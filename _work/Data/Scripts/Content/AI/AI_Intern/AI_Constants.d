// **************************
// AIVAR-Kennungen fьr Humans 
// **************************

// ------ NEWS und MEMORY Aivars ------
// - NUR Augenzeugen
// - NUR persцnliches News-Gedдchtnis, geregelt ьber die folgenden Aivars
// - schwere Tat ьberschreibt leichte Tat
// - KEINE automatische News-Verbreitung, AIV-Check bei aNSCs in Dialogen = Ersatz fьr News-Verbreitung
// - News-Vergabe ьber Dialog-Module
...
const int 		CRIME_ATTACK			= 2;	//Kampf self-SC --> feinere Analys in DIA-Modul ьber AIV_LastFightAgainstPlayer
...
// ------ ZS_Talk ------------------------------
const int   AIV_INVINCIBLE					= 4;	//ist TRUE fьr beide Teilnehmer eines Dialogs
...
const int 		AR_MonsterVsHuman		= 4; 			//Monster kдmpft gegen Human - ich helfe Human
const int 		AR_MonsterMurderedHuman = 5;			//Monster hat Human getцtet
const int 		AR_SheepKiller			= 6;			//Schaf wurde angegriffen oder getцtet (von Mensch oder Monster)
...
const int 		AR_ReactToWeapon		= 10;			//Tдter hat trotz zweimaliger Warnung Waffe nicht weggesteckt ODER ich fliehe direkt
const int 		AR_ReactToDamage		= 11;   	 	//Tдter hat mich verletzt
const int 		AR_GuardStopsFight		= 12;   	 	//Wache beendet Kampf, greift Tдter an
const int 		AR_GuardCalledToKill	= 13;   	 	//Wache durch WARN zum Mit-Tцten gerufen
...
const int 		AR_SuddenEnemyInferno	= 18;			//EnemyOverride Blockierung fьr mich selbst und alle NPCs im Umkreis aufheben.
const int 		AR_KILL					= 19;   	 	//Spieler aus Dialog heraus tцten (SC hat keine Chance)

// ------ ZS_RansackBody ------------------------
const int 	AIV_RANSACKED					= 10; 	//damit nur EIN NSC einen Body plьndert

// ------ ZS_Dead -------------------------------
const int 	AIV_DeathInvGiven				= 11;	// Fьr Mensch und Monster!

// ------ TA_GuardPassage -----------------------
const int	AIV_Guardpassage_Status			= 12;	// Enthдlt den Status einer Durchgangswache
const int		GP_NONE					= 0;		// ...Anfang; SC noch nicht in der Nдhe
...
const int	AIV_LastDistToWP				= 13;	// enthдlt die letzte ermittelte Distanz zum Referenz-Waypoint (Checkpoint) (wird beim SPIELER gesetzt!) 
const int   AIV_PASSGATE					= 14;   // TRUE = Spieler kann durch, FALSE = Spieler wird aufgehalten

// ------ XP-Vergabe ----------------------------
const int	AIV_PARTYMEMBER					= 15;	//fьr XP-Vergabe, AUCH fьr Monster
const int 	AIV_VictoryXPGiven				= 16;	//wenn NSC schonmal besiegt, gibts keine XP mehr fьr den Spieler (z.B. bei unconscious Humans)

// ------ Geschlecht ----------------------------
const int AIV_Gender						= 17; 	//wird zusammen mit Visual ьber B_SetVisual gesetzt
const int 		MALE					= 0;
const int 		FEMALE					= 1;
...
// ------  TA Hilfsvariable -----------------------	//notwendig da lokale Hilfsvariablen anscheinend nicht immer neu initialisiert werden bitte nur fьr TAs verwenden!
...
const int AIV_MM_FollowInWater 				= 28;	//AUCH fьr HUMANS! - folgt einem der NSC auch in Wasser?
// ----------------------------------------------
const int AIV_MM_PRIORITY 					= 29;	//ist meine Prioritдt gerade FRESSEN oder KДMPFEN (immer fressen, es sei denn ich werde getroffen)
...
const int AIV_ToughGuy						= 57;	//Jubelt beim Kampf, hat keine News bei Attack (kann ьber AIV_LastFightAgainstPlayer reagieren)
...
// ------ diverse ZS -------
const int AIV_StateTime						= 68; //um mit zwei StateTime-Triggern arbeiten zu kцnnen.
...
const int AIV_NPCIsRanger					= 72;	//Der Typ gehцrt zum 'Ring des Wassers'
const int AIV_IgnoresArmor					= 73;	//Keine Reaktion oder Konsequenzen auf die Rьstung des SC
const int AIV_StoryBandit					= 74;	//Banditen, mit denen gekдmpft werden darf
...
// ***************************************************
// Globalvariablen fьr Petzen/Absolution/News - System
// ***************************************************
...
const int PERC_DIST_HEIGHT				= 1000; // ab welchem Hцhenunterschied wird Wn ignoriert
const int PERC_DIST_INDOOR_HEIGHT		= 250;  // dasselbe fьr Indoor (zum Ignorieren von anderen Stockwerken wenn ganzes Haus (Hцhlensystem) EIN Portalraum ist)
//---------------------------------------------
const int FIGHT_DIST_MELEE				= 600;	// Bis zu welcher Entfernung Bedrohung durch SC-Waffe
const int FIGHT_DIST_RANGED_INNER		= 900;  // Ab welcher Entfernung NK wдhlen, wenn NSC im FK ist
const int FIGHT_DIST_RANGED_OUTER		= 1000; // Ab welcher Entfernung FK wдhlen, wenn NSC im NK ist oder Waffe zum ersten Mal gezogen wird
...
**					Zeit-Konstanten									*
********************************************************************/
CONST INT NPC_ANGRY_TIME 				= 120;	// MUSS SO HEISSEN, ist vom Programm ausgelagert - Spielsekunden, die die Temp_Att aufrechterhalten wird, bevor sie wieder auf Perm_Att gesetzt wird (gilt fьr alle At, nicht nur fьr angry)
// -------------------------------------------
const int HAI_TIME_UNCONSCIOUS			= 20;	// MUSS SO HEISSEN, ist vom Programm ausgelagert (Default = 20) - Zeit in Sekunden, die der SC und NSCs bewuЯtlos bleiben
// -------------------------------------------
...
**	Mindestschaden 				                                   **
********************************************************************/
const int NPC_MINIMAL_DAMAGE 	= 5; 	//MUSS SO HEISSEN, ist vom Programm ausgelagert - Untere Genze des Mindestschadens fьr Menschen (*** und Monster ??? ***)
...
const int FAI_Bear						= 45	; 	//Bдr
const int FAI_Stoneguardian				= 46	;	//Steinwдchter
...
//werden benцtigt um eine Unterscheidung der Mobsi beim PC_Hero Dialog vorzunehmen.
...
// *****************************
// Konstanten fьr B_SetNpcVisual
// *****************************

// ------ Nacktmesh-Texturen fьr Mдnner und Frauen (je 4) ------
...
const int BodyTexBabe_B		= 7;	//Black Babe   //Frauen werden auch mit den "Mдnner"-Konstanten angegeben, dann vom Script +4 addiert, d.h. diese Konstanten werden nicht gebraucht
const int BodyTex_Player	= 8;

//---------ADD ON----------------------
const int BodyTex_T	= 10; //tдtowierte psionikerhaut
