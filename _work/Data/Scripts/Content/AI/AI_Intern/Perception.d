...
FUNC VOID InitPerceptions()
{
	...
	// PERC_ASSESSENEMY		// der nдchste NSC, zu dem ich Att_Hostile habe, wird wahrgenommen! (wenn Temp_Att != Gil_Att, wird Temp_att genommen)
	...
	// Aktive Wahrnehmungen werden NUR ьber die aktivierten senses getriggert -Humans haben KEIN sense_smell, d.h. ein Aufruf von z.B. B_AssessPlayer beinhaltet schon einen Sichtcheck (etwa 180° vor dem NSC)

	// *** ACHTUNG *** Passive Wahrnehmungen sind sense-unabhдngig (sie gehen immer an alle NSCs, egal ob's gesehen/gehцrt wird oder nicht)
	...
	// wird an alle NSCs gesendet, wenn ein NSC bewuЯtlos wird
	Perc_SetRange		(PERC_ASSESSMURDER		, PERC_DIST_ACTIVE_MAX 			); //eigentlich INTERMEDIAT, ist hier grцЯer, weil Mцrder direkt hinter mir, aber Opfer 11m entfernt stehen kann
	...
	Perc_SetRange		(PERC_ASSESSFIGHTSOUND	, 3000							); //wird in fast allen Fдllen auf PERC_DIST_INTERMEDIAT verkьrzt!
	...
	// wird gesendet von VisualFXInst.d - Wahrnehmungs-Sendung hдngt am Magie Effekt
	// Ein NSC, der diese Wahrnehmung bekommt, kann in der AI ZUSДTZLICH auf Magie reagieren (i.d.R. mit ZS-Wechsel)
	...
	// Wird an das / die Opfer gesendet, wenn der Spell aufhцrt zu wirken
	...
	// Wenn NSC eine Tьr im Weg ist

	// --- SC verwandelt sich / - zurьck ------------------------------------------
	
	Perc_SetRange		(PERC_ASSESSSURPRISE	, FIGHT_DIST_CANCEL			 	);	// nur in ZS_Attack und ZS_MM_Attack
	// geht an alle, wenn der SC sich zurьckverwandelt
...
func void Perception_Set_Normal ()				//Fьr Humans
{
	// --- Die Senses bei jedem Zustand neu setzen (Weil in ZS_Guide_Player die Senses & Ranges verдndert werden) ------
	...
	Npc_PercEnable  	(self, 	PERC_MOVEMOB			,	B_MoveMob					); //цffnet Tьren, die auf dem Weg liegen
};
...
func void Perception_Set_Minimal ()				//Fьr Mдnner und Frauen
{
	// --- Die Senses bei jedem Zustand neu setzen (nur falls in ZS mit Perc_Set_Minimal noch ZUSДTLZLICH (lokal) aktive Wahrnehmungen angemeldet werden) ------
	self.senses			=	SENSE_HEAR | SENSE_SEE;
	self.senses_range	=	PERC_DIST_ACTIVE_MAX;
...
// ЬBERALL DORT aufgerufen werden, wo ein neuer ZS aufgerufen wird
// Beachten: auch in B_Befehlen, die wiederum aus diesem B_Befehlen aufgerufen werden
// mьssen alle ZS_Aufrufe mit B_ClearPerceptions versehen werden
// AUF KEINEN FALL einfach B_ClearPerceptions in jeden B_Befehl, der durch eine Wahrnehmung aufgerufen wird,
// reinmachen (so daЯ er IMMER aufgerufen wьrde) - das wьrde dazu fьhren, daЯ ein B_Befehl, in einem Fall in dem
