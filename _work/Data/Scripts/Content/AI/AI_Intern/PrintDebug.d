...
func void PrintGlobals (var int channel)
{
	...
	{
		pipe = ConcatStrings("...sebe:   ",	self.name);
	...
	{
		pipe = ConcatStrings("...jiný:  ",	other.name);
		PrintDebugNpc 		(channel,		pipe);
	};
	...
	{
		pipe = ConcatStrings("...oběť: ",	victim.name);
		PrintDebugNpc 		(channel,		pipe);
	};
	...
	{
		pipe = ConcatStrings("...hrdina:   ",	hero.name);
		PrintDebugNpc 		(channel,		pipe);
	};
	...
	{
		pipe = ConcatStrings("...předmět:   ",	item.name);
		PrintDebugNpc 		(channel,		pipe);
	};
