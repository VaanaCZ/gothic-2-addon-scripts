...
// HUMAN
	...
	STEP_HEIGHT		 		[GIL_HUMAN] = 60;				// Stufenhцhe fьr autom. Hцhenьberwindung
	JUMPLOW_HEIGHT	 		[GIL_HUMAN] = 105;				// Hцhe bis zu der JumpUpLow benutzt wird (in cm)
	JUMPMID_HEIGHT	 		[GIL_HUMAN] = 205;				// Hцhe bis zu der JumpUpMid benutzt wird (in cm)
	JUMPUP_HEIGHT 			[GIL_HUMAN] = 200; 				// Sprunghцhe wenn hцher als JumpUpMid (in cm) Ist effektiv hцher, da von Boden bis Kцrpermitte gemessen!!!

	SLIDE_ANGLE		 		[GIL_HUMAN] = 50;				// Winkel ab der Model in Slide (in Grad)
	DISABLE_AUTOROLL		[GIL_HUMAN] = 0;
	SURFACE_ALIGN			[GIL_HUMAN] = 0;

	TURN_SPEED				[GIL_HUMAN] = 240;				// dieser Wert gilt nur fьr NSCs!
															// Turn-Speed fьr den Spieler ist PROGRAMMINTERN auf 100° im Normalmodus und 200° im Kampfmodus eingestellt!
	// Kletterkanten - Winkel
	CLIMB_HEADING_ANGLE		[GIL_HUMAN] = 50;
	CLIMB_HORIZ_ANGLE		[GIL_HUMAN] = 50;
	CLIMB_GROUND_ANGLE  	[GIL_HUMAN] = 50;

	// Fallhцhe / Fallschaden
	FALLDOWN_HEIGHT			[GIL_HUMAN] = 500;				// Fallhцhe in cm ohne Schaden zu nehmen
	FALLDOWN_DAMAGE			[GIL_HUMAN] = 20;				// Schaden in HPs fьr jeden weiteren Meter
...
// GOBBO SKELETT AUCH SUMMONED Gobbo-Skelett (s.u.) beachten!
	...
	// Fallhцhe / Fallschaden                      // Fallhцhe / Fallschaden          
...
// .Der Wert "999999" fьr fiegende Monster fьhrt dazu, dass Wasserpolys wie statische Polys behandelt werden
//	auch auf diesen "laufen" und niemals ins Schwimmen/Tauchen kommen kцnnen.
...
// SURFACE_ALIGN 2 ist WICHTIG damit der Swampshark nich an jeder Kante hдngenbleibt!
...
// DRACHE (Feuerdrache / Eisdrache / Felsdrache / Sumpfdrache / Untoter Drache)
	...
	STEP_HEIGHT		 		[GIL_DRAGON] = 200; //sonst "fдllt" der Drache in manchen Drehungsframes (geht in FallDown)
	...
	FIGHT_RANGE_FIST		[GIL_DRAGON] = 250; //W = Base + Fist = 600 //Schiebefaktor NICHT 450 (*0.75) sondern Kьrzer!!!
