...
// Printbefehl, der aus den angegebenen Strings einen Auswдhlt und auf den Bildschirm schreibt
...
// Die Position ist fьr jede Koordinate eine Zahl zwischen 0 und 99 und gibt die prozentuale Position an.
...
// erzeugt einen Zufallswert (wobei n0 Obergrenze : [0..n0-1] ) Rьckgabewert integer
FUNC INT    Hlp_StrCmp			(VAR STRING s1, VAR STRING s2) { return 0; };
// Hilfsfunktion um zwei Strings miteinander zu vergleichen liefert bei Gleichheit eins zurьck
FUNC INT  	Hlp_IsValidNpc			(VAR C_NPC self)					{ return 0; };
// Prьfe ob NSC-Instanz gьltig und initialisiert ist. True/False
FUNC INT  	Hlp_IsValidItem			(VAR C_ITEM item)					{ return 0; };
// Prьfe ob Item-Instanz gьltig und initialisiert ist True/False
// -------------------------------------------------------------------------------------------
FUNC INT 	Hlp_IsItem 				(VAR C_Item item, VAR INT instanceName)
// Prьft, ob der Gegenstand 'item' den Instanznamen 'instanceName' trдgt. Will man z.B. testen,
// ob die globale Variable item Pfeile sind (ItMuArrow) ruft man Hlp_IsItem(item,ItMuArrow) auf.
// Gribt bei Gleichheit TRUE, sonst FALSE zurьck.
FUNC C_NPC 	Hlp_GetNpc				(VAR INT instanceName) { };
// Ermittle einen NSC ьber den Instanznamen. Dieser kann einer Variablen zugewiesen werden und ist somit gezielt verfьgbar
FUNC INT 	Hlp_GetInstanceID 		( VAR C_NPC npc ) { return 0; };
// liefert die interne ID ( nicht Var aus der Instanz) zurьck, um z.B. other mit self vergleichen zu kцnnen ( integer Vergleich)
FUNC INT 	Hlp_GetInstanceID 		( VAR C_ITEM item ) { return 0; };
// liefert die interne ID ( nicht Var aus der Instanz) zurьck, um zwei items miteinander vergleichen zu kцnnen ( integer Vergleich)
// *******************************************************************************************
// ***                                      WARTEN / ZEIT                                  ***
// *******************************************************************************************

FUNC VOID 	AI_Wait 			( VAR C_NPC n0, VAR float n1 ) { };
// Character wird fьr n1 Sekunden in einen Wait-Zustand versetzt,
...
//--> Das ist dann ьber Npc_GetStateTime abfragbar.

FUNC INT	Wld_GetDay			() {};
// Liefert den aktuellen Tag zurьck (Zдhler), hierbei ist der StartTag (Gamestart) = 0.
...
// Fьge NSC in Welt ein. Wobei SPawnpoint entweder ein WP oder ein FP sein darf.
func void 	Wld_InsertNpcAndRespawn (VAR INT instance, VAR STRING spawnPoint, VAR FLOAT spawnDelay)
// !!!!!!!! weiЯ ich nicht Genau Ulf
...
// - Ist der Nsc in einem Animatinsstate, wird die passende Rьcktransition abgespielt.
// - Benutzt der NSC gerade ein MOBSI, poppt er ins stehen.
func void	AI_StandUpQuick 		(VAR C_NPC self)
// Wie AI_StandUp(), jedoch werden keine Rьcktransitionen abgespielt, sondern auch dort wird
// sofort in den Grundzustand "gepoppt". Wichtig fьr sehr eilige Situationen!
...
// und der angegebene 'targetState' ist bereits vorhanden, lдuft der NSC zwar trotzdem zum MOB, tut dann aber nichts
FUNC INT  	Wld_IsMobAvailable		(VAR C_NPC self, VAR STRING schemeName) {};
// Sucht sich ein Mobsi im Umkreis von 10m und liefert TRUE falls gefunden. MOB wird nur zurьckgeliefert, wenn es nicht besetzt ist.
func int	Wld_GetMobState			(var C_NPC self, var string schemeName );
// Liefert den State zum nдchsten Mob mit 'schemeName' zurьck bzw. '-1' wenn kein solches Mob gefunden wurde

// -------------------------------------------- GEHEN ----------------------------------------

FUNC VOID 	AI_SetWalkmode 			(VAR C_NPC n, VAR INT n0 ) { };
// gibt an mit welchem Walkmode Run etc der Character durch das Level lдuft
//NPC_RUN			: Rennen
//NPC_WALK			: Gehen
...
// Npc-Instanz lдuft zum namentlich angegeben Waypoint
...
// liefert den Namen des am nдchsten gelegenen Waypoints zurьck
FUNC string	Npc_GetNextWP			(VAR C_NPC self) {return ""; };
// Liefert den zweitnahesten WP vom NSC zurьck
// -------------------------------------------------------------------------------------------
FUNC INT  Wld_IsFPAvailable			(VAR C_NPC self, VAR STRING fpName) {};
// Sucht einen Freepoint im Umkreis von 20m vom NSC und liefert TRUE falls vorhanden und frei ('self' zдhlt als Blockierer nicht!) und sichtbar
func int  Wld_IsNextFPAvailable     (var C_NPC self, VAR STRING fpName)
//  wie Wld_IsFPAvailable(), aber es wird immer der nahegelegenste genommen und 'self' zдhlt als Blockierer!
...
// Der Nsc weicht ein Stьck nach hinten aus
...
// Fьgt Zufallsanimationen (am Kopf kratzen, an den Sack packen etc (s2)) fьr bestimmte Animationszustдnde (s1) ein
FUNC VOID 	Mdl_ApplyRandomAniFreq 	(VAR C_NPC n0, VAR STRING s1, VAR FLOAT f2 ) { };
// hiermit kann die Frequenz betimmt werden, wie oft die fьr den Animationszustand (s1) deklarierten Randomanis abgespielt werden
...
// Starte zufдllige Gesichtsanimation
...
// liefert eins zurьck, wenn der Charakter im angegebenen Fightmode (z.B. FMODE_MAGIC) ist
...
// gibt eins zurьck, wenn die Waffe schon in der Hand ist
...
// Gibt TRUE zurьck, wenn 'npc' irgendeine Fernkampfwaffe im Inventory oder in der Hand hat UND
// dazu auch passende Munition vorhanden ist. Sonst FALSE.

// ----------------------------------------- ANGRIFF -----------------------------------------
FUNC INT  	Npc_GetTarget				(VAR C_NPC self)
// Befьllt 'other' mit dem aktuellen Ziel. Das aktuelle Ziel wird intern gespeichert, wird
...
// ьbernommen und in 'other' geschrieben, wenn kein Gegner gefunden wurde wird das
// interne Ziel gelцscht und 'other' ungьltig gemacht.
// Kriterien:	1. gesetzter Gegner, der nicht tot/bewuЯtlos ist wird genommen...
//				2. ...dann der nдchstbeste aNSC demgegenьber 'self' HOSTILE ist (auch weder tot noch bewuЯtlos)
...
//				Wahrnehmungen benutzt, muЯ vorher ein Npc_PerceiveAll() aufgerufen werden
FUNC INT 	Npc_IsNextTargetAvailable 	(var C_NPC self )
// Sucht genauso wie Npc_GetNextTarget nach einem neuen Ziel, ьberschreibt aber weder
...
// Fьhrt den logischen Finishing Move inklusive Anis aus den Skripten heraus aus
...
//sonst JA (= wir SIND stдrker, nicht echt, aber ab doppelter Gegnerstдrke zu fliehen ist frьh genug, sonst kommen
//die Jungs aus dem Rennen nicht mehr raus
//Beachten:
//1) Monster kцnnen feindlich zu beiden Menschen sein --> egal, zдhlen auf beiden Seiten
...
// liefert eins zurьck, wenn der zweite Character auf den ersten zielt (Fernkampfwaffen und Zauber)
...
// Fьge Item in Welt ein entweder an einem WP oder einem FP
...
// gibt die Mцglichkeit nach bestimmten Items zu suchen (z.B.:Das goldene Schwert der Zerstцrung, wenn vorhanden)
func INT  	Wld_RemoveItem          	(VAR C_ITEM item)		{ };
// !!! hiermit wird das globale Item aus der Welt gelцscht ???
...
// Mit diesem Befehl lдЯt sich nachsehen, ob in einem bestimmten Slot einer bestimmten Kategorie ein item vorhanden ist
...
// !!! das globale Item wird gelцscht ???
func void	Npc_RemoveInvItems			(VAR C_NPC owner, VAR int itemInstance, VAR INT amount )	{ };
// !!! wie Npc_RemoveInvItem, nur das Multislotgegenstдnde gelцscht werden ???
// ------------------------------------------ TRUHEN -----------------------------------------
...
// gibt dem Nsc direkt das angegebene Item an den Gьrtel
FUNC VOID 	AI_EquipBestMeleeWeapon		(VAR C_NPC self) {};
// sucht im Inventory nach der besten Nahkampfwaffe und hдngt sie an den Gьrtel
FUNC VOID 	AI_EquipBestRangedWeapon	(VAR C_NPC self) {};
// sucht im Inventory nach der besten Fernkampfwaffe und ploppt sie auf den Rьcken der Instanz
FUNC VOID 	AI_EquipBestArmor			(VAR C_NPC self) {};
// Wunder, Wunder hier wird die beste im Inventory vorhandene Rьstung angezogen
FUNC VOID 	AI_UnequipWeapons			(VAR C_NPC self) {};
// Unequippe alle Waffen
FUNC VOID 	AI_UnequipArmor				(VAR C_NPC self) {};
// Unequippe aktuelle Rьstung
...
// Liefert die angelegte Rьstung des NSCs.
// -------------------------------------------------------------------------------------------
FUNC INT  	Npc_HasEquippedWeapon		(VAR C_NPC self) { return 0; };
// gibt eins zurьck, wenn die abgefragte Instanz (self oder other) eine Waffe sichtbar mit sich rumtrдgt
FUNC INT  	Npc_HasEquippedMeleeWeapon	(VAR C_NPC self) { return 0; };
// gibt eins zurьck, wenn eine Nahkampfwaffe Equipped ist
FUNC INT  	Npc_HasEquippedRangedWeapon	(VAR C_NPC self) { return 0; };
// Fernkampwaffe auf dem Rьcken True/False
...
// Liefert "1", wenn dem NSC das Item gehцrt ( persцnliches Besitzflag )
FUNC INT  	Npc_OwnedByGuild			(VAR C_ITEM item, VAR int guild) { return 0; };
// Liefert "1", wenn der angegebenen Gilde das Item gehцrt ( Gildenbesitz )
...
// Prьfe alle ItemReact-Module von "self" auf Gegenstand "item" von Geber "other" und starte passende Reaction-Funktion
...
//Bietet Spieler dem NSC einen Gegenstand ьbers Trade-Modul an ? True/False
...
// wie AI_OutputSVM, wartet jedoch NICHT mit der Ausfьhrung des nдchsten AI_...-Befehls, bis
// das SVM zuende gespielt wird. (Fьr Kommentare kurz vor und wдhrend dem Kampf!)
...
// die fьr die Fдlle Spieler sagt ja (Daumen nach oben) und Spieler sagt nein (Daumen unten) vorhanden sein mьssen
FUNC VOID 	AI_AskText              	(VAR C_NPC self, VAR FUNC funcYes, VAR FUNC funcNo, VAR STRING strYes, VAR STRING strNo);
// wie AI_Ask, nur das auЯer den Funktionen auch noch Strings fьr die Antworten Ja/Nein mit angegeben werden kцnnen
FUNC VOID 	AI_WaitForQuestion			(var C_NPC self, VAR FUNC scriptFunc) { };
// NSC wartet 20 Sekunden, wird er in dieser Zeit vom Spieler angesprochen, wird die angegebene SkriptFunktion ausgefьhrt.
// -------------------------------------------------------------------------------------------
...
// self:	NSC, bei dem News eingespeist werden soll, other:	Tдter victim :	Opfer
FUNC VOID	Npc_MemoryEntryGuild		(VAR C_NPC self, VAR INT source, VAR C_NPC offender, VAR INT newsid, VAR C_NPC victimguild) { };
// wie MemoryEntry nur, das die Memory fьr die ganze Gilde kreiert wird
FUNC INT    Npc_HasNews					(VAR C_NPC self, VAR INT newsID, VAR C_NPC offender, VAR C_NPC victim)	{};
// Liefert Newsnummer>0 (fьr weitere Referenzen) falls entsprechende News vorhanden.
// Nicht benцtigte Parameter kцnnen mit "NULL" leergelassen werden
...
// Ermittle Zeuge der News und gibt eine INstanz der Klasse C_Npc zurьck
FUNC C_NPC  Npc_GetNewsVictim			(VAR C_NPC self, VAR INT newsNumber) {};
// Ermittle Opfer der News und gibt eine INstanz der Klasse C_Npc zurьck
FUNC C_NPC 	Npc_GetNewsOffender 		(VAR C_NPC self, VAR INT newsNumber ) { return 0; };
// Ermittelt Tдter der News und gibt eine INstanz der Klasse C_Npc zurьck
...
// Funktion liefert einen Wert zurьck, falls Dead true ist
...
// VORSICHT: auch wenn eine permanente Info schon einmal dem Spieler erzдhlt wurde, so gibt diese Funktion trotzdem FALSE zurьck!
FUNC INT  	Npc_CheckInfo				(VAR C_NPC npc, var int important) { return 0; };
// Ьberprьft,ob der NSC gьltige Infos fьr den Spieler hat und startet diese gegebenenfalls (Returnwert "1").
FUNC INT  	NPC_GiveInfo				(VAR C_NPC npc, VAR INT important) { return 0; };
// Ьberprьft,ob der NSC eine (!) gьltige Info fьr den Spieler hat und startet diese gegebenenfalls (Returnwert "1").
FUNC INT  	Npc_CheckAvailableMission	(VAR C_NPC npc, VAR INT missionState, VAR INT important) { return 0; };
// Prьfe ob zwischen dem NSC eine aktuelle Mission (AVAILABLE,RUNNING) besteht und vergleiche den Status mit den angegebenen.
// Falls dieser identisch ist, wird "1" geliefert.
FUNC INT  	Npc_CheckRunningMission		(VAR C_NPC npc, VAR INT important) { return 0; };
// Ьberprueft ob zwischen dem NSC und dem Spieler eine laufende Mission besteht und startet den entsprechenden Skriptblock.
// (Returnwert 1 : Mission aktiv)
FUNC INT  	Npc_CheckOfferMission		(VAR C_NPC npc, VAR INT important) { return 0; };
// Ьberprueft ob der NSC dem Spieler einen Auftrag anbieten kann, wenn ja, wird der Offer-Block gestartet und "1" zurьckgeliefert.
...
// Liefert aktuellen Status einer Mission zurьck ( Bezogen auf den Spieler ) -> RUNNING, SUCCESS, FAILED etc.
FUNC INT  	Mis_OnTime					(VAR INT missionName ) { return 0; };
// Liefert TRUE, wenn sich Spieler noch innerhalb des Zeitlimits fьr diese Mission befindet
...
// liefert eins zurьck, wenn der geprьfte Charakter der Spieler himself ist
FUNC INT	Wld_DetectPlayer			(VAR C_NPC self) { return 0; };
// liefert eins zurьck, wenn der Player in der Nдhe ist
FUNC INT	Npc_HasDetectedNpc			(VAR C_NPC self, VAR C_NPC other) { return 0; };
// liefert eins zurьck, wenn der Spieler in Sinnesreichweite und entdeckt ist                                     // SPIELER ???
...
// liefert eins zurьck, wenn geprьfte Instanz sich im Bereich von drei Metern zur prьfenden Instanz befindet
FUNC INT  	Npc_GetDistToNpc			(VAR C_NPC npc1, VAR C_NPC npc2) { return 0; };
// Liefert Entfernung ( нn cm ! ) zwischend den beiden NSCs
FUNC INT  	Npc_GetHeightToNpc			(VAR C_NPC npc1, VAR C_NPC npc2) { return 0; };
// Liefert Hцhendifferenz ( нn cm ! ) zwischend den beiden NSCs
...
// Liefert Entfernung ( нn cm ! ) zwischend NSC und Gegenstand
...
// liefert immer!!! die wahre Gilde zurьck, ignoriert also auch ohne die Regeln die Verkleidung
FUNC VOID	Npc_SetAttitude				(VAR C_NPC self, VAR INT att) { };
//  setzt die permanente Attitьde auf einen festen Wert
FUNC VOID	Npc_SetTempAttitude			(VAR C_NPC self, VAR INT att) { };
//   setzt die temporдre Attitьde auf einen Wert (att)
FUNC INT	Npc_GetAttitude				(VAR C_NPC self, VAR C_NPC other) { return 0; };
// Gibt die Attitьde von self zu other zurьck (temp. / perm. / Gilden )
...
// Ermittelt die Gildenattitьde von zwei Nscґs direkt im Gegensatz zu Wld_GetGuildAttitude
// -------------------------------------------------------------------------------------------
...
// stateBehaviour sagt : "0"-aktuellen Zustand abbrechen "1"-aktuellen Zustand erst ordnungsgemдЯ beenden (End-Funktion aufrufen) ).
...
// Tausche meherere Tagesablдufe aus
...
// (Name wird automatisch mit "RTN_" am Anfang und NSC-Skript-ID am Ende ergдnzt)
...
// zum skalieren des Meshes (Breite,Hцhe,Tiefe) 1 = 100%, also NormalgrцЯe!
...
// дndert den Wert des Attributs (atr) um (value) Einheiten.
...
// liefert eins zurьck, wenn der geprьfte Charakter das Talent tal hat
...
// Setze Zeitdelta fьr aktive Wahrnehmungen, alle Zeitdelta-Sekunden wird  WN gesendet
// -------------------------------------------------------------------------------------------
...
// Sende eine passive Wahrnehmung aus.Npc1 = wer schickt Npc2 = Opfer, Npc3 = Tдter
...
// ansonsten wird "0" geliefert und "other" wird nicht verдndert.
VAR INT 	Wld_DetectNpcEx ( VAR C_NPC self, VAR INT npcInstance, VAR FUNC aiState, VAR INT guild, VAR INT detectPlayer)
// Wie Wld_DetectNpc(). Zusдtzlich kann per detectPlayer=0 der Spieler ignoriert werden.
FUNC INT 	Wld_DetectItem				(VAR C_NPC self, VAR INT flags) { return 0; };
// liefert eins zurьck, wenn ein Item mit dem Entsprechende Flag (z.B.ITEM_KAT_FOOD )gefunden wurde
...
// liefert den Schemanamen des Mobsi zurьck, das der Nsc entdeckt hat. Schemaname ist das String-Kьrzel des Mobs, daЯ im Visualnamen vor dem ersten "_"-Zeichen steht, also z.B. "DOOR" wenn der Visualname "DOOR_OCR__135" ist.
// VORSICHT: Diese Funktion ist nur in Verbindung mit PERC_MOVEMOB erlaubt !!!
// WORKAROUND: zur Zeit werden hiermit nur Tьr-MOBSIs zurьckgegeben, dies soll aber wieder zurьckgebaut werden
// --------------------------------------------------------------------------------------------
FUNC INT  	Npc_CanSeeNpc				(VAR C_NPC npc1, VAR C_NPC npc2) { return 0; };
// Prueft ob Npc1 den Npc2 sehen kann ( ein Ray wird gecastet -> Bitte sparsam einsetzen ) True/False Blickwinkelabhдngig (+,-100°)+LOS von der Hьfte aus
FUNC INT  	Npc_CanSeeNpcFreeLOS		(VAR C_NPC self, VAR C_NPC other) 	{};
// Prьft ob NSC anderen NSC sehen kann, ohne dabei den Winkel zu berьcksichtigen (nur LineOfSight-Check)
...
// Cutscene an den zuletzt angegebenen Tagesablaufpunkt hдngen
// csName	: Name der Cutscene ( der Name des "CS" - Files )
// roleName : Die Rolle die der NSC dabei ьbernehmen soll.
...
// liefert eins zurьck, wenn der entsprechende Nsc in einer Cutscene ist
...
// liefert den NSC des aktiven Raums, in dem sich der SC gerade befindet, zurьck
// - wenn der SC 'drauЯen' ist, dann ist der Rьckgabe-Npc 'notValid'
// - wenn der aktive Raum besitzerlos ist, dann ist der Rьckgabe-Npc 'notValid'
FUNC INT 	Wld_GetPlayerPortalGuild 	()
// liefert Gilde des aktiven Raums, in dem sich der SC gerade befindet, zurьck
// - wenn der SC 'drauЯen' ist, dann wird GIL_NONE zurьckgegeben
// - wenn der aktive Raum gildenlos ist, dann wird GIL_NONE zurьckgeliefert
FUNC C_NPC 	Wld_GetFormerPlayerPortalOwner ()
// liefert den NSC des Raums, in dem sich der SC vor dem letzten "Raumwechsel" befunden hat, zurьck
// Raumwechsel ist dabei: AuЯen->Raum1, Raum1->Raum2, -> Raum1->AuЯen
// - wenn der SC 'drauЯen' ist, dann ist der Rьckgabe-Npc 'notValid'
// - wenn der aktive Raum besitzerlos ist, dann ist der Rьckgabe-Npc 'notValid'
...
// gibt TRUE zurьck, wenn sich SC im Raum des 'npc' oder seiner Gilde befindet, sonst FALSE
FUNC int 	Npc_WasPlayerInMyRoom ( VAR C_NPC npc )
// gibt TRUE zurьck, wenn sich SC vor dem letzten Raumwechsel im Raum des 'npc' oder seiner Gilde befindet, sonst FALSE
// Raumwechsel ist dabei: AuЯen->Raum1, Raum1->Raum2, -> Raum1->AuЯen
...
// !!!Funktion ьbertrдgt eine Integer variable in eine Stringvariable (Rьckgabewert : string)???

func int	FloatToInt			( var float x)	{ };
// !!! konvertiert einen float in einen int, hier wдre es noch ganz schцn zu wissen, wie mit dem float umgesprungen wird --> schneiden oder runden ???

func float	IntToFloat			( var int x)	{ };
// !!! wie float to int, nur daЯ ein integer in float umgewandelt wird ???
func string ConcatStrings		(var string str1, var string str2)
// !!!Erzeugt eine Kopie von dem ersten angegebenen String, hдngt den zweiten an diesen an und gibt den neuen String zurьck ???


//Die folgenden Methoden geben den Text nur unter bestimmten zusдtzlichen Bedingungen aus. Fьr die Methoden mit dem Suffix Inst trifft dies zu, wenn die Instanz von der die Debug-Methode aufgerufen wird, zum Debuggen aktiviert wurde. Die Methoden mit dem Suffix Ch geben den Text nur aus, wenn der angegebene Channel zum Debuggen aktiviert wurde.
...
// gibt nur den Text eines bestimmten Channels wieder, unabhдngig von der Instanz ???

// *******************************************************************************************
// ***                               Log-Entries (ab v1.01)                                ***
// *******************************************************************************************
func void Log_CreateTopic( VAR STRING name, VAR INT section )
//	Der Befehl fьgt unter der Sektion 'section' ein neues Topic mit Namen 'name' ein.
//	Sollte bereits ein Topic dieses Names in irgendeiner Sektion existieren, wird das Topic nicht hinzugefьgt.
//
//	Parameter:
...
//	Der Befehl дndert den Status des Topics 'name' in 'status'.
//	Er sollte nur fьr Topics verwendet werden, die fьr die Sektion LOG_MISSION erzeugt wurden.
//
//	Parameter:
...
//	Der Befehl fьgt dem Topics topic ein neues Entrz namens entry hinzu. Wie auch bei der Erzeugung von Topics dient hier der
//	Name sowohl zur Identifikation als auch zur Anzeige und muss somit eindeutig sein. Doppelte Eintrдge werden auch hier ignoriert.
//
//	Parameter
