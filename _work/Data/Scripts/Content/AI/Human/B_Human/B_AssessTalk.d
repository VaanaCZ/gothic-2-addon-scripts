...
func void B_AssessTalk ()
{
	...
	{
		PrintScreen (ConcatStrings("Voix :", IntToString(self.voice)), -1, 70, FONT_Screen, 2);
		
		PrintScreen ("AUCUN HEROS !", -1, -1, FONT_Screen, 2);	
		PrintScreen (IntToString(self.aivar[AIV_FollowDist]), -1, 70, FONT_Screen, 2);
		
		if (C_NpcIsInQuarter (self) == Q_KASERNE)
		{
			PrintScreen ("Q_CASERNE", -1, 30, FONT_Screen, 2);
		};
		
		if (C_NpcIsInQuarter (self) == Q_GALGEN)
		{
			PrintScreen ("Q_GEOLE", -1, 30, FONT_Screen, 2);
		};
		
		if (C_NpcIsInQuarter (self) == Q_MARKT)
		{
			PrintScreen ("Q_MARCHE", -1, 30, FONT_Screen, 2);
		};
		...
		{
			PrintScreen ("Q_BASSE VILLE", -1, 30, FONT_Screen, 2);
		};
		
		if (C_NpcIsInQuarter (self) == Q_HAFEN)
		{
			PrintScreen ("Q_PORT", -1, 30, FONT_Screen, 2);
		};
		
		if (C_NpcIsInQuarter (self) == Q_OBERSTADT)
		{
			PrintScreen ("Q_HAUTE VILLE", -1, 30, FONT_Screen, 2);
		};
