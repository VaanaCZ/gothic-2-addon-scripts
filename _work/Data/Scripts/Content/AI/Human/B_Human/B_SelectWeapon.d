...
// B_SelectWeapon
// --------------
// Bei Fernkдmpfern wird Muni ins Inv gemogelt
// *******************************************
...
func void B_SelectWeapon(var C_NPC slf, var C_NPC oth)
{
	...
	// ------ Magie wдhlen oder behalten ------
	if (B_SelectSpell (slf, oth))		//bei HUMANS wдhlen nur Magier, Paladine oder Dementoren Magie
	{
		return; 			//Magie gewдhlt!
	};
	...
	if (Npc_IsInFightMode(slf,FMODE_MAGIC))		//Habe Magie gezogen, will aber keine benutzen!!!
	{
		...
		AI_ReadyMeleeWeapon	(slf); //Schwert ziehen, sonst Fдuste
		return;
	};
	...
	if (Npc_IsInFightMode (slf,FMODE_NONE))
	{
		...
		// ----- wenn gar keine Waffe: Default (Fдuste) ------
