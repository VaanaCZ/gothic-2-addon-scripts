...
func void B_AssessFightSound ()
{
	...
	// ------ im KLOSTER: wenn Hцhenunterschied zu groЯ -------
	...
	// ------ FightSound wird NUR bei anderen NSCs (nicht self) berьcksichtigt ------
	...
	// ------ Tдter Monster (Opfer Human wegen oberer Abfrage) ------
	if (other.guild > GIL_SEPERATOR_HUM)
	...
	// ------ Opfer Monster (Tдter Human wegen oberer Abfrage) ------
	...
	// ---------------------------------------------
	// IM FOLGENDEN sind Tдter und Opfer BEIDE Human
	// ---------------------------------------------
	
	// --------------------------- 
	// WAS ist ein FakeBanditDuel?
	// ---------------------------
	// --> Player wird als Bandit ANGESEHEN und kдmpf gegen Bandit
	// ACHTUNG: Player muЯ nicht nur Banditenrьstung anhaben, 
	// sondern muЯ auch vom NPC 'self' als Bandit ANGESEHEN werden
	...
	// ----------------------------------------------------------------
	// ------ Wachen kommen zum Tцten (helfen other) ------------------
	// ----------------------------------------------------------------
	...
	&& (Npc_GetAttitude(self,other)==ATT_FRIENDLY)
	{
		// ------ Pack schlдgt sich, Pack vertrдgt sich -------
	...
	// ------ NUR in Watchfight gehen, wenn nдher als 10m ------ (wichtig z.B. fьr GIL_OUT)
