...
func void B_Say_AttackEnd()
{
	...
	|| (self.aivar[AIV_ATTACKREASON] == AR_MonsterMurderedHuman)
	{
		if (other.guild < GIL_SEPERATOR_HUM)
		{
			...
			{
				B_Say (self, other, "$ENEMYKILLED");			//Das war's fьr dich, Penner.
			};
		}
		else
		{
	...
	// ------ Schaf wurde angegriffen oder getцtet (von Mensch oder Monster) ------
	...
	if (self.aivar[AIV_ATTACKREASON] == AR_HumanMurderedHuman)
	{
		if (!Npc_IsDead (other))
		{
			if (self.aivar[AIV_LASTTARGET] == Hlp_GetInstanceID (other))
			{
				B_Say (self, other, "$KILLMURDERER");	//Stirb, Mцrder!
			}
			else
			{
				B_Say (self, other, "$GOODKILL");			//(ruft) Ja, mach das Schwein fertig! 
	...
	// ------ Monster kдmpft gegen Human - ich helfe Human ------
	...
	{
		B_Say (self, other, "$STUPIDBEASTKILLED");			//So ein saublцdes Vieh!
		return;
	};
	
	// ------ Tдter hat mich verletzt ------
	...
	// ------ Tдter hat trotz zweimaliger Warnung Waffe nicht weggesteckt ------
	if (self.aivar[AIV_ATTACKREASON] == AR_ReactToWeapon)
	{
		B_Say (self, other, "$YOUBETTERSHOULDHAVELISTENED");	//Du hдttest auf mich hцren sollen!
		return;
	};
	...
	|| (self.aivar[AIV_ATTACKREASON] == AR_GuardCalledToRoom)
	{
		...
		{
			B_Say (self, other, "$NEVERENTERROOMAGAIN");		//Und daЯ ich dich nie wieder da drin erwische!
		};
		return;
	};
	...
	// ------ Wache beendet Kampf, greift Tдter an ------
	if (self.aivar[AIV_ATTACKREASON] == AR_GuardStopsFight)
	{
		...
		{
			B_Say (self, other, "$THEREISNOFIGHTINGHERE");		//Hier wird nicht gekдmpft, klar!? Lass dir das eine Lehre sein!
		};
		return;
	};
