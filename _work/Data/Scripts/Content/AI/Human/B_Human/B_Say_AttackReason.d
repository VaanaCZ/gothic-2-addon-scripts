...
func void B_Say_AttackReason()
{
	...
	if (self.aivar[AIV_ATTACKREASON] == AR_SheepKiller)
	{
		...
		{
			B_Say_Overlay (self, other, "$SHEEPKILLERMONSTER");		//Das verdammte Mistvieh friЯt unsere Schafe!
		};
		return;
	};
	...
	{
		B_Say_Overlay (self, other, "$YOUMURDERER");				//Mцrder!
		Npc_SendPassivePerc	(self, PERC_ASSESSFIGHTSOUND, self, other);
		return;
	};
	
	// ------ Monster hat Human getцtet ------
	...
	{
		B_Say_Overlay (self, other, "$THENIBEATYOUOUTOFHERE");		//Dann muss ich dich eben rausPRЬGELN!
		return;
	};
	...
	{
		B_Say_Overlay (self, other, "$WILLYOUSTOPFIGHTING");		//Wollt ihr wohl damit aufhцren!?
		return;
	};
