...
// und setzt Attack Reason (bzw. ьberschreibt ihn evtl.)
// ********************************************************
...
func void B_Attack (var C_NPC slf, var C_NPC oth, var int attack_reason, var int wait)
{
	...
	if (attack_reason == AR_SuddenEnemyInferno)
	{
		...
		Npc_SendPassivePerc	(slf, PERC_ASSESSFIGHTSOUND, slf, oth); //EnemyOverride-Aivars von allen anderen Npcs wegschieЯen!
	};
	...
	//MEMORIZE CRIMES				//AR ДNDERT sich NUR, wenn schwerer als vorheriger (s.o.)
	...
	// ------ AR_KILL ------						//HACK - geht davon aus, daЯ Spieler auf jeden Fall JETZT stirbt (daher brauchen die Werte spдter nicht resettet zu werden)
	...
	if (!Npc_IsInState (slf, ZS_Talk)) //HIER RAUS: fьhrt aus DIA heraus zum Lцschen des AI_StopProcessInfo
	{
