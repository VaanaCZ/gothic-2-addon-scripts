...
// ---------------------	
// Neue Tat ьberschreibt alte Tat, wenn sie schwerer oder gleichschwer ist
// Tag wird gemerkt
...
{	
	...
	// ------ PETZCRIME fьr HomeLocation ьberschreiben, wenn schwerer als vorherige ------
	...
	// ------ neue Straftat ьberschreibt alte, wenn sie schwerer (oder gleichschwer) ist ------	
	if (crime >= B_GetPlayerCrime(slf))
	{
		slf.aivar[AIV_NpcSawPlayerCommit] 		= crime;
		slf.aivar[AIV_NpcSawPlayerCommitDay] 	= Wld_GetDay();
		slf.aivar[AIV_CrimeAbsolutionLevel] 	= B_GetCurrentAbsolutionLevel(slf); //bleibt fьr NSCs, die NICHT zu City/Monastery/Farm gehцren, immer 0 
	};
};
