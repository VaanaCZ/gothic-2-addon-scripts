...
func void B_AssessPlayer ()
{
	...
	// ------ Ignorieren, wenn SC tot, ohnmдchtig oder in magischem Schlaf ------
	...
	{
		if (C_NpcIsGateGuard (self)) //Wachen mьssen auch freudliches/neutrales SC-Monster aufhalten 
		{
	...
	// -------- Spieler hat Banditen Rьstung an ------
	...
	// ------ SC ist in einem Portalraum, der mir oder meinen Freuden gehцrt ------
	...
	//------------------------------------
	//Joly: ImportantInfos fьr AmbientDMT! 
	B_AssignDementorTalk (self);
	//------------------------------------

	// ------ Hat NSC Important Info fьr Spieler? Auch GUARDPASSAGE ------
	if (Npc_GetDistToNpc(self,other) <= PERC_DIST_DIALOG)
	&& (Npc_CheckInfo (self, 1))	//REIHENFOLGE nicht vertauschen! (Check ist dahingehend buggy (darf nicht als erste Abfrage in &&-verknьpfter If-Abfrage stehen))
	{
		...
		{
			// ------- Spieler fдllt, schwimmt und taucht NICHT ------
	...
	// ------ sonst den Spieler einfach grьЯen (wenn ich ihm entgegenkomme) ------
	...
	&& (Npc_RefuseTalk(other) == FALSE)		//Damit kein GruЯ-Maschinengewehr losgeht
	&& (!C_NpcIsGateGuard (self))	//GateGuards benutzen RefuseTalk-Counter, um zu verhindern, daЯ der Spieler von hinten kommend auch angequatscht wird
	&& (C_PlayerHasFakeGuild (self,other) == FALSE)			
	{	
