...
func void B_AssessSurprise()
{
	...
	// ------ HACK: AR дndern wegen B_Says, mцgl. GateGuards, mцgl. Mцrder, etc. -------
...
func void ZS_Attack ()
{
	...
	// ------ lokale Wahrnehmung fьr verwandelten SC ------
	...
	// ------ Fьr Magier ------
	self.aivar[AIV_SelectSpell] = 0; //Fьr Magier;
};
...
func int ZS_Attack_Loop ()
{
	...
	if (self.aivar[AIV_PursuitEnd] == TRUE)
	{
		...
		{
			// ------ Feind kommt zurьck ODER bleibt stehen ------
	...
	B_CreateAmmo(self);							//muЯ VOR SelectWeapon kommen, weil sonst Bogen bei fehlender Muni nicht gezogen wird
	...
	&& (C_NpcIsDown(other) == FALSE))
	{
		...
		{
												//fьhrt Angriff mit Waffe oder Spell aus (Aktion wird durch FAI bestimmt)
	...
	else //target ungьltig (tot) oder down		// falls NPC_GetTarget(self) == FALSE --> other hier automatisch gelцscht
	{
		// ------ noch in der Queue befindliche Attacks lцschen ------
		Npc_ClearAIQueue(self);
		...
		// ------ NUR neues Ziel wдhlen, wenn AR NICHT Kill ------
		if (self.aivar [AIV_ATTACKREASON] != AR_KILL)
		{
			Npc_PerceiveAll	(self);					// nцtig, da Npc_GetNextTarget() auf der Liste der zuletzt Wahrgenommenen VOBs beruht, und das kann hier schon ne Weile her sein, denn ZS_Attack hat keine aktiven Wahrnehmungen
		...
		else 									// wenn false, wird other gelцscht!
		{
			Npc_ClearAIQueue(self);
...
func void ZS_Attack_End ()
{
	...
	{
		// ------ wenn Spieler wдhrend der Verfolgung Absolution erlangt hat ------
		if (B_GetCurrentAbsolutionLevel(self) > self.aivar[AIV_LastAbsolutionLevel])
		{
	...
	// ------ mein Gegner vom Spieler getцtet ------
	...
	{
		// ------ Opfer wird markiert, damit nur EINER plьndert - abgefragt vor Aufruf von ZS_RansackBody -------
