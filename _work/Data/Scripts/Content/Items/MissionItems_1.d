...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "K pâípravę magických lektvarů potâebuje alchymista tyto vęci:");
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "Ohnivá kopâiva");
					Doc_PrintLine	( nDocID,  0, "Ohnivé býlí");
					Doc_PrintLine	( nDocID,  0, "Ohnivý koâen");
					Doc_PrintLines	( nDocID,  0, "Dále se musí postarat, aby mu nechybęlo toto:"						);
					Doc_PrintLine	( nDocID,  0, "Luční pohanka"						);
					Doc_PrintLine	( nDocID,  0, ""						);
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, "        pohybovat v dolní části Khorinidu."		);
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_SetFont 	( nDocID, -1, FONT_Book		   		); 	
					Doc_SetMargins	( nDocID, -1, 50, 50, 50, 50, 1   	);  
					Doc_PrintLine	( nDocID,  0, "Milý Isgarothe,"					);
					Doc_PrintLines	( nDocID,  0, "Cítím, že tu s námi dlí ještę jiná moc, která den ode dne sílí."					);
					Doc_PrintLines	( nDocID,  0, "Její podstata mi ale zcela uniká. Že by se k nám blížili Beliarovi vazalové?"					);
...
INSTANCE ItKe_ThiefGuildKey_Hotel_MIS (C_Item)
{
	...
	TEXT[2]				=	"Klíč od sklepa hostince.";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""								);
					Doc_PrintLines	( nDocID,  0, "Oba víme, jaká je Vaše situace, a proto navrhuji následující ujednání:" 	);
					...
					Doc_PrintLines	( nDocID,  0, "Já sám krom toho osobnę dohlédnu, aby se moji lidé drželi dál od okolních statků - samozâejmę vyjma toho Onarova, kde budeme sídlit i nadále." );
					...
					Doc_PrintLines	( nDocID,  0, "Prosím, rozhodnęte se moudâe." );
					Doc_PrintLine	( nDocID,  0, ""								);
					Doc_PrintLine	( nDocID,  0, "podepsán   generál Lee"		);
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   				);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
...
{   
		...
		*/
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, "Jestli na nękoho takového narazíš, pak"				);
...
INSTANCE ItMi_EddasStatue (C_Item)
{
	...
	TEXT[4]				= 	"a chraŕ mne ode všeho zlého.";
...
INSTANCE ItKe_Salandril(C_Item)
{
	...
	TEXT[2]				=	"která patâí alchymistovi Salandrilovi.";
	TEXT[3]				=   "";
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já mistr Thorben, khorinidský tesaâ, dlužím ctihodnému Lehmarovi 200 zlaăáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					Doc_PrintLine	( nDocID,  0, "             Thorben");
					Doc_PrintLine	( nDocID,  0, ""					);	
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já Coragon, khorinidský hostinský, dlužím ctihodnému Lehmarovi 150 zlaăáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Já Hanna, khorinidská hostinská, dlužím ctihodnému Lehmarovi 250 zlaăáků.");
					Doc_PrintLine	( nDocID,  1, ""					);
