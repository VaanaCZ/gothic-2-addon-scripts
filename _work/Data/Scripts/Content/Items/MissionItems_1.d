...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, " Ostatnie ostrzeżenie"								);
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_SetFont		(nDocID,  0, FONT_Book);
					Doc_PrintLine	(nDocID,  0, " Wielokrotnie już ostrzegałem cię"				);
					Doc_PrintLine	(nDocID,  0, " w niezwykle przyjazny sposób,"				);
					Doc_PrintLine	(nDocID,  0, " że broń zamówiona u ciebie tydzień"				);
					Doc_PrintLine	(nDocID,  0, " temu jest mi wyjątkowo pilnie"			);
					Doc_PrintLine	(nDocID,  0, " potrzebna."										);
					Doc_PrintLine	(nDocID,  0, " Jeśli szybko się nie odezwiesz w tej"		);
					Doc_PrintLine	(nDocID,  0, " sprawie, to zmusisz mnie do zmiany"			);
					Doc_PrintLine	(nDocID,  0, " sposobu, w jaki cię traktuję. Mam"			);
					Doc_PrintLine	(nDocID,  0, " nadzieję, że się dobrze rozumiemy!!!!"					);
					Doc_PrintLine	(nDocID,  0, ""													);
...
INSTANCE ItRi_ValentinosRing(C_Item) //bei Cassia als Belohnung
{
	...
	description				= "Pierścień Valentina";
...
INSTANCE ItWr_Kraeuterliste (C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	"Lista ziół Constantina.";
};
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "Zioła alchemiczne"	);
					Doc_PrintLine	( nDocID,  0, ""						);
					...
					Doc_PrintLine	( nDocID,  0, "Ognisty korzeń"  );
					Doc_PrintLine	( nDocID,  0, "Goblinie jagody"     );
					Doc_PrintLine	( nDocID,  0, "Smoczy korzeń");
					Doc_PrintLine	( nDocID,  0, "Zębate ziele"   );
					Doc_PrintLine	( nDocID,  0, "Rdest polny"      	);
					Doc_PrintLine	( nDocID,  0, "Szczaw królewski"       );
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "Niech rośliny zbiera dla ciebie kandydat na ucznia - przekonasz się, czy się do czegoś nadaje. "						);
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "Z poważaniem, Zuris."						);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "Do warzenia mikstur magicznych alchemik potrzebuje - oprócz umiejętności - także:");
					Doc_PrintLine	( nDocID,  0, ""						);
...
INSTANCE ItWr_Passierschein	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[3]				=	"Dzięki tym dokumentom nie";
	TEXT[4]				=   "zatrzymają mnie strażnicy miejscy.";
};
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, " do swobodnego poruszania się po dolnej części"		);
...
INSTANCE ItMi_HerbPaket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Ciężka, lepiąca się paczka,";
	TEXT[3]				=	"śmierdząca bagiennym zielem.";
...
{	
	...
	description			= 	"Dziwnie wyglądająca ryba.";
	TEXT[2]				= 	"Z tą rybą jest coś nie tak.";		
	TEXT[3]				= 	"Wygląda, jakby ktoś ją pozszywał...";
};
...
INSTANCE ItWr_HalvorMessage	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[2]				=   "W rybie była schowana ta notatka.";
};
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, "    Nie wychylajcie się! "				);
					Doc_PrintLine	(nDocID,  0, "    Straż zaczyna coś podejrzewać."				);
					Doc_PrintLine	(nDocID,  0, "    Nic nie róbcie, dopóki znów się nie odezwę!");
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "wyczuwam coraz mocniej obecność nie znanej nam siły."					);
					Doc_PrintLines	( nDocID,  0, "Czy to możliwe, że sługi Beliara nadchodzą? Być może się mylę,"					);
					Doc_PrintLines	( nDocID,  0, "sądzę jednak, że Ojciec Pyrokar powinien wziąć sprawy w swoje ręce. "					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Oby nasze modły zostały wysłuchane."					);
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItWr_VatrasMessage_Open	(C_Item)
{
	...
	TEXT[2]				=   "Wiadomość od Vatrasa";
	TEXT[3]				=   "do Magów Ognia.";
	TEXT[4]				=   "Pieczęć została złamana.";
};
...
INSTANCE ItKe_ThiefGuildKey_MIS (C_Item)
{
	...
	TEXT[2]				=	"Klucz ten nie oparł się działaniu słonej, morskiej wody.";
...
{	
	...
	//FIXME_Noki: TAB
	TEXT[2]				=  "Obrażenia:                   ??";
	TEXT[3] 			=  "Wymagana siła:          ??";
	TEXT[4] 			=  "Broń dwuręczna";
	TEXT[5]				=  "Wartość:  niemożliwa do ustalenia";					
	/*
	description			= name;
	TEXT[2]				= NAME_Damage;					COUNT[2]	= damageTotal;
	TEXT[3] 			= NAME_Str_needed;				COUNT[3]	= cond_value[2];
...
INSTANCE ItWr_Passage_MIS	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[3]				=	"Dzięki tej wiadomości być może uda";
	TEXT[4]				=   "mi się porozmawiać z Lordem Hagenem.";
};
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""								);
					Doc_PrintLines	( nDocID,  0, "obydwaj zdajemy sobie sprawę z sytuacji, więc przejdę od razu do rzeczy:" 	);
					Doc_PrintLines	( nDocID,  0, "uniewinnij mnie i moich ludzi wobec Jego Wysokości i samego Innosa."				);
					Doc_PrintLines	( nDocID,  0, "W zamian zobowiązujemy się pomóc Ci w obronie miasta i okolicznych ziem." );
					Doc_PrintLines	( nDocID,  0, "Osobiście dopilnuję też, by moi ludzie trzymali się z dala od tutejszych farm - naturalnie oprócz zagród Onara, gdzie nadal będziemy stacjonowali." );
					Doc_PrintLines	( nDocID,  0, "Wiem, że statek, którym zamierzacie wrócić na kontynent, może zabrać kilka dodatkowych osób. Gdy będziecie ruszać w drogę, chcę popłynąć z wami." );
					Doc_PrintLines	( nDocID,  0, "Zaklinam cię: rozważ dobrze moje słowa i podejmij rozsądną decyzję." );
					Doc_PrintLine	( nDocID,  0, ""								);
					Doc_PrintLine	( nDocID,  0, "Podpisano   Generał Lee"		);
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   				);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
...
{   
		...
		*/
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, "przyjdą od strony gór."						);
					Doc_PrintLine	(nDocID,  0, "Jeśli ktoś przekroczy przełęcz,"				);
					Doc_PrintLine	(nDocID,  0, "będzie to prawdopodobnie pewien"			);
					Doc_PrintLine	(nDocID,  0, "starzec. Nie dajcie się"				);
					Doc_PrintLine	(nDocID,  0, "zwieść - to niebezpieczny mag."				);
					Doc_PrintLine	(nDocID,  0, "Miejcie na niego oko."						);
					Doc_PrintLine	(nDocID,  0, ""											);
					Doc_PrintLine	(nDocID,  0, "Może się z nim skontaktować człowiek,"		);
					Doc_PrintLine	(nDocID,  0, "którego szukamy."						);
					Doc_PrintLine	(nDocID,  0, "Wraz z tym listem otrzymujecie"				);
					Doc_PrintLine	(nDocID,  0, "30 sztuk złota."							);
					Doc_PrintLine	(nDocID,  0, "Kolejne 30 otrzyma osoba,"				);
					Doc_PrintLine	(nDocID,  0, "która go zabije."							);
					Doc_PrintLine	(nDocID,  0, ""													);
					Doc_PrintLine	(nDocID,  0, "Głowę przynieście do starej kopalni.");
					Doc_PrintLine	(nDocID,  0, ""													);
...
INSTANCE ItMi_EddasStatue (C_Item)
{
	...
	TEXT[2]				= 	"Innosie, Władco Prawości,";
	TEXT[3]				= 	"pobłogosław mnie,";
	TEXT[4]				= 	"i chroń przed wrogiem.";
...
INSTANCE ItKe_PaladinTruhe(C_Item)
{
	...
	TEXT[2]				=	"Niewielki mosiężny klucz";
...
INSTANCE ItKe_Fingers(C_Item)
{
	...
	TEXT[2]				=	"Zardzewiały klucz do drzwi";
	TEXT[3]				=   "z kanałów.";
...
{	
	...
	scemeName				=	"MAP";
	description				= 	"Księga Długów Lehmara.";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages 
 					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Umowy i długi"	);
					...
					Doc_PrintLine	( nDocID,  0, ""					);	
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Ja, Coragon, właściciel gospody w Khorinis, jestem winien sz.p. Lehmarowi 150 monet.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Ja, Hanna, właścicielka gospody w Khorinis, jestem winna sz.p. Lehmarowi 250 monet.");
					Doc_PrintLine	( nDocID,  1, ""					);
...
INSTANCE ITKE_ORLAN_HOTELZIMMER(C_Item)
{
	...
	TEXT[2]				=	"gospody 'Pod Martwą Harpią'.";
	TEXT[3]				=   "";
