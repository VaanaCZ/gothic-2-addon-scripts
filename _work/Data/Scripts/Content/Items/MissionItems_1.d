...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "K přípravě magických lektvarů potřebuje alchymista tyto věci:");
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "Ohnivá kopřiva");
					Doc_PrintLine	( nDocID,  0, "Ohnivé býlí");
					Doc_PrintLine	( nDocID,  0, "Ohnivý kořen");
					Doc_PrintLines	( nDocID,  0, "Dále se musí postarat, aby mu nechybělo toto:"						);
					Doc_PrintLine	( nDocID,  0, "Luční pohanka"						);
					Doc_PrintLine	( nDocID,  0, ""						);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Cítím, že tu s námi dlí ještě jiná moc, která den ode dne sílí."					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Já sám krom toho osobně dohlédnu, aby se moji lidé drželi dál od okolních statků - samozřejmě vyjma toho Onarova, kde budeme sídlit i nadále." );
					...
					Doc_PrintLines	( nDocID,  0, "Prosím, rozhodněte se moudře." );
					Doc_PrintLine	( nDocID,  0, ""								);
...
{   
		...
		*/
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, "Jestli na někoho takového narazíš, pak"				);
...
INSTANCE ItMi_EddasStatue (C_Item)
{
	...
	TEXT[4]				= 	"a chraň mne ode všeho zlého.";
...
INSTANCE ItKe_Salandril(C_Item)
{
	...
	TEXT[2]				=	"která patří alchymistovi Salandrilovi.";
	TEXT[3]				=   "";
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já mistr Thorben, khorinidský tesař, dlužím ctihodnému Lehmarovi 200 zlaťáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					Doc_PrintLine	( nDocID,  0, "             Thorben");
					Doc_PrintLine	( nDocID,  0, ""					);	
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já Coragon, khorinidský hostinský, dlužím ctihodnému Lehmarovi 150 zlaťáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Já Hanna, khorinidská hostinská, dlužím ctihodnému Lehmarovi 250 zlaťáků.");
					Doc_PrintLine	( nDocID,  1, ""					);
