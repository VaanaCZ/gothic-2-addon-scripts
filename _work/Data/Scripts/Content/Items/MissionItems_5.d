...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Jestli vše probęhne podle mého očekávání,"					);
					Doc_PrintLines	( nDocID,  0, "męl bys teë vyhledat Irdorathské sínę."					);
	...
	{
		B_LogEntry (TOPIC_BuchHallenVonIrdorath,"V tomto dopise mi Xardas sdęlil, jaká slova otevâou knihu Irdorathské sínę."); 
	};
...
INSTANCE ItKe_MonastarySecretLibrary_Mis (C_Item)
{
	...
	TEXT[0]				= 	"z knihy 'Irdorathské sínę'.";
	...
	var int nDocID;
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages 
 					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages 
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "...nyní vím jistę, že tou stavbou jsou Irdorathské sínę. Leží na ostrovę nedaleko khorinidského pâístavu. Už je tedy jasné, proč męl Beliar o rudné doly takový zájem...");
					Doc_PrintLines	( nDocID,  0, "");		
					...
					if (hero.guild == GIL_PAL)
					{
						...
						Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages 
						Doc_PrintLine	( nDocID,  1, ""					);
						Doc_PrintLines	( nDocID,  1, "...promęnęný paladin už na nic nereaguje. Jeho zbroj a ostatní vęci jsem proto uložil do zadní místnosti. Je dobâe zajištęná - dveâe se dají otevâít pouze zevnitâ a dovnitâ se dá dostat pouze pomocí teleportační runy, kterou jsem vytvoâil. Všechny pokyny jsem zapsal do almanachu, aby je On jednou mohl najít..."	);
					};
					...
					{
						Doc_PrintLines	( nDocID,  0, "...pâijde válka, válka pro všechny vęâící na svętę. Ale zdá se mi, že znamení dokážu rozpoznat jenom já -  ostatní si jich vůbec nevšímají. Já však vím, že se objeví Vyvolený a jeho pâíchod bude vęstit válku. Válku tak starou jako sám čas - konec je však už na obzoru.");
						
						//2.Seite
						Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
						Doc_SetFont 	( nDocID,  1, FONT_BookHeadline	   			); 	// -1 -> all pages 
						Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages 
						Doc_PrintLines	( nDocID,  1, "Nebudu tu jen tak stát s rukama v klínę a pozorovat, jak ostatní berou do rukou osud svęta!"					);
						Doc_PrintLine	( nDocID,  1, ""					);
...
INSTANCE ItWr_UseLampIdiot_Mis	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[0]				=	"od knihy 'Irdorathské sínę'.";
};
...
//Joly:VORSICHT : Dieser Brief ist ein ERMÄCHTIGUNGSSCHREIBEN für das Schiff und kann von LEE oder dem Richter stammen. NICHT zwangsläufig von Lord Hagen oder gefälscht!!!!!!!!!
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, "        volnę plavit na královské válečné galéâe"		);
