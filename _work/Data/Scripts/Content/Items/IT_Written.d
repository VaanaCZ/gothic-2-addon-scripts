...
func void UseStandardBrief ()
{
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager
					...
					Doc_PrintLine	( nDocID,  0, "Lettera comune"					);
...
INSTANCE StandardBuch (C_ITEM)
{
	...
	FUNC VOID UseStandardBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Libro comune, pagina 1"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
...
{
	name 		= "Mappa della città di Khorinis";  // 
...
{
	name 		= "Mappa della Valle delle Miniere";
...
INSTANCE ItWr_EinhandBuch (C_ITEM)
{
	...
	scemeName				=	"MAP";
	description				= 	"Arte della difesa del Sud.";
	
	TEXT[2] 				=  "Un libro sull'arte del";
	TEXT[3] 				=  "combattimento a una mano";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseEinhandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Un meridionale usa meno forza fisica di un settentrionale, invece fa affidamento sull'agilità. Visto il clima caldo della zona d'origine, preferisce usare un'armatura leggera che gli permette di muoversi liberamente. Il meridionale ha quindi sviluppato uno stile di combattimento che è profondamente diverso da quello cui siamo abituati."					);
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Probabilmente la tattica più famosa impiegata da un meridionale è la parata armata e l'arretramento. L'arretramento permette al combattente di smorzare il colpo dell'assalitore, portandosi allo stesso tempo in una posizione favorevole per il contrattacco."	);
					Doc_PrintLine	( nDocID,  1, ""					);
...
{
	name 					=	"Tattiche di combattimento";
	...
	scemeName				=	"MAP";
	description				= 	"Doppi blocchi";
	
	TEXT[2] 				=  "Un libro sull'arte del";
	TEXT[3] 				=  "combattimento a due mani";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseZweihandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Doppi blocchi"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Bloccare la lama avversaria con un'arma a due mani è una tecnica utilizzata dai combattenti robusti per fermare l'impeto degli attaccanti e costringere gli assalitori a interrompere immediatamente la propria combinazione di colpi."					);
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "La pausa dei movimenti avversari che generalmente si crea è un'ottima opportunità per prendere l'iniziativa e sconfiggere il nemico con dei colpi ben piazzati."	);
