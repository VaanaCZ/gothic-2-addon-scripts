...
INSTANCE StandardBuch (C_ITEM)
{
	...
	FUNC VOID UseStandardBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "TypowaKsiążka Strona 1"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "TypowaKsiążka Strona 2"	);
...
INSTANCE ItWr_EinhandBuch (C_ITEM)
{
	...
	scemeName				=	"MAP";
	description				= 	"Kunszt obronny południowców";
	
	TEXT[2] 				=  "Książka opisująca sztukę";
	TEXT[3] 				=  "walki broniami jednoręcznymi.";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseEinhandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Kunszt obronny południowców"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Południowiec w porównaniu z mieszkańcem północy nie polega tak bardzo na sile fizycznej, preferując zwinność, a gorący klimat jego ojczyzny zmusza go do korzystania z lżejszych i dających większą swobodę ruchów pancerzy. W związku z tym styl walki ludzi południa jest całkowicie inny od tego, do którego jesteśmy przyzwyczajeni. "					);
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Prawdopodobnie najsłynniejszym manewrem wywodzącym się z południa jest jednoręczny blok połączony z krokiem wstecz: cofnięcie się pozwala na zmniejszenie siły wrogiego ciosu i umożliwia przyjęcie postawy ułatwiającej wyprowadzenie kontrataku."	);
					Doc_PrintLine	( nDocID,  1, ""					);
...
INSTANCE ItWr_ZweihandBuch (C_ITEM)
{
	...
	scemeName				=	"MAP";
	description				= 	"Bloki dwuręczne";
	
	TEXT[2] 				=  "Książka opisująca sztukę";
	TEXT[3] 				=  "walki broniami dwuręcznymi.";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseZweihandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Bloki dwuręczne"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Blokowanie ciosów przeciwnika za pomocą broni dwuręcznej umożliwia silnym wojownikom gwałtowne powstrzymanie wroga i przerwanie ewentualnej kombinacji jego ciosów. "					);
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Zatrzymany w ten sposób przeciwnik traci inicjatywę i łatwiej go potem skutecznie zaatakować."	);
