...
INSTANCE ItWr_SaturasFirstMessage_Addon_Sealed	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "Tato zpráva byla peèlivì zapeèetìna.";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Dostali jsme se až k portálu. Mìl jsi pravdu,"					);
					Doc_PrintLines	( nDocID,  0, "opravdu to vypadá, že šlo o stoupence Adanovy. Prosím, vezmi si k ruce mé poznámky a ještì jednou to ovìø."					);
					Doc_PrintLines	( nDocID,  0, "Už pár dní jsme nezahlédli žádnou z tìch podivných kamenných bytostí."					);
					Doc_PrintLines	( nDocID,  0, "Ale zemì se pøesto stále bezdùvodnì tøese."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že naše studia nám brzy pøinesou další osvícení."					);
					Doc_PrintLines	( nDocID,  0, "Ten ornament, který jsme našli, je mnohem dùležitìjší, než jsme si prve mysleli. Nejspíš se jedná o nìjaký klíèový artefakt. Ale bohužel není dokonèen - musíme jej dále studovat a zjistit, k èemu skuteènì sloužil."					);
					Doc_PrintLines	( nDocID,  0, "Vyšli prosím nìkteré z dítek 'kruhu vody', aby nám jej pøineslo. Ale pokud možno tím nepovìøuj Cavalorna."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že vìtší úkol než doruèení tohoto dopisu by byl nad jeho síly."					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že dìláme správnou vìc."					);
		...
		&& (MIS_Addon_Cavalorn_Letter2Vatras != LOG_SUCCESS)
		{
			...
			B_LogEntry (TOPIC_Addon_KDW,"Tu zprávu, kterou mìl Cavalorn doruèit vodnímu mágovi Vatrasovi, jsem sebral jednomu banditovi. Takže teï je to na mnì."); 
		...
		if (SC_KnowsRanger == FALSE)
		{
			...
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Jedna zdejší komunita si øíká 'kruh vody' a v jejím èele podle všeho stojí vodní mágové."); 
		};
		...
		if (SC_IsRanger == FALSE)
		{
			Log_CreateTopic (TOPIC_Addon_RingOfWater, LOG_MISSION);
			Log_SetTopicStatus(TOPIC_Addon_RingOfWater, LOG_RUNNING);
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Cavalorn patøil právì k tomutu 'kruhu vody'."); 
		};
...
INSTANCE ItWr_SaturasFirstMessage_Addon	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[2]				=   "Saturasùv dopis Vatrasovi";
};
...
instance ItWr_Map_NewWorld_Ornaments_Addon (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Na této mapì jsou vyznaèena místa,";
	TEXT[1]		= "kde se podle Nefaria nacházejí chybìjící ornamenty.";
	TEXT[2]		= "";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
instance ItWr_Map_NewWorld_Dexter (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Skip mi na mapì vyznaèil,";
	TEXT[1]		= "kde najdu náèelníka banditù.";
	TEXT[2]		= "Dexter.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
{
	TEXT[1]					= "Tento prsten patøi Laresovi";
};

//PCs Ring
INSTANCE ItRi_Ranger_Addon (Rangerring_Prototype)
{
	TEXT[1]					= "Tento prsten patøí mnì";
};
//Lance Ring
...
{
	name 				=	"Martinùv doporuèující dopis";
	...
	scemeName			=	"MAP";
	description			=  	"Doporuèující dopis pro lorda Andreho";
	
	TEXT[2]				= 	"Tento dopis by mi mìl pomoci";
	TEXT[3]				= 	"pøi pøijetí do øad domobrany.";
...
{   
	...
	var int nDocID;
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLines	(nDocID,  0, "Spolu s tímto dopisem k Vám posílám i nového èlena domobrany.");
					Doc_PrintLines	(nDocID,  0, "Už nìkolikrát mi pomohl v rùzných obtížných a dùležitých záležitostech.");
					Doc_PrintLines	(nDocID,  0, "Proto se domnívám, že stejnì dobøe bude chránit krále i obèany tohoto krásného mìsta.");
					Doc_PrintLine	(nDocID,  0, "Nech Innos žehná králi!"							);
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
 	...
 	&& (MIS_Addon_Vatras_WhereAreMissingPeople != 0)
 	{
		...
		B_LogEntry (TOPIC_Addon_WhoStolePeople,"Tak teï už to mám èerné na bílém. Za tìmi únosy khorinidských obyvatel vìzí bývalý rudobaron Havran. Skrývá se kdesi za horami severovýchodnì od mìsta. Tento dokument by mìl vidìt Vatras."); 
	...
	nDocID = 	Doc_Create		();
				...
				Doc_PrintLine	(nDocID,  0, ""														);
				Doc_PrintLines	(nDocID,  0, "Když jsem byl ještì rudobaronem, nebyl jsi tak dìsnì nespolehlivý!");
				Doc_PrintLines	(nDocID,  0, "Jestli v nejbližší dobì neuneseš z Khorinidu nìjaký další lidi a nepošleš mi je, èekaj tì vážný potíže s hochy z naší základny.");
				Doc_PrintLines	(nDocID,  0, "Nutnì potøebuju další otroky, nebo budu mít brzo na krku vzpouru. A ty urèitì víš, co to znamená, viï?");
				Doc_PrintLines	(nDocID,  0, "Už jsem se skoro dostal do chrámu, takže teï si rozhodnì nemùžu dovolit žádný takovýhle potíže.");
				Doc_PrintLines	(nDocID,  0, "A ještì jeden problém:"								);
				Doc_PrintLines	(nDocID,  0, "Døív nebo pozdìjc se musíme dostat pøes ty vysoký hory severovýchodnì od Khorinidu. Piráti už nás moc dlouho pøepravovat nebudou, jestli jim pøestaneme platit.");
				Doc_PrintLine	(nDocID,  0, ""														);
...
{
	name 				=	"Vatrasùv doporuèující dopis";
	...
	scemeName			=	"MAP";
	description			=  	"Vatrasùv doporuèující dopis";
	
	TEXT[2]				= 	"S pomocí tohoto dopisu bych se mìl";
	TEXT[3]				= 	"dostat do kláštera ohnivých mágù,";
	TEXT[4]				= 	"aniž bych musel nìco platit.";
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Následovníci ohnì.");
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Právì jsem se dozvìdìl, že za vstup do vašich øad vyžadujete nìjaký poplatek.");
...
{
	name 				=	"Luciin dopis na rozlouèenou";
...
{   
	...
	B_LogEntry (TOPIC_Addon_Lucia,"Lucie napsala Elvrichovi dopis na rozlouèenou. To by ho mohlo zajímat."); 
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Nedokážeš si pøedstavit, s jakou bolestí ti tohle píšu.");
					Doc_PrintLines	( nDocID,  0, "Vím, že to nepochopíš, ale hodnì jsem pøemýšlela a došla jsem k závìru, že bude pro nás oba lepší, když si najdeš nìjakou slušnìjší ženu, než jsem já."					);
					Doc_PrintLines	( nDocID,  0, "Odcházím a už se nevrátím. Zapomeò na mì. Holka jako já se pro slušnýho chlapa jako ty prostì nehodí. Sbohem.");
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItRi_Addon_BanditTrader(C_Item)	//Händlergildenring
{
	...
	TEXT[2]					= 	"námoønì-obchodního cechu Araxa";
...
INSTANCE ItWr_Addon_BanditTrader		(C_Item)
{
	...
	TEXT[3]				= 	"u banditù za Sekobovým statkem";
...
{   
 	...
 	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "15 krátkých meèù");
					Doc_PrintLines	( nDocID,  0, "20 rapírù"					);
					Doc_PrintLines	( nDocID,  0, "25 bochníkù chleba");
					Doc_PrintLines	( nDocID,  0, "15 lahví vína"					);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "To bylo naposledy."					);
					Doc_PrintLines	( nDocID,  0, "Už to zaèíná být moc o hubu."					);
					Doc_PrintLines	( nDocID,  0, "");
	...
	{
		B_LogEntry (TOPIC_Addon_BanditTrader,"Našel jsem dokument s dùkazem, že zbrojíøem, kterého hledám, je Fernando."); 
...
INSTANCE ItWr_Vatras2Saturas_FindRaven	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "Vatrasùv dopis Saturasovi";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLine	( nDocID,  0, "Milý Saturasi."					);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že brzy dosáhneš svého cíle. Tady ve mìstì je poslední dobou klid, ale bojím se, aby to nebylo jen ticho pøed bouøí. Mìl by sis pospíšit, potøebuji tì tu."					);
					Doc_PrintLines	( nDocID,  0, "Prostudoval jsem tvé záznamy a mohu potvrdit tvé podezøení. Ten dopis zcela jistì poukazuje na nìjakou kulturu, která uctívala Adana. Buï opatrný a nenechej se zaslepit falešnými proroky."					);
					Doc_PrintLines	( nDocID,  0, "Zmizelí obèané Khorinidu byli odvleèeni k bývalému rudobaronovi Havranovi tam, kam pravdìpodobnì ústí onen portál."					);
					Doc_PrintLines	( nDocID,  0, "A ještì jedna vìc: posílám ti pomoc. Doruèitel této zprávy je opravdu výjimeèný èlovìk. Má v oèích zvláštní svit - úplnì jistì to øíci nemohu, ale mohl by to být ON, i když jsme si ho pøedstavovali ponìkud jinak."					);
	...
	INSTANCE  ItAm_Addon_WispDetector (C_Item)  
{
	...
	description		=  "Amulet pátrací bludièky";
...
INSTANCE ItMi_Addon_Steel_Paket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Tenhle balík je POØÁDNÌ tìžký.";
	TEXT[3]				=	"Obsahuje znaèné množství oceli.";
...
func void Use_StonePlateCommon ()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		...
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kasta váleènická na naše hlavy hnìv boží seslala.");	
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Quarhodronùv následovník RHADEMES byl zapuzen, leè zlomoc jeho i z místa jeho vyhnanství k nám pronikla.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Byli jsme proti ní zcela bezmocní.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kéž JHERENDARA hnìv ADANÙV sklátí!");	
	}
...
INSTANCE ItMi_Addon_Stone_01(C_Item) //Esteban und Thorus  //Joly:Die Banditen benutzen diese Steintafeln als Zahlungsmittel.
{
	...
	description			= 	name;
	TEXT[2]				= 	"Tabulka z èerveného kamene.";
	TEXT[3]				=	"";
};
//---------------------------------------------------------------------------
FUNC VOID Use_Addon_Stone_01()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "My, tøi poslední vùdcové z Rady pìti, jali jsme se v síních chrámových rozlièné pasti rozmístiti a vchod ukrýti, by meè již nikdy svìtla denního nespatøil.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_05()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "První past vytvoøil jsem já, jenž proti rozhodnutí Tøí jsem brojil. A pouze já vchodu správného znám.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_03()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Druhou past KHARDIMON sestrojil. A pouze on, jenž cestu Svìtla následuje, do tøetí komnaty nakonec vejde.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_04()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Tøetí past jest dílem QUARHODRONOVÝM. A pouze on ví, jak portálu otevøíti.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_02()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "A byl to též Quarhodron, kdo s pomocí KHARDIMONOVOU vnìjší bránu chrámu uzavøel. Nikdo z nich obøad nepøežil.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Zùstal jsem pouze já, bych vám celý pøíbìh vyprávìl.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Doufám, že RHADEMES navìky shnije v chrámu!");
	}
	else
	{
...
INSTANCE ItSE_Addon_FrancisChest	(C_Item)
{
	...
	TEXT[0]				= 	"Truhlice je velmi tìžká.";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"";
	TEXT[3]				= 	"";
	TEXT[4]				= 	"";
	...
	FUNC VOID UseFrancisAbrechnung_Mis()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Obchodní loï 'Moøská panna'"	);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Celková koøist: 14560 zlatých");
					Doc_PrintLine	( nDocID,  0, "-----------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Posádka: 9840");
					Doc_PrintLine	( nDocID,  0, "Dùstojníci: 2500");
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Obchodní loï 'Miriam'"	);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Celková koøist: 4890 zlatých");
					Doc_PrintLine	( nDocID,  0, "-----------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Posádka: 2390");
					Doc_PrintLine	( nDocID,  0, "Dùstojníci: 500");
					...
					Doc_PrintLine	( nDocID,  1, "Obchodní loï 'Nico'"	);
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, "Celková koøist: 9970");
					Doc_PrintLine	( nDocID,  1, "----------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Posádka: 5610");
					Doc_PrintLine	( nDocID,  1, "Dùstojníci: 1500");
					...
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Obchodní loï 'Maria'"	);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, "Celková koøist: 7851 zlatých");
					Doc_PrintLine	( nDocID,  1, "----------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Posádka: 4400");
					Doc_PrintLine	( nDocID,  1, "Dùstojníci: 750");
					Doc_PrintLine	( nDocID,  1, "Kapitán: 1000");
					Doc_PrintLine	( nDocID,  1, "----------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
...
INSTANCE ITWR_Addon_GregsLogbuch_Mis (C_ITEM)
{
	...
	scemeName				=	"MAP";
	description				= 	"Gregùv palubní deník";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseGregsLogbuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					//1.Seite
 					...
 					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1   		);  //  0 -> margins are in pixels
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  0, "Už mám té pláže plné zuby. Všude nic než písek. Už se ani poøádnì nevyspím, protože mì všecko svìdí. Havran už by mìl koneènì navalit zlato za vìznì, abysme mohli znova vyrazit na moøe. Musím si s tím nafrnìným kreténem promluvit.");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Co si to ten šmejd dovoluje? Naøídil tìm svejm smradlavejm vlezdoprdelkùm, aby mì nepustili do brány! Tak tohle si šerednì odskáèe! Vlastníma rukama udìlám z toho jeho patolízala Bloodwyna žrádlo pro ryby!"	);
					Doc_PrintLines	( nDocID,  0, "Jestli brzo nezaplatí, pøestanu si brát servítky.");
					

					//2.Seite
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  1, "Ti banditi zaèínají být drzí jak opice. Ještì poøád mi nezaplatili za poslední dodávku. Asi toho ještì dost nevíme. Musím zjistit, co tu ten Havran vlastnì chce."	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Vezmu si vìtšinu posádky na pevninu a zmocníme se rudy. Francis s pár lidma zatím zùstane tady a opevní náš tábor."	);
					Doc_PrintLines	( nDocID,  1, "Abysme neztráceli èas, dal jsem Bonesovi brnìní. Proplíží se do tábora banditù a zjistí, co má Havran za lubem.");
...
INSTANCE ITKE_Addon_Heiler (C_Item)
{
	...
	description			= 	"Klíè kamenného strážce";
	
	TEXT[2]				=	"Otevírá truhlici";
	TEXT[3]				=   "v podivné staré stavbì v bažinách";
...
instance ItWR_Addon_TreasureMap (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "";
	TEXT[1]		= "Na mapì jsou vyznaèena nìkterá místa.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
{
	name 				=	"Poškrábaný støíbrný pohár";
