...
INSTANCE ItWr_SaturasFirstMessage_Addon_Sealed	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "Ta wiadomość została dokładnie zapieczętowana.";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Dotarliśmy do portalu. Miałeś rację."					);
					Doc_PrintLines	( nDocID,  0, "Są znaki wskazujące na to, że naprawdę czcili Adanosa. Sprawdź, proszę, moje notatki i potwierdź nasze ustalenia."					);
					Doc_PrintLines	( nDocID,  0, "Od kilku dni nie widzieliśmy tych dziwnych kamiennych istot."					);
					Doc_PrintLines	( nDocID,  0, "Ziemia wciąż trzęsie się z niewiadomych powodów."					);
					Doc_PrintLines	( nDocID,  0, "Sądzę, że nasze badania wkrótce wskażą nam, co może być tego przyczyną."					);
					Doc_PrintLines	( nDocID,  0, "Znaleziony przez nas ornament ma większe znaczenie, niż wcześniej sądziliśmy. Wydaje nam się, że jest to jakiś klucz-artefakt. Niestety, nie jest kompletny. Musimy go dokładniej zbadać."					);
					Doc_PrintLines	( nDocID,  0, "Wyślij jednego z członków Wodnego Kręgu, by nam go dostarczył. Jeśli to możliwe, nie przysyłaj Cavalorna."					);
					Doc_PrintLines	( nDocID,  0, "Poprosiłem go już o dostarczenie Ci tego listu. Wydaje mi się, że już wystarczająco nam pomógł."					);
					Doc_PrintLines	( nDocID,  0, "Mam nadzieję, że postępujemy właściwie."					);
		...
		&& (MIS_Addon_Cavalorn_Letter2Vatras != LOG_SUCCESS)
		{
			...
			B_LogEntry (TOPIC_Addon_KDW,"Zabrałem bandycie wiadomość, którą Cavalorn miał dostarczyć Vatrasowi, Magowi Wody. To teraz moje zadanie."); 
		...
		if (SC_KnowsRanger == FALSE)
		{
			...
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Istnieje stowarzyszenie zwące się Wodnym Kręgiem. Wygląda na to, że rządzą nim Magowie Wody."); 
		};
		...
		if (SC_IsRanger == FALSE)
		{
			Log_CreateTopic (TOPIC_Addon_RingOfWater, LOG_MISSION);
			Log_SetTopicStatus(TOPIC_Addon_RingOfWater, LOG_RUNNING);
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Cavalorn należy do Wodnego Kręgu."); 
		};
...
INSTANCE  ItMi_Ornament_Addon (C_ITEM)
{
	...
	TEXT	[0]			=	"Fragment dużego, ozdobnego pierścienia";
...
instance ItWr_Map_NewWorld_Ornaments_Addon (C_Item)
{
	...
	TEXT[1]		= "gdzie według Nefariusa znajdują się brakujące ornamenty.";
	TEXT[2]		= "";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
instance ItWr_Map_NewWorld_Dexter (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Skip zaznaczył miejsce,";
	TEXT[1]		= "gdzie mogę znaleźć szefa bandytów,";
	TEXT[2]		= "Dextera.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
PROTOTYPE Rangerring_Prototype (C_ITEM)
{
	...
	description				= "Symbol przynależności do Wodnego Kręgu";
...
{
	TEXT[1]					= "Ten pierścień należy do Laresa.";
};

//PCs Ring
INSTANCE ItRi_Ranger_Addon (Rangerring_Prototype)
{
	TEXT[1]					= "To mój pierścień.";
};
//Lance Ring
INSTANCE ItRi_LanceRing (Rangerring_Prototype)
{
	TEXT[1]					= "Ten pierścień należy do Lance'a.";
};
...
INSTANCE ItMi_PortalRing_Addon (C_Item)
{
	...
	description			= 	"Ten pierścień otwiera portal";
...
{
	name 				=	"List polecający od Martina";
	...
	scemeName			=	"MAP";
	description			=  	"List polecający dla Lorda Andre";
	
	TEXT[2]				= 	"powinien mi umożliwić";
	TEXT[3]				= 	"wstąpienie w szeregi straży.";
...
{   
	...
	var int nDocID;
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLines	(nDocID,  0, "Wraz z tym listem przysyłam Ci nowego rekruta do naszej straży.");
					Doc_PrintLines	(nDocID,  0, "Udowodnił swoją przydatność, wykonując trudne i odpowiedzialne zadania.");
					Doc_PrintLines	(nDocID,  0, "Jestem pewien, że będzie się dobrze spisywał, broniąc króla oraz mieszkańców tego miasta.");
					Doc_PrintLine	(nDocID,  0, "Innosie, chroń króla."							);
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
 	...
 	&& (MIS_Addon_Vatras_WhereAreMissingPeople != 0)
 	{
		...
		B_LogEntry (TOPIC_Addon_WhoStolePeople,"Teraz mam to na piśmie. Kruk, dawny magnat, stoi za porwaniami mieszkańców Khorinis. Jego kryjówka znajduje się gdzieś za górami, w północno-wschodniej części Khorinis. Vatras powinien zobaczyć ten dokument."); 
	...
	nDocID = 	Doc_Create		();
				...
				Doc_PrintLine	(nDocID,  0, ""														);
				Doc_PrintLines	(nDocID,  0, "Kiedy jeszcze byłem magnatem, można było bardziej na tobie polegać.");
				Doc_PrintLines	(nDocID,  0, "Jeśli nie zdołasz porwać więcej ludzi z miasta i przysłać ich do mnie, to będziemy mieli niezły problem z chłopakami w naszej kryjówce.");
				Doc_PrintLines	(nDocID,  0, "Potrzebuję tu więcej niewolników albo chłopaki zaczną się buntować. Chyba nie muszę ci mówić, co to znaczy, prawda?");
				Doc_PrintLines	(nDocID,  0, "Jestem bardzo bliski dostania się do świątyni. Nie mogę sobie pozwolić na żadne, absolutnie żadne kłopoty.");
				Doc_PrintLines	(nDocID,  0, "A, jest jeszcze jeden problem:"								);
				Doc_PrintLines	(nDocID,  0, "Prędzej czy później będziemy musieli znaleźć drogę przez wysokie góry w północno-wschodnim Khorinis. Piraci nie zapewnią nam transportu, kiedy przestaniemy im płacić.");
				Doc_PrintLine	(nDocID,  0, ""														);
...
{
	name 				=	"List polecający od Vatrasa";
	...
	scemeName			=	"MAP";
	description			=  	"List polecający od Vatrasa";
	
	TEXT[2]				= 	"powinien pomóc mi";
	TEXT[3]				= 	"wejść do klasztoru Magów Ognia";
	TEXT[4]				= 	"bez zapłaty.";
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Właśnie poinformowano mnie, że aby dołączyć do waszego stowarzyszenia, trzeba wnieść opłatę.");
					Doc_PrintLines	( nDocID,  0, "List ten dostarczy oddany człowiek, który chciałby zostać nowicjuszem w waszych szeregach."					);
...
{   
	...
	B_LogEntry (TOPIC_Addon_Lucia,"Lucia napisała list pożegnalny dla Elvricha. Z pewnością będzie chciał go przeczytać."); 
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Najdroższy Elvrichu,");
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Żadne słowa nie oddadzą tego, jak mi przykro.");
					Doc_PrintLines	( nDocID,  0, "Wiem, że ciężko będzie ci to zrozumieć. Doszłam jednak do wniosku, że lepiej będzie dla nas obojga, jeśli znajdziesz sobie porządniejszą dziewczynę."					);
					Doc_PrintLines	( nDocID,  0, "Udaję się w miejsce, z którego już nie wrócę. Zapomnij o mnie. Nieprzyzwoita dziewczyna jak ja nie zasługuje na takiego dobrego chłopaka. Żegnaj.");
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItRi_Addon_BanditTrader(C_Item)	//Händlergildenring
{
	...
	description				= 	"Rzeźbiony pierścień";
...
INSTANCE ItWr_Addon_BanditTrader		(C_Item)
{
	...
	TEXT[2]				= 	"Zabrałem ten list bandytom";
	TEXT[3]				= 	"za farmą Sekoba.";
...
{   
 	...
 	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "20 pałaszy"					);
					Doc_PrintLines	( nDocID,  0, "25 bochenków chleba");
					Doc_PrintLines	( nDocID,  0, "15 butelek wina"					);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "To już ostatni raz."					);
					Doc_PrintLines	( nDocID,  0, "Zaczyna robić się za gorąco."					);
					Doc_PrintLines	( nDocID,  0, "");
	...
	{
		B_LogEntry (TOPIC_Addon_BanditTrader,"Znalazłem dokument potwierdzający, że Fernando jest handlarzem bronią, którego szukam."); 
...
INSTANCE ItWr_Vatras2Saturas_FindRaven	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "Wiadomość Vatrasa dla Saturasa";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Mam nadzieję, że zbliżasz się do końca. Sytuacja w mieście ostatnio się uspokoiła. Jednak obawiam się, że to cisza przed burzą. Pospiesz się, proszę. Potrzebuję cię tutaj."					);
					Doc_PrintLines	( nDocID,  0, "Przestudiowałem zapiski i potwierdzam twoje podejrzenia. Symbole rzeczywiście wskazują na kulturę, która czciła Adanosa. Dlatego musimy pozostać czujni i nie dać się zaślepić słowom fałszywych proroków."					);
					Doc_PrintLines	( nDocID,  0, "Zaginieni mieszkańcy zostali porwani przez Kruka, byłego magnata. Są w rejonie, do którego z pewnością zaprowadzi nas portal."					);
					Doc_PrintLines	( nDocID,  0, "Jeszcze jedno – przysyłam Ci pomocnika. Osoba, która przyniesie ten list, jest dość wyjątkowa. Za każdym razem, gdy na ciebie spogląda, ma w oku ten charakterystyczny błysk. Nie jestem jeszcze tego całkiem pewien, ale możliwe, że to ON, nawet jeśli inaczej go sobie wyobrażaliśmy."					);
					Doc_PrintLines	( nDocID,  0, "Sprawdź go. Wierzę, że mam rację."					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "     Vatras"					);
					Doc_Show		( nDocID );
	...
	INSTANCE  ItAm_Addon_WispDetector (C_Item)  
{
	...
	description		=  "Amulet szukającego ognika";
...
{	
	...
	TEXT[1]			= "Pierścień zdobiony jest mnóstwem pięknych run.";
...
INSTANCE ItMi_Addon_Steel_Paket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Ta paczka jest NAPRAWDĘ ciężka.";
	TEXT[3]				=	"Zawiera sporą bryłę stali.";
...
func void Use_StonePlateCommon ()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		...
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kasta wojowników wywołała gniew naszego boga.");	
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "RADEMES, następca QUARHODRONA, został wypędzony. Ale jego złowroga moc dotarła do nas nawet z jego miejsca wygnania.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Byliśmy wobec niej bezsilni.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Na JARKENDAR spadł gniew ADANOSA!");	
	}
...
FUNC VOID Use_Addon_Stone_01()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "My, trzej pozostali przywódcy Rady Pięciu, założyliśmy pułapki w świątyni i zapieczętowaliśmy jej wejście. Oby miecz na zawsze już pozostał w mroku.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_05()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Ja, który sprzeciwiłem się rozkazom trójki, zbudowałem pierwszą pułapkę. I tylko ja znam właściwe wrota.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_03()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "KHARDIMON przygotował drugą pułapkę. Tylko ten, kto nie zejdzie ze ścieżki światła, dotrze do trzeciej komnaty.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_04()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Trzecia pułapka została zbudowana przez QUARHODRONA i jedynie on wie, jak otworzyć bramę.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_02()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "QUARHODRON był też tym, który z pomocą KHARDIMONA zapieczętował zewnętrzną bramę świątyni. Żaden z nich nie przeżył tego rytuału.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Jedynie ja pozostałem, aby być świadkiem tych wydarzeń.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Oby RADEMES przez całą wieczność gnił wewnątrz świątyni!");
	}
	else
	{
...
INSTANCE ItSE_Addon_FrancisChest	(C_Item)
{
	...
	TEXT[0]				= 	"Skrzynia jest dosyć ciężka.";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"";
	TEXT[3]				= 	"";
	TEXT[4]				= 	"";
...
FUNC VOID FrancisChest ()
{
		...
		Snd_Play ("Geldbeutel");
		Print ("Zdobyłem stertę różnych rzeczy.");
};
...
INSTANCE ITWR_Addon_FrancisAbrechnung_Mis (C_ITEM)
{
	...
	FUNC VOID UseFrancisAbrechnung_Mis()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Suma łupów: 14560");
					Doc_PrintLine	( nDocID,  0, "-----------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Załoga       : 9840");
					Doc_PrintLine	( nDocID,  0, "Oficerowie   : 2500");
					Doc_PrintLine	( nDocID,  0, "Kapitan      : 1200");
					Doc_PrintLine	( nDocID,  0, "----------------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "moja działka  : 2220");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Fregata 'Miriam'"	);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Suma łupów: 4890");
					Doc_PrintLine	( nDocID,  0, "----------------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "Załoga       : 2390");
					Doc_PrintLine	( nDocID,  0, "Oficerowie   : 500");
					Doc_PrintLine	( nDocID,  0, "Kapitan      : 500");
					Doc_PrintLine	( nDocID,  0, "----------------------------"	);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "moja działka  : 1000");
					Doc_PrintLine	( nDocID,  0, "");
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, "Suma łupów: 9970");
					Doc_PrintLine	( nDocID,  1, "----------------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Załoga       : 5610");
					Doc_PrintLine	( nDocID,  1, "Oficerowie   : 1500");
					Doc_PrintLine	( nDocID,  1, "Kapitan      : 1000");
					Doc_PrintLine	( nDocID,  1, "----------------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "moja działka  : 1860");
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Statek kupiecki 'Maria'"	);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, "Suma łupów: 7851");
					Doc_PrintLine	( nDocID,  1, "----------------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Załoga       : 4000");
					Doc_PrintLine	( nDocID,  1, "Oficerowie   : 750");
					Doc_PrintLine	( nDocID,  1, "Kapitan      : 1000");
					Doc_PrintLine	( nDocID,  1, "----------------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "moja działka  : 1701");
					Doc_PrintLine	( nDocID,  1, "");
	...
	FUNC VOID UseGregsLogbuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					//1.Seite
 					...
 					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1   		);  //  0 -> margins are in pixels
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  0, "Mam już dość tej plaży! Wokół tylko piasek. Nie mogę nawet spać, bo mnie wszędzie swędzi. Czas, żeby Kruk w końcu sypnął złotem za więźniów i żebyśmy w końcu stąd odpłynęli. Coś mi się zdaje, że muszę poważnie pogadać z tym bufonem.");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Co ten pacan sobie wyobraża? Kazał odprawić mnie z kwitkiem swoim śmierdzącym najemnikom. Wkrótce się dowie, z kim ma do czynienia. Na początek załatwię tego służalca Bloodwyna."	);
					Doc_PrintLines	( nDocID,  0, "Jeśli wkrótce nie zapłaci, będę musiał zrobić się naprawdę niemiły.");
					

					//2.Seite
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  1, "Bandyci się rozzuchwalili. Wciąż nie zapłacili za ostatni transport. Po prostu za mało wiemy. Muszę wybadać, co planuje Kruk."	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Wezmę większość załogi na kontynent i zabezpieczymy rudę. Francis zostanie z paroma ludźmi i ufortyfikuje obóz."	);
					Doc_PrintLines	( nDocID,  1, "Aby mieć pewność, że każdy ma zajęcie, dałem Bonesowi zbroję. Przekradnie się w niej do obozu bandytów i dowie się, co knuje Kruk.");
...
INSTANCE ITKE_Addon_Heiler (C_Item)
{
	...
	description			= 	"Klucz kamiennego strażnika";
...
{	
	...
	TEXT[2]		= "Klucz do świątyni Adanosa";
...
instance ItWR_Addon_TreasureMap (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "";
	TEXT[1]		= "Na mapie są zaznaczone jakieś miejsca.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
INSTANCE ItMi_Addon_GregsTreasureBottle_MIS	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "W środku jest kawałek papieru";
