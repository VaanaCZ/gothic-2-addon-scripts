...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Dostali jsme se až k portálu. Měl jsi pravdu,"					);
					Doc_PrintLines	( nDocID,  0, "opravdu to vypadá, že šlo o stoupence Adanovy. Prosím, vezmi si k ruce mé poznámky a ještě jednou to ověř."					);
					Doc_PrintLines	( nDocID,  0, "Už pár dní jsme nezahlédli žádnou z těch podivných kamenných bytostí."					);
					Doc_PrintLines	( nDocID,  0, "Ale země se přesto stále bezdůvodně třese."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že naše studia nám brzy přinesou další osvícení."					);
					Doc_PrintLines	( nDocID,  0, "Ten ornament, který jsme našli, je mnohem důležitější, než jsme si prve mysleli. Nejspíš se jedná o nějaký klíčový artefakt. Ale bohužel není dokončen - musíme jej dále studovat a zjistit, k čemu skutečně sloužil."					);
					Doc_PrintLines	( nDocID,  0, "Vyšli prosím některé z dítek 'kruhu vody', aby nám jej přineslo. Ale pokud možno tím nepověřuj Cavalorna."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že větší úkol než doručení tohoto dopisu by byl nad jeho síly."					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že děláme správnou věc."					);
		...
		&& (MIS_Addon_Cavalorn_Letter2Vatras != LOG_SUCCESS)
		{
			...
			B_LogEntry (TOPIC_Addon_KDW,"Tu zprávu, kterou měl Cavalorn doručit vodnímu mágovi Vatrasovi, jsem sebral jednomu banditovi. Takže teď je to na mně."); 
		...
		if (SC_KnowsRanger == FALSE)
		{
			...
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Jedna zdejší komunita si říká 'kruh vody' a v jejím čele podle všeho stojí vodní mágové."); 
		};
		...
		if (SC_IsRanger == FALSE)
		{
			Log_CreateTopic (TOPIC_Addon_RingOfWater, LOG_MISSION);
			Log_SetTopicStatus(TOPIC_Addon_RingOfWater, LOG_RUNNING);
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Cavalorn patřil právě k tomutu 'kruhu vody'."); 
		};
...
instance ItWr_Map_NewWorld_Ornaments_Addon (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Na této mapě jsou vyznačena místa,";
	TEXT[1]		= "kde se podle Nefaria nacházejí chybějící ornamenty.";
	TEXT[2]		= "";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
instance ItWr_Map_NewWorld_Dexter (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Skip mi na mapě vyznačil,";
...
{
	TEXT[1]					= "Tento prsten patři Laresovi";
};

//PCs Ring
INSTANCE ItRi_Ranger_Addon (Rangerring_Prototype)
{
	TEXT[1]					= "Tento prsten patří mně";
};
//Lance Ring
...
INSTANCE ItWr_Martin_MilizEmpfehlung_Addon		(C_Item)
{
	...
	TEXT[3]				= 	"při přijetí do řad domobrany.";
...
{   
	...
	var int nDocID;
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLines	(nDocID,  0, "Už několikrát mi pomohl v různých obtížných a důležitých záležitostech.");
					Doc_PrintLines	(nDocID,  0, "Proto se domnívám, že stejně dobře bude chránit krále i občany tohoto krásného města.");
					Doc_PrintLine	(nDocID,  0, "Nechť Innos žehná králi!"							);
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
 	...
 	&& (MIS_Addon_Vatras_WhereAreMissingPeople != 0)
 	{
		...
		B_LogEntry (TOPIC_Addon_WhoStolePeople,"Tak teď už to mám černé na bílém. Za těmi únosy khorinidských obyvatel vězí bývalý rudobaron Havran. Skrývá se kdesi za horami severovýchodně od města. Tento dokument by měl vidět Vatras."); 
	...
	nDocID = 	Doc_Create		();
				...
				Doc_PrintLine	(nDocID,  0, ""														);
				Doc_PrintLines	(nDocID,  0, "Když jsem byl ještě rudobaronem, nebyl jsi tak děsně nespolehlivý!");
				Doc_PrintLines	(nDocID,  0, "Jestli v nejbližší době neuneseš z Khorinidu nějaký další lidi a nepošleš mi je, čekaj tě vážný potíže s hochy z naší základny.");
				Doc_PrintLines	(nDocID,  0, "Nutně potřebuju další otroky, nebo budu mít brzo na krku vzpouru. A ty určitě víš, co to znamená, viď?");
				Doc_PrintLines	(nDocID,  0, "Už jsem se skoro dostal do chrámu, takže teď si rozhodně nemůžu dovolit žádný takovýhle potíže.");
				Doc_PrintLines	(nDocID,  0, "A ještě jeden problém:"								);
				Doc_PrintLines	(nDocID,  0, "Dřív nebo pozdějc se musíme dostat přes ty vysoký hory severovýchodně od Khorinidu. Piráti už nás moc dlouho přepravovat nebudou, jestli jim přestaneme platit.");
				Doc_PrintLine	(nDocID,  0, ""														);
...
INSTANCE ItWr_Vatras_KDFEmpfehlung_Addon		(C_Item)
{
	...
	TEXT[2]				= 	"S pomocí tohoto dopisu bych se měl";
	TEXT[3]				= 	"dostat do kláštera ohnivých mágů,";
	TEXT[4]				= 	"aniž bych musel něco platit.";
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Následovníci ohně.");
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Právě jsem se dozvěděl, že za vstup do vašich řad vyžadujete nějaký poplatek.");
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Nedokážeš si představit, s jakou bolestí ti tohle píšu.");
					Doc_PrintLines	( nDocID,  0, "Vím, že to nepochopíš, ale hodně jsem přemýšlela a došla jsem k závěru, že bude pro nás oba lepší, když si najdeš nějakou slušnější ženu, než jsem já."					);
					Doc_PrintLines	( nDocID,  0, "Odcházím a už se nevrátím. Zapomeň na mě. Holka jako já se pro slušnýho chlapa jako ty prostě nehodí. Sbohem.");
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItRi_Addon_BanditTrader(C_Item)	//Händlergildenring
{
	...
	TEXT[2]					= 	"námořně-obchodního cechu Araxa";
...
{   
	...
	{
		B_LogEntry (TOPIC_Addon_BanditTrader,"Našel jsem dokument s důkazem, že zbrojířem, kterého hledám, je Fernando."); 
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLine	( nDocID,  0, "Milý Saturasi."					);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že brzy dosáhneš svého cíle. Tady ve městě je poslední dobou klid, ale bojím se, aby to nebylo jen ticho před bouří. Měl by sis pospíšit, potřebuji tě tu."					);
					Doc_PrintLines	( nDocID,  0, "Prostudoval jsem tvé záznamy a mohu potvrdit tvé podezření. Ten dopis zcela jistě poukazuje na nějakou kulturu, která uctívala Adana. Buď opatrný a nenechej se zaslepit falešnými proroky."					);
					Doc_PrintLines	( nDocID,  0, "Zmizelí občané Khorinidu byli odvlečeni k bývalému rudobaronovi Havranovi tam, kam pravděpodobně ústí onen portál."					);
					Doc_PrintLines	( nDocID,  0, "A ještě jedna věc: posílám ti pomoc. Doručitel této zprávy je opravdu výjimečný člověk. Má v očích zvláštní svit - úplně jistě to říci nemohu, ale mohl by to být ON, i když jsme si ho představovali poněkud jinak."					);
...
INSTANCE ItMi_Addon_Steel_Paket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Tenhle balík je POŘÁDNĚ těžký.";
...
func void Use_StonePlateCommon ()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		...
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kéž JHERENDARA hněv ADANŮV sklátí!");	
	}
...
FUNC VOID Use_Addon_Stone_04()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Třetí past jest dílem QUARHODRONOVÝM. A pouze on ví, jak portálu otevříti.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_02()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "A byl to též Quarhodron, kdo s pomocí KHARDIMONOVOU vnější bránu chrámu uzavřel. Nikdo z nich obřad nepřežil.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Zůstal jsem pouze já, bych vám celý příběh vyprávěl.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Doufám, že RHADEMES navěky shnije v chrámu!");
	}
	else
	{
...
INSTANCE ItSE_Addon_FrancisChest	(C_Item)
{
	...
	TEXT[0]				= 	"Truhlice je velmi těžká.";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"";
	TEXT[3]				= 	"";
	TEXT[4]				= 	"";
...
INSTANCE ITWR_Addon_FrancisAbrechnung_Mis (C_ITEM)
{
	...
	FUNC VOID UseFrancisAbrechnung_Mis()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, "Celková kořist: 9970");
					Doc_PrintLine	( nDocID,  1, "----------------------"	);
					Doc_PrintLine	( nDocID,  1, "");
	...
	FUNC VOID UseGregsLogbuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					//1.Seite
 					...
 					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1   		);  //  0 -> margins are in pixels
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  0, "Už mám té pláže plné zuby. Všude nic než písek. Už se ani pořádně nevyspím, protože mě všecko svědí. Havran už by měl konečně navalit zlato za vězně, abysme mohli znova vyrazit na moře. Musím si s tím nafrněným kreténem promluvit.");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Co si to ten šmejd dovoluje? Nařídil těm svejm smradlavejm vlezdoprdelkům, aby mě nepustili do brány! Tak tohle si šeredně odskáče! Vlastníma rukama udělám z toho jeho patolízala Bloodwyna žrádlo pro ryby!"	);
					...
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  1, "Ti banditi začínají být drzí jak opice. Ještě pořád mi nezaplatili za poslední dodávku. Asi toho ještě dost nevíme. Musím zjistit, co tu ten Havran vlastně chce."	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Vezmu si většinu posádky na pevninu a zmocníme se rudy. Francis s pár lidma zatím zůstane tady a opevní náš tábor."	);
					Doc_PrintLines	( nDocID,  1, "Abysme neztráceli čas, dal jsem Bonesovi brnění. Proplíží se do tábora banditů a zjistí, co má Havran za lubem.");
...
INSTANCE ITKE_Addon_Heiler (C_Item)
{
	...
	TEXT[2]				=	"Otevírá truhlici";
	TEXT[3]				=   "v podivné staré stavbě v bažinách";
...
instance ItWR_Addon_TreasureMap (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "";
	TEXT[1]		= "Na mapě jsou vyznačena některá místa.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
{
	name 				=	"Poškrábaný stříbrný pohár";
