...
INSTANCE ItWr_SaturasFirstMessage_Addon_Sealed	(C_Item)
{
	...
	scemeName			=	"MAPSEALED";
	description			= 	name;
	TEXT[2]				=   "Tato zpráva byla peèlivê zapeèetêna.";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Dostali jsme se až k portálu. Mêl jsi pravdu,"					);
					Doc_PrintLines	( nDocID,  0, "opravdu to vypadá, že šlo o stoupence Adanovy. Prosím, vezmi si k ruce mé poznámky a ještê jednou to ovêâ."					);
					Doc_PrintLines	( nDocID,  0, "Už pár dní jsme nezahlédli žádnou z têch podivných kamenných bytostí."					);
					Doc_PrintLines	( nDocID,  0, "Ale zemê se pâesto stále bezdùvodnê tâese."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že naše studia nám brzy pâinesou další osvícení."					);
					Doc_PrintLines	( nDocID,  0, "Ten ornament, který jsme našli, je mnohem dùležitêjší, než jsme si prve mysleli. Nejspíš se jedná o nêjaký klíèový artefakt. Ale bohužel není dokonèen - musíme jej dále studovat a zjistit, k èemu skuteènê sloužil."					);
					Doc_PrintLines	( nDocID,  0, "Vyšli prosím nêkteré z dítek 'kruhu vody', aby nám jej pâineslo. Ale pokud možno tím nepovêâuj Cavalorna."					);
					Doc_PrintLines	( nDocID,  0, "Myslím, že vêtší úkol než doruèení tohoto dopisu by byl nad jeho síly."					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že dêláme správnou vêc."					);
		...
		&& (MIS_Addon_Cavalorn_Letter2Vatras != LOG_SUCCESS)
		{
			...
			B_LogEntry (TOPIC_Addon_KDW,"Tu zprávu, kterou mêl Cavalorn doruèit vodnímu mágovi Vatrasovi, jsem sebral jednomu banditovi. Takže teë je to na mnê."); 
		...
		if (SC_KnowsRanger == FALSE)
		{
			...
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Jedna zdejší komunita si âíká 'kruh vody' a v jejím èele podle všeho stojí vodní mágové."); 
		};
		...
		if (SC_IsRanger == FALSE)
		{
			Log_CreateTopic (TOPIC_Addon_RingOfWater, LOG_MISSION);
			Log_SetTopicStatus(TOPIC_Addon_RingOfWater, LOG_RUNNING);
			Log_AddEntry (TOPIC_Addon_RingOfWater,"Cavalorn patâil právê k tomutu 'kruhu vody'."); 
		};
...
instance ItWr_Map_NewWorld_Ornaments_Addon (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Na této mapê jsou vyznaèena místa,";
	TEXT[1]		= "kde se podle Nefaria nacházejí chybêjící ornamenty.";
	TEXT[2]		= "";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
...
instance ItWr_Map_NewWorld_Dexter (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "Skip mi na mapê vyznaèil,";
...
INSTANCE ItWr_Martin_MilizEmpfehlung_Addon		(C_Item)
{
	...
	TEXT[2]				= 	"Tento dopis by mi mêl pomoci";
...
{   
	...
	var int nDocID;
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLines	(nDocID,  0, "Už nêkolikrát mi pomohl v rùzných obtížných a dùležitých záležitostech.");
					Doc_PrintLines	(nDocID,  0, "Proto se domnívám, že stejnê dobâe bude chránit krále i obèany tohoto krásného mêsta.");
					Doc_PrintLine	(nDocID,  0, "Nechã Innos žehná králi!"							);
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
 	...
 	&& (MIS_Addon_Vatras_WhereAreMissingPeople != 0)
 	{
		...
		B_LogEntry (TOPIC_Addon_WhoStolePeople,"Tak teë už to mám èerné na bílém. Za têmi únosy khorinidských obyvatel vêzí bývalý rudobaron Havran. Skrývá se kdesi za horami severovýchodnê od mêsta. Tento dokument by mêl vidêt Vatras."); 
	...
	nDocID = 	Doc_Create		();
				...
				Doc_PrintLine	(nDocID,  0, ""														);
				Doc_PrintLines	(nDocID,  0, "Když jsem byl ještê rudobaronem, nebyl jsi tak dêsnê nespolehlivý!");
				Doc_PrintLines	(nDocID,  0, "Jestli v nejbližší dobê neuneseš z Khorinidu nêjaký další lidi a nepošleš mi je, èekaj tê vážný potíže s hochy z naší základny.");
				Doc_PrintLines	(nDocID,  0, "Nutnê potâebuju další otroky, nebo budu mít brzo na krku vzpouru. A ty urèitê víš, co to znamená, vië?");
				Doc_PrintLines	(nDocID,  0, "Už jsem se skoro dostal do chrámu, takže teë si rozhodnê nemùžu dovolit žádný takovýhle potíže.");
				Doc_PrintLines	(nDocID,  0, "A ještê jeden problém:"								);
				Doc_PrintLines	(nDocID,  0, "Dâív nebo pozdêjc se musíme dostat pâes ty vysoký hory severovýchodnê od Khorinidu. Piráti už nás moc dlouho pâepravovat nebudou, jestli jim pâestaneme platit.");
				Doc_PrintLine	(nDocID,  0, ""														);
...
INSTANCE ItWr_Vatras_KDFEmpfehlung_Addon		(C_Item)
{
	...
	TEXT[2]				= 	"S pomocí tohoto dopisu bych se mêl";
	TEXT[3]				= 	"dostat do kláštera ohnivých mágù,";
	TEXT[4]				= 	"aniž bych musel nêco platit.";
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Následovníci ohnê.");
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "Právê jsem se dozvêdêl, že za vstup do vašich âad vyžadujete nêjaký poplatek.");
...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Vím, že to nepochopíš, ale hodnê jsem pâemýšlela a došla jsem k závêru, že bude pro nás oba lepší, když si najdeš nêjakou slušnêjší ženu, než jsem já."					);
					Doc_PrintLines	( nDocID,  0, "Odcházím a už se nevrátím. Zapomeà na mê. Holka jako já se pro slušnýho chlapa jako ty prostê nehodí. Sbohem.");
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItRi_Addon_BanditTrader(C_Item)	//Händlergildenring
{
	...
	TEXT[2]					= 	"námoânê-obchodního cechu Araxa";
...
{   
	...
	{
		B_LogEntry (TOPIC_Addon_BanditTrader,"Našel jsem dokument s dùkazem, že zbrojíâem, kterého hledám, je Fernando."); 
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLine	( nDocID,  0, "Milý Saturasi."					);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Doufám, že brzy dosáhneš svého cíle. Tady ve mêstê je poslední dobou klid, ale bojím se, aby to nebylo jen ticho pâed bouâí. Mêl by sis pospíšit, potâebuji tê tu."					);
					Doc_PrintLines	( nDocID,  0, "Prostudoval jsem tvé záznamy a mohu potvrdit tvé podezâení. Ten dopis zcela jistê poukazuje na nêjakou kulturu, která uctívala Adana. Buë opatrný a nenechej se zaslepit falešnými proroky."					);
					Doc_PrintLines	( nDocID,  0, "Zmizelí obèané Khorinidu byli odvleèeni k bývalému rudobaronovi Havranovi tam, kam pravdêpodobnê ústí onen portál."					);
					Doc_PrintLines	( nDocID,  0, "A ještê jedna vêc: posílám ti pomoc. Doruèitel této zprávy je opravdu výjimeèný èlovêk. Má v oèích zvláštní svit - úplnê jistê to âíci nemohu, ale mohl by to být ON, i když jsme si ho pâedstavovali ponêkud jinak."					);
...
INSTANCE ItMi_Addon_Steel_Paket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Tenhle balík je POÂÁDNÊ têžký.";
...
func void Use_StonePlateCommon ()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		...
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kasta váleènická na naše hlavy hnêv boží seslala.");	
		Doc_PrintLine	( nDocID,  0, "");
		...
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Kéž JHERENDARA hnêv ADANÙV sklátí!");	
	}
...
FUNC VOID Use_Addon_Stone_01()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "My, tâi poslední vùdcové z Rady pêti, jali jsme se v síních chrámových rozlièné pasti rozmístiti a vchod ukrýti, by meè již nikdy svêtla denního nespatâil.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_03()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "Druhou past KHARDIMON sestrojil. A pouze on, jenž cestu Svêtla následuje, do tâetí komnaty nakonec vejde.");
	}
	else
	{
...
FUNC VOID Use_Addon_Stone_02()
{
	...
	if (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines	( nDocID,  0, "A byl to též Quarhodron, kdo s pomocí KHARDIMONOVOU vnêjší bránu chrámu uzavâel. Nikdo z nich obâad nepâežil.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Zùstal jsem pouze já, bych vám celý pâíbêh vyprávêl.");
		Doc_PrintLine	( nDocID,  0, "");
		Doc_PrintLines	( nDocID,  0, "Doufám, že RHADEMES navêky shnije v chrámu!");
	}
	else
	{
...
INSTANCE ItSE_Addon_FrancisChest	(C_Item)
{
	...
	TEXT[0]				= 	"Truhlice je velmi têžká.";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"";
	TEXT[3]				= 	"";
	TEXT[4]				= 	"";
	...
	FUNC VOID UseGregsLogbuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					//1.Seite
 					...
 					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1   		);  //  0 -> margins are in pixels
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  0, "Už mám té pláže plné zuby. Všude nic než písek. Už se ani poâádnê nevyspím, protože mê všecko svêdí. Havran už by mêl koneènê navalit zlato za vêznê, abysme mohli znova vyrazit na moâe. Musím si s tím nafrnêným kreténem promluvit.");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Co si to ten šmejd dovoluje? Naâídil têm svejm smradlavejm vlezdoprdelkùm, aby mê nepustili do brány! Tak tohle si šerednê odskáèe! Vlastníma rukama udêlám z toho jeho patolízala Bloodwyna žrádlo pro ryby!"	);
					...
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLines	( nDocID,  1, "Ti banditi zaèínají být drzí jak opice. Ještê poâád mi nezaplatili za poslední dodávku. Asi toho ještê dost nevíme. Musím zjistit, co tu ten Havran vlastnê chce."	);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Vezmu si vêtšinu posádky na pevninu a zmocníme se rudy. Francis s pár lidma zatím zùstane tady a opevní náš tábor."	);
					Doc_PrintLines	( nDocID,  1, "Abysme neztráceli èas, dal jsem Bonesovi brnêní. Proplíží se do tábora banditù a zjistí, co má Havran za lubem.");
...
INSTANCE ITKE_Addon_Heiler (C_Item)
{
	...
	TEXT[2]				=	"Otevírá truhlici";
	TEXT[3]				=   "v podivné staré stavbê v bažinách";
...
instance ItWR_Addon_TreasureMap (C_Item)
{
	...
	description	= name;
	TEXT[0]		= "";
	TEXT[1]		= "Na mapê jsou vyznaèena nêkterá místa.";
	TEXT[5]		= NAME_Value;
	COUNT[5]	= value;
};
