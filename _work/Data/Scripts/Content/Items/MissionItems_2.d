...
{   
		...
		var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Expedice do Hornického údolí selhala. Utrpêli jsme têžké ztráty a ti, kteâí pâežili, jsou nyní uvêznêni na hradê a obklíèeni skâety.");
					Doc_PrintLines	( nDocID,  0, "Také na nás mnohokrát zaútoèili draci. Znièili a spálili veškerou zem v okolí hradu. S Innosovou pomocí snad vytrváme, dokud nedorazí posily. Stejnê nám nic jiného nezbývá, neboã žádný výpad podniknout nemùžeme.");					
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Nechã nás Innos ochraàuje.");
					Doc_PrintLine	( nDocID,  0, ""					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLines	( nDocID,  0, "Garond souhlasil, že tê propustí - za 1000 zlaãákù.");
					Doc_PrintLines	( nDocID,  0, "Takže jestli máš nêkde ulitý nêjaký zlato, bylo by naèase se s ním vytasit.");
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLine	( nDocID,  0, " "					);
...
{   
	...
	nDocID = 	Doc_Create		()			  ;							// DocManager 
				...
				Doc_PrintLine	( nDocID,  0, "");
				Doc_PrintLines	( nDocID,  0, "Tak nás èeká další den plný kutání. Dnes se chceme probourat skrze zeë a prodolovat se hloubêji do kamene.");
				Doc_PrintLines	( nDocID,  0, "Trápí mê ale jakési neblahé tušení, takže radši všechnu rudu nêkam schovám.");
				Doc_PrintLines	( nDocID,  0, "Diego to tady jako jediný z nás zná. Musím mu vêâit. Urèitê ty bedny ukryje nêkam do bezpeèí - ale nedovolím, aby to udêlal sám.");	
				Doc_PrintLines	( nDocID,  0, "Rudu musíme ochránit za každou cenu, a tak s ním posílám ještê dva rytíâe.");	
				Doc_PrintLine	( nDocID,  0, "");
				Doc_PrintLine	( nDocID,  0, "Aã žije král!");
				Doc_PrintLine	( nDocID,  0, "");
...
INSTANCE ItSe_Olav	(C_Item)
{
	...
	description			= 	"Olavùv mêšec";
	
	TEXT[0]				= 	"";
	TEXT[1]				= 	"";
	TEXT[2]				= 	"V mêšci cinká nêkolik mincí.";
	TEXT[3]				= 	"";
	TEXT[4]				= 	"";
	TEXT[5]				= 	NAME_Value;	COUNT[5]	= value;
};
...
INSTANCE ITKE_RUNE_MIS(C_Item)
{
	...
	TEXT[1]				= 	"Tento klíè patâí ke truhlici";
	TEXT[2]				= 	"pod mostem.";
...
{   
		...
		var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Žihadlo krvavé mouchy obsahuje prudký jed, a proto se ho nikdo, kdo má všech pêt pohromadê, nedotýká."					);
					Doc_PrintLines	( nDocID,  0, "Pokud tedy nezná tajemství, jak je spolehlivê vyjmout.");
					Doc_PrintLines	( nDocID,  0, "Žihadlo lze èistê otevâít, pokud je ostrým nožem po délce opatrnê naâíznete");					
					Doc_PrintLines	( nDocID,  0, "a sloupnete tak svrchní kùži. Pak už jen staèí odstranit tkáà kolem jedových žláz."					);
					Doc_PrintLines	( nDocID,  0, "Zvláštní tekutina z této tkánê má léèivé úèinky,");
					Doc_PrintLines	( nDocID,  0, "ale lidské têlo se vùèi nim bohužel èasem stává imunní."					);
					Doc_PrintLine	( nDocID,  0, "");
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_SetMargins	( nDocID, -1, 50, 50, 50, 50, 1   		);  //  0 -> margins are in pixels
					Doc_PrintLine	( nDocID,  0, " "					);
					Doc_PrintLine	( nDocID,  0, "Pâi platbê nevyrovnaných dluhù lze"					);
					Doc_PrintLine	( nDocID,  0, "pâijmout do zástavy i nêjakou vêc, pokud"					);
					Doc_PrintLine	( nDocID,  0, "její hodnota daný závazek vyrovná."					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "V zástavê: zdobený zlatý kalich"					);
...
{   
		...
		var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Innos stojí pâi mnê i pâi mých bratrech.");					
					Doc_PrintLines	( nDocID,  0, "Každého, kdo se mê pokusí zastavit, podâíznu jak ovci,"					);
					Doc_PrintLines	( nDocID,  0, "neboã On mi v srdci zažehl oheà."					);
					...
					Doc_PrintLines	( nDocID,  0, "vlastnoruènê skolím,");
					Doc_PrintLines	( nDocID,  0, "neboã já jsem Innosovým váleèníkem."					);
...
{   
		...
		var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Opustili jsme tábor. Teë, když bariéra padla, hledáme místo, kde celá ta zkáza zaèala. Snad tam najdeme odpovêdi, po kterých se pídíme už po dlouhá léta. Jen Adanos ví, kam nás cesta zavede."					);
...
{   
		...
		var int nDocID;
			...
			nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLine	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Zaèíná mê to pêknê štvát. Už jsem se tu schovával dost dlouho a jestli jsem dobâe slyšel, bariéra koneènê padla."					);
					Doc_PrintLines	( nDocID,  0, "Teë už mê nikdo nebude hledat. Mám plné zuby téhle hnusné jeskynê i toho zatraceného údolí. Je naèase vrátit se domù.");
					Doc_PrintLines	( nDocID,  0, "");					
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, ""					);
