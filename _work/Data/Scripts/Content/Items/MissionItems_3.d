...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		();
						...
						Doc_PrintLine	(nDocID,  0, ""												);
						Doc_PrintLine	(nDocID,  0, ""												);
						Doc_PrintLines	(nDocID,  0, "Nositel tohoto listu mùže volnê vstoupit do posvátných síní kláštera.");
						Doc_PrintLines	(nDocID,  0, "Nechã je jeho pâáním ohlednê oka Innosova okamžitê vyhovêno.");
						...
						Doc_PrintLines	(nDocID,  0, "V této vêci již neuèiním žádná další prohlášení a oèekávám, že mé rozkazy budete respektovat.");
						Doc_PrintLine	(nDocID,  0, ""												);
						Doc_PrintLine	(nDocID,  0, "Nechã Innos požehná králi!"						);
						Doc_PrintLine	(nDocID,  0, ""												);
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Vèera veèer se mi stala prapodivná vêc. Pâišel ke mnê jeden z têch èerných zakuklencù, o kterých jsme toho v poslední dobê tolik slyšeli. Nebyl jsem moc nervózní, i když z tajemného návštêvníka pâímo èišelo nêjaké zlo a jeho moc byla skoro hmatatelná. Mluvil však tak klidným a pâíjemným hlasem, že jsem mêl pocit, že mu prostê musím vêâit.");

					//2.Seite
					Doc_SetMargins	( nDocID,  -1, 30, 20, 275, 20, 1   		);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Nabídl mi 20 000 zlaãákù, když se postarám, aby byl jeden z têch žoldákù uznán vinným. Prý vím, co tím myslí."	);
					Doc_PrintLines	( nDocID,  1, "Samozâejmê že jsem souhlasil, protože ta chátra si stejnê nic jiného nezaslouží. Nebýt jich, mêl bych se teë mnohem líp. A zlato, které za to dostanu, mi zajistí slušné živobytí až do smrti."					);
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLines	( nDocID,  1, "Jednoho z têch žoldákù už mají pod zámkem, takže své slovo opravdu hravê splním.");
					Doc_Show		( nDocID );
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Urèitê se divíš, že ti píšeme. Donesli jsme totiž mistru Marlasovi všechny úspory a požádali ho, aby tyto âádky napsal za nás. Pâejeme ti do budoucna jen to nejlepší a jako dárek na rozlouèenou ti posíláme obrázek, abys mêl na co vzpomínat za dlouhých veèerù v klášteâe.");
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID,  -1, FONT_Book	   			); 	// -1 -> all pages 
 					Doc_SetMargins	( nDocID,  0,  275, 20, 30, 20, 1   		);  //  0 -> margins are in pixels					
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "...ale když se síla vola spojí s váleènickými ideály, pak se stâezte událostí, které mohou nastat.");
					Doc_PrintLines	( nDocID,  0, "Nespoutaná volská síla a vytrvalost váleèníka dokáže narušit pradávnou rovnováhu sil. Hranice mezi dimenzemi pak zaène slábnout - až nakonec bude tak tenká, že k nám ze svêta stínù snadno proniknou Beliarovy nestvùry.");
					//2.Seite
					Doc_SetMargins	( nDocID,  -1, 30, 20, 275, 20, 1   		);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Války ze starých èasù nám budiž pâíkladem, jak nêco takového dopadá. I když tehdy byla vazba mezi svêty ještê stále pevná, stoupenci zla sem pâinesli smrt a zkázu, pâed kterou lidstvo ochránil pouze Innos a Jeho vyvolený."	);
					Doc_PrintLines	( nDocID,  1, "Innos nás chraà, jestli by se mêla podobná hrozba znovu objevit, protože žádný další Innosùv vyvolený nepâišel už víc jak sto let."					);
					Doc_PrintLine	( nDocID,  1, "");
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Teë jsem zpátky ve mêstê a hledám tê. A až tê najdu, nadêlám z tebe sekanou. Znáš mê už dost dlouho, abys vêdêl, že se jen tak lehce oblbnout nenechám."); 
					Doc_PrintLines	( nDocID,  0, "Poèítám, že když jsi mê házel pâes bariéru, ani ve snu tê nenapadlo, že bych se mohl vrátit. A to byla chyba.");
					Doc_PrintLines	( nDocID,  0, "Vezmu si zpátky, co mi patâí.");
					Doc_PrintLines	( nDocID,  0, "Jestli je ti život milej, tak si sbalíš fidlátka a svùj obchod pâenecháš mnê. Tím si budeme kvit.");				
					Doc_PrintLine	( nDocID,  0, "");
...
INSTANCE ItSe_DiegosTreasure_Mis(C_Item)
{
	...
	description			= 	"Diegùv starý mêšec.";
	TEXT[5]				= 	NAME_Value;	COUNT[5]	= value;
};
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLines	(nDocID,  0, "Podle listiny o velikosti dolù èiní jedno malé královské pole 16 yardù na šíâku a 3 yardy na výšku.");
					Doc_PrintLines	(nDocID,  0, "Královský prospektor není povinen zajistit volný pâístup a ochranu dùlních cel.");
	...
	{
		B_LogEntry (TOPIC_MinenAnteile,"Ten chlapík, který prodal dùlní akcie obchodníkùm, se jmenoval Salandril. Najdu ho nejspíš v horní ètvrti Khorinidu, jestli tedy už pâed spravedlností neutekl nêkam dál."); 
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_SetFont 	( nDocID,  0, FONT_Book		); 	// -1 -> all pages 
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Morgaharde, ty mizerný budižknièemu! Buë vyplníš moje rozkazy, nebo se naštvu a nechám vás všechny zavâít, až zèernáte!");
					Doc_PrintLines	( nDocID,  0, "Musíte to provést dneska v noci, Larius zaèíná nêco tušit.");
					Doc_PrintLines	( nDocID,  0, "Jestli mu kapsy nevybereme teë, pak už mùže být pozdê. Postarám se, aby ses s ním setkal dneska veèer na trhu.");
					Doc_PrintLines	( nDocID,  0, "Ale ne abys z toho dêlal estrádu, jinak tê stráže zatknou, než se staèíš rozkoukat.");
...
INSTANCE ItWr_VinosKellergeister_Mis (C_ITEM)
{
	...
	FUNC VOID Use_VinosKellergeister_Mis()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "...už jsem toho v životê vyzkoušel spoustu, ale tohle ovoce, které mi minulý týden pâivezli z ciziny, pâedèilo všechna má oèekávání..."					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					//Absatz
					Doc_PrintLines	( nDocID,  0, "...šãáva z têchto hroznù je daleko vydatnêjší než cokoliv, co lze získat v têchto konèinách..."					);
					Doc_PrintLines	( nDocID,  0, "");
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "...takže zatím to jde dobâe. Ale poâád mi nedá spát pomyšlení, co by se mohlo stát, kdyby nêkdo našel mùj tajný destilaèní aparát. Radši ani nechci vêdêt, co by mi za to drábové provedli - nakonec by mê mohli hodit pâes bariéru..."	);
					Doc_PrintLine	( nDocID,  1, ""					);
					//Absatz
					Doc_PrintLines	( nDocID,  1, "...zaèínají mít podezâení. Radši se odsud uklidím a chvíli poèkám, až se vêci trochu uklidní a budu se moci opêt vrátit k práci. Hlavnê musím mít jistotu, že po mnê už nejdou...");
					Doc_Show		( nDocID );
};
