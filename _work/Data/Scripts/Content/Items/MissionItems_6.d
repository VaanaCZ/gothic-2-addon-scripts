...
INSTANCE ItSe_XardasNotfallBeutel_MIS	(C_Item)
{
	...
	TEXT[0]				= 	"";
	TEXT[1]				= 	"Wydaje się zawierać";
	TEXT[2]				= 	"coś twardego i jakiś";
	TEXT[3]				= 	"dokument.";
	TEXT[4]				= 	"";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Mój młody przyjacielu - wielce mnie rozczarowała twoja postawa. Jak mogłeś odpłynąć bez Oka Innosa?"					);
					Doc_PrintLines	( nDocID,  0, "Liczyć mogę jedynie na to, że nawet twe niedbalstwo ma granice. W przeciwnym wypadku nie wygnasz Zła ze świata i będę musiał osobiście ocalić cię od twej głupoty."					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Zawiodłem. Płonne me nadzieje na zatrzymanie Inubisa na ścieżce dobra."					);
					Doc_PrintLines	( nDocID,  0, "Na początku sądziłem, że nie żyję, ale w starych kościach Ivana drzemie jeszcze siła."					);
					Doc_PrintLines	( nDocID,  0, "Inubis jednak powstał z martwych i szuka zemsty za wygnanie przez prastary zakon paladynów."					);
					Doc_PrintLines	( nDocID,  0, "Towarzyszy mu wielu wyznawców... Jak wojownik pokroju Inubisa mógł stać się tak zły?"					);
					Doc_PrintLines	( nDocID,  0, "Znalazłem w tej krypcie jego grób, nie wiem jednak, czy zdołam przekazać komuś wieści o moim odkryciu, piszę więc te słowa w nadziei na to, że zostaną odnalezione."					);
					Doc_PrintLines	( nDocID,  0, "Strzeżcie się. Potężny nieprzyjaciel sięga po dusze sprawiedliwych. Inubis nie będzie ostatni..."					);
					Doc_PrintLine	( nDocID,  0, " "					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, "Niech Oko Mocy oświetli twą ścieżkę.");
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_SetMargins	( nDocID, -1, 200, 50, 50, 50, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus
	...
	B_LogEntry (TOPIC_HallenVonIrdorath,"Zwój czarnego maga zawiera słowa KHADOSH EMEM KADAR. Brzmi to jak jakieś zaklęcie, ale do czego służy - i czym jest Oko Mocy?"); 
};
//**********************************************************************************
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages 
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Już od 36 dni czekam na wezwanie. Zaczynam wątpić, czy się doczekam - mimo że zrobiłem wszystko, o co mnie proszono. Latałem dla nich i przynosiłem różne rzeczy jak jakiś lokaj."					);
					//Absatz
					Doc_PrintLines	( nDocID,  0, "Klucznik nakazał mi zamknięcie krat, ale jeszcze się do tego nie zabrałem. Jak tak dalej pójdzie, to każdy będzie sobie mógł po prostu wejść przez bramę...");
					Doc_PrintLines	( nDocID,  0, "Szkoda, że nie pamiętam właściwych kombinacji.");

					//2.Seite
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages 
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Bardzo dawno już nie zakradałem się do wielkiej komnaty i nie mogę się doczekać spotkania z Mistrzem. Ciekaw jestem, czy pozwolą mi na nie, gdy już do nich dołączę."	);
					Doc_PrintLine	( nDocID,  1, ""					);
					//Absatz
					Doc_PrintLines	( nDocID,  1, "Wczoraj podjąłem próbę, ale dotarłem tylko do komnaty z dwoma dźwigniami - nawet nie zdołałem wypróbować kombinacji przełączników w zachodnim skrzydle! Ta świnia pozamykała komnaty. Jutro spróbuję wydostać od niego klucz...");
					Doc_Show		( nDocID );

	B_LogEntry (TOPIC_HallenVonIrdorath,"Dziennik ucznia czarnego maga wspomina o komnatach z dźwigniami, kluczniku, kombinacji przełączników... Trzeba to zapamiętać, nigdy nie wiadomo, kiedy może się przydać."); 
};
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Nic mnie nie obchodzi, czy uda się wam przeleźć przez most z drugiej strony. Dla mnie możecie piec się w piekle."					);
					Doc_PrintLines	( nDocID,  0, "Tak długo, jak jestem w swoich włościach, most będzie podniesiony, a każdego mędrka, który będzie próbował ustrzelić z łuku przełącznik, żeby się tu dostać, osobiście wypatroszę i powieszę na najbliższym drzewie!"	);
					Doc_PrintLines	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
