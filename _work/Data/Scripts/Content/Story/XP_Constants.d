...
const int 	XP_Addon_BringMissingOrnament			= 200;	//fьr jedes der 3 Ornamente
const int 	XP_Addon_Martin_GotRangar_Report		= 100;	//(nur Milizanwдrter)
const int 	XP_Addon_Martin_GotRangar				= 300;	//(nur Milizanwдrter)
const int 	XP_Addon_ReturnedLostInnosStatue_Daron	= 300;	//(nur KDFanwдrter)
const int 	XP_Addon_ReportLostInnosStatue2Daron	= 100;	//(nur KDFanwдrter)
const int 	XP_Addon_Cord_Look4Patrick				= 300;	//(nur SLDanwдrter)
const int 	XP_Addon_Dexter_KnowsPatrick			= 100;	//(nur SLDanwдrter)
...
const int 	XP_Addon_BringStoneplate				= 50;	//fьr jede Steintafel
...
const int 	XP_Addon_Vatras_MissingPeopleReport		= 50;	//pro Report bei Vatras fьr die missing People
const int 	XP_Addon_AllNWTeleporststones			= 200;
const int	XP_Greg_NW_GiveBauArmor					= 100;	

//Kapitel 2 ADDON	
const int 	XP_Addon_LookForMorgan					= 100;	//Fьr AlligatorJack Moran suchen
...
const int	XP_Addon_HenryProofCrew					= 200;	//Ьberzeuge HenrysMдnner
const int	XP_Addon_MorganProofCrew				= 200;	//Ьberzeuge MorgansMдnner
...
const int	XP_Addon_Riordian_FoundHouse			= 100; //5 mal fьr jedes Haus einen
const int 	XP_addon_ClearCanyon					= 250;
const int 	XP_Bones_GetBDTArmor					= 200;

//Kapitel 3 ADDON
const int   XP_Addon_Andre_MissingPeople			= 600;  //(nur MIL oder PAL)Report bei Andre fьr Returning Missing People.
const int   XP_Addon_ForOneToken					= 400;  //fьr jede der 5 Steintafeln (Token) an Saturas abgegeben.
const int   XP_Addon_Myxir_GeistBeschwoeren			= 400;  //SC berichtet von geglьckter Geistbeschwцrung.
...
const int   XP_Addon_ActivatedTeleportStone			= 500;  //(5 mal) fьr jeden aktivierten Teleportstein. bei Merdarion.
...
const int   XP_Addon_SkipsGrog						= 200;  //Skip den Grog zurьck gegeben.
const int   XP_Addon_Morgan_LurkerPlatt				= 300;  //SC sдubert Strand fьr Morgan.
...
//-----------------------------------------------------------------
//Banditenlager Addon Kapitel 2? Drei? Wer weiЯ es?
//-----------------------------------------------------------------				
...
const int  XP_Addon_MINE				= 150;	//3x fьr Buddler in die mine schicken	
...
CONST int  XP_Addon_HunoStahl			= 200; //Stahl fьr Huno
CONST int  XP_Addon_LennarPaket			= 200; //Dietriche fьr lennar
const int  XP_Addon_Tom					= 100; //Tom nicht verpetzen

const int  XP_Addon_Auftraggeber		= 500; //Auftraggeber gefunden (Snaf)
const int  XP_Addon_Loushammer			= 150; //Schnapsbrennen fьr Snaf
...
// ------ AMBIENT XP -----
var int   XP_Ambient;		// kann fьr alle kleinen Ambient Sachen verwendet werden M.F.
...
const int 	XP_BringOldCoin			= 10;	//Joly: fьr jede alte Mьnze, die SC zu Wasili bringt.	(kann bis zu 800 XP bringen! bis Kapitel 5 )	
...
const int 	XP_Andre_Auslieferung	= 100; //gibt's fьr jeden ausgelieferten NPC 
...
const int	XP_HannaRetrieveLetter		= 300; //fьr alle
const int 	XP_HelpDiegoNW				= 600; //Finde Diegos Schatz	
const int   XP_DiegosResidence			= 400; //ein neues Haus fьr Diego	
...
const int	XP_BringSerpentesMinenAnteil= 75; 	//Fьr jedes Minenanteil, dass SC zu Serpentes bringt ,  10 X  XP_BringSerpentesMinenAnteil = 750 (nur PAL)
const int	XP_SalandrilInKloster		= 400; 	//Salandril bьhst seine Sьnden im Kloster (KDF und SLD,DJG)
const int	XP_Akil_SchafDiebe			= 400; 	//SC hat Schafdiebe getцtet
const int	XP_AkilsSchaf				= 200; 	//Schaf ist wieder da.
const int	XP_FoundVinosKellerei		= 400; 	//Moleratefett anngewendet und VinosKellerei geцffnet
...
const int	XP_Lester_KnowsMadPsi 		= 300; 	//Lester weiЯ jetzt, dass die Suchenden vorher Psioniker waren (KDF)
const int	XP_Angar_KnowsMadPsi 		= 300; 	//Angar weiЯ jetzt, dass die Suchenden vorher Psioniker waren (KDF)
...
const int	XP_KillMorgahard			= 300; 	//SC tцtet Morgahard fьr den Richter. 						(SLD) 
const int 	XP_FoundMorgahard			= 500;	//Morgahard gefunden (SLD) 
const int 	XP_SwordBlessed				= 400;	//Schwert wurde geweiht
const int 	XP_Torlof_DMT				= 300;  //fьr torlof den DMT getцtet
...
const int   XP_KDF_BringAlmanach   	= 450; 		//Joly: nur Kdf XP fьr einen gefundenen Almanach 		-> ca. 10 x XP_KDF_BringAlmanach
...
const int 	XP_Crewmember_Success		= 500;	//Fьr jedes Crewmitglied!!	 
const int	XP_Captain_Success			= 2000; //ich habe einen Kapitдn lalalalala
