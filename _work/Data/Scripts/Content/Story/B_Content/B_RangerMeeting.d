...
func VOID B_SchlussMitRangerMeeting () //im Vatras dialog
{
	...
	B_StartOtherRoutine	(Lares,"Začít"); 				

	B_StartOtherRoutine	(Cord,"Začít");					
	if ((Gaan.aivar[AIV_TalkedToPlayer] == FALSE))
	{
		B_StartOtherRoutine	(Gaan,"PreStart");}else{B_StartOtherRoutine	(Gaan,"Začít");					
	};
	if (MIS_Addon_Martin_GetRangar == 0)
	{
		B_StartOtherRoutine	(Martin,"PreStart");}else{B_StartOtherRoutine (Martin,"Začít");
	};
	B_StartOtherRoutine	(Orlan,"Začít");	
