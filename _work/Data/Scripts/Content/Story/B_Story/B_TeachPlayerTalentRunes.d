...
func int B_TeachPlayerTalentRunes (var C_NPC slf, var C_NPC oth, var int spell)
{
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'svętlo': 1 zlaăák.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'léčba lehkých zranęní': 1 léčivá rostlina.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'vyvolání vlka': 1 vlčí kožešina.");	};		
	if (spell == SPL_WINDFIST)				{	PLAYER_TALENT_RUNES[SPL_WINDFIST] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_WINDFIST, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'vętrná pęst': 1 uhlík.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'léčba stâednę tęžkých zranęní': 1 léčivá bylina.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'strach': 1 černá perla.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'zničení nemrtvých': 1 svęcená voda.");	};		
	if (spell == SPL_Pyrokinesis)			{	PLAYER_TALENT_RUNES[SPL_Pyrokinesis] 			= TRUE;	CreateInvItems (ScrollTrader, ItSc_Pyrokinesis, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'velká ohnivá bouâe': 1 síra a 1 jazyk ohnivého ještęra.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'léčba tęžkých zranęní': 1 léčivý koâen");	};	
	if (spell == SPL_Firerain)				{	PLAYER_TALENT_RUNES[SPL_Firerain] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Firerain, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'ohnivý déšă': 1 smola a 1 jazyk ohnivého ještęra.");	};		
	if (spell == SPL_BreathOfDeath)			{	PLAYER_TALENT_RUNES[SPL_BreathOfDeath]			= TRUE;	CreateInvItems (ScrollTrader, ItSc_BreathOfDeath, 1);			
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'dech smrti': 1 uhlík a 1 černá perla");	};		
	if (spell == SPL_MassDeath)				{	PLAYER_TALENT_RUNES[SPL_MassDeath] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_MassDeath, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'vlna smrti': 1 kost kostlivce a 1 černá perla");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'armáda temnot': 1 kost z kostlivce, 1 černá perla, 1 srdce kamenného golema a 1 srdce démona");	};		
	if (spell == SPL_Shrink)				{	PLAYER_TALENT_RUNES[SPL_Shrink] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Shrink, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Ingredience pro runu 'zmenšení nestvůry': 1 gobliní kost a 1 trolí zub");	};		
