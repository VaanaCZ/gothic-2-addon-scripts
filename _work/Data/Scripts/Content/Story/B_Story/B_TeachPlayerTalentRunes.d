...
func int B_TeachPlayerTalentRunes (var C_NPC slf, var C_NPC oth, var int spell)
{
	...
	B_LogEntry (TOPIC_TalentRunes,"Per creare una runa mi serve una pergamena dell'incantesimo desiderato e alcuni ingredienti, a seconda della runa. Utilizzando questi ingredienti a una pietra runica vuota posso creare la runa desiderata su un tavolo runico.");
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Luce': 1 moneta d'oro.");	};		
	if (spell == SPL_Firebolt)				{	PLAYER_TALENT_RUNES[SPL_Firebolt] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Firebolt, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Freccia infuocata': 1 zolfo.");	};		
	if (spell == SPL_Icebolt)				{	PLAYER_TALENT_RUNES[SPL_Icebolt] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Icebolt, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Freccia ghiacciata': 1 quarzo del ghiacciaio.");};		
	if (spell == SPL_LightHeal)				{	PLAYER_TALENT_RUNES[SPL_LightHeal] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_LightHeal, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Cura ferite leggere': 1 pianta curativa.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Crea scheletro goblin': 1 osso di goblin.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Fulmine': 1 cristallo di roccia e 1 quarzo del ghiacciaio.");	};	
	...
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Armata delle tenebre': 1 osso di scheletro, 1 perla nera, 1 cuore di golem di pietra e 1 cuore di demone. ");	};		
	if (spell == SPL_Shrink)				{	PLAYER_TALENT_RUNES[SPL_Shrink] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Shrink, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Ingredienti per la runa 'Restringi mostro': 1 osso di goblin e 1 dente di troll");	};		
