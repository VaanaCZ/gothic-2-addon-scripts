...
{
	AI_Output (self, other,"DIA_KDF_507_Talamon_SecondWarn_04_00"); //Innos na tebe sešle svůj hněv, jestli nepůjdeš zpátky!
...
FUNC VOID DIA_Talamon_KAP5_Stop_Info()
{
	...
	AI_Output (other,self ,"DIA_Talamon_KAP5_Stop_15_01"); //Pyrokar říká, že mám povoleno prohlédnout si Xardasovu knihu.
	AI_Output (self, other,"DIA_Talamon_KAP5_Stop_04_02"); //No, když to říká. Dobrá, můžeš vstoupit. Kniha, kterou hledáš, je tamhle u té alchymistické kolony.
...
FUNC VOID DIA_Talamon_FoundSecretDoor_Info()
{
	...
	AI_Output (self ,other,"DIA_Talamon_FoundSecretDoor_04_01"); //(překvapeně) Cože? Kde?
	...
	AI_Output (self ,other,"DIA_Talamon_FoundSecretDoor_04_05"); //To jsou důležité zprávy, okamžitě je předám Pyrokarovi.
	AI_Output (self ,other,"DIA_Talamon_FoundSecretDoor_04_06"); //Mezitím bys měl zjistit, co v tom sklepě je.
...
INSTANCE DIA_Talamon_ScoutSecretLibrary   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Byl jsem v té kryptě.";
};
...
{
	AI_Output (other,self ,"DIA_Talamon_ScoutSecretLibrary_15_00"); //Byl jsem v té kryptě.
	AI_Output (self ,other,"DIA_Talamon_ScoutSecretLibrary_04_01"); //A co jsi zjistil?
	
	if (Npc_IsDead (SecretLibrarySkeleton))
	{
		AI_Output (other,self ,"DIA_Talamon_ScoutSecretLibrary_15_02"); //Byl tam kostlivec válečník, který hlídal dveře. Zabil jsem ho.
	...
	{
		AI_Output (other,self ,"DIA_Talamon_ScoutSecretLibrary_15_05"); //Tam dole se to hemží příšerami.
		AI_Output (self ,other,"DIA_Talamon_ScoutSecretLibrary_04_06"); //Musí tam něco být, hledej dál. A s těmi potvorami neměj žádné slitování.
	};
};
