...
{
	AI_Output			(other, self, "DIA_Sagitta_HALLO_15_00"); //Co dêláš tak sama, tady venku?
	AI_Output			(self, other, "DIA_Sagitta_HALLO_17_01"); //Âekni, co ode mê chceš, a pak vypadni. Mám práci.

	Info_ClearChoices	(DIA_Sagitta_HALLO);
	Info_AddChoice	(DIA_Sagitta_HALLO, DIALOG_BACK, DIA_Sagitta_HALLO_ende );
	Info_AddChoice	(DIA_Sagitta_HALLO, "Mùžeš mê vyléèit?", DIA_Sagitta_HALLO_Heil );
	Info_AddChoice	(DIA_Sagitta_HALLO, "Co tady dêláš?", DIA_Sagitta_HALLO_was );
...
func void DIA_Sagitta_HALLO_wer ()
{
	...
	AI_Output			(self, other, "DIA_Sagitta_HALLO_wer_17_01"); //Tak tys o mnê ještê neslyšel?
	AI_Output			(self, other, "DIA_Sagitta_HALLO_wer_17_02"); //Bylinkáâka, alespoà se tomu tak âíká. Šarlatánka, nebo tak nêjak mê nazývají.
...
{
	AI_Output			(other, self, "DIA_Sagitta_HALLO_was_15_00"); //Co tady dêláš?
...
{
	AI_Output			(other, self, "DIA_Sagitta_HALLO_Heil_15_00"); //Mùžeš mê vyléèit?
	AI_Output			(self, other, "DIA_Sagitta_HALLO_Heil_17_01"); //To je ten dùvod, kvùli kterému jsi tady, co? Tak povídej, co je v nepoâádku.
...
instance DIA_Sagitta_TeachAlchemyRequest		(C_INFO)
{
	...
	description	 = 	"Mùžeš mê nauèit nêco o bylinkách?";
};
...
{
	AI_Output			(other, self, "DIA_Sagitta_TeachAlchemyRequest_15_00"); //Mùžeš mê nauèit nêco o bylinkách?

	if (DIA_Sagitta_TeachAlchemyRequest_OneTime ==FALSE)
	{
		AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_17_01"); //Jak zajímavé. O nêco takového mê lidé nežádají zrovna èasto.
		AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_17_02"); //Takže ty by ses chtêl stát mým studentem? To bys mêl nejdâív prokázat, že to s tím zájmem myslíš vážnê.
		AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_17_03"); //Právê pâipravuju velmi vzácný lektvar, který vyžaduje vybrané bylinky a extrakty.
		AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_17_04"); //Když mi seženeš jednu ingredienci - velmi vzácnou bylinu, kterou tady bohužel nemám - mùžu tê nêèemu nauèit.
	...
	Info_AddChoice	(DIA_Sagitta_TeachAlchemyRequest, "Promià, nemám zájem.", DIA_Sagitta_TeachAlchemyRequest_nein );
...
func void DIA_Sagitta_TeachAlchemyRequest_was ()
{
	...
	AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_was_17_01"); //Je to nesmírnê vzácná bylina - âíká se jí sluneèní aloe. Poznáš ji podle pronikavé mandlové vùnê.
...
func void DIA_Sagitta_TeachAlchemyRequest_wo ()
{
	...
	AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_wo_17_03"); //Což je taky ten dùvod, proè je pro mê tak obtížné tu kytku získat.
	Info_AddChoice	(DIA_Sagitta_TeachAlchemyRequest, "Uvidíme, co se s tím dá dêlat.", DIA_Sagitta_TeachAlchemyRequest_wo_ja );
};
func void DIA_Sagitta_TeachAlchemyRequest_wo_ja ()
{
	AI_Output			(other, self, "DIA_Sagitta_TeachAlchemyRequest_wo_ja_15_00"); //Uvidíme, co se s tím dá dêlat.
	AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_wo_ja_17_01"); //Dobrá tedy. Tak hodnê štêstí pâi hledání.
	Info_ClearChoices	(DIA_Sagitta_TeachAlchemyRequest);
	...
	B_LogEntry (TOPIC_SagittaHerb,"Sagitta potâebuje zvláštní rostlinu zvanou sluneèní aloe, která roste pouze na exkrementech èerného trola.");
...
{
	AI_Output			(other, self, "DIA_Sagitta_TeachAlchemyRequest_nein_15_00"); //Promià, nemám zájem.
	AI_Output			(self, other, "DIA_Sagitta_TeachAlchemyRequest_nein_17_01"); //Tak pâestaà plýtvat mým èasem s têmi tvými nesmysly.
	Info_ClearChoices	(DIA_Sagitta_TeachAlchemyRequest);
};
...
func void DIA_Sagitta_Sagitta_Herb_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sagitta_Sagitta_Herb_17_01"); //Díky. A teë se mê ohlednê lektvarù mùžeš ptát, na co chceš.
...
INSTANCE DIA_Sagitta_Teach   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Jaké lektvary mê mùžeš nauèit pâipravovat?";
};
//----------------------------------------
...
FUNC VOID DIA_Sagitta_Teach_Info()
{
	var int talente;
	talente = 0;
	AI_Output (other, self,"DIA_Sagitta_Teach_15_00");//Jaké lektvary mê mùžeš nauèit pâipravovat?
	...
	if (talente > 0)
	{
		...
		{
			AI_Output (self, other,"DIA_Sagitta_Teach_17_01"); //Pâedtím, než tê nauèím nêco z mé alchymie, ti musím âíct, co všechno budeš na pâípravu lektvarù potâebovat.
			AI_Output (self, other,"DIA_Sagitta_Teach_17_02"); //Alchymistická kolona slouží jako kuchyà pro pâípravu lektvarù. Budeš potâebovat prázdné laboratorní baàky, ve kterých se lektvary uchovávají.
			AI_Output (self, other,"DIA_Sagitta_Teach_17_03"); //Pak pâidáš ingredience a máš požadovaný lektvar.
			AI_Output (self, other,"DIA_Sagitta_Teach_17_04"); //Tomu se samozâejmê mùžeš ode mê nauèit.
			Alchemy_Explain = TRUE;
		}
		else
		{
			AI_Output (self, other,"DIA_Sagitta_Teach_17_05"); //Tak jaký lektvar by to mêl být?
		};
	}
	else 
	{
		AI_Output (self, other,"DIA_Sagitta_Teach_17_06"); //Už znáš vše, co bych tê mohla nauèit.
...
{
	AI_Output			(other, self, "DIA_Sagitta_HEAL_15_00"); //Vyleè mê.
	
	if hero.attribute [ATR_HITPOINTS] < hero.attribute[ATR_HITPOINTS_MAX]
	{
		AI_Output			(self, other, "DIA_Sagitta_HEAL_17_01"); //Ukaž, podívám se. Mmh. Na ty nejhorší zranêní by mêla moje mastièka staèit.
		hero.attribute [ATR_HITPOINTS] = hero.attribute[ATR_HITPOINTS_MAX];
		PrintScreen (PRINT_FullyHealed, - 1, - 1, FONT_Screen, 2);   
	}
	else 
	{	
		AI_Output			(self, other, "DIA_Sagitta_HEAL_17_02"); //V téhle chvíli nepotâebuješ žádné léèení.
	};	
};
...
instance DIA_Sagitta_Thekla		(C_INFO)
{
	...
	description	 = 	"Thekla mê poslala pro nêjaký balík.";
};
...
{
	AI_Output			(other, self, "DIA_Sagitta_Thekla_15_00"); //Thekla mê poslala pro nêjaký balík.
	AI_Output			(self, other, "DIA_Sagitta_Thekla_17_01"); //Ach, ano. Už jsem ji oèekávala vèera.
	AI_Output			(self, other, "DIA_Sagitta_Thekla_17_02"); //Tady je ten balík. Dávej na nêj dobrý pozor!
...
instance DIA_Sagitta_HEALRANDOLPH		(C_INFO)
{
	...
	description	 = 	"Randolph má absãák.";
};
...
{
	AI_Output			(other, self, "DIA_Sagitta_HEALRANDOLPH_15_00"); //Randolph má absãák.

	if (DIA_Sagitta_HEALRANDOLPH_KnowsPrice == FALSE)
		{
			AI_Output			(self, other, "DIA_Sagitta_HEALRANDOLPH_17_01"); //Kdy už se ten chlap koneènê pouèí?
			DIA_Sagitta_HEALRANDOLPH_KnowsPrice = TRUE;
		};
	
	if(DIA_Sagitta_HEALRANDOLPH_GotOne == TRUE)
		{
			AI_Output			(self, other, "DIA_Sagitta_HEALRANDOLPH_17_02"); //Už jsem ti pro nêj pâichystala medicínu. Tak se s tím nikde moc nepoflakuj.
		}
	else
		{
			AI_Output			(self, other, "DIA_Sagitta_HEALRANDOLPH_17_03"); //Dám mu na to lék. To mu bude na nêkolik dní staèit.
		};
	...
	if (DIA_Sagitta_HEALRANDOLPH_KnowsPrice == FALSE)
	{
		...
		AI_Output			(self, other, "DIA_Sagitta_HEALRANDOLPH_17_06"); //Jediná vêc, kterou dostaneš úplnê zadarmo, je smrt, chlapèe.
	};	
...
func void DIA_Sagitta_HEALRANDOLPH_geld ()
{
	...
	if (B_GiveInvItems (other, self, ItMi_Gold,300))
		{
			AI_Output			(self, other, "DIA_Sagitta_HEALRANDOLPH_geld_17_01"); //Výbornê. Vždycky ty peníze mùžeš dostat zpátky od nêj.
