...
INSTANCE DIA_Lester_DI_Hello (C_INFO)
{
	...
	description = "Nechtêl bys mi nêco âíct?";
};    
...
{	
	AI_Output	(other ,self,"DIA_Lester_DI_Hello_15_00");	//Nechtêl bys mi nêco âíct?

	if (Npc_IsDead(Mario_DI) == FALSE)
	&& (ORkSturmDI  == TRUE)
	&& (DIA_Lester_DI_SCGotWarning1 == FALSE)
	{
		AI_Output	(self ,other,"DIA_Lester_DI_Hello_13_01");	//Když skâeti zaútoèili, Mario utekl. I když tady na lodi byl zmatek, neztratil jsem ho z oèí.
		AI_Output	(self ,other,"DIA_Lester_DI_Hello_13_02");	//Jednoduše prošel skrz skâetí linie - a skâeti se ho ani nedotkli.
		AI_Output	(self ,other,"DIA_Lester_DI_Hello_13_03");	//Moc se mi ten parchant nelíbil už od samého zaèátku. Je ve spolku s nepâítelem.
	...
	{
		AI_Output	(self ,other,"DIA_Lester_DI_Hello_13_06");	//Teë ne. Možná pozdêji.
	};
};
...
INSTANCE DIA_Lester_DI_MarioArsch (C_INFO)
{
	...
	description = "Mêl jsi pravdu. Mario nás zradil.";
};    
...
{	
	AI_Output	(other ,self,"DIA_Lester_DI_MarioArsch_15_00");	//Mêl jsi pravdu. Mario nás zradil.
	AI_Output	(self ,other,"DIA_Lester_DI_MarioArsch_13_01");	//Ano. Možná nejsem zrovna nejbystâejší, ale tohle mi bylo jasné.
	AI_Output	(self ,other,"DIA_Lester_DI_MarioArsch_13_02");	//Pâíštê buë pâi vybírání spoleèníkù opatrnêjší.
	B_GivePlayerXP (XP_Ambient);
};
		...
		if ((Npc_GetDistToWP(self,"DI_DRACONIANAREA_19")<4000) )
			{
				...
				AI_Output (self ,other,"DIA_PC_Lester_DI_UndeadDragonDead_13_02"); //Jak jsi to udêlal?
				AI_Output (other,self ,"DIA_PC_Lester_DI_UndeadDragonDead_15_03"); //Znièil jsem nepâítele.
				AI_Output (self ,other,"DIA_PC_Lester_DI_UndeadDragonDead_13_04"); //Muselo to být opravdu nêco hodnê mocného. Co to pâesnê bylo?
		...
		if (SC_KnowsMadPsi == TRUE)
			{
				...
				AI_Output (self ,other,"DIA_PC_Lester_DI_UndeadDragonDead_13_08"); //Budu potâebovat víc èasu, než se pâes to dokážu úplnê pâenést.
			};
		DIA_PC_Lester_DI_UndeadDragonDead_OneTime = TRUE;
	};
	AI_Output (self ,other,"DIA_PC_Lester_DI_UndeadDragonDead_13_09"); //Doufejme, že se už nikdy nic takového nestane. Na celém svêtê není dost trávy, kolik bych jí chtêl teë vykouâit.
	AI_StopProcessInfos (self);
