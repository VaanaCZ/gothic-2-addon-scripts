...
func void DIA_Akil_NachKampf_Info ()
{
	...
	AI_Output (other, self, "DIA_Akil_NachKampf_15_02"); //Kdo byli ti lidé?
		...
		{
			AI_Output (self, other, "DIA_Akil_NachKampf_13_04"); //To byli žoldáci z Onarovy farmy. Ty bastardi neznají nic než vraždění a rabování.
		};
...
instance DIA_Addon_Akil_MissingPeople		(C_INFO)
{
	...
	description	 = 	"Nezaslechl jsi něco o pohřešovaných lidech z města?";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Akil_MissingPeople_15_00"); //Nevíš něco o tom, že se tu ztratili nějací lidé?
	AI_Output	(self, other, "DIA_Addon_Akil_MissingPeople_13_01"); //To bys mě rozesmál! Jestli jsem o tom slyšel? Vždyť se mi ztratilo i pár mých vlastních lidí!
	AI_Output	(self, other, "DIA_Addon_Akil_MissingPeople_13_02"); //Tonak a Telbor pracovali na mých polích, ale před třemi dny najednou zmizeli.
	AI_Output	(self, other, "DIA_Addon_Akil_MissingPeople_13_03"); //Vím určitě, že ještě ráno úplně normálně odcházeli ze statku.
	AI_Output	(self, other, "DIA_Addon_Akil_MissingPeople_13_04"); //Prostě zmizeli a nikdo neví, kam mohli jít.
	AI_Output	(self, other, "DIA_Addon_Akil_MissingPeople_13_05"); //Kdybys na ně někde narazil, dáš mi vědět, viď že jo?
	B_GivePlayerXP (XP_Ambient);
...
instance DIA_Addon_Akil_ReturnPeople		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"K těm tvým čeledínům...";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Akil_ReturnPeople_15_00"); //Co se týče těch tvých pacholků...
	
	if (Npc_GetDistToWP (Tonak_NW,"NW_FARM2_FIELD_TANOK")<= 1000)
	&& (Npc_GetDistToWP (Telbor_NW,"NW_FARM2_FIELD_TELBOR")<= 1000)
	{
		AI_Output	(self, other, "DIA_Addon_Akil_ReturnPeople_13_01"); //Tys je přivedl zpátky! Seš dobrej chlap.
	}
	else 
	{
		AI_Output	(self, other, "DIA_Addon_Akil_ReturnPeople_13_02"); //Aspoň jeden z nich přežil.
	};
	AI_Output	(self, other, "DIA_Addon_Akil_ReturnPeople_13_03"); //Vezmi si tohle zlato, abys viděl, že jsem ti opravdu vděčný.
...
func void DIA_Akil_Wald_Info ()
{
	...
	AI_Output (self, other, "DIA_Akil_Gegend_Wald_13_01"); //Jenom nějaký příšery - a vlci jsou z nich ty nejmíň nebezpečný.
...
func void DIA_Akil_Perm_Info ()
{
	...
	else //Kapitel 4 & 5
	{
		...
		if ((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
		{
			AI_Output (self, other, "DIA_Akil_Perm_13_04"); //Taky tu spatřili spoustu ještěrů. Být tebou, do jeskyní bych vůbec nechodil.
...
instance DIA_Akil_SCHAFDIEB		(C_INFO)
{
	...
	description	 = 	"Ještě nějaké novinky?";
};
