...
instance DIA_Engor_ABOUTPARLAF		(C_INFO)
{
	...
	description	 = 	"Ty máš na starosti příděly potravin, nemýlím-li se.";
};
...
instance DIA_Engor_RSkaufen		(C_INFO)
{
	...
	permanent 	 =  TRUE;
	description	 = 	"Koupit těžkou zbroj domobrany (Ochrana: zbraně 60, šípy 60. Cena: 2500 zlatých)"; 
};
//--------------------------------------
...
{
	AI_Output (other, self, "DIA_Engor_HELP_15_00"); //Možná ti mohl s něčím pomoci.
...
FUNC VOID DIA_Engor_HELP_YES()
{
	...
	AI_Output (self, other, "DIA_Engor_HELP_YES_13_01"); //Takové dva tucty pořádných kusů masa by mi pomohly nasytit nejméně sto hladových krků. Vrať se, až budeš mít maso. Teď se musím vrátit ke své práci.
	...
	B_LogEntry  (TOPIC_BringMeat,"Nezáleží na tom, půjde-li o klobásy, šunku, pečené či syrové maso. Jenom aby chlapi měli co zakousnout.");  
...
instance DIA_Engor_BRINGMEAT		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Tumáš, něco jsem pro tebe sehnal. (předat maso)";
};
...
// ***** Rohes Fleisch *****
	if (Npc_HasItems (hero,ItFoMuttonRaw) >= 1) 
	{
		...
		&& (Meat_Counter < Meat_Amount) 
		{
			...
			ConcatRaw = ConcatStrings (ConcatRaw, " Předat syrové maso");
...
// ***** gebratenes Fleisch *****
	if (Npc_HasItems (hero,ItFoMutton) >= 1) 
	{
		...
		&& (Meat_Counter < Meat_Amount) 
		{
			...
			ConcatMutton = ConcatStrings (ConcatMutton, " Předat pečené maso");
...
// ***** Schinken *****	
	if (Npc_HasItems (hero,ItFo_Bacon) >= 1) 
	{
		...
		&& (Meat_Counter < Meat_Amount) 
		{
			...
			ConcatBacon = ConcatStrings (ConcatBacon, " Předat šunku");
	...
	if (Npc_HasItems (hero,ItFo_Sausage) >= 1) 
	{
		...
		&& (Meat_Counter < Meat_Amount) 
		{
			...
			ConcatSausage = ConcatStrings (ConcatSausage, " Předat klobásu");
	...
	{	
		...
		GesamtFleisch = ConcatStrings ("Celkem předáno masa:",GesamtFleisch);
