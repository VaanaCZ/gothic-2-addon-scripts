	...
	else // ------ normales Reinkommen ------
	{
		...
		|| (Hlp_IsItem(itm, ItAr_Bau_M) == TRUE)
		{
			...
			if (Npc_KnowsInfo (other, DIA_Mil_310_Stadtwache_MilizWerden))
			{
				...
				AI_Output (self, other,"DIA_Mil_333_Stadtwache_FirstWarn_06_14"); //(ošklivě se zasměje) No potěš! Že oni nám sem posílají vždycky samou smetánku.
			};
		}
...
{
	AI_Output (other, self,"DIA_Mil_333_Stadtwache_Bribe_15_00"); //Tady je 100 zlatých. Nech mě projít!
	
	if (B_GiveInvItems (other, self, itmi_gold, 100))
	{
		AI_Output (self, other,"DIA_Mil_333_Stadtwache_Bribe_06_01"); //(lakomě) 100 zlatých zní hezky. Tak běž dovnitř.
		
		if (B_GetGreatestPetzCrime(self) >= CRIME_ATTACK)
		{
			AI_Output (self, other,"DIA_Mil_333_Stadtwache_Bribe_06_02"); //A jdi rovnou za Andrem! Nebo po tobě budu příště chtít dalších 100 zlatých!
		};
	...
	{
		AI_Output (self, other,"DIA_Mil_333_Stadtwache_Bribe_06_03"); //Co?! Kde?! Nevidím žádné zlaťáky. Zmiz!
	};
