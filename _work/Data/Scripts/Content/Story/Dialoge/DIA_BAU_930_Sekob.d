...
{
	AI_Output (other, self, "DIA_Sekob_ZAHLPACHT_15_00"); //Tahle půda patâí velkostatkáâi a ty jsi tady jenom v nájmu. Onar chce, abys mu zaplatil rentu. Dlužíš už za nękolik týdnů.
...
instance DIA_Sekob_DMTWEG		(C_INFO)
{
	...
	description	 = 	"Tvůj dům je prázdný. Ty černé kápę už odešly.";
};
...
func void DIA_Sekob_DMTWEG_Info ()
{
	...
	{
		B_StartOtherRoutine (Rosi,"Začít"); 
		B_StartOtherRoutine (Till,"Začít"); 
	};
	B_StartOtherRoutine (Balthasar,"Začít"); 
	B_StartOtherRoutine (BAU_933_Rega,"Začít"); 
	B_StartOtherRoutine (BAU_934_Babera,"Začít"); 
	B_StartOtherRoutine (BAU_937_BAUER,"Začít"); 
	B_StartOtherRoutine (BAU_938_BAUER,"Začít"); 
	...
	&& (Kapitel >= 5)
	...
	else
		{
			...
			&& (MIS_bringRosiBackToSekob != LOG_SUCCESS)
			{
				...
				B_LogEntry (TOPIC_bringRosiBackToSekob,"Sekobova žena Rosi zmizela a její muž by byl rád, kdyby jí nękdo 'dopomohl' k návratu."); 
			}
			else
			{
...
instance DIA_Sekob_ROSINEVERBACK		(C_INFO)
{
	...
	description	 = 	"Rosi se k tobę už nevrátí. Odvedl jsem ji do bezpečí.";
};
