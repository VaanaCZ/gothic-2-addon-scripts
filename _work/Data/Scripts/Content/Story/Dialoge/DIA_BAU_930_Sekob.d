...
{
	AI_Output (other, self, "DIA_Sekob_ZAHLPACHT_15_00"); //Tahle pùda patøí velkostatkáøi a ty jsi tady jenom v nájmu. Onar chce, abys mu zaplatil rentu. Dlužíš už za nìkolik týdnù.
...
func void DIA_Sekob_Defeated_Info ()
{
	...
	AI_Output (self, other, "DIA_Sekob_Defeated_01_03"); //(naøíkavì) Ale pane, já nic nemám. Jsem jen chudý ubožák, co skoro umírá hlady.
...
instance DIA_Sekob_DMTWEG		(C_INFO)
{
	...
	description	 = 	"Tvùj dùm je prázdný. Ty èerné kápì už odešly.";
};
	...
	&& (Kapitel >= 5)
	...
	else
		{
			...
			&& (MIS_bringRosiBackToSekob != LOG_SUCCESS)
			{
				...
				B_LogEntry (TOPIC_bringRosiBackToSekob,"Sekobova žena Rosi zmizela a její muž by byl rád, kdyby jí nìkdo 'dopomohl' k návratu."); 
			}
			else
			{
...
instance DIA_Sekob_ROSINEVERBACK		(C_INFO)
{
	...
	description	 = 	"Rosi se k tobì už nevrátí. Odvedl jsem ji do bezpeèí.";
};
