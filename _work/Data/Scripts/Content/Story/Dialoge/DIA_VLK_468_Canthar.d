...
{	
	...
	{
		AI_Output (self, other,"DIA_Canthar_PersonalCRIMES_09_00"); //To od tebe bylo opravdu hloupé, že jsi mê napadl.
	}
		
	// ------ ich habe Spieler besiegt ODER keiner wurde besiegt ------
	else // FIGHT_WON oder FIGHT_CANCEL
	{
		AI_Output (self, other,"DIA_Canthar_PersonalCRIMES_09_01"); //(posmêšnê) To sis myslel, že mê pâemùžeš tak snadno?
	};
	
	AI_Output (self, other,"DIA_Canthar_PersonalCRIMES_09_02"); //Buë mi to nêjak vynahradíš, nebo se budu dívat na to, jak hoâce lituješ svých èinù!
	AI_Output (other, self,"DIA_Canthar_PersonalCRIMES_15_03"); //A co pâesnê myslíš?
	B_Say_Gold(self,other, 100);
	
	Info_ClearChoices	(DIA_Canthar_PersonalCRIMES);
	Info_AddChoice		(DIA_Canthar_PersonalCRIMES, "Nemêl jsem toho tolik!"	,DIA_Canthar_PersonalCRIMES_NotEnough);
	if (Npc_HasItems (other, itmi_gold) >= 100)
	{
		Info_AddChoice		(DIA_Canthar_PersonalCRIMES, "Tady je zlato - zapomeneme na tu vêc!",DIA_Canthar_PersonalCRIMES_Pay);
	};
};

func void DIA_Canthar_PersonalCRIMES_Pay()
{
	AI_Output (other, self,"DIA_Canthar_PersonalCRIMES_Pay_15_00"); //Tady je zlato - zapomeneme na tu vêc!
	AI_Output (self, other,"DIA_Canthar_PersonalCRIMES_Pay_09_01"); //To je od tebe hodnê rozumné!
...
{
	AI_Output (other, self,"DIA_Canthar_PersonalCRIMES_NotEnough_15_00"); //Nemêl jsem toho tolik!
	AI_Output (self, other,"DIA_Canthar_PersonalCRIMES_NotEnough_09_01"); //Tak proè plýtváš mým èasem?
...
{	
	...
	if (Npc_HasEquippedArmor(other) == FALSE) 
	{
		...
		AI_Output	(self, other,"DIA_Canthar_Hallo_09_01"); //Jsi na cestê do mêsta, nemám pravdu?
		AI_Output	(other, self,"DIA_Canthar_Hallo_15_02"); //Možná.
		AI_Output	(self, other,"DIA_Canthar_Hallo_09_03"); //Hm! (odfrkne) Vypadáš jako uprchlík! Všichni uprchlíci mají namíâeno do mêsta.
		AI_Output	(self, other,"DIA_Canthar_Hallo_09_04"); //Dokonce bys mohl být bývalým vêznêm z hornické kolonie.
		AI_Output	(self, other,"DIA_Canthar_Hallo_09_05"); //(kvapnê) Je mi jedno, odkud jdeš. Ale vêâím, že bych pro tebe mêl zajímavou nabídku.
	...
	|| 		(Hlp_IsItem (itm, ItAr_Bau_M))
	{
		...
		Info_AddChoice		(DIA_Canthar_Hallo, "Nemohu si stêžovat."		,DIA_Canthar_Hallo_Bauer);
	}
	
	// ------ alle anderen Rüstungen (höchst unwahrscheinlich) ------
	else 
	{
		AI_Output (self, other,"DIA_Canthar_Hallo_09_07"); //Co pro tebe mohu udêlat?
	};
};

func void DIA_Canthar_Hallo_Bauer()
{
	AI_Output	(other, self,"DIA_Canthar_Hallo_Bauer_15_00");	//Nemùžu si stêžovat.
	AI_Output	(self, other,"DIA_Canthar_Hallo_Bauer_09_01");	//Mùžu pro tebe nêco udêlat?
...
func void DIA_Canthar_Hallo_NoBauer()
{
	...
	AI_Output	(self, other,"DIA_Canthar_Hallo_NoBauer_09_01"); //Pâesnê jako rolník. Ale nemluvíš tak!
	AI_Output	(self, other,"DIA_Canthar_Hallo_NoBauer_09_02"); //(hloubavê) Jestli nejsem úplnê mimo, tak bych pro tebe mêl zajímavou nabídku.
...
{	
	...
	if (Canthar_GotME == TRUE)
	{
		if (Npc_HasEquippedArmor(other) == FALSE)
		{	
			AI_Output (self, other,"DIA_Canthar_WhatOffer_09_01"); //Tak, jak si to ty pâedstavuješ, tê stráže do mêsta nikdy nepustí.
		}
		else
		{
			AI_Output (self, other,"DIA_Canthar_WhatOffer_09_02"); //(ironicky) Jestli budeš v oèích stráží tak "pâesvêdèivý" jako v têch mých, nikdy tê do mêsta nepustí.
		};
		
		AI_Output (self, other,"DIA_Canthar_WhatOffer_09_03"); //Mùžu ti pomoct dostat se do mêsta.
		AI_Output (self, other,"DIA_Canthar_WhatOffer_09_04"); //Mám tady kousek papíru. S královskou peèetí a podpisem místodržícího. Propustku.
				
		if (Npc_HasEquippedArmor(other) == FALSE)
		{	
			AI_Output (self, other,"DIA_Canthar_WhatOffer_09_05"); //S tímhle kouskem papíru v kapse mùžeš vypadat tak otrhanê, jak se ti zachce, a stráže tê nechají na pokoji.
		};
		...
		Info_AddChoice		(DIA_Canthar_WhatOffer, "Jak se dostaneš do mêsta TY?" 			,DIA_Canthar_WhatOffer_HowYouIn);
		Info_AddChoice		(DIA_Canthar_WhatOffer, "V tom musí být nêjaký háèek..."	,DIA_Canthar_WhatOffer_Strings);
	}
	else
	{
	...
	B_LogEntry (Topic_CityTrader,"Canthar obchoduje se všemožnými zbranêmi."); 
};

func void DIA_Canthar_WhatOffer_Strings()
{
	AI_Output (other, self,"DIA_Canthar_WhatOffer_Strings_15_00"); //V tom musí být nêjaký háèek.
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Strings_09_01"); //(pâehrává) Ne. Žádný háèek. Jenom mi budeš dlužit laskavost.
};

func void DIA_Canthar_WhatOffer_HowYouIn()
{
	AI_Output (other, self,"DIA_Canthar_WhatOffer_HowYouIn_15_00");  //Jak se dostaneš do mêsta TY?
	AI_Output (self, other,"DIA_Canthar_WhatOffer_HowYouIn_09_01"); //Stráže už mê znají. Prostê jim âeknu, že jsem svoji propustku ztratil.
};

func void DIA_Canthar_WhatOffer_Price()
{
	AI_Output (other, self,"DIA_Canthar_WhatOffer_Price_15_00"); //Co za tu propustku chceš?
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Price_09_01"); //(spokojenê) VÊDÊL JSEM, že ty jsi ten pravý!
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Price_09_02"); //Poslouchej. Teë ode mê dostaneš tu propustku.
	AI_Output (other, self,"DIA_Canthar_WhatOffer_Price_15_03"); //Jen tak?
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Price_09_04"); //Jen tak. ALE: Až tê ve mêstê potkám, mám u tebe laskavost.
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Price_09_05"); //Dohodnuto?
		
	Info_AddChoice (DIA_Canthar_WhatOffer, "Jasnê. Dej mi ten papír.",DIA_Canthar_WhatOffer_Ok);
};

func void DIA_Canthar_WhatOffer_Ok()
{
	AI_Output (other, self,"DIA_Canthar_WhatOffer_Ok_15_00"); //Jasnê. Dej mi ten papír.
	...
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Ok_09_02"); //Ještê jedna vêc: Ani nepomysli na to, že bys porušil slovo!
	AI_Output (self, other,"DIA_Canthar_WhatOffer_Ok_09_03"); //Jako obchodník se ve mêstê têším velké vážnosti - nedopadlo by to pro tebe dobâe, vêâ mi!
	...
	B_LogEntry (TOPIC_City,"Od obchodníka Canthara jsem dostal propustku, díky níž se dostanu mêsta. Dlužím mu za to protislužbu.");
...
func void DIA_Canthar_WhatOffer_No()
{
	...
	AI_Output (self, other,"DIA_Canthar_WhatOffer_No_09_01"); //Hm. Vypadá to, že jsem se v tobê spletl, co?
	AI_Output (self, other,"DIA_Canthar_WhatOffer_No_09_02"); //Dobrá. Chceš ještê nêco? Mùžu ti ukázat své zboží?
...
func void DIA_Canthar_PAYPRICEINCITY_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_09_00"); //Ještê stále mi dlužíš laskavost. Pâišel èas splatit dluh.
	}
	else
	{
	...
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_09_03"); //Chci zpátky svùj stánek na tržišti. Sarah mi ho ukradla.
	...
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_09_07"); //Pak pùjdeš za lordem Andrem a âekneš mu, že Sarah prodává zbranê Onarovi.
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_09_08"); //Skonèí ve vêzení, ty shrábneš peníze a já dostanu zpátky svùj stánek.
	...
	if (Canthar_Gefallen == FALSE)
	{
		...
		AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_09_11"); //Jakmile budu mít zpátky stánek, dostaneš ode mê zbraà - zatracenê dobrou zbraà.
	};
	...
	Info_AddChoice	(DIA_Canthar_PAYPRICEINCITY, "No tak dobâe, udêlám to.", DIA_Canthar_PAYPRICEINCITY_Ok );
...
func void DIA_Canthar_PAYPRICEINCITY_if ()
{
	...
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_if_09_02"); //Pokud se to roznese, mùže to ošklivê ublížit tvé povêsti ve mêstê.
};
...
func void DIA_Canthar_PAYPRICEINCITY_Nein ()
{
	...
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_Nein_09_01"); //To bylo špatné rozhodnutí, chlapèe. Ještê si spolu promluvíme.
...
func void DIA_Canthar_PAYPRICEINCITY_Ok ()
{
	...
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_Ok_09_03"); //Nech ji, aã ti ukáže své zboží -  a mezitím k ní ten dopis schovej.
	AI_Output (self, other, "DIA_Canthar_PAYPRICEINCITY_Ok_09_04"); //A nezapomeà, že mám ve mêstê znaèný vliv, takže žádné hlouposti.
	...
	if (Canthar_Gefallen == TRUE)
	{
		B_LogEntry (TOPIC_Canthar,"Canthar po mnê nyní chce tu laskavost, kterou mu stále dlužím.");
	}
	else
	{
		B_LogEntry (TOPIC_Canthar,"Canthar mi za onu laskavost slíbil pêknou zbraà.");
	};
	B_LogEntry (TOPIC_Canthar,"Mám obchodnici Sarah nenápadnê podstrèit dopis, který budí zdání, že prodává zbranê Onarovi. Pak mám zajít za lordem Andreem a udat ji.");
...
func void DIA_Canthar_SARAHERLEDIGT_Info ()
{
	if ((Npc_IsDead(Sarah)) == FALSE)
	{
		if (MIS_Canthars_KomproBrief_Day <= (Wld_GetDay()+ 2))
		{
			...
			AI_Output (self, other, "DIA_Canthar_SARAHERLEDIGT_09_01"); //Nejdâív ten dopis podstrè Sarah, pak zajdi za Andrem a obvià ji.
			AI_Output (self, other, "DIA_Canthar_SARAHERLEDIGT_09_02"); //Udêlej to a nepokoušej se mê podvést. Litoval bys toho.
		}
		else  
		{
			AI_Output (self, other, "DIA_Canthar_SARAHERLEDIGT_09_03"); //Utíká ti èas. Zaèni s tím už!
		};
	}
	else
	{
			AI_Output (other, self, "DIA_Canthar_SARAHERLEDIGT_15_04"); //Sarah je mrtvá.
			AI_Output (self, other, "DIA_Canthar_SARAHERLEDIGT_09_05"); //Opravdu? Výbornê. Tak to jsi splnil polovinu našeho malého plánu.
...
func void DIA_Canthar_Success_Info ()
{
	...
	AI_Output (self, other, "DIA_Canthar_Success_09_01"); //Pâevezmu její zboží, takže jestli potâebuješ zbraà, stav se u mê.
	...
	if (Canthar_Gefallen == FALSE)
	{
		AI_Output (other, self, "DIA_Canthar_Success_15_02"); //Slíbils mi zbraà.
		AI_Output (self, other, "DIA_Canthar_Success_09_03"); //Správnê. Tady, tahle zbraà je mistrovským kouskem kováâského umêní.
...
func void DIA_Canthar_Again_Info ()
{
	...
	{
		AI_Output (other,self , "DIA_Canthar_Again_15_01"); //Jak to, že nejsi ve vêzení?
		AI_Output (self,other, "DIA_Canthar_Again_09_02"); //Byl jsem propuštên. Jak už jsem ti âíkal - mám ve mêstê velký vliv.
	}
	else if (MIS_Canthars_KomproBrief == LOG_RUNNING)
	{
		AI_Output (self, other, "DIA_Canthar_Again_09_03"); //A mezitím jsem se se Sarah dohodl sám. Mùžeš na tu vêc s dopisem zapomenout.
		
	}
	else if (MIS_Canthars_KomproBrief == LOG_OBSOLETE)
	{
		AI_Output (self, other, "DIA_Canthar_Again_09_04"); //Copak jsem tê nevaroval? Bylo by líp, kdybys udêlal, co jsem ti âekl.
	};
	AI_Output (self, other, "DIA_Canthar_Again_09_05"); //Teë se mezi obchodníky povídá, že jsi trestanec.
	AI_Output (self, other, "DIA_Canthar_Again_09_06"); //Politováníhodná vêc - teë tady na tržišti nedostaneš ani kousek chleba.
	AI_Output (self, other, "DIA_Canthar_Again_09_07"); //Samozâejmê bych tu vêc mohl zase urovnat, âeknême...
...
func void DIA_Canthar_Pay_Info ()
{
	...
	Info_AddChoice (DIA_Canthar_Pay,"Nemám u sebe dost penêz...",DIA_Canthar_Pay_Nein);
...
FUNC VOID DIA_Canthar_Pay_Ja()
{
	...
	AI_Output (self, other, "DIA_Canthar_Pay_Ja_09_01"); //(smêje se) Výbornê. Tak to bych se mêl postarat o to, aby tvá reputace mezi obchodníky zase stoupla.
...
{
	AI_Output (other, self, "DIA_Canthar_Pay_Nein_15_00"); //Nemám u sebe dost penêz.
	AI_Output (self, other, "DIA_Canthar_Pay_Nein_09_01"); //Tak si je sežeà.
	Info_ClearChoices (DIA_Canthar_Pay);
	AI_StopProcessInfos (self);
};
...
{
	AI_Output (self, other, "DIA_Canthar_CANTHARANGEPISST_09_00"); //Varoval jsem tê, ale tys neposlouchal. Probereme to pozdêji.
	AI_Output (self, other, "DIA_Canthar_CANTHARANGEPISST_09_01"); //A teë se ztraã, chci si odpoèinout.
...
func void DIA_Canthar_MinenAnteil_Info ()
{
	...
	AI_Output (self, other, "DIA_Canthar_Minenanteil_09_01"); //No a? Co teda! Kdybych to nedêlal já, chytne se toho nêkdo jiný.
	B_GivePlayerXP (XP_Ambient);
};
