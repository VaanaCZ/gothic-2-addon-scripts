...
{	
	AI_Output (self ,other,"DIA_Addon_PIR_6_GimmeGrog_06_00"); //Uf, ten voheŕ tak hâeje, že z toho pęknę vysychá v krku!
	AI_Output (self ,other,"DIA_Addon_PIR_6_GimmeGrog_06_01"); //Hele, máš pro mę ŕákou koâalku?
	
	Log_CreateTopic (TOPIC_Addon_RoastGrog,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_Addon_RoastGrog,LOG_RUNNING);
	B_LogEntry (TOPIC_Addon_RoastGrog,"Pirát u ohništę zoufale potâebuje koâalku.");
...
FUNC VOID DIA_Addon_RoastPirate_GimmeGrog_HereIsGrog()
{
	...
	PIR_1364_Grog = TRUE;
	Info_ClearChoices (DIA_Addon_RoastPirate_GimmeGrog);
	B_LogEntry (TOPIC_Addon_RoastGrog,"Koâalka ho očividnę zachránila pâed smrtí žízní.");
...
func void DIA_Addon_RoastPirate_SeichtesWasser_Info ()
{
	...
	AI_Output	(self, other, "DIA_Addon_PIR_6_SeichtesWasser_06_03"); //Radši bych ale brousil po pláži a po męlčinę a hledal vyplavený vęci.
	...
	AI_Output	(self, other, "DIA_Addon_PIR_6_SeichtesWasser_06_06"); //Fakt doufám, že to tu za mę brzo nękdo vezme.
};
...
{	
	AI_Output (other,self ,"DIA_Addon_PIR_6_FRANCIS_15_00"); //Co bys mi povędęl o Francisovi?
	...
	AI_Output (self ,other,"DIA_Addon_PIR_6_Francis_06_04"); //Abych byl upâímnej, tak tu každýho šidí. Vsadím se, že si tu už pęknę namastil kapsu.
	AI_Output (self ,other,"DIA_Addon_PIR_6_Francis_06_05"); //Ale nic jsem ti neâek. Nemám totiž žádnej důkaz, tak to radši nikomu neâíkej.
};	
...
{	
	AI_Output (other,self ,"DIA_Addon_Matt_Job_15_00"); //Ještę nęco?
	...
	if (GregIsBack == TRUE)
	{
		...
		{
			AI_Output (self ,other,"DIA_Addon_PIR_6_Chef_06_01"); //Když se Greg vrátil, je tu víc práce, ale aspoŕ za ni dostanem poâádnę zaplacíno.
		};
	}
	else
	{
		...
		&& (!Npc_IsDead(Francis))
		{
			AI_Output (self ,other,"DIA_Addon_PIR_6_Chef_06_06"); //Francis je totální vůl. Vod tý doby, co si tu hraje na kapitána, tu jde všecko vod desíti k pęti.
		}
		else if (randy == 1)
		{
			AI_Output (self ,other,"DIA_Addon_PIR_6_Chef_06_05"); //Jen aă si banditi zaútočej. Budou bradou vzhůru dâív, než si uvędomí, která bije.
		}
		else
		{
...
{	
	AI_Output (other,self ,"DIA_Addon_RoastPirate_GoHome_15_00"); //Už tę nepotâebuju.
	B_Say(self, other, "$ABS_GOOD");
