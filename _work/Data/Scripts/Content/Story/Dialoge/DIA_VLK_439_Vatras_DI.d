...
func void DIA_Vatras_DI_HEAL_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Vatras_DI_HEAL_05_01"); //(zbožně) Adane, požehnej tomuto tělu. Je předurčeno obnovit rovnováhu tohoto světa.
		hero.attribute [ATR_HITPOINTS] = hero.attribute[ATR_HITPOINTS_MAX];
		PrintScreen (PRINT_FullyHealed, - 1, - 1, FONT_Screen, 2);   
	}
	else 
	{	
		AI_Output			(self, other, "DIA_Vatras_DI_HEAL_05_02"); //Tvé tělo je teď bez zranění.
	};	
};
	...
	if (Got_HealObsession_Day<=(Wld_GetDay()-2))  
		{
			...
			if (Npc_HasItems (self,ItPo_HealObsession_MIS))
			{
				...
				AI_Output			(self, other, "DIA_Vatras_DI_OBSESSION_05_02"); //Nicméně vždycky pamatuj: mé prostředky na odvrácení těch nočních můr jsou velmi omezené.
...
func void DIA_Vatras_DI_RAT_Info ()
{
	...
	{
		AI_Output			(other, self, "DIA_Vatras_DI_RAT_15_02"); //Nic se neděje, mám amulet na vyvolávání duší.
	};
...
func void DIA_Vatras_DI_PEDROTOT_Info ()
{
	...
	&& (Npc_IsDead(Pedro_DI)) 
	{
		...
		AI_Output			(self, other, "DIA_Vatras_DI_PEDROTOT_05_04"); //Vím o tvé odporné dohodě s těmi hady. Jsem hluboce otřesen tím, jak jsem nakonec pochybil.
		DIA_Vatras_DI_PEDROTOT_VatrasSucked = TRUE;
		AI_StopProcessInfos (self);
	}
	else
	{
		AI_Output			(self, other, "DIA_Vatras_DI_PEDROTOT_05_05"); //Tak ho přiveď sem na palubu. Předáme ho úřadům v Khorinidu.
...
{	
	...
	Info_AddChoice	  (DIA_Vatras_DI_Talente, "Alchymie - umění míchat lektvary", DIA_Vatras_DI_Talente_ALCHIMIE);
};
...
func void DIA_Vatras_DI_UndeadDragonDead_Info ()
{
	...
	if (DIA_Vatras_DI_UndeadDragonDead_OneTime == FALSE)
	{
		...
		{
			AI_Output (other, self, "DIA_Vatras_DI_UndeadDragonDead_15_03"); //Můžu teď odejít, nebo máte vy mágové v zásobě ještě nějaké kostlivce, co potřebujou opustit tenhle svět?
		}
		else
		{
			AI_Output (self, other, "DIA_Vatras_DI_UndeadDragonDead_05_04"); //Ale měj na paměti, že to byla jen pouhá epizoda z věčné války mezi dobrem a zlem.
		};
		
		AI_Output			(self, other, "DIA_Vatras_DI_UndeadDragonDead_05_05"); //Zlo si vždycky najde způsob, jak se dostat na tenhle svět. Válka nikdy neskončí.
		
		if (hero.guild == GIL_PAL)
		{
			AI_Output (self, other, "DIA_Vatras_DI_UndeadDragonDead_05_06"); //Jako bojovník dobra bys to měl vědět.
		};
			
		AI_Output (self, other, "DIA_Vatras_Add_05_15"); //Jedině sám Adanos stojí mezi válčícími bohy, aby zajistil rovnováhu sil.
		AI_Output (other, self, "DIA_Vatras_Add_15_16"); //Měl mi radši pomoct.
		AI_Output (self, other, "DIA_Vatras_Add_05_17"); //(uličnicky) Ale vždyť právě to udělal - tím si můžeš být jist.
					
		DIA_Vatras_DI_UndeadDragonDead_OneTime = TRUE;
	};	

	AI_Output (self, other, "DIA_Vatras_DI_UndeadDragonDead_05_09"); //Řekni kapitánovi, že má co nejdříve zvednout kotvy. Momentální mír může být dost ošidný.
};
