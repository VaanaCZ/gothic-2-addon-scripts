...
FUNC VOID DIA_Lee_PMSchulden_Info()
{
	...
	if (B_GetTotalPetzCounter(self) > Lee_LastPetzCounter)
	{
		...
		{
			AI_Output (self, other, "DIA_Lee_PMSchulden_04_03"); //Bude teda po tobì chtít taky víc penìz.
...
FUNC VOID DIA_Lee_PETZMASTER_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Lee_PETZMASTER_04_00"); //Kýho èerta - (pøekvapenì) - to jsi TY, ten nováèek, co jde z jednoho maléru do druhého?
...
INSTANCE DIA_Lee_Paladine   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Musím nutnì hovoøit s paladiny ve mìstì...";
};
...
func void DIA_Addon_Lee_Ranger_Info ()
{
	...
	AI_Output	(self, other, "DIA_Addon_Lee_Ranger_04_01"); //(smìje se) To jsem mìl vìdìt. Ty prostì musíš strkat nos do všeho, co?
	AI_Output	(other, self, "DIA_Addon_Lee_Ranger_15_02"); //No tak, øekni mi to.
	...
	AI_Output	(self, other, "DIA_Addon_Lee_Ranger_04_04"); //Od pádu bariéry už mì ale k vodním mágùm nepoutá žádná smlouva jako tenkrát.
	AI_Output	(self, other, "DIA_Addon_Lee_Ranger_04_05"); //Samozøejmì že pomùžu, když to pùjde. Ale mám tu dost vlastních problémù a na ostatní vìci nemám moc èas.
	AI_Output	(self, other, "DIA_Addon_Lee_Ranger_04_06"); //Jestli se o tom spolku chceš dozvìdìt víc, promluv si s Cordem. Co já vím, tak patøí k nim.
...
FUNC VOID DIA_Lee_JoinNOW_Info()
{
	...
	{		
		...
		else //Onar ist einverstanden
		{
			...
			AI_Output (self, other, "DIA_Lee_JoinNOW_04_17"); //Má to co do èinìní s paladiny. Je naèase, aby ses za nimi vypravil.
...
FUNC VOID DIA_Lee_AngebotSuccess_Info()
{
	...
	AI_Output (other, self, "DIA_Lee_AngebotSuccess_15_02"); //Øekl, že tobì mùže zajistit prominutí tvých èinù, ale tvým lidem ne.
	...
	AI_Output (self, other, "DIA_Lee_AngebotSuccess_04_06"); //Že bych sám vytáhl krk z oprátky a opustil své lidi, to vùbec nepøipadá v úvahu.
...
{	
	...
	{
		AI_Output (self ,other,"DIA_Lee_ArmorM_04_03"); //Torlof dostal od Onara úkol, o který se mìl už pøed èasem postarat.
		AI_Output (self ,other,"DIA_Lee_ArmorM_04_04"); //Takže to nejdøív zaøiï - pak si mùžeme promluvit o lepším brnìní!
	};
};
...
instance DIA_Lee_BuyArmorM (C_INFO)
{
	...
	permanent	= TRUE;
	description = "Koupit si støednì tìžkou žoldnéøskou zbroj. Ochrana: zbranì 45, šípy 45, cena 1000 zlaákù.";
};                       
...
instance DIA_Lee_BuyArmorH (C_INFO)
{
	...
	permanent	= TRUE;
	description = "Koupit tìžkou žoldnéøskou zbroj. Ochrana: zbranì 60, šípy 60, cena: 2500 zlaákù.";
};                       
...
{	
	...
	B_LogEntry (TOPIC_RichterLakai,"Lee chce, abych našel nìjaký inkriminující dùkaz proti khorinidskému soudci. Mám mu proto nabídnout své služby a pøitom mít oèi na stopkách."); 
...
{	
	...
	AI_Output (other,self ,"DIA_Lee_RichterBeweise_15_02"); //Najal si pár ranaøù, aby pøepadli místodržícího Khorinidu.
...
{	
	...
	AI_Output (self ,other,"DIA_Lee_TalkAboutBennet_04_01"); //Takže už jsi o tom slyšel. Ti bastardi ho zavøeli, jen tak pro nic za nic.
	if ((hero.guild != GIL_MIL)&&(hero.guild != GIL_PAL))
	{
...
{	
	...
	AI_Output (self ,other,"DIA_Lee_DoAboutBennet_04_01"); //Ještì nevím. Pár chlapcù by rádo vtrhlo do mìsta, aby povyráželi lordu Hagenovi všechny zuby, jeden po druhém.
	...
	B_LogEntry (TOPIC_RESCUEBENNET,"Když dostateènì rychle neprokážu Bennetovu nevinu, Lee za další následky už neponese zodpovìdnost. Jeho lidé bez váhání kdykoliv napadnou mìsto a Benneta osvobodí násilím."); 
	...
	&& ((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
	{ 
		AI_Output (self ,other,"DIA_Lee_DoAboutBennet_04_07"); //Jo, a abych nezapomnìl... Buster s tebou chce mluvit. Nechtìl mi øíct, o co jde. Možná by ses za ním mìl stavit!
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Lee_CanHelpYou_04_01"); //Jasnì, troška rozumu a rozvahy nemùže být na škodu.
...
instance DIA_Lee_DRACHENEI		(C_INFO)
{
	...
	description	 = 	"Ještìøi roznášejí draèí vejce po celé zemi.";
};
...
{
	AI_Output			(other, self, "DIA_Lee_DRACHENEI_15_00"); //Ještìøi roznášejí draèí vejce po celé zemi.
	B_GivePlayerXP (XP_Ambient);
	AI_Output			(self, other, "DIA_Lee_DRACHENEI_04_01"); //Mìlo mì to napadnout. Je naèase vypadnout.
	...
	if (hero.guild == GIL_DJG)
	{
		...
		AI_Output			(self, other, "DIA_Lee_DRACHENEI_04_04"); //Ze skoøápek by možná šlo udìlat brnìní nebo tak nìco.
...
{	
	...
	AI_Output (self ,other,"DIA_Lee_KAP4_Perm_04_03"); //Práce bohužel neubylo. Chlapi jsou èím dál tím nespokojenìjší, protože musejí pracovat i za Sylviovy lidi.
...
func void DIA_Lee_GetShip_Info ()
{
	...
	Log_SetTopicStatus(TOPIC_Ship, LOG_RUNNING);                                                                                                                                                                                                                                                 
		...
		&& ((Npc_IsDead(Richter))== FALSE)
		{
			AI_Output	(self, other, "DIA_Lee_GetShip_04_04"); //Koneckoncù máme soudce ve své moci. Zkus za ním zajít a vymáèknout z nìj dopis, který by tì k tomu opravòoval.
		...
		{
			AI_Output	(self, other, "DIA_Lee_GetShip_04_05"); //Mám tady falešné oprávnìní. S jeho pomocí tì stráže nechají projít.
...
func void DIA_Lee_GetShip_crew ()
{
	...
	B_LogEntry (TOPIC_Crew,"Ve vìci posádky toho pro mì Lee moc udìlat nemohl. Ale poradil mi, že bych mìl najmout jen lidi, kterým mohu vìøit. Mám se zeptat ještì Benneta, mohlo by ho to zajímat.");
};
