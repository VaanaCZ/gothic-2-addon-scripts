...
func void DIA_Balthasar_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_HALLO_05_01"); //Ach, nová tváâ na farmê. Že by mi nakonec ta stará držgrešle pâece jen sehnala uèedníka?
	AI_Output			(other, self, "DIA_Balthasar_HALLO_15_02"); //Nejsem žádný uèedník.
	AI_Output			(self, other, "DIA_Balthasar_HALLO_05_03"); //Hm, to vidím. Nejsem už tak mladý jako dâív, však víš. A už léta èekám na nêkoho, kdo by to po mnê pâevzal.
...
func void DIA_Balthasar_WASMACHSTDU_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_WASMACHSTDU_05_01"); //No, hlídám ovce a dêlám to tak dlouho, co si jen pamatuju.
	AI_Output			(self, other, "DIA_Balthasar_WASMACHSTDU_05_02"); //Ta zvíâata jsou velmi oddaná a chytrá, však víš. Když se o nê dobâe staráš, budou tê poslouchat až do konce života.
...
instance DIA_Addon_Balthasar_Rangerbandits		(C_INFO)
{
	...
	description	 = 	"Nevšiml sis, že by tudy procházeli nêjací bandité?";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Balthasar_Rangerbandits_15_00"); //Nevidêls tudy procházet nêjaké bandity?
	AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_01"); //Samozâejmê. Šli podél pole u Sekobova statku a na severu vstoupili do lesa.
	
	if (MIS_Vatras_FindTheBanditTrader == LOG_RUNNING)	
	{
		AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_02"); //Mêli s sebou spoustu zbraní. Vyzbrojili by tím i královu armádu.
	};
	
	if (SC_KnowsLuciaCaughtByBandits == TRUE)
	{
		AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_03"); //Jestli mê nešálil zrak, tak do lesa táhli i nêjakou mladou ženu.
...
func void DIA_Balthasar_AERGERMITNACHBARN_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_01"); //Tráva na téhle stranê lánu je vždycky zakrslá a suchá. Tak vêtšinou chodím na pastviny jiných farmáâù.
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_02"); //Velkostatkáâi to nevadí, ale Bengar, farmáâ z náhorních pastvin, dêlá hroznej virvál, když tam vezmu naše zvíâata.
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_03"); //Âíká, že by mu mêl Sekob platit za to, že beru ovce na Bengarovy pastviny.
...
func void DIA_Balthasar_WOBENGAR_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_WOBENGAR_05_01"); //Jdi po téhle cestê, až narazíš na kâižovatku.
...
instance DIA_Balthasar_TALKTOBENGAR		(C_INFO)
{
	...
	description	 = 	"Možná bych si mêl promluvit s Bengarem.";
};
...
{
	AI_Output			(other, self, "DIA_Balthasar_TALKTOBENGAR_15_00"); //Možná bych si mêl promluvit s Bengarem.
	AI_Output			(self, other, "DIA_Balthasar_TALKTOBENGAR_05_01"); //Ty bys to pro mê udêlal? Co mu âekneš? Totiž, nechtêl bych žádný problémy, rozumíš?
...
instance DIA_Balthasar_BENGARUEBERREDET		(C_INFO)
{
	...
	description	 = 	"Mùžeš své ovce opêt pâivést za Bengarovy pastviny.";
};
...
{
	AI_Output			(other, self, "DIA_Balthasar_BENGARUEBERREDET_15_00"); //Už zase mùžeš svoje ovce brát na Bengarovy pastviny. Právê jsem s ním mluvil.
	AI_Output			(self, other, "DIA_Balthasar_BENGARUEBERREDET_05_01"); //Díky. Hned vyrazím na cestu.
	AI_Output			(self, other, "DIA_Balthasar_BENGARUEBERREDET_05_02"); //Tady, vezmi si na dùkaz mé vdêènosti tuhle ovèí kùži.
...
func void DIA_Balthasar_PERMKAP1_Info ()
{
	...
	if 	(
				...
				{
					AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_01"); //Nemùžu si stêžovat. Díky za optání.
					AI_StopProcessInfos (self);
				}
	else
			{
				AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_02"); //Jsou to teë têžké èasy. Nemùžu najít své ovce!
				if (Npc_IsDead(Sekob)== FALSE)
				{
					AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_03"); //Hádám, že bych o tom mêl âíct Sekobovi.
				};
				AI_StopProcessInfos (self);
				Npc_ExchangeRoutine	(self,"Zaèít"); //Joly: falls er auf der 2. Weide bei Bengar ist.
			};
};
