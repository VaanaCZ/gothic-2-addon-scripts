...
func void DIA_Balthasar_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_HALLO_05_01"); //Ach, nová tvář na farmě. Že by mi nakonec ta stará držgrešle přece jen sehnala učedníka?
	AI_Output			(other, self, "DIA_Balthasar_HALLO_15_02"); //Nejsem žádný učedník.
	AI_Output			(self, other, "DIA_Balthasar_HALLO_05_03"); //Hm, to vidím. Nejsem už tak mladý jako dřív, však víš. A už léta čekám na někoho, kdo by to po mně převzal.
...
func void DIA_Balthasar_WASMACHSTDU_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_WASMACHSTDU_05_01"); //No, hlídám ovce a dělám to tak dlouho, co si jen pamatuju.
	AI_Output			(self, other, "DIA_Balthasar_WASMACHSTDU_05_02"); //Ta zvířata jsou velmi oddaná a chytrá, však víš. Když se o ně dobře staráš, budou tě poslouchat až do konce života.
...
instance DIA_Addon_Balthasar_Rangerbandits		(C_INFO)
{
	...
	description	 = 	"Nevšiml sis, že by tudy procházeli nějací bandité?";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Balthasar_Rangerbandits_15_00"); //Neviděls tudy procházet nějaké bandity?
	AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_01"); //Samozřejmě. Šli podél pole u Sekobova statku a na severu vstoupili do lesa.
	
	if (MIS_Vatras_FindTheBanditTrader == LOG_RUNNING)	
	{
		AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_02"); //Měli s sebou spoustu zbraní. Vyzbrojili by tím i královu armádu.
	};
	
	if (SC_KnowsLuciaCaughtByBandits == TRUE)
	{
		AI_Output	(self, other, "DIA_Addon_Balthasar_Rangerbandits_05_03"); //Jestli mě nešálil zrak, tak do lesa táhli i nějakou mladou ženu.
...
func void DIA_Balthasar_AERGERMITNACHBARN_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_01"); //Tráva na téhle straně lánu je vždycky zakrslá a suchá. Tak většinou chodím na pastviny jiných farmářů.
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_02"); //Velkostatkáři to nevadí, ale Bengar, farmář z náhorních pastvin, dělá hroznej virvál, když tam vezmu naše zvířata.
	AI_Output			(self, other, "DIA_Balthasar_AERGERMITNACHBARN_05_03"); //Říká, že by mu měl Sekob platit za to, že beru ovce na Bengarovy pastviny.
...
func void DIA_Balthasar_WOBENGAR_Info ()
{
	...
	AI_Output			(self, other, "DIA_Balthasar_WOBENGAR_05_01"); //Jdi po téhle cestě, až narazíš na křižovatku.
...
instance DIA_Balthasar_TALKTOBENGAR		(C_INFO)
{
	...
	description	 = 	"Možná bych si měl promluvit s Bengarem.";
};
...
{
	AI_Output			(other, self, "DIA_Balthasar_TALKTOBENGAR_15_00"); //Možná bych si měl promluvit s Bengarem.
	AI_Output			(self, other, "DIA_Balthasar_TALKTOBENGAR_05_01"); //Ty bys to pro mě udělal? Co mu řekneš? Totiž, nechtěl bych žádný problémy, rozumíš?
...
instance DIA_Balthasar_BENGARUEBERREDET		(C_INFO)
{
	...
	description	 = 	"Můžeš své ovce opět přivést za Bengarovy pastviny.";
};
...
{
	AI_Output			(other, self, "DIA_Balthasar_BENGARUEBERREDET_15_00"); //Už zase můžeš svoje ovce brát na Bengarovy pastviny. Právě jsem s ním mluvil.
	AI_Output			(self, other, "DIA_Balthasar_BENGARUEBERREDET_05_01"); //Díky. Hned vyrazím na cestu.
	AI_Output			(self, other, "DIA_Balthasar_BENGARUEBERREDET_05_02"); //Tady, vezmi si na důkaz mé vděčnosti tuhle ovčí kůži.
...
{
	AI_Output			(other, self, "DIA_Balthasar_PERMKAP1_15_00"); //A co jinak. Všechno v pořádku?
	...
	if 	(
				...
				{
					AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_01"); //Nemůžu si stěžovat. Díky za optání.
					AI_StopProcessInfos (self);
				}
	else
			{
				AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_02"); //Jsou to teď těžké časy. Nemůžu najít své ovce!
				if (Npc_IsDead(Sekob)== FALSE)
				{
					AI_Output			(self, hero, "DIA_Balthasar_PERMKAP1_05_03"); //Hádám, že bych o tom měl říct Sekobovi.
				};
				AI_StopProcessInfos (self);
