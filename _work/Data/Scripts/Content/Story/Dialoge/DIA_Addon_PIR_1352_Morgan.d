...
func int DIA_Addon_Morgan_EXIT_Info ()
{
	if (GregIsBack == FALSE)
	{
		AI_Output	(other, self, "DIA_Addon_Morgan_Perm_15_00"); //Bìž se natáhnout.
...
{
	AI_Output (self, other, "DIA_Addon_Morgan_Anheuern_07_00"); //To je úžasný, teï budu muset furt øezat prkna.
...
{	
	...
	if (GregIsBack == FALSE)
	{
		...
		AI_Output (self ,other,"DIA_Addon_Morgan_Meat_07_02"); //To je lepší. Takže ještì jednou – co chceš?
		AI_Output (other,self ,"DIA_Addon_Morgan_Meat_15_03"); //Mám ti donést nìjaké maso od Krokodýla Jacka.
	};
...
{	
	...
	AI_Output (self, other, "DIA_Addon_Morgan_Job_07_01"); //Greg ze mì udìlal velitele jedný ze dvou nájezdnickejch skupin.
	...
	if (GregIsBack == FALSE)
	{
		AI_Output (self, other, "DIA_Addon_Morgan_Job_07_05"); //Øek jsem jim, že až se Greg vrátí, nechci tu vidìt ani jednu potvoru (zívá).
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Morgan_Sleep_07_02"); //Mùj úkol je nejdùležitìjší ze všech.
	AI_Output (self ,other,"DIA_Addon_Morgan_Sleep_07_03"); //Já svý lidi cvièím.
	AI_Output (self ,other,"DIA_Addon_Morgan_Sleep_07_04"); //Dìlám z nich nejlepší a nejobávanìjší bojovníky, který se po tìchhle vodách kdy plavili.
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_01"); //(smìje se) K mýmu voddílu? Ten se potlouká nìkde po pláži.
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_03"); //Ti chlapi nehnou prstem, dokud se kapitán nevrátí, to je jistý.
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_04"); //Ale jestli chceš všem ukázat, že za nìco stojíš, klidnì tu práci na severní pláži udìlej za nì.
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_05"); //Je tam plno èíhavcù a kdo ví èeho ještì.
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_06"); //Jestli je zlikviduješ sám, všichni si tì (zííív) budou vážit.
	AI_Output (self ,other,"DIA_Addon_Morgan_JoinMorgan_07_07"); //Tak tì vítám ve svým voddíle. Ha! A teï si zase lehnu!
	
	Log_CreateTopic (TOPIC_Addon_MorganBeach,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_Addon_MorganBeach,LOG_RUNNING);
	B_LogEntry (TOPIC_Addon_MorganBeach,"Když jsem se pøidal k Morganovým lupièùm, dostal jsem nový úkol: mám vyèistit severní pláž ode všech potvor.");
...
{
	AI_Output (other, self, "DIA_Addon_Morgan_LurkerPlatt_15_00"); //Severní pláž už je úplnì vyèištìná.
	AI_Output (self, other, "DIA_Addon_Morgan_LurkerPlatt_07_01"); //A co ta jeskynì? Tam už jsi taky byl?
	...
	if (Npc_IsDead(BeachShadowbeast1))
	{
		...
		B_LogEntry (TOPIC_Addon_MorganBeach,"Pláž už je vyèištìná - všechny potvory jsou pryè.");
	...
	else
	{
		...
		AI_Output	(self, other, "DIA_Addon_Morgan_LurkerPlatt_07_06"); //No, ta k tomu taky patøí. Vra se, až to budeš mít celý.
...
{	
	...
	if (Morgan_AngusStory == FALSE)
	{
		...
		AI_Output (self ,other,"DIA_Addon_Morgan_FOUNDTHEM_07_04"); //No, svým zpùsobem byl ten prsten JEHO, páè ho na mnì vyhrál v kostkách.
...
func void DIA_Addon_Morgan_FOUNDTHEM_NoRing()
{
	...
	AI_Output (self ,other,"DIA_Addon_Morgan_FOUNDTHEM_NoRing_07_01"); //(zdìšenì) Cože? Vra se tam podívej se po nìm ještì jednou! Museli ho mít!
...
func void DIA_Addon_Morgan_FOUNDTHEM_GiveRing()
{
	...
	AI_Output (self, other,"DIA_Addon_Morgan_FOUNDTHEM_GiveRing_07_01"); //(šastný jako dìcko) Jo, to je von! Hodnej kluk!
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Morgan_Francis_07_04"); //(škodolibì) Ale až se vrátí Greg, Francis bude zase jen øezat fošny.
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Morgan_TRAIN_07_01"); //Jasnì, mùžu ti ukázat, jak bojovat jednoruèníma zbranìma.
...
func VOID DIA_Addon_Morgan_Teach_Back()
{
	if (other.HitChance[NPC_TALENT_1H] > Morgan_Merke1h)
	{
		...
		{
			AI_Output (self,other,"DIA_Addon_Morgan_CommentFightSkill_07_00"); //Zapomeò na ty kecy vo cti a tak. Když nezabiješ, skonèíš zabitej.
			Morgan_Labercount = 1;	
		}
		...
		{
			AI_Output (self,other,"DIA_Addon_Morgan_CommentFightSkill_07_02"); //Ha, ha, ha, aspoò že teï udržíš meè.
			Morgan_Labercount = 0;	
		};
	}
	else if (other.HitChance[NPC_TALENT_1H] >= 75)
	{
		AI_Output (self ,other,"DIA_Addon_Morgan_Teach_Back_07_00"); //Jestli chceš bejt ještì lepší, budeš se muset uèit u nìkoho jinýho.
	};
	Info_ClearChoices (DIA_Addon_Morgan_Teach);
};
