...
func void DIA_Valentino_HALLO_Info ()
{
	...
	|| (hero.guild == GIL_PAL)
	{
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_01"); //Jmenuji se Valentino. Snažím se, abych tento den, který mi Innos dal, neznesvêtil žádnou prací.
		AI_Output			(other, self, "DIA_Valentino_HALLO_15_02"); //Dávej pozor na to, co âíkáš.
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_03"); //Ach, omluv mê. Nechtêl jsem Tvou Milost nijak urazit, ó, prosím za odpuštêní, pane.
	}
	else
	{
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_04"); //Copak se neumíš chovat? To je nechutné!
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_05"); //Poâád tahle špinavá chátra! Nemáš být náhodou nêkde v práci?
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_06"); //Ale poèítám, že takového ošuntêlého chlápka jako ty si každý všimne, co?
...
instance DIA_Valentino_WhoAgain		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Ptal jsem se tê, kdo jsi!";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_01"); //Ptal jsem se tê, kdo jsi!
	...
	AI_Output			(self, other, "DIA_Valentino_HALLO_wasmachstdu_03_03"); //Jestli máš problémy, nech si je pro sebe, já na nê zvêdavý nejsem.
};
...
instance DIA_Valentino_Manieren		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Zdá se mi, že bych ti mêl uštêdâit menší lekci slušného chování!";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_02"); //Zdá se mi, že bych ti mêl uštêdâit menší lekci slušného chování!
	//AI_Output			(other, self, "DIA_Valentino_HALLO_klappe_15_00"); //Für deine dünnen Ärmchen hast du 'ne ganz schön große Klappe!
	AI_Output			(self, other, "DIA_Valentino_HALLO_klappe_03_01"); //Mnê to nevadí. Jen si mê klidnê zbij. Až se zítra ráno probudím, zase mi bude dobâe.
...
instance DIA_Valentino_WASNUETZLICHES		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Chceš mi âíct ještê nêco jiného?";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_03"); //(klidnê) Chceš mi âíci ještê nêco jiného?
	...
	&& (other.guild != GIL_KDF)
	{
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_01"); //Získej si vêhlas, nikdy nikomu nic neslibuj, ber si, co chceš, a dávej si pozor, aby ses nedostal do sporu s domobranou.
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_02"); //Nebo se žárlivými manželi, samozâejmê - ti jsou ze všech nejhorší, to ti povím.
		Valentino_Lo_Perm = TRUE;
	}
	else if (Valentino_Hi_Perm == FALSE)
	&& ( (other.guild == GIL_PAL) || (other.guild == GIL_KDF) )
	{
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_03"); //Pro muže tvého postavení není nic nemožné. Prostê jen musíš poznat, kdy je èas k èinu.
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_04"); //Tak neztrácej èas a jednej!
		Valentino_Hi_Perm = TRUE;
	}
	else
	{
		AI_Output (self ,other,"DIA_Valentino_WASNUETZLICHES_03_05"); //Už jsem ti âekl všechno, co potâebuješ vêdêt - zbytek je na tobê.
	};
};
