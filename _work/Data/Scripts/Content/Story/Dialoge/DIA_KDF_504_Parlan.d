...
{
	AI_Output (self, other, "DIA_Parlan_HAMMER_05_00"); //(pâísnê) Poèkej chvíli, synu.
	AI_Output (self, other, "DIA_Parlan_HAMMER_05_01"); //Âíká se, že ze svatynê 'zmizel' cenný artefakt.
...
FUNC VOID DIA_Parlan_PMSchulden_Info()
{
	...
	AI_Output (self, other, "DIA_Parlan_PMSchulden_05_00"); //Porušil jsi zákon našeho spoleèenství.

	if (B_GetTotalPetzCounter(self) > Parlan_LastPetzCounter)
	{
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_01"); //A i když jsi už byl obvinên, provinil ses ještê víc.
		
		if (Parlan_Schulden < 1000)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_02"); //Tvá vina mùže být smyta jen vêtším darem klášteru.
		...
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_03"); //Kdo si myslíš, že jsi? Modli se k Innosovi za odpuštêní svých skutkù!
		};
	}
	else if (B_GetGreatestPetzCrime(self) < Parlan_LastPetzCrime)
	{
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_04"); //Tvoje situace se zmênila.
		
		if (Parlan_LastPetzCrime == CRIME_MURDER)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_05"); //Nejsou už žádní svêdkové tvé vraždy!
		};
		
		if (Parlan_LastPetzCrime == CRIME_THEFT)
		|| ( (Parlan_LastPetzCrime > CRIME_THEFT) && (B_GetGreatestPetzCrime(self) < CRIME_THEFT) )
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_06"); //Nikdo už nebude svêdèit, že tê vidêl krást!
		};
		
		if (Parlan_LastPetzCrime == CRIME_ATTACK)
		|| ( (Parlan_LastPetzCrime > CRIME_ATTACK) && (B_GetGreatestPetzCrime(self) < CRIME_ATTACK) )
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_07"); //Už tê nikdo neobviàuje z toho, že by ses pral!
		};
		
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_08"); //Všechna obvinêní proti tobê byla stažena!
		};
		
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_09"); //Možná se ti podaâí mê ošálit - ale Innos zná tvé pravé skutky!
				
		// ------- Schulden erlassen oder trotzdem zahlen ------
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_10"); //Tvé hâíchy ti byly odpuštêny.
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_11"); //A snaž se, aby to tak také zùstalo!
		...
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_12"); //Zaplatíš pokutu - taková je vùle Innosova!
			B_Say_Gold (self, other, Parlan_Schulden);
	...
	if (B_GetGreatestPetzCrime(self) != CRIME_NONE)
	{
		...
		Info_AddChoice		(DIA_Parlan_PMSchulden,"Nemám dost penêz!",DIA_Parlan_PETZMASTER_PayLater);
...
FUNC VOID DIA_Parlan_PETZMASTER_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_00"); //Ještê jsme spolu nemluvili, a už na sobê máš bâemeno viny!
	};
	
	if (B_GetGreatestPetzCrime(self) == CRIME_MURDER) 
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_01"); //Provinil ses nejhorším ze všech zloèinù! Vraždou!
		...
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_02"); //A zatížil ses ještê další vinou!
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_03"); //Kdyby záleželo na mnê, na místê bych tê odsoudil, vrahu!
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_04"); //Ale když za svùj zloèin zaplatíš pokutu, zbavím tê tvé viny.
	};
		
	if (B_GetGreatestPetzCrime(self) == CRIME_THEFT) 
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_05"); //Krádež je porušením zákonù kláštera!
		if ((PETZCOUNTER_City_Attack + PETZCOUNTER_City_Sheepkiller) > 0)
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_06"); //Kromê toho jsi porušil i další zákony.
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_07"); //Tyto prohâešky musí být potrestány. Tak zní Innosovo pâikázání.
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_08"); //Zaplatíš klášteru pâimêâenou pokutu.
	...
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_09"); //V klášteâe netolerujeme rvaèky. Takové chování se protiví našim zákonùm.
		
		if (PETZCOUNTER_City_Sheepkiller > 0)
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_10"); //A proè zabíjíš naše ovce?
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_11"); //Žijeme tady podle pâikázání. A ta se vztahují i na tebe.
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_12"); //Za svùj zloèin musíš dát klášteru dar.
	...
	Info_ClearChoices  	(DIA_Parlan_PMSchulden);
	Info_ClearChoices  	(DIA_Parlan_PETZMASTER);
	Info_AddChoice		(DIA_Parlan_PETZMASTER,"Nemám dost penêz!",DIA_Parlan_PETZMASTER_PayLater);
	if (Npc_HasItems(other, itmi_gold) >= Parlan_Schulden)
	{
...
func void DIA_Parlan_PETZMASTER_PayNow()
{
	...
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayNow_05_01"); //Pâijímám tvùj dar. Tvé hâíchy jsou odpuštêny. Nechã ti Innos dá moudrost, abys stejné chyby již víckrát neopakoval.
...
{
	AI_Output (other, self, "DIA_Parlan_PETZMASTER_PayLater_15_00"); //Nemám dost penêz!
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayLater_05_01"); //Na to jsi mêl myslet, než ses svých prohâeškù dopustil.
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayLater_05_02"); //Možná se z toho ponauèíš. Nedêlej žádné další potíže, nebudeš muset platit.
...
{
			AI_Output (self, other, "DIA_Parlan_WELCOME_05_09"); //Až to udêláš, promluvíme si o tvé práci tady v klášteâe.
};
...
func void DIA_Parlan_WELCOME_Info ()
{
	...
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_02"); //(pâeruší tê)... NOVIC. Není dùležité, kdo jsi byl pâedtím. Teë jsi Innosùv služebník. To je vše, na èem záleží.
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_03"); //A jakmile budeš pâijat do bratrstva ohnê, budou všechny tvé pâedchozí prohâešky odpuštêny.
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_04"); //Nechã Innos požehná tomuto muži, který zasvêtí svùj život jeho službê, a smyje z nêj všechny jeho hâíchy.
	...
	Snd_Play ("LEVELUP");
		...
		B_LogEntry (Topic_Gemeinschaft, "Jako novic mám povinnost plnit úkoly, které poslouží celému spoleèenství.");
	...
	if (Pedro_NOV_Aufnahme_LostInnosStatue_Daron == FALSE)//ADDON
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_WELCOME_05_08"); //Své zlato mùžeš dát Goraxovi, našemu správci. Najdeš ho ve stáèírnê vína.
...
func void DIA_Parlan_Auge_Info ()
{
	...
	AI_Output (self, other, "DIA_Parlan_Auge_05_01"); //Nevím, kdo ti o nêm âekl, ale k tomuto posvátnému artefaktu nemá nikdo pâístup.
	AI_Output (other, self, "DIA_Parlan_Auge_15_02"); //Kde je Innosovo oko uschováno?
	AI_Output (self, other, "DIA_Parlan_Auge_05_03"); //Èím ménê lidí toto tajemství zná, tím lépe. Hledej ho - a nenajdeš jej.
};
...
instance DIA_Parlan_Amulett		(C_INFO)
{
	...
	permanent 	 =  FALSE; 
	description	 =  "Povêz mi nêco o Innosovê oku.";
};
...
{
	AI_Output (other, self, "DIA_Parlan_Amulett_15_00"); //Âekni mi nêco o Innosovê oku.
	AI_Output (self, other, "DIA_Parlan_Amulett_05_01"); //Dobrá. Innosovo oko obsahuje èást Innosovy svaté moci. Nosit tento amulet mùže jen JEDINÝ vyvolený Innosùv služebník.
	AI_Output (self, other, "DIA_Parlan_Amulett_05_02"); //Bylo vytvoâeno, aby nás v pravý èas ochránilo pâed silami zla.
};
///////////////////////////////////////////////////////////////////////
//	Info Hagen
///////////////////////////////////////////////////////////////////////
instance DIA_Parlan_Hagen		(C_INFO)
{
	...
	permanent 	 =  FALSE; 
	description	 =  "Potâebuji si promluvit s vùdcem paladinù!";
};
...
{
	AI_Output (other, self, "DIA_Parlan_Hagen_15_00"); //Potâebuji si promluvit s vùdcem paladinù!
...
instance DIA_Parlan_WORK		(C_INFO)
{
	...
	permanent	 =  TRUE;
	description	 = 	"Jak mohu posloužit zdejší komunitê?";
};
//------------------------------------
...
{
	AI_Output			(other, self, "DIA_Parlan_WORK_15_00"); //Jak mohu posloužit zdejší komunitê?
	...
	{
		AI_Output (self, other, "DIA_Parlan_WORK_NIGHT_05_01"); //Je èas klidu. Práce novicù zaèíná za rozbâesku a pokraèuje až do setmêní.
		AI_Output (self, other, "DIA_Parlan_WORK_NIGHT_05_02"); //Mêl by ses vyspat. Pâijë, až nadejde èas práce.
	...
	else
	{
		...
		AI_Output (self, other, "DIA_Parlan_WORK_05_04"); //Já budu na tvou práci dohlížet, a až své povinnosti uspokojivê splníš, dostaneš povolení vstoupit do knihovny a studovat Innosovo uèení.
		
		DIA_Parlan_WORK_perm = TRUE;
		MIS_KlosterArbeit = LOG_RUNNING; 
		B_LogEntry (Topic_Gemeinschaft, "Když splním úkoly pro mágy, dovolí mi pak studovat v knihovnê.");
...
func void DIA_Parlan_Stand_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_03"); //Zdolal jsi èerného vlka, který ohrožoval svatyni.
		Kloster_Punkte = (Kloster_Punkte + 1);
	};
	if (MIS_ParlanFegen == LOG_SUCCESS)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_04"); //Podaâilo se ti najít ètyâi novice a tím pádem i uklidit pokoje.
		Kloster_Punkte = (Kloster_Punkte + 3);
	};
	if (MIS_GoraxEssen == LOG_SUCCESS)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_05"); //Rozdêlil jsi jídlo mezi novice spravedlivê, pâesnê jak ti bratr Gorax nakázal.
		Kloster_Punkte = (Kloster_Punkte + 1);
	}
	else if (MIS_GoraxEssen == LOG_FAILED)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_06"); //Gorax s tebou nebyl spokojen, protože jsi jídlo nerozdêlil spravedlivê.
	};	
	if (MIS_GoraxWein == LOG_SUCCESS)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_07"); //Prodal jsi víno ke Goraxovê spokojenosti.
		Kloster_Punkte = (Kloster_Punkte + 1);
	}
	else if (MIS_GoraxWein == LOG_FAILED)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_08"); //Èím ménê toho bude âeèeno o té záležitosti s vínem, tím lépe.
	};	
	
	if (Kloster_Punkte >= 8)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_09"); //Ano, už jsi pracoval dost. Nadešel èas, abys studoval Innosovo uèení. Promluv si s mistrem Hyglasem v knihovnê.
		AI_Output (self, other, "DIA_Parlan_Stand_05_10"); //Zde ti dávám klíè.
	...
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_11"); //Nepracoval jsi málo, ale tvou povinností je dál sloužit spoleèenství.
	}
	else  //0 Punkte
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_12"); //Nejprve splà své úkoly ve službê spoleèenství. Až budeš se vším hotov, mùžeme si o tom promluvit znovu.
	};
};
///////////////////////////////////////////////////////////////////////
//	Info Aufgabe
...
{
	AI_Output (other, self, "DIA_Parlan_Aufgabe_15_00"); //Máš pro mê nêjaký úkol?
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_01"); //Hmm... Ano, opravdu tady je nêco, co bys mohl pro naši komunitu udêlat.
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_02"); //Komnaty novicù by potâebovaly poâádnê zamést. Postarej se o to.
	AI_Output (other, self, "DIA_Parlan_Aufgabe_15_03"); //Ale to bude trvat vêènê.
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_04"); //V tom pâípadê bys mêl zaèít co nejdâív, nezdá se ti?
	
	MIS_ParlanFegen = LOG_RUNNING;
	
	Log_CreateTopic (Topic_ParlanFegen,LOG_MISSION);
	Log_SetTopicStatus (Topic_ParlanFegen,LOG_RUNNING);
	B_LogEntry 	(Topic_ParlanFegen,"Mistr Parlan chce, abych zametl ètyâi cely pro novice. To potrvá celou vêènost!"); 
};
///////////////////////////////////////////////////////////////////////
...
instance DIA_Parlan_Fegen		(C_INFO)
{
	...
	permanent	 =  TRUE;
	description	 = 	"Co se têch komnat novicù týèe...";
};
//------------------------------------
...
{
	AI_Output (other, self, "DIA_Parlan_Fegen_15_00"); //Co se têch komnat novicù týèe...
	
	if (NOV_Helfer >= 4)
	{
		AI_Output (self, other, "DIA_Parlan_Fegen_05_01"); //Podaâilo se ti najít ètyâi novice, aby ti pomohli. Myslím, že se teë mohou zase vrátit ke své bêžné práci.
	...
	{
		AI_Output (self, other, "DIA_Parlan_Fegen_05_03"); //O tom není sporu - není to jednoduchý úkol. Ale s Innosovou pomocí to jistê zvládneš.
	};
};
///////////////////////////////////////////////////////////////////////
//	Info LEARN
...
instance DIA_Parlan_LEARN		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"Jak se mohu nauèit umêní magie?";
};
...
{
	AI_Output (other, self, "DIA_Parlan_LEARN_15_00"); //Jak se mohu nauèit umêní magie?
	...
	B_LogEntry (Topic_KlosterTeacher,"Mistr Parlan mi pomùže vylepšit magická umêní.");
};
///////////////////////////////////////////////////////////////////////
//	Info KNOWSJUDGE
///////////////////////////////////////////////////////////////////////
...
{
	AI_Output (other, self, "DIA_Parlan_KNOWSJUDGE_15_00"); //Chtêl bych podstoupit zkoušku ohnê.
	AI_Output (self, other, "DIA_Parlan_KNOWSJUDGE_05_01"); //Cože...? Hm... (naléhavê) Myslíš to vážnê?
	AI_Output (other, self, "DIA_Parlan_KNOWSJUDGE_15_02"); //Trvám na tom.
	AI_Output (self, other, "DIA_Parlan_KNOWSJUDGE_05_03"); //Jsi výjimeèný novic. Dobrá, pokud jsi rozhodnut, promluv si s mistrem Pyrokarem.
	
	B_LogEntry (TOPIC_FireContest,"Kdybych nêkdy chtêl podstoupit zkoušku ohnê, musím si promluvit s mistrem Pyrokarem.");
};
///////////////////////////////////////////////////////////////////////
//	Info TEACH_MANA
///////////////////////////////////////////////////////////////////////
...
{
		AI_Output (other, self, "DIA_Parlan_TEACH_MANA_15_00"); //Chtêl bych posílit svoji magickou moc.
...
FUNC VOID DIA_Parlan_TEACH_MANA_BACK()
{
	if (other.attribute[ATR_MANA_MAX] >= T_MED)  
	{
		...
		AI_Output (self, other, "DIA_Parlan_TEACH_MANA_05_01"); //Jestli se chceš uèit dál, požádej Pyrokara.
		Parlan_Sends = TRUE;
	};
...
func void DIA_Parlan_MAGE_Info ()
{
	...
	AI_Output (self, other, "DIA_Parlan_MAGE_05_01"); //Až budeš dostateènê zkušený, budu tê uèit v kruzích magie.
	AI_Output (self, other, "DIA_Parlan_MAGE_05_02"); //Na dùkaz, že ve tvých rukou nyní dâímá moc, si vezmi tenhle runový kámen.
...
instance DIA_Parlan_CIRCLE1		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauè mê první kruh magie.";
};
...
{
	AI_Output (other, self, "DIA_Parlan_TECH_CIRCLE1_15_00"); //Nauè mê první kruh magie.
	
	if B_TeachMagicCircle (self, other, 1)
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_01"); //V prvním kruhu magie mùžeš používat runy, které jsi sám vytvoâil.
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_02"); //Ke stvoâení run se budeš muset nauèit kouzelné formule.
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_03"); //Každý z nás se soustâedí na jeden okruh magie. Dobâe si tedy rozmysli, jaké by ses chtêl vênovat ty.
	};
};	
///////////////////////////////////////////////////////////////////////
//	Info TECH_CIRCLE_02
...
instance DIA_Parlan_CIRCLE2		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauè mê druhý kruh magie.";
};
...
{
	AI_Output (other, self, "DIA_Parlan_TECH_CIRCLE2_15_00"); //Nauè mê druhý kruh magie.
	
	if (Kapitel >= 2)
	{
		if B_TeachMagicCircle (self, other, 2)
		{
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_01"); //Vstup do druhého kruhu a nauè se ještê silnêjší magii.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_02"); //Ale mêj na pamêti, že na sebe bereš zodpovêdnost. Ne ke mnê nebo k Pyrokarovi - ale k Innosovi.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_03"); //On ti ukazuje smêr - cestu však tvoâí tvé skutky.
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_04"); //Ještê nenadešel správný èas.
	};
};	
///////////////////////////////////////////////////////////////////////
//	Info TECH_CIRCLE_03
...
instance DIA_Parlan_CIRCLE3		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauè mê tâetí kruh magie.";
};
...
{
	AI_Output (other, self, "DIA_Parlan_TECH_CIRCLE3_15_00"); //Nauè mê tâetí kruh magie.
	
	if (Kapitel >= 3)
	{
		if B_TeachMagicCircle (self, other, 3)
		{
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_01"); //Ano, už nadešel ten správný èas. Vstup do tâetího kruhu magie. Nová kouzla tê již oèekávají.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_02"); //Budeš je potâebovat, protože zlo se stahuje a odrazit jej mùže jen Innosova síla.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_03"); //Už jsem tê nauèil všem kruhùm, jimiž jsem tê mohl provést. Vyšší kruhy tê nauèí Karas.
			
			B_LogEntry (Topic_KlosterTeacher,"Bratr Parlan mê nauèil první tâi kruhy. Do dalších mê uvede bratr Karras.");
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_04"); //Ještê nenadešel pravý èas, aby ses nauèil tâetí kruh.
	};
};		
///////////////////////////////////////////////////////////////////////
//	Info TEACH
...
func void DIA_Parlan_TEACH_Info ()
{
	...
	AI_Output (other, self, "DIA_Parlan_TEACH_15_00"); //Uè mê!
	...
	if (abletolearn < 1)
	{
		AI_Output (self, other, "DIA_Parlan_TEACH_05_01"); //V tuhle chvíli tê nemohu nauèit žádné další formule.
	};
};
...
{
	AI_Output (self ,other,"DIA_Parlan_EXIT_05_00"); //Nechã tê Innos ochraàuje.
};
...
FUNC VOID DIA_Parlan_IAmParlan_Info()
{
	if (other.guild == GIL_PAL)
	{
		...
		AI_Output (self ,other,"DIA_Parlan_IAmParlan_05_01"); //Bude se nám hodit každá ruka. Bêž si promluvit pâímo s Mardukem, ten zodpovídá za vás paladiny.
	}
	else
	{
		AI_Output (self ,other,"DIA_Parlan_IAmParlan_05_02"); //Netuším, proè ti byl povolen vstup sem do kláštera. Jsem Parlan a zodpovídám za naše nováèky.
		...
		Info_AddChoice (DIA_Parlan_IAmParlan,"Budu si dêlat, co se mi zlíbí.",DIA_Parlan_IAmParlan_MyChoice);
		Info_AddChoice (DIA_Parlan_IAmParlan,"Jasnê.",DIA_Parlan_IAmParlan_OK);
	};
...
{
	AI_Output (other,self ,"DIA_Parlan_IAmParlan_MyChoice_15_00"); //Budu si dêlat, co se mi zlíbí.
	AI_Output (self ,other,"DIA_Parlan_IAmParlan_MyChoice_05_01"); //Urèitê je nêjaký dùvod, proè tu jsi trpên, ale všechno má své meze.
	AI_Output (self ,other,"DIA_Parlan_IAmParlan_MyChoice_05_02"); //Když budeš dêlat problémy, zaplatíš za to. Tak se drž od novicù dál.
...
{
	AI_Output (other,self ,"DIA_Parlan_IAmParlan_OK_15_00"); //Jasnê.
...
INSTANCE DIA_Parlan_Bibliothek   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Máš pro mê ještê nêco?";
};
...
{
	AI_Output (other ,self,"DIA_Parlan_Bibliothek_15_00"); //Máš pro mê ještê nêco?
	AI_Output (self ,other,"DIA_Parlan_Bibliothek_05_01"); //Ano. Jakožto èlen knihovny bys mêl dostat klíè ke knihovnê. Najdeš tam bratry Karase a Hyglase.
	AI_Output (self ,other,"DIA_Parlan_Bibliothek_05_02"); //Pokud chceš, mùžeš si tam s nimi promluvit
	
	if (other.guild != GIL_PAL)
	{
		AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_03"); //Ale od novicù se drž dál!
	};
	B_GiveInvItems (self,other,ItKe_KlosterBibliothek,1);
};
///////////////////////////////////////////////////////////////////////
...
{
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_00"); //(výhrùžnê) Nerad bych se opakoval. Drž se od novicù dál.
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_01"); //Mêli by oèistit své duše fyzickou prací a pâipravit se na život v klášteâe.
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_02"); //(ráznê) Další vyrušování nebudu trpêt!
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Church()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_00"); //Bože všemohoucí! Copak tê Innos oslepil?
	AI_Output (self, other, "DIA_Parlan_Add_05_01"); //(sarkasticky) Kdepak je kostel? Božíèku, kde by asi tak mohl být?
};
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Chapel()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_03"); //Kaple je místnost v polovinê kolonády nalevo. Paladinové se v ní modlí k Innosovi.
};
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Cellar()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_04"); //Vstup do sklepení je v polovinê kolonády napravo.
};
