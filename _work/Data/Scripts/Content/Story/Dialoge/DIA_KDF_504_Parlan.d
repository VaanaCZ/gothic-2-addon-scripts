...
{
	AI_Output (self, other, "DIA_Parlan_HAMMER_05_00"); //(přísně) Počkej chvíli, synu.
	AI_Output (self, other, "DIA_Parlan_HAMMER_05_01"); //Říká se, že ze svatyně 'zmizel' cenný artefakt.
	AI_Output (self, other, "DIA_Parlan_HAMMER_05_02"); //Ani o tom nechci slyšet nic dalšího - předpokládám, že se vrátí na své místo.
...
FUNC VOID DIA_Parlan_PMSchulden_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_01"); //A i když jsi už byl obviněn, provinil ses ještě víc.
		
		if (Parlan_Schulden < 1000)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_02"); //Tvá vina může být smyta jen větším darem klášteru.
		...
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_03"); //Kdo si myslíš, že jsi? Modli se k Innosovi za odpuštění svých skutků!
		};
	}
	else if (B_GetGreatestPetzCrime(self) < Parlan_LastPetzCrime)
	{
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_04"); //Tvoje situace se změnila.
		
		if (Parlan_LastPetzCrime == CRIME_MURDER)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_05"); //Nejsou už žádní svědkové tvé vraždy!
		};
		
		if (Parlan_LastPetzCrime == CRIME_THEFT)
		|| ( (Parlan_LastPetzCrime > CRIME_THEFT) && (B_GetGreatestPetzCrime(self) < CRIME_THEFT) )
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_06"); //Nikdo už nebude svědčit, že tě viděl krást!
		};
		
		if (Parlan_LastPetzCrime == CRIME_ATTACK)
		|| ( (Parlan_LastPetzCrime > CRIME_ATTACK) && (B_GetGreatestPetzCrime(self) < CRIME_ATTACK) )
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_07"); //Už tě nikdo neobviňuje z toho, že by ses pral!
		};
		
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_08"); //Všechna obvinění proti tobě byla stažena!
		};
		
		AI_Output (self, other, "DIA_Parlan_PMSchulden_05_09"); //Možná se ti podaří mě ošálit - ale Innos zná tvé pravé skutky!
				
		// ------- Schulden erlassen oder trotzdem zahlen ------
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_10"); //Tvé hříchy ti byly odpuštěny.
		...
		else
		{
			...
			B_Say_Gold (self, other, Parlan_Schulden);
			AI_Output (self, other, "DIA_Parlan_PMSchulden_05_13"); //Osvoboď se od své viny!
		};
	};
	...
	if (B_GetGreatestPetzCrime(self) != CRIME_NONE)
	{
		...
		Info_AddChoice		(DIA_Parlan_PMSchulden,"Nemám dost peněz!",DIA_Parlan_PETZMASTER_PayLater);
...
FUNC VOID DIA_Parlan_PETZMASTER_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_00"); //Ještě jsme spolu nemluvili, a už na sobě máš břemeno viny!
	};
	...
	if (B_GetGreatestPetzCrime(self) == CRIME_MURDER) 
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_02"); //A zatížil ses ještě další vinou!
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_03"); //Kdyby záleželo na mně, na místě bych tě odsoudil, vrahu!
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_04"); //Ale když za svůj zločin zaplatíš pokutu, zbavím tě tvé viny.
	};
	...
	if (B_GetGreatestPetzCrime(self) == CRIME_THEFT) 
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_06"); //Kromě toho jsi porušil i další zákony.
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_07"); //Tyto prohřešky musí být potrestány. Tak zní Innosovo přikázání.
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_08"); //Zaplatíš klášteru přiměřenou pokutu.
	...
	{
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_09"); //V klášteře netolerujeme rvačky. Takové chování se protiví našim zákonům.
		
		if (PETZCOUNTER_City_Sheepkiller > 0)
		{
			AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_10"); //A proč zabíjíš naše ovce?
		};
		AI_Output (self, other, "DIA_Parlan_PETZMASTER_05_11"); //Žijeme tady podle přikázání. A ta se vztahují i na tebe.
	...
	Info_ClearChoices  	(DIA_Parlan_PMSchulden);
	Info_ClearChoices  	(DIA_Parlan_PETZMASTER);
	Info_AddChoice		(DIA_Parlan_PETZMASTER,"Nemám dost peněz!",DIA_Parlan_PETZMASTER_PayLater);
	if (Npc_HasItems(other, itmi_gold) >= Parlan_Schulden)
	{
...
func void DIA_Parlan_PETZMASTER_PayNow()
{
	...
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayNow_05_01"); //Přijímám tvůj dar. Tvé hříchy jsou odpuštěny. Nechť ti Innos dá moudrost, abys stejné chyby již víckrát neopakoval.
...
{
	AI_Output (other, self, "DIA_Parlan_PETZMASTER_PayLater_15_00"); //Nemám dost peněz!
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayLater_05_01"); //Na to jsi měl myslet, než ses svých prohřešků dopustil.
	AI_Output (self, other, "DIA_Parlan_PETZMASTER_PayLater_05_02"); //Možná se z toho ponaučíš. Nedělej žádné další potíže, nebudeš muset platit.
...
{
			AI_Output (self, other, "DIA_Parlan_WELCOME_05_09"); //Až to uděláš, promluvíme si o tvé práci tady v klášteře.
};
func void B_DIA_Parlan_WELCOME_BringTribute2Gorax ()
{
			AI_Output (self, other, "DIA_Addon_Parlan_WELCOME_05_00"); //Svou platbu hned zanes Goraxovi do kláštera. Dál už to zařídí on.
};
...
func void DIA_Parlan_WELCOME_Info ()
{
	...
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_02"); //(přeruší tě)... NOVIC. Není důležité, kdo jsi byl předtím. Teď jsi Innosův služebník. To je vše, na čem záleží.
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_03"); //A jakmile budeš přijat do bratrstva ohně, budou všechny tvé předchozí prohřešky odpuštěny.
	AI_Output (self, other, "DIA_Parlan_WELCOME_05_04"); //Nechť Innos požehná tomuto muži, který zasvětí svůj život jeho službě, a smyje z něj všechny jeho hříchy.
	AI_Output (other, self, "DIA_Parlan_WELCOME_15_05"); //A teď?
	...
	if (Pedro_NOV_Aufnahme_LostInnosStatue_Daron == FALSE)//ADDON
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_WELCOME_05_07"); //Nejprve však doveď svou ovci k Opolosovi, postará se o ni.
		};
		if (DIA_Gorax_GOLD_perm == FALSE) 
		{
			AI_Output (self, other, "DIA_Parlan_WELCOME_05_08"); //Své zlato můžeš dát Goraxovi, našemu správci. Najdeš ho ve stáčírně vína.
...
func void DIA_Parlan_Auge_Info ()
{
	...
	AI_Output (self, other, "DIA_Parlan_Auge_05_01"); //Nevím, kdo ti o něm řekl, ale k tomuto posvátnému artefaktu nemá nikdo přístup.
	AI_Output (other, self, "DIA_Parlan_Auge_15_02"); //Kde je Innosovo oko uschováno?
	AI_Output (self, other, "DIA_Parlan_Auge_05_03"); //Čím méně lidí toto tajemství zná, tím lépe. Hledej ho - a nenajdeš jej.
};
...
instance DIA_Parlan_Amulett		(C_INFO)
{
	...
	permanent 	 =  FALSE; 
	description	 =  "Pověz mi něco o Innosově oku.";
};
...
{
	AI_Output (other, self, "DIA_Parlan_Amulett_15_00"); //Řekni mi něco o Innosově oku.
	...
	AI_Output (self, other, "DIA_Parlan_Amulett_05_02"); //Bylo vytvořeno, aby nás v pravý čas ochránilo před silami zla.
};
///////////////////////////////////////////////////////////////////////
//	Info Hagen
///////////////////////////////////////////////////////////////////////
instance DIA_Parlan_Hagen		(C_INFO)
{
	...
	permanent 	 =  FALSE; 
	description	 =  "Potřebuji si promluvit s vůdcem paladinů!";
};
...
{
	AI_Output (other, self, "DIA_Parlan_Hagen_15_00"); //Potřebuji si promluvit s vůdcem paladinů!
	AI_Output (self, other, "DIA_Parlan_Hagen_05_01"); //Lord Hagen novice nepřijímá - setkat se s ním mohou pouze mágové.
};
///////////////////////////////////////////////////////////////////////
//	Info WORK
/////////////////////////////////////////////////////////////////////// 
instance DIA_Parlan_WORK		(C_INFO)
{
	...
	permanent	 =  TRUE;
	description	 = 	"Jak mohu posloužit zdejší komunitě?";
};
//------------------------------------
...
{
	AI_Output			(other, self, "DIA_Parlan_WORK_15_00"); //Jak mohu posloužit zdejší komunitě?
	...
	{
		AI_Output (self, other, "DIA_Parlan_WORK_NIGHT_05_01"); //Je čas klidu. Práce noviců začíná za rozbřesku a pokračuje až do setmění.
		AI_Output (self, other, "DIA_Parlan_WORK_NIGHT_05_02"); //Měl by ses vyspat. Přijď, až nadejde čas práce.
	...
	else
	{
		...
		AI_Output (self, other, "DIA_Parlan_WORK_05_04"); //Já budu na tvou práci dohlížet, a až své povinnosti uspokojivě splníš, dostaneš povolení vstoupit do knihovny a studovat Innosovo učení.
		
		DIA_Parlan_WORK_perm = TRUE;
		MIS_KlosterArbeit = LOG_RUNNING; 
		B_LogEntry (Topic_Gemeinschaft, "Když splním úkoly pro mágy, dovolí mi pak studovat v knihovně.");
...
func void DIA_Parlan_Stand_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_01"); //Sehnal jsi ohnivé kopřivy pro bratra Neorase.
	...
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_04"); //Podařilo se ti najít čtyři novice a tím pádem i uklidit pokoje.
		Kloster_Punkte = (Kloster_Punkte + 3);
	};
	if (MIS_GoraxEssen == LOG_SUCCESS)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_05"); //Rozdělil jsi jídlo mezi novice spravedlivě, přesně jak ti bratr Gorax nakázal.
		Kloster_Punkte = (Kloster_Punkte + 1);
	}
	else if (MIS_GoraxEssen == LOG_FAILED)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_06"); //Gorax s tebou nebyl spokojen, protože jsi jídlo nerozdělil spravedlivě.
	};	
	if (MIS_GoraxWein == LOG_SUCCESS)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_07"); //Prodal jsi víno ke Goraxově spokojenosti.
		Kloster_Punkte = (Kloster_Punkte + 1);
	}
	else if (MIS_GoraxWein == LOG_FAILED)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_08"); //Čím méně toho bude řečeno o té záležitosti s vínem, tím lépe.
	};	
	
	if (Kloster_Punkte >= 8)
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_09"); //Ano, už jsi pracoval dost. Nadešel čas, abys studoval Innosovo učení. Promluv si s mistrem Hyglasem v knihovně.
	...
	{
		AI_Output (self, other, "DIA_Parlan_Stand_05_12"); //Nejprve splň své úkoly ve službě společenství. Až budeš se vším hotov, můžeme si o tom promluvit znovu.
	};
};
///////////////////////////////////////////////////////////////////////
//	Info Aufgabe
...
{
	AI_Output (other, self, "DIA_Parlan_Aufgabe_15_00"); //Máš pro mě nějaký úkol?
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_01"); //Hmm... Ano, opravdu tady je něco, co bys mohl pro naši komunitu udělat.
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_02"); //Komnaty noviců by potřebovaly pořádně zamést. Postarej se o to.
	AI_Output (other, self, "DIA_Parlan_Aufgabe_15_03"); //Ale to bude trvat věčně.
	AI_Output (self, other, "DIA_Parlan_Aufgabe_05_04"); //V tom případě bys měl začít co nejdřív, nezdá se ti?
	
	MIS_ParlanFegen = LOG_RUNNING;
	
	Log_CreateTopic (Topic_ParlanFegen,LOG_MISSION);
	Log_SetTopicStatus (Topic_ParlanFegen,LOG_RUNNING);
	B_LogEntry 	(Topic_ParlanFegen,"Mistr Parlan chce, abych zametl čtyři cely pro novice. To potrvá celou věčnost!"); 
};
///////////////////////////////////////////////////////////////////////
...
instance DIA_Parlan_Fegen		(C_INFO)
{
	...
	permanent	 =  TRUE;
	description	 = 	"Co se těch komnat noviců týče...";
};
//------------------------------------
...
{
	AI_Output (other, self, "DIA_Parlan_Fegen_15_00"); //Co se těch komnat noviců týče...
	
	if (NOV_Helfer >= 4)
	{
		AI_Output (self, other, "DIA_Parlan_Fegen_05_01"); //Podařilo se ti najít čtyři novice, aby ti pomohli. Myslím, že se teď mohou zase vrátit ke své běžné práci.
	...
	{
		AI_Output (self, other, "DIA_Parlan_Fegen_05_03"); //O tom není sporu - není to jednoduchý úkol. Ale s Innosovou pomocí to jistě zvládneš.
	};
};
///////////////////////////////////////////////////////////////////////
//	Info LEARN
...
instance DIA_Parlan_LEARN		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"Jak se mohu naučit umění magie?";
};
...
{
	AI_Output (other, self, "DIA_Parlan_LEARN_15_00"); //Jak se mohu naučit umění magie?
	...
	B_LogEntry (Topic_KlosterTeacher,"Mistr Parlan mi pomůže vylepšit magická umění.");
};
///////////////////////////////////////////////////////////////////////
//	Info KNOWSJUDGE
///////////////////////////////////////////////////////////////////////
...
{
	AI_Output (other, self, "DIA_Parlan_KNOWSJUDGE_15_00"); //Chtěl bych podstoupit zkoušku ohně.
	AI_Output (self, other, "DIA_Parlan_KNOWSJUDGE_05_01"); //Cože...? Hm... (naléhavě) Myslíš to vážně?
	...
	B_LogEntry (TOPIC_FireContest,"Kdybych někdy chtěl podstoupit zkoušku ohně, musím si promluvit s mistrem Pyrokarem.");
};
///////////////////////////////////////////////////////////////////////
//	Info TEACH_MANA
///////////////////////////////////////////////////////////////////////
...
{
		AI_Output (other, self, "DIA_Parlan_TEACH_MANA_15_00"); //Chtěl bych posílit svoji magickou moc.
...
{
	AI_Output (self, other, "DIA_Parlan_MAGE_05_00"); //Složil jsi slib, bratře. Vítej mezi námi.
	AI_Output (self, other, "DIA_Parlan_MAGE_05_01"); //Až budeš dostatečně zkušený, budu tě učit v kruzích magie.
	AI_Output (self, other, "DIA_Parlan_MAGE_05_02"); //Na důkaz, že ve tvých rukou nyní dřímá moc, si vezmi tenhle runový kámen.
...
instance DIA_Parlan_CIRCLE1		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauč mě první kruh magie.";
};
...
func void DIA_Parlan_CIRCLE1_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_01"); //V prvním kruhu magie můžeš používat runy, které jsi sám vytvořil.
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_02"); //Ke stvoření run se budeš muset naučit kouzelné formule.
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE1_05_03"); //Každý z nás se soustředí na jeden okruh magie. Dobře si tedy rozmysli, jaké by ses chtěl věnovat ty.
	};
};	
///////////////////////////////////////////////////////////////////////
//	Info TECH_CIRCLE_02
...
instance DIA_Parlan_CIRCLE2		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauč mě druhý kruh magie.";
};
...
func void DIA_Parlan_CIRCLE2_Info ()
{
	...
	if (Kapitel >= 2)
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_01"); //Vstup do druhého kruhu a nauč se ještě silnější magii.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_02"); //Ale měj na paměti, že na sebe bereš zodpovědnost. Ne ke mně nebo k Pyrokarovi - ale k Innosovi.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_03"); //On ti ukazuje směr - cestu však tvoří tvé skutky.
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE2_05_04"); //Ještě nenadešel správný čas.
	};
};	
///////////////////////////////////////////////////////////////////////
//	Info TECH_CIRCLE_03
...
instance DIA_Parlan_CIRCLE3		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauč mě třetí kruh magie.";
};
...
func void DIA_Parlan_CIRCLE3_Info ()
{
	...
	if (Kapitel >= 3)
	{
		...
		{
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_01"); //Ano, už nadešel ten správný čas. Vstup do třetího kruhu magie. Nová kouzla tě již očekávají.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_02"); //Budeš je potřebovat, protože zlo se stahuje a odrazit jej může jen Innosova síla.
			AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_03"); //Už jsem tě naučil všem kruhům, jimiž jsem tě mohl provést. Vyšší kruhy tě naučí Karas.
			
			B_LogEntry (Topic_KlosterTeacher,"Bratr Parlan mě naučil první tři kruhy. Do dalších mě uvede bratr Karras.");
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Parlan_TECH_CIRCLE3_05_04"); //Ještě nenadešel pravý čas, aby ses naučil třetí kruh.
	};
};		
///////////////////////////////////////////////////////////////////////
//	Info TEACH
...
func void DIA_Parlan_TEACH_Info ()
{
	...
	if (abletolearn < 1)
	{
		AI_Output (self, other, "DIA_Parlan_TEACH_05_01"); //V tuhle chvíli tě nemohu naučit žádné další formule.
	};
};
...
{
	AI_Output (self ,other,"DIA_Parlan_EXIT_05_00"); //Nechť tě Innos ochraňuje.
};
...
FUNC VOID DIA_Parlan_IAmParlan_Info()
{
	if (other.guild == GIL_PAL)
	{
		...
		AI_Output (self ,other,"DIA_Parlan_IAmParlan_05_01"); //Bude se nám hodit každá ruka. Běž si promluvit přímo s Mardukem, ten zodpovídá za vás paladiny.
	}
	else
	{
		...
		Info_AddChoice (DIA_Parlan_IAmParlan,"Budu si dělat, co se mi zlíbí.",DIA_Parlan_IAmParlan_MyChoice);
		Info_AddChoice (DIA_Parlan_IAmParlan,"Jasně.",DIA_Parlan_IAmParlan_OK);
	};
...
{
	AI_Output (other,self ,"DIA_Parlan_IAmParlan_MyChoice_15_00"); //Budu si dělat, co se mi zlíbí.
	AI_Output (self ,other,"DIA_Parlan_IAmParlan_MyChoice_05_01"); //Určitě je nějaký důvod, proč tu jsi trpěn, ale všechno má své meze.
	AI_Output (self ,other,"DIA_Parlan_IAmParlan_MyChoice_05_02"); //Když budeš dělat problémy, zaplatíš za to. Tak se drž od noviců dál.
...
{
	AI_Output (other,self ,"DIA_Parlan_IAmParlan_OK_15_00"); //Jasně.
...
INSTANCE DIA_Parlan_Bibliothek   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Máš pro mě ještě něco?";
};
...
{
	AI_Output (other ,self,"DIA_Parlan_Bibliothek_15_00"); //Máš pro mě ještě něco?
	AI_Output (self ,other,"DIA_Parlan_Bibliothek_05_01"); //Ano. Jakožto člen knihovny bys měl dostat klíč ke knihovně. Najdeš tam bratry Karase a Hyglase.
...
{
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_00"); //(výhrůžně) Nerad bych se opakoval. Drž se od noviců dál.
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_01"); //Měli by očistit své duše fyzickou prací a připravit se na život v klášteře.
	AI_Output (self ,other,"DIA_Parlan_DontDisturb_05_02"); //(rázně) Další vyrušování nebudu trpět!
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Church()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_00"); //Bože všemohoucí! Copak tě Innos oslepil?
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Library()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_02"); //Knihovna je na konci kolonády, přímo naproti kostelu.
};
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Chapel()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_03"); //Kaple je místnost v polovině kolonády nalevo. Paladinové se v ní modlí k Innosovi.
};
...
FUNC VOID DIA_Parlan_Kap3U4U5_PERM_Cellar()
{
	...
	AI_Output (self, other, "DIA_Parlan_Add_05_04"); //Vstup do sklepení je v polovině kolonády napravo.
};
