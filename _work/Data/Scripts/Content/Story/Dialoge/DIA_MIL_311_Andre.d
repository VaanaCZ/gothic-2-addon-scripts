...
{
	AI_Output (self ,other,"DIA_Andre_Add_08_00"); //Jeden z mých mužù mi âekl, že bandité roznášejí plakáty s vypsanou odmênou a že je na nich tvoje tváâ.
	...
	AI_Output (other, self,"DIA_Andre_Add_15_03"); //Nevím, proè mê ti lidé hledají...
	AI_Output (self ,other,"DIA_Andre_Add_08_04"); //Pro tvoje vlastní dobrou doufám, že âíkáš pravdu.
	AI_Output (self ,other,"DIA_Andre_Add_08_05"); //Nechci v domobranê nikoho, kdo nemá èistý rejstâík.
	AI_Output (self ,other,"DIA_Andre_Add_08_06"); //Vêtšina z têch banditù jsou bývalí vêzni z têžaâské kolonie.
	AI_Output (self ,other,"DIA_Andre_Add_08_07"); //Doufám, že ses s têmi hrdloâezy nijak nezapletl!
...
{
	AI_Output (self, other, "B_Andre_CantharFalle_08_00"); //Byl tady kupec Canthar. Povídal, že jsi uprchlý trestanec z têžaâské kolonie.
	AI_Output (self, other, "B_Andre_CantharFalle_08_01"); //Nevím, jestli to je pravda, a radši bych se tê neptal, ale mêl bys to nêjak objasnit.
...
FUNC VOID DIA_Andre_PMSchulden_Info()
{
	...
	if (B_GetTotalPetzCounter(self) > Andre_LastPetzCounter)
	{
		...
		AI_Output (self, other, "DIA_Andre_PMSchulden_08_02"); //Zdá se, že se poèet žalob na tebe ještê znásobil!
		if (Andre_Schulden < 1000)
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_03"); //Varoval jsem tê! Pokuta, kterou musíš zaplatit, je ještê vyšší!
		...
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_04"); //Vážnê jsi mê zklamal!
		};
	}
	else if (B_GetGreatestPetzCrime(self) < Andre_LastPetzCrime)
	{
		AI_Output (self, other, "DIA_Andre_PMSchulden_08_05"); //Objevilo se pár nových vêcí.
		
		if (Andre_LastPetzCrime == CRIME_MURDER)
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_06"); //Už není nikdo, kdo by tê obviàoval z vraždy.
		};
		
		if (Andre_LastPetzCrime == CRIME_THEFT)
		|| ( (Andre_LastPetzCrime > CRIME_THEFT) && (B_GetGreatestPetzCrime(self) < CRIME_THEFT) )
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_07"); //Už si nikdo nevzpomíná, že by tê vidêl pâi krádeži.
		};
		
		if (Andre_LastPetzCrime == CRIME_ATTACK)
		|| ( (Andre_LastPetzCrime > CRIME_ATTACK) && (B_GetGreatestPetzCrime(self) < CRIME_ATTACK) )
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_08"); //Už nejsou žádní svêdkové, kteâí by tvrdili, že tê vidêli úèastnit se rvaèky.
		};
		
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_09"); //Zdá se, že se všechna obvinêní proti tobê nakonec vyjasnila.
		};
		
		AI_Output (self, other, "DIA_Andre_PMSchulden_08_10"); //Nevím, jak se to stalo, ale varuji tê: nehraj si se mnou.
				
		// ------- Schulden erlassen oder trotzdem zahlen ------
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_11"); //Každopádnê jsem se rozhodl odpustit ti tvoje dluhy.
		...
		{
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_13"); //Jedno si vyjasnême: tak jako tak budeš muset zaplatit pokutu v plné výši.
			B_Say_Gold (self, other, Andre_Schulden);
			AI_Output (self, other, "DIA_Andre_PMSchulden_08_14"); //Tak kolik to dêlá?
		};
	};
	...
	if (B_GetGreatestPetzCrime(self) != CRIME_NONE)
	{
		...
		Info_AddChoice		(DIA_Andre_PMSchulden,"Nemám dost penêz!",DIA_Andre_PETZMASTER_PayLater);
...
FUNC VOID DIA_Andre_PETZMASTER_Info()
{
	...
	if (self.aivar[AIV_TalkedToPlayer] == FALSE)
	{
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_00"); //Ty musíš být ten nový chlapík, který tady ve mêstê dêlá tolik problémù.
	};
	...
	{
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_01"); //Dobâe že jsi za mnou pâišel, než to zaèalo být pro tebe ještê horší.
		...
		{
			AI_Output (self, other, "DIA_Andre_PETZMASTER_08_03"); //A to nemluvím o têch dalších vêcech, které jsi napáchal.
		};
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_04"); //Strážní mají rozkazy trestat každého vraha hned na místê.
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_05"); //A vêtšina obèanù nehodlá vraždy na území mêsta tolerovat!
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_06"); //Nemám žádný zájem na tom tê povêsit. Jsme ve válce a potâebujeme každého muže.
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_07"); //Ale nebude lehké pâesvêdèit lidi, aby o tobê smýšleli zase lépe.
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_08"); //Mêl bys projevit svou lítost zaplacením pokuty - pochopitelnê pâimêâenê vysoké.
	};
		
	if (B_GetGreatestPetzCrime(self) == CRIME_THEFT) 
	{
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_09"); //Dobâe že jsi pâišel! Jsi obvinên z krádeže! Jsou na to svêdci!
		if ((PETZCOUNTER_City_Attack + PETZCOUNTER_City_Sheepkiller) > 0)
		{
			AI_Output (self, other, "DIA_Andre_PETZMASTER_08_10"); //A to nemluvím o dalších vêcech, které se mi donesly.
		};
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_11"); //Takové chování nebudu ve mêstê trpêt!
	...
	{
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_13"); //Je nêco jiného, když se popereš s lùzou v pâístavu...
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_14"); //Ale jakmile zaútoèíš na obèany nebo královské vojáky, musíš jít pâed soud.
		if (PETZCOUNTER_City_Sheepkiller > 0)
		{
			AI_Output (self, other, "DIA_Andre_PETZMASTER_08_15"); //A to s tou ovcí bylo také zcela zbyteèné.
		};
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_16"); //Kdybych tê nechal odejít jen tak, bude si tady za chvíli každý dêlat, co se mu zlíbí.
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_17"); //Takže zaplatíš pâimêâenou pokutu - a na celou vêc zapomeneme.
	...
	if (B_GetGreatestPetzCrime(self) == CRIME_SHEEPKILLER) 
	{
		...
		AI_Output (self, other, "DIA_Andre_PETZMASTER_08_19"); //Uvêdom si, že to nemohu nechat být jen tak.
	...
	Info_ClearChoices  	(DIA_Andre_PMSchulden);
	Info_ClearChoices  	(DIA_Andre_PETZMASTER);
	Info_AddChoice		(DIA_Andre_PETZMASTER,"Nemám dost penêz!",DIA_Andre_PETZMASTER_PayLater);
	if (Npc_HasItems(other, itmi_gold) >= Andre_Schulden)
	{
...
func void DIA_Andre_PETZMASTER_PayNow()
{
	...
	AI_Output (self, other, "DIA_Andre_PETZMASTER_PayNow_08_01"); //Dobrá! Postarám se o tom, aby se o tom dozvêdêl každý ve mêstê - tím se tvoje povêst zase trošku vylepší.
...
{
	AI_Output (other, self, "DIA_Andre_PETZMASTER_PayLater_15_00"); //Nemám dost penêz.
	AI_Output (self, other, "DIA_Andre_PETZMASTER_PayLater_08_01"); //Tak si nêjaké co nejrychleji sežeà.
	AI_Output (self, other, "DIA_Andre_PETZMASTER_PayLater_08_02"); //A varuji tê: jestli se proviníš ještê nêèím, jen si to zhoršíš.
...
{
	AI_Output (self, other, "DIA_Andre_Hallo_08_00"); //Innos tê provázej, cizinèe! Co tê ke mnê pâivádí?
};
...
func void DIA_Andre_Message_Info ()
{
	...
	AI_Output (self, other, "DIA_Andre_Message_08_01"); //No, stojíš pâed jedním z jeho mužù. Èeho se to týká?
...
{
	AI_Output (self, other, "B_Andre_LordHagenNichtZuSprechen_08_00"); //Lord Hagen pâijímá jen paladiny a ty, kdo jsou u paladinù ve službê.
	AI_Output (self, other, "B_Andre_LordHagenNichtZuSprechen_08_01"); //Je pod jeho dùstojnost ztrácet èas s bêžnými lidmi.
};
...
func void DIA_Andre_Message_EyeInnos()
{
	...
	AI_Output (self, other, "DIA_Andre_Message_EyeInnos_08_01"); //Innosovo oko - nikdy jsem o nêm neslyšel. Ale to ještê nemusí nic znamenat.
	AI_Output (self, other, "DIA_Andre_Message_EyeInnos_08_02"); //Pokud nêjaký artefakt tohoto jména opravdu existuje, stejnê o nêm vêdí jen nejvyšší èlenové našeho âádu.
...
{
	AI_Output (other, self, "DIA_Andre_Message_Dragons_15_00"); //Zástupy skâetù vedou DRA-
	AI_Output (self, other, "DIA_Andre_Message_Dragons_08_01"); //(pâeruší tê) JÁ VÍM, že armáda skâetù je èím dál tím silnêjší.
	...
	AI_Output (self, other, "DIA_Andre_Message_Dragons_08_04"); //A jsem si jist, že jsi dost chytrý na to, abys to vêdêl sám.
...
func void DIA_Andre_Message_Personal()
{
	...
	AI_Output (self, other, "DIA_Andre_Message_Personal_08_01"); //Jak myslíš. Ale jedno bys mêl vêdêt:
	B_Andre_LordHagenNichtZuSprechen();
...
instance DIA_Andre_Paladine		(C_INFO)
{
	...
	permanent 	 =  FALSE; 
	description	 = 	"Proè jsou ve mêstê paladinové?";	
};
...
{
	AI_Output (other, self, "DIA_Andre_Paladine_15_00"); //Proè jsou ve mêstê paladinové?
	AI_Output (self, other, "DIA_Andre_Paladine_08_01"); //Cíle naší mise jsou tajné.
	AI_Output (self, other, "DIA_Andre_Paladine_08_02"); //Mohu âíct jen to, že obèané mêsta nejsou v ohrožení.
	AI_Output (self, other, "DIA_Andre_Paladine_08_03"); //Nemusíš si dêlat starosti.
};
// *********************************************************
//	Warum seid ihr Paladine in der Stadt? 
// *********************************************************
...
func void DIA_Andre_PaladineAgain_Info ()
{
	...
	{
		AI_Output (other, self, "DIA_Andre_PaladineAgain_15_00"); //Âekneš mi teë, proè jste vy paladinové pâijeli sem do Khorinidu?
	}
	else
	{
		AI_Output (other, self, "DIA_Andre_PaladineAgain_15_01"); //Proè jsou ve mêstê paladinové?
	};
	
	AI_Output (self, other, "DIA_Andre_PaladineAgain_08_02"); //Teë patâíš k mêstské stráži, takže jsi také podâízený paladinùm.
	AI_Output (self, other, "DIA_Andre_PaladineAgain_08_03"); //Koneènê tê tedy mohu zasvêtit.
	...
	AI_Output (self, other, "DIA_Andre_PaladineAgain_08_05"); //Proto musíme získat rudu a dopravit ji na pevninu. Z té rudy vykováme nové zbranê a zaženeme skâety zpátky.
...
func void DIA_Andre_AskToJoin_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Andre_AskToJoin_08_01"); //Dobrá. Hodí se mi každý schopný muž. Nezáleží na dùvodech, proè se k nám chce pâidat.
		AI_Output (self, other, "DIA_Andre_AskToJoin_08_02"); //Pokud vstoupíš do služby k paladinùm, mohl bych ti pomoci zaâídit slyšení u lorda Hagena.
	}
	else
	{
	...
	AI_Output (self, other, "DIA_Andre_AskToJoin_08_04"); //Mám však rozkazy pâijímat do domobrany jen obèany mêsta.
	AI_Output (self, other, "DIA_Andre_AskToJoin_08_05"); //Mùj velitel se bojí, že by se do ní jinak mohli vetâít špehové nebo sabotéâi.
	AI_Output (self, other, "DIA_Andre_AskToJoin_08_06"); //Tímhle zpùsobem chce to riziko minimalizovat.
	AI_Output (self, other, "DIA_Andre_AskToJoin_08_07"); //Nejprve tedy musíš být obèanem mêsta - mùže sice být sporné, jestli tohle pravidlo má smysl nebo ne, ale rozkazy jsou rozkazy.
	
	Log_CreateTopic (TOPIC_BecomeMIL,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_BecomeMIL,LOG_RUNNING);
	B_LogEntry (TOPIC_BecomeMIL,"Chci-li se stát èlenem mêstské gardy, musím být nejdâív obèanem mêsta.");
};
...
instance DIA_Andre_AboutMiliz (C_INFO)
{
	...
	permanent    =  FALSE;
	description	 = 	"Co mùžu od služby v domobranê èekat?";
};
...
{
	AI_Output (other, self, "DIA_Andre_AboutMiliz_15_00"); //Co mùžu od služby v domobranê èekat?
	AI_Output (self, other, "DIA_Andre_AboutMiliz_08_01"); //Jedno si vyjasnême. Být vojákem v domobranê obnáší mnohem víc než jen promenádovat se mêstem v uniformê.
	AI_Output (self, other, "DIA_Andre_AboutMiliz_08_02"); //Je to špinavá a nêkdy i krvavá práce. Jakmile budeš jedním z nás, bude tê èekat spousta povinností.
	AI_Output (self, other, "DIA_Andre_AboutMiliz_08_03"); //Ale stojí to za to. Kromê platu budeš mít také možnost stát se jednoho dne Innosovým svatým váleèníkem.
};
...
func void DIA_Addon_Andre_MartinEmpfehlung_Info ()
{
	...
	AI_Output	(self, other, "DIA_Addon_Andre_MartinEmpfehlung_08_01"); //(nevêâícnê) Cože? Ukaž mi ho.
	B_GiveInvItems (other, self, ItWr_Martin_MilizEmpfehlung_Addon,1);
	B_UseFakeScroll();
	AI_Output	(self, other, "DIA_Addon_Andre_MartinEmpfehlung_08_02"); //(ohromenê) No tohle...! To ti muselo dát poâádnou práci. Martin nêco takového hned tak nêkomu nedá.
	AI_Output	(self, other, "DIA_Addon_Andre_MartinEmpfehlung_08_03"); //No dobâe, pâesvêdèil jsi mê. Když se za tebe pâimluvil i Martin, pâijmu tê mezi nás. Dej mi vêdêt, až budeš pâipravený.
...
instance DIA_Andre_Alternative (C_INFO)
{
	...
	permanent 	= FALSE; 
	description = "Není nêjaký rychlejší zpùsob, jak se k vám pâidat?";
};
...
{
	AI_Output (other, self, "DIA_Andre_Alternative_15_00"); //Není nêjaký rychlejší zpùsob, jak se k vám pâidat?
	AI_Output (self, other, "DIA_Andre_Alternative_08_01"); //Hmm (na chvíli se odmlèí) - ty to asi myslíš vážnê, co?
	AI_Output (self, other, "DIA_Andre_Alternative_08_02"); //Dobrá, poslouchej. Mám problém. Když ho za mê vyâešíš, dohlédnu na to, abys byl pâijat do âad domobrany.
	AI_Output (self, other, "DIA_Andre_Alternative_08_03"); //Ale je velice dùležité, abys o tom nikomu neâekl ani slovo!
...
func void DIA_Andre_GuildOfThieves_Info ()
{
	...
	AI_Output (self, other, "DIA_Andre_GuildOfThieves_08_01"); //Poslední dobou ve mêstê došlo k množství krádeží. Zatím se nám nepodaâilo nikoho chytit, ti zlodêji jsou prostê pâíliš dobâí.
	AI_Output (self, other, "DIA_Andre_GuildOfThieves_08_02"); //Ti bastardi se vyznají. Jsem si jistý, že máme co do èinêní s organizovanou bandou.
	AI_Output (self, other, "DIA_Andre_GuildOfThieves_08_03"); //Nepâekvapilo by mê, kdyby byl tady v Khorinidu zlodêjský cech. Najdi vùdce toho gangu a sejmi ho.
	if (other.guild == GIL_NONE)
	{
		AI_Output (self, other, "DIA_Andre_GuildOfThieves_08_04"); //Já se pak postarám o to, abys byl pâijat do domobrany - bez ohledu na to, jestli jsi obèanem mêsta, nebo ne.
		AI_Output (self, other, "DIA_Andre_GuildOfThieves_08_05"); //Ale nesmíš se o naší dohodê nikomu zmínit ani slovem!
	};
	
	MIS_Andre_GuildOfThieves = LOG_RUNNING;
	B_LogEntry (TOPIC_BecomeMIL,"K mêstské gardê se mohu pâidat i jinak, a to tak, že vypátrám a znièím khorinidský zlodêjský cech.");
...
instance DIA_Andre_WhereThieves (C_INFO)
{
	...
	permanent 	= FALSE; 
	description = "Kde bych mêl ty zlodêje zaèít hledat?";
};
...
{
	AI_Output (other, self, "DIA_Andre_WhereThieves_15_00"); //Kde bych mêl ty zlodêje zaèít hledat?
	AI_Output (self, other, "DIA_Andre_WhereThieves_08_01"); //Kdybych to vêdêl, udêlám to sám!
	AI_Output (self, other, "DIA_Andre_WhereThieves_08_02"); //Mùžu ti âíct jenom tohle: Nedávno jsme obrátili celou pâístavní ètvrã vzhùru nohama a nenašli jsme nic, vùbec nic.
	AI_Output (self, other, "DIA_Andre_WhereThieves_08_03"); //Lidé odtamtud nejsou zrovna upovídaní, zvlášã když na sobê máš paladinské brnêní.
	AI_Output (self, other, "DIA_Andre_WhereThieves_08_04"); //Ale ty nejsi odsud, nebudou k tobê tak nedùvêâiví.
	AI_Output (self, other, "DIA_Andre_WhereThieves_08_05"); //Nejdâív by ses mêl poptat po pâístavu. Ale buë opatrný. Jestli tamním lidem dojde, že pracuješ pro paladiny, nedozvíš se vùbec NIC!
	
	B_LogEntry (TOPIC_BecomeMIL,"S pátráním po zlodêjském cechu mohu zaèít v pâístavní ètvrti.");
};
...
instance DIA_Andre_WhatToDo (C_INFO)
{
	...
	permanent 	= FALSE; 
	description = "Co mám udêlat, až najdu nêkterého z têch zlodêjù?";
};
...
{
	AI_Output (other, self, "DIA_Andre_WhatToDo_15_00"); //Co mám udêlat, až najdu nêkterého z têch zlodêjù?
	AI_Output (self, other, "DIA_Andre_WhatToDo_08_01"); //Pokud se bude jednat o nêjakého poskoka, pomahaèe nebo malou rybu, bude nejlepší, když to nebudeš hnát na ostâí nože.
	AI_Output (self, other, "DIA_Andre_WhatToDo_08_02"); //Spíš pâijë za mnou a podej mi o tom zprávu. Já se pak postarám o to, aby ten chlapík skonèil za mâížemi.
	AI_Output (self, other, "DIA_Andre_WhatToDo_08_03"); //Do pâímého boje by náhodou mohla zakroèit mêstská stráž a têžko bys pak mêl pâíležitost vysvêtlovat, o co vlastnê jde.
	AI_Output (self, other, "DIA_Andre_WhatToDo_08_04"); //Navíc za každýho zmetka, kterej skonèí pod zámkem, dostaneš odmênu.
	AI_Output (self, other, "DIA_Andre_WhatToDo_08_05"); //Ale jestli se ti podaâí najít skrýš jejich vùdcù - no - pak se boji už asi nevyhneš.
	
	B_LogEntry (TOPIC_BecomeMIL,"Pokud chytnu nêkterého z pâisluhovaèù zlodêjského cechu, mám ho pâedvést pâímo pâed lorda Andreho. Jestliže mám ale znièit celý cech, musím zjistit, kde se skrývají.");
};
...
instance DIA_Andre_Auslieferung		(C_INFO)
{
	...
	permanent    =  TRUE;
	description	 = 	"Pâišel jsem si vybrat odmênu za zloèince.";
};
...
{
	AI_Output (other, self, "DIA_Andre_Auslieferung_15_00"); //Pâišel jsem si vybrat odmênu za zloèince.
	
	Info_ClearChoices (DIA_Andre_Auslieferung);
	
	Info_AddChoice (DIA_Andre_Auslieferung,"Vrátím se pozdêji. (ZPÊT)",DIA_Andre_Auslieferung_Back);
	...
	&& (MIS_Canthars_KomproBrief_Day > (Wld_GetDay()-2))
	{
		Info_AddChoice (DIA_Andre_Auslieferung,"Sarah prodává zbranê Onarovi.",DIA_Andre_Auslieferung_Sarah);
	};
};
...
func void DIA_Andre_Auslieferung_Halvor()
{
	...
	AI_Output (other, self, "DIA_Andre_Auslieferung_Halvor_15_00"); //Halvor je pâekupník. Prodává zboží, které bandité ukradnou kupcùm.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Halvor_08_01"); //Takže on za tím vêzí. Mí muži ho okamžitê zatknou.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Halvor_08_02"); //Neâekl bych, že bude dêlat nêjaké problémy. Hned ti dám tvou odmênu.
	B_GiveInvItems (self, other, itmi_gold, Kopfgeld);
...
func void DIA_Andre_Auslieferung_Nagur()
{
	...
	AI_Output (other, self, "DIA_Andre_Auslieferung_Nagur_15_00"); //Nagur zabil Baltramova posla. Pokusil se nastrèit mê jako nového posla, aby zachytil Akilovu zásilku.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Nagur_08_01"); //Na toho chlapíka èeká trest. Okamžitê ho nechám zavâít.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Nagur_08_02"); //Tady máš odmênu, kterou sis po právu zasloužil.
	B_GiveInvItems (self, other, itmi_gold, Kopfgeld);
...
func void DIA_Andre_Auslieferung_Canthar()
{
	...
	AI_Output (self, other, "DIA_Andre_Auslieferung_Canthar_08_01"); //Sarah? Té prodavaèky zbraní z tržištê?
	AI_Output (other,self , "DIA_Andre_Auslieferung_Canthar_15_02"); //Mêl jsem jí podstrèit falešný dopis, ve kterém se tvrdí, že dodává zbranê Onarovi.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Canthar_08_03"); //Aha. Za toho parchanta ti rád vyplatím odmênu. Jako by už byl za mâížemi.
	B_GiveInvItems (self, other, itmi_gold, Kopfgeld);
...
func void DIA_Andre_Auslieferung_Sarah()
{
	...
	AI_Output (other, self, "DIA_Andre_Auslieferung_Sarah_15_00"); //Sarah prodává zbranê Onarovi.
	AI_Output (self, other, "DIA_Andre_Auslieferung_Sarah_08_01"); //Sarah? Ta prodavaèka zbraní z tržištê? Máš nêjaký dùkaz?
...
instance DIA_Andre_DGRunning (C_INFO)
{
	...
	permanent    =  TRUE;
	description	 = 	"Co se týèe toho cechu zlodêjù...";
};
...
{
	AI_Output (other, self, "DIA_Andre_DGRunning_15_00"); //Co se týèe toho cechu zlodêjù...
	...
	if (Andre_Diebesgilde_aufgeraeumt == TRUE)
	{
		...
		AI_Output (self, other, "DIA_Andre_DGRunning_08_02"); //Cech zlodêjù je už jen smutná kapitola v historii tohoto mêsta.
	...
	{
		Info_AddChoice(DIA_Andre_DGRunning, "Našel jsem úkryt cechu zlodêjù!", DIA_Andre_DGRunning_Verrat);
	};
};
...
func void DIA_Andre_DGRunning_BACK()
{
	...
	{
		AI_Output (self, other, "DIA_Andre_DGRunning_BACK_08_01"); //Dobrá. Dám ti na splnêní toho úkolu trochu víc èasu.
	}
	else
	{
		AI_Output (self, other, "DIA_Andre_DGRunning_BACK_08_02"); //Dobrá! Dej mi o všem vêdêt.
	};
	Info_ClearChoices(DIA_Andre_DGRunning);
};
...
{	
	AI_Output (other, self, "DIA_Andre_DGRunning_Verrat_15_00"); //Našel jsem úkryt cechu zlodêjù!
	AI_Output (self, other, "DIA_Andre_DGRunning_Verrat_08_01"); //Kde?
	AI_Output (other, self, "DIA_Andre_DGRunning_Verrat_15_02"); //Ve stokách pod mêstem.
...
func void DIA_Andre_DGRunning_Success()
{
	...
	AI_Output (self, other, "DIA_Andre_DGRunning_Success_08_01"); //Prokázal jsi mêstu cennou službu.
	DG_gefunden = TRUE;
	...
	if (other.guild == GIL_NONE)
	{
		AI_Output (self, other, "DIA_Andre_DGRunning_Success_08_02"); //Jestli máš poâád zájem o místo v domobranê, staèí âíct.
	}
	...
	AI_Output (self, other, "DIA_Andre_DGRunning_Success_08_04"); //Právem ti náleží odmêna za ty bandity. Tady je.
...
instance DIA_Andre_JOIN	(C_INFO) 
{
	...
	permanent    =  TRUE;
	description	 = 	"Jsem pâipraven pâidat se k domobranê!";
};
...
{
	AI_Output (other, self, "DIA_Andre_JOIN_15_00"); //Jsem pâipraven pâidat se k domobranê!
	
	//ADDON
	if (Andre_Knows_MartinEmpfehlung == TRUE)
	{
		AI_Output (self, other, "DIA_Addon_Andre_JOIN_08_00"); //Skladník Martin pro tebe nejen hlasuje, ale také se za tebe pâimluvil. A to samotné mi staèí.
	}
	//ADDON
	else if (MIS_Andre_GuildOfThieves == LOG_SUCCESS)
	&& (Player_IsApprentice == APP_NONE)
	{
		AI_Output (self, other, "DIA_Andre_JOIN_08_01"); //Dodržím svou èást úmluvy a pâijímám tê do âad domobrany, aèkoliv nejsi obèanem mêsta.
		AI_Output (self, other, "DIA_Andre_JOIN_08_02"); //Ale nikomu na svêtê o tom neâekni ani slovo! Èím ménê lidí bude o této výjimce vêdêt, tím ménê to budu muset pozdêji obhajovat.
	}
	else if (Player_IsApprentice > APP_NONE)
	{
		...
		{
			AI_Output (other, self, "DIA_Andre_JOIN_15_04"); //Kováâ mê pâijal za uèedníka.
			AI_Output (self, other, "DIA_Andre_JOIN_08_05"); //Harad? Znám ho. Pracuje pro nás. Je to dobrý chlap.
		};
		
		if (Player_IsApprentice == APP_Constantino)
		{
			AI_Output (other, self, "DIA_Andre_JOIN_15_06"); //Jsem alchymistùv uèeà!
			AI_Output (self, other, "DIA_Andre_JOIN_08_07"); //V domobranê nemáme zrovna moc uèencù. Možná by se nám mohly tvoje schopnosti hodit.
			AI_Output (self, other, "DIA_Andre_JOIN_08_08"); //Moc toho o tom alchymistovi nevím. Ale lidé âíkají, že to je èestný muž.
		};
		
		if (Player_IsApprentice == APP_Bosper)
		{
			AI_Output (other, self, "DIA_Andre_JOIN_15_09"); //Výrobce lukù Bosper mê vzal do uèení.
			AI_Output (self, other, "DIA_Andre_JOIN_08_10"); //Takže víš také nêco o životê v divoèinê? To je dobâe, protože èlenové domobrany plní i úkoly mimo okruh mêstských hradeb.
			AI_Output (self, other, "DIA_Andre_JOIN_08_11"); //Lidé, kteâí se vyznají v divoèinê, se nám budou urèitê hodit.
			AI_Output (self, other, "DIA_Andre_JOIN_08_12"); //A ten výrobce lukù je pro mêsto velice významný èlovêk.
		};
		
		AI_Output (self, other, "DIA_Andre_JOIN_08_13"); //Pokud pro tebe bude hlasovat, nestojí tvému pâijetí do domobrany nic v cestê.
		
		// ------- Außerdem noch GuildOfThieves ------
		if (MIS_Andre_GuildOfThieves == LOG_SUCCESS)
		{
			AI_Output (self, other, "DIA_Andre_JOIN_08_14"); //Navíc se ti podaâilo zbavit nás toho cechu zlodêjù. Už jen za to bych tê do domobrany pâijal.
		};
	}
	else //Player_IsApprentice == APP_NONE
	{
		AI_Output (self, other, "DIA_Andre_JOIN_08_15"); //To je možné - ale stále nejsi obèanem našeho mêsta a já mám své rozkazy.
		return;
	};
		
	AI_Output (self, other, "DIA_Andre_JOIN_08_16"); //Mùžeš se k nám pâidat, jestli chceš. Ale tvé rozhodnutí bude koneèné.
	AI_Output (self, other, "DIA_Andre_JOIN_08_17"); //Jakmile na sobê budeš mít zbroj domobrany, nebudeš si ji moci jen tak sejmout a pâestat patâit mezi nás.
	AI_Output (self, other, "DIA_Andre_JOIN_08_18"); //Jsi pâipraven spoleènê s námi bojovat za Innose a krále?
	
	Info_ClearChoices (DIA_Andre_JOIN);
	Info_AddChoice (DIA_Andre_JOIN,"Ještê si nejsem tak úplnê jistý.",DIA_Andre_JOIN_No);
...
func void DIA_Andre_JOIN_Yes()
{
	...
	AI_Output (self, other, "DIA_Andre_JOIN_Yes_08_01"); //Tak budiž. Vítej v domobranê.
	...
	AI_Output (self, other, "DIA_Andre_JOIN_Yes_08_03"); //Nos ji hrdê a dùstojnê.
...
{
	AI_Output (other, self, "DIA_Andre_JOIN_No_15_00"); //Ještê si nejsem tak úplnê jistý.
	AI_Output (self, other, "DIA_Andre_JOIN_No_08_01"); //Pokud máš stále nêjaké pochyby o svém rozhodnutí, nemohu tê pâijmout do âad domobrany.
...
instance DIA_Andre_LORDHAGEN		(C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 = 	"Mohu teë koneènê jít za lordem Hagenem?";
};
...
{
	AI_Output (other, self, "DIA_Andre_LORDHAGEN_15_00"); //Mohu teë koneènê jít za lordem Hagenem?
	AI_Output (self, other, "DIA_Andre_LORDHAGEN_08_01"); //Nyní jsi ve službách paladinù. Vpustí tê dovnitâ. Ale ve vlastním zájmu bys mêl mít na srdci nêco opravdu dùležitého.
	AI_Output (other, self, "DIA_Andre_LORDHAGEN_15_02"); //To také mám, nemêj obavy.
	AI_Output (self, other, "DIA_Andre_LORDHAGEN_08_03"); //Mêj na pamêti, že budeš mluvit s velitelem paladinù. Chovej se podle toho. Nebudeš tam stát jen za sebe, ale za celou domobranu.
};
// *********************************************************
//				Waffe
// *********************************************************
instance DIA_Andre_Waffe		(C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 = 	"Dostanu také nêjakou zbraà?";
};
...
{
	AI_Output (other, self, "DIA_Andre_Waffe_15_00"); //Dostanu také nêjakou zbraà?
	AI_Output (self, other, "DIA_Andre_Waffe_08_01"); //Samozâejmê. Obvykle se o to stará Peck. Ale zdá se mi, že jsem ho už nêjakou dobu nevidêl.
	AI_Output (self, other, "DIA_Andre_Waffe_08_02"); //Zjisti, kde se zašívá, a pâiveë ho sem. Pak od nêj nafasuješ zbraà.
	AI_Output (self, other, "DIA_Andre_Waffe_08_03"); //A pokud bys chtêl spát, mùžeš si lehnout na nêkterou z postelí v kasárnách.
	
	MIS_Andre_Peck = LOG_RUNNING;
	
	Log_CreateTopic (TOPIC_Peck,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_Peck,LOG_RUNNING);
	B_LogEntry (TOPIC_Peck,"Peck se toulá nêkde po mêstê. Pokud ho pâivedu zpátky do kasáren, mohu od nêj dostat zbraà.");
...
func void DIA_Andre_FOUND_PECK_Info ()
{
	...
	AI_Output (self, other, "DIA_Andre_FOUND_PECK_08_01"); //Ano, už je zpêt na svém místê a plní své povinnosti. Kde jsi ho našel?
...
{
	AI_Output (other, self, "DIA_Andre_FOUND_PECK_SOMEWHERE_15_00"); //Narazil jsem na nêj ve mêstê.
	AI_Output (self, other, "DIA_Andre_FOUND_PECK_SOMEWHERE_08_01"); //Dobrá, tak za ním zajdi a vyzvedni si zbraà.
...
FUNC VOID DIA_Andre_FOUND_PECK_REDLIGHT()
{
	...
	AI_Output (self, other, "DIA_Andre_FOUND_PECK_REDLIGHT_08_01"); //Aha, takže místo aby se staral o své povinnosti, vysedává s dêvèaty.
...
{
	AI_Output (other, self, "DIA_Andre_FIRSTMISSION_15_00"); //Máš pro mê nêjaký úkol?
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_01"); //V poslední dobê se prý ve mêstê obchoduje s trávou z bažin.
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_02"); //Nemùžeme dopustit, aby se to tady nêjak rozmohlo.
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_03"); //Jinak by každý zaèal kouâit a nikdo by nebyl schopný pracovat, natož udržet zbraà.
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_04"); //To si nemùžeme dovolit zvlášã teë, když hrozí útok skâetù nebo žoldákù.
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_05"); //Mám podezâení, že za tím jsou ti žoldáci. Vsadím se, že to oni pâináší to svinstvo do mêsta.
	AI_Output (other, self, "DIA_Andre_FIRSTMISSION_15_06"); //Co je na práci?
	AI_Output (self, other, "DIA_Andre_FIRSTMISSION_08_07"); //Mortis, jeden z našich mužù, v pâístavní krèmê zaslechl, že nêkam do pâístavní ètvrti mêl dorazit balík trávy z bažin.
	...
	B_LogEntry (TOPIC_WAREHOUSE,"Do pâístavu dorazil balík trávy z bažin - Mortis se o tom dozvêdêl v místní hospodê. Já mám tu zásilku najít a donést lordu Andreovi.");
...
FUNC VOID DIA_Andre_FOUND_STUFF_Becken()
{
	...
	AI_Output (self, other, "DIA_Andre_FOUND_STUFF_Becken_08_01"); //Vážnê? No, hlavnê že už nemùže padnout do špatných rukou.
	B_AndreSold ();
...
instance DIA_Andre_FIND_DEALER		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"Máš pro mê nêjakou další práci?";
};
...
{
	AI_Output (other, self, "DIA_Andre_FIND_DEALER_15_00"); //Máš pro mê nêjakou další práci?
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_01"); //Stáhnul jsi ten balík trávy z obêhu - to je dobâe.
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_02"); //Rád bych ale zjistil, kdo to rozšiâuje mezi lidi.
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_03"); //Musí to být nêkdo z pâístavní ètvrti.
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_04"); //Kdyby do mêsta nêkdo pravidelnê pâicházel zvenèí, už bychom ho dávno chytili.
	AI_Output (other, self, "DIA_Andre_FIND_DEALER_15_05"); //Co pâesnê mám udêlat?
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_06"); //Najdi toho pâekupníka a nech si od nêj nêjakou tu trávu prodat. Nebude to snadné, ale jinak ho nemùžeme zatknout.
	AI_Output (self, other, "DIA_Andre_FIND_DEALER_08_07"); //Promluv si s Mortisem, vyzná se v pâístavní ètvrti. Možná by ti mohl nêjak pomoci.
	...
	B_LogEntry (TOPIC_REDLIGHT,"Mám najít chlápka, co v pâístavišti kšeftuje s trávou z bažin. Musím ho pâimêt, aby mi nêco ze svých zásob prodal. Mortis mi v tom mùže pomoci.");
};
///////////////////////////////////////////////////////////////////////
...
func void DIA_Andre_REDLIGHT_SUCCESS_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_01"); //Nemyslím, že bychom se v pâístavní ètvrti nêco dozvêdêli.
		...
		{
			AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_03"); //Ta dívka z Èervené lucerny, Nadja, je mrtvá. Musela to být nêjaká podivná nehoda.
			B_RemoveNpc (Nadja);
		};
		if (Undercover_Failed == TRUE)
		{
			AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_04"); //Nebyl jsi dostateènê opatrný.
		};
	...
	{
		AI_Output (other, self, "DIA_Andre_REDLIGHT_SUCCESS_15_05"); //Vím, kdo tu trávu ve mêstê prodává. Je to Borka, dveâník od Èervené lucerny.
		AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_06"); //Vážnê? Máme nêjaký dùkaz?
		AI_Output (other, self, "DIA_Andre_REDLIGHT_SUCCESS_15_07"); //Trochu té trávy mi prodal.
		AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_08"); //Výbornê, to nám staèí. Okamžitê ho nechám zatknout.
	...
	{
		AI_Output (other, self, "DIA_Andre_REDLIGHT_SUCCESS_15_09"); //Ještê na tom pracuju.
		AI_Output (self, other, "DIA_Andre_REDLIGHT_SUCCESS_08_10"); //Dobrá, a nezapomeà, že musíš toho chlapíka pâesvêdèit, aby s tebou uzavâel obchod.
	};
};
///////////////////////////////////////////////////////////////////////
...
{
	AI_Output (other, self, "DIA_Andre_HILFBAUERLOBART_15_00"); //Máš pro mê nêjaký další úkol?
	AI_Output (self, other, "DIA_Andre_HILFBAUERLOBART_08_01"); //Farmáâ Lobart má na svých polích nêjaké problémy.
	AI_Output (self, other, "DIA_Andre_HILFBAUERLOBART_08_02"); //Pokud mu pomùžeme, upevníme jeho vztahy s mêstem. Zajdi tedy za ním a zjisti, co se dêje.

	
	Log_CreateTopic (TOPIC_Feldraeuber, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Feldraeuber, LOG_RUNNING);
	B_LogEntry (TOPIC_Feldraeuber,"Andre mê poslal na Lobartùv statek. Mám mu pomoci ho dát znovu do poâádku."); 
...
func void DIA_Andre_LOBART_SUCCESS_Info ()
{
	...
	AI_Output (self, other, "DIA_Andre_LOBART_SUCCESS_08_01"); //Výbornê. Pokud bude Lobart spokojený, bude mêstu i nadále prodávat své tuâíny.
...
{
	AI_Output (other, self, "DIA_Addon_Andre_MissingPeople_15_00"); //Co je s têmi pohâešovanými lidmi?
	AI_Output (self, other, "DIA_Addon_Andre_MissingPeople_08_01"); //Co s nimi?
	AI_Output (other, self, "DIA_Addon_Andre_MissingPeople_15_02"); //Nemêli bychom je zkusit najít?
	AI_Output (self, other, "DIA_Addon_Andre_MissingPeople_08_03"); //Lord Hagen mi nakázal chránit mêsto a pâilehlé statky.
	AI_Output (self, other, "DIA_Addon_Andre_MissingPeople_08_04"); //To znamená, že se staráme o lidi, kteâí jsou ještê TADY.
	AI_Output (self, other, "DIA_Addon_Andre_MissingPeople_08_05"); //V noci stâeží ulice domobrana. Víc dêlat nemohu.
	AI_Output (self, other, "DIA_Addon_Andre_MissingPeople_08_06"); //A TY by sis mêl taky hledêt hlavnê úkolù, které jsem ti zadal, jasné?
...
instance DIA_Addon_Andre_MissingPeople2	(C_INFO)
{
	...
	description	= "K têm zmizelým lidem...";
};
...
{
	AI_Output (other, self, "DIA_Addon_Andre_MissingPeople2_15_00"); //Co se týèe têch zmizelých lidí...
...
{
	AI_Output	(other, self, "DIA_Addon_Andre_ReturnedMissingPeople_15_00"); //Nêkteré z têch pohâešovaných jsem osvobodil.
	if (other.guild == GIL_MIL)
	{
		AI_Output (self, other, "DIA_Addon_Andre_ReturnedMissingPeople_08_01"); //A já si âíkal, kde jsi sakra byl tak dlouho!
		AI_Output (self, other, "DIA_Addon_Andre_ReturnedMissingPeople_08_02"); //Jsi èlenem domobrany! A k nièemu takovému jsi nemêl rozkazy!
	...
	if (other.guild == GIL_MIL)
	{
		AI_Output (self, other, "DIA_Addon_Andre_ReturnedMissingPeople_08_07"); //...jsem na tebe hrdý! A jsem rád, že ses pâidal právê k nám!
		B_AndreSold ();
	};
	AI_Output (self, other, "DIA_Addon_Andre_ReturnedMissingPeople_08_08"); //To byl takâka hrdinský èin.
...
{
	AI_Output (self ,other,"DIA_Andre_Add_08_11"); //Mêl bys jít pâímo za ním.
};
...
instance DIA_Andre_BerichtDrachen		(C_INFO)
{
	...
	permanent	 =	FALSE;
	description	 = 	"Byl jsem v Hornickém údolí a vidêl jsem draky!";
};
...
func void DIA_Andre_BerichtDrachen_Info ()
{
	...
	AI_Output (other, self,"DIA_Andre_Add_15_13"); //Byl jsem v Hornickém údolí a vidêl jsem draky!
...
func void DIA_Andre_BennetInPrison_Info ()
{
	...
	AI_Output (self, other, "DIA_Andre_BennetInPrison_08_01"); //Myslíš toho žoldáka? Sedí ve vêzení, kam patâí.
	AI_Output (other,self ,"DIA_Andre_BennetInPrison_Talk_15_00"); //Mohu s ním mluvit?
	AI_Output (self ,other,"DIA_Andre_BennetInPrison_Talk_08_01"); //Jistê, jak je libo. Ale pokud se pokusíš pomoci mu k útêku, skonèíš tam také.
};
...
FUNC VOID DIA_Andre_Cornelius_Liar_No()
{
	...
	AI_Output (self ,other,"DIA_Andre_Cornelius_Liar_No_08_01"); //V tom pâípadê bys nemêl své podezâení tak zeširoka roztrubovat.
	AI_Output (self ,other,"DIA_Andre_Cornelius_Liar_No_08_02"); //Cornelius je vlivný muž. Mohl by ti udêlat ze života peklo, kdyby chtêl.
...
FUNC VOID DIA_Andre_Cornelius_Liar_Yes()
{
	...
	if (Cornelius_IsLiar == TRUE)
	{
		...
		AI_Output (self ,other,"DIA_Andre_Cornelius_Liar_Yes_08_03"); //Pokud to je skuteènê tak, musíš jít okamžitê za lordem Hagenem.
	...
	else
	{
		...
		AI_Output (self ,other,"DIA_Andre_Cornelius_Liar_Yes_08_06"); //Potâebuji dùkaz, ne podezâení. Sežeà dùkaz, pak pro tebe budu moci nêco udêlat.
		AI_Output (self ,other,"DIA_Andre_Cornelius_Liar_Yes_08_07"); //Do té doby by sis mêl radêji dávat pozor na jazyk.
	};
...
instance DIA_Andre_PERM		(C_INFO)
{
	...
	permanent	 = TRUE;
	description	 = "Jak to vypadá ve mêstê?";
};
...
{
	AI_Output (other, self, "DIA_Andre_PERM_15_00"); //Jak to vypadá ve mêstê?
	...
	if (other.guild == GIL_PAL)
	{
		AI_Output (self, other, "DIA_Andre_PERM_08_03"); //Od této chvíle se hlásíš pâímo lordu Hagenovi. Bêž si s ním promluvit.
	};
};
...
instance DIA_Andre_BerichtTorAuf (C_INFO)
{
	...
	permanent	 =	FALSE;
	description	 = 	"Na hrad v Hornickém údolí zaútoèili skâeti!";
};
...
{
	AI_Output (other, self,"DIA_Andre_Add_15_16"); //Na hrad v Hornickém údolí zaútoèili skâeti!
	AI_Output (self ,other,"DIA_Andre_Add_08_09"); //Ne! O tom se musí dozvêdêt lord Hagen.
	B_Andre_GotoLordHagen();
};
