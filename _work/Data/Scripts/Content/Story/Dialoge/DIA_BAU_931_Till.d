...
func void DIA_Till_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Till_HALLO_03_01"); //Takhle se mnou žádnej usoplenej rolník nebude mluvit - kolikrát ti to ještě budu připomínat?
...
func void DIA_Till_HALLO_selber ()
{
	...
	AI_Output			(self, other, "DIA_Till_HALLO_selber_03_01"); //(ustrašeně) Eh. Ne, jsem jen Sekobův syn. Ale až můj starý otec jednou nebude mezi námi, bude mi to tu všechno patřit.
...
func void DIA_Till_FELDARBEITER_Info ()
{
	...
	AI_Output			(self, other, "DIA_Till_FELDARBEITER_03_01"); //Jasně - potřebujou to. A není proč se pohoršovat, takhle to prostě funguje.
...
func void DIA_Till_SEKOB_Info ()
{
	...
	AI_Output			(other, self, "DIA_Till_SEKOB_15_02"); //Představ mě svému otci.
	AI_Output			(self, other, "DIA_Till_SEKOB_03_03"); //Ty jsi opravdu neústupný chlapík a zřejmě i dobrý bojovník, co?
...
func void DIA_Till_WASMACHSTDU_Info ()
{
	...
	AI_Output			(other, self, "DIA_Till_WASMACHSTDU_15_05"); //Ale jistě!
...
{
	AI_Output			(other, self, "DIA_Till_BRONKO_15_00"); //Je tady chlápek, co chce po každém kolemjdoucím mýtné a tvrdí o sobě, že je farmář.
	AI_Output			(self, other, "DIA_Till_BRONKO_03_01"); //(rozpačitě) Eh. Ano. Já vím. To je Bronco. Dělá si, co chce.
	...
	AI_Output			(self, other, "DIA_Till_BRONKO_03_04"); //Prostě to neudělal.
	
	if (Npc_IsDead(Sekob)== FALSE)
	{
		AI_Output			(self, other, "DIA_Till_BRONKO_03_05"); //Můj otec je na mě pěkně naštvanej, že jsem ho ještě nedokázal přinutit, aby se vrátil do práce.
	};
};
...
func void DIA_Till_BRONKOZURARBEIT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Till_BRONKOZURARBEIT_03_01"); //Myslíš s ním? Fajn, poslouchej, když se ti podaří Bronca dostat zpátky do práce, zaplatím ti. Řekněme 10 zlatých. Co říkáš?
...
func void DIA_Till_BRONKOZURARBEIT_ok ()
{
	...
	B_LogEntry (TOPIC_Bronkoeingeschuechtert,"Till nedokáže Bronca přimět k práci, a proto mě požádal, abych se o to pokusil."); 
	...
	if (Till_IchMachsNurEinmal == TRUE)
	 ...
	 if (Till_Angebot == 10)
		...
		{	
			...
			Info_ClearChoices	(DIA_Till_BRONKOZURARBEIT);
			Info_AddChoice	(DIA_Till_BRONKOZURARBEIT, "Dobrá. Uvidím, co se dá dělat.", DIA_Till_BRONKOZURARBEIT_mehr );
			Info_AddChoice	(DIA_Till_BRONKOZURARBEIT, "Dobrá. Uvidím, co se dá dělat.", DIA_Till_BRONKOZURARBEIT_ok );
			Till_Angebot = 50;
		}
		...
		{	
			...
			AI_Output			(self, other, "DIA_Till_BRONKOZURARBEIT_mehr_03_06"); //Tak dobře, dobře! Dám ti 100 zlatých. Ale to je všechno, co mám.
			Info_ClearChoices	(DIA_Till_BRONKOZURARBEIT);
...
func void DIA_Till_BRONKOWIEDERANARBEIT_Info ()
{
	...
	if (Till_Angebot <= 50)
		{
			AI_Output			(self, other, "DIA_Till_BRONKOWIEDERANARBEIT_03_03"); //(váhavě) Mmh. dobrá. Obchod je obchod, co?
	...
	if (Kapitel == 5)
		{
			...
			AI_Output			(self, other, "DIA_Till_PERMKAP1_03_03"); //Tak mě nech na pokoji.
			AI_StopProcessInfos (self);	
		}
		else
		{		
			if 	(self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_LOST)
				{
					AI_Output			(other, self, "DIA_Till_PERMKAP1_15_04"); //Možná bys měl víc jíst, abys víc vyrostl a stal se silnějším.
			...
			else
				{
					if (Till_HatSeinGeldBehalten == TRUE)
						...
						{
							AI_Output			(other, self, "DIA_Till_PERMKAP1_15_08"); //Máš někoho, kdo ti dneska bude foukat na nosánek?
							AI_Output			(self, other, "DIA_Till_PERMKAP1_03_09"); //Nech mě být, ty pitomče.
						};
				};
		};
};
