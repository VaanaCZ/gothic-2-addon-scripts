...
func void DIA_Kati_SLDNOCHDA_Info ()
{
	...
	if (Hilfe == FALSE)
	{
		AI_Output (self, other, "DIA_Kati_SLDNOCHDA_16_00"); //Ti hrdloâezové mému muži vyhrožují! Jsme âádnými obèany mêsta, vêrnými králi, a žoldáci nás chtêjí oloupit o všechno!
		Hilfe = TRUE;
	};
...
func void DIA_Kati_HALLO_Info ()
{
	...
	if (Npc_IsDead (Akil))
	{
		AI_Output (self, other, "DIA_Kati_HALLO_16_01"); //(vzlyká) Mùj milovaný manžel je mrtvý! Ó Innosi, zaè mê tak tvrdê trestáš?
...
instance DIA_Kati_ESSEN		(C_INFO)
{
	...
	permanent 	 =  FALSE;
	description	 = 	"Akil âíkal, že pro mê budeš mít nêco k jídlu.";
};
...
{
	AI_Output (other, self, "DIA_Kati_ESSEN_15_00"); //Akil âíkal, že pro mê budeš mít nêco k jídlu.
...
{
	AI_Output (other, self, "DIA_Kati_Baltram_15_00"); //Poslal mê Baltram. Mám pro nêj vyzvednout nêjaký balík.
	AI_Output (self, other, "DIA_Kati_Baltram_16_01"); //Ano, samozâejmê. Tady, už jsem to zabalil.
...
func void DIA_Kati_BAUERNAUFSTAND_Info ()
{
	...
	AI_Output (self, other, "DIA_Kati_BAUERNAUFSTAND_16_01"); //Pro nás, co máme farmy blízko mêsta, má vêtší smysl zùstat na stranê domobrany než se spoléhat na Onarovy žoldáky.
	AI_Output (self, other, "DIA_Kati_BAUERNAUFSTAND_16_02"); //Na druhou stranu, jsou tu i takoví jako Bengar a Sekob, co by se radêji vzdali svých farem, než by pracovali pro krále.
...
func void DIA_Kati_ANDEREHOEFE_Info ()
{
	...
	AI_Output (self, other, "DIA_Kati_ANDEREHOEFE_16_01"); //Není to daleko od farmy, na které velkostatkáâ žije. Nalezneš je, když odsud pùjdeš na východ.
};
...
instance DIA_Kati_HIERWEG		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"Pomýšleli jste nêkdy na to, že byste odsud odešli?";
};
...
{
	AI_Output (other, self, "DIA_Kati_HIERWEG_15_00"); //Pomýšleli jste nêkdy na to, že byste odsud odešli?
	AI_Output (self, other, "DIA_Kati_HIERWEG_16_01"); //Není zrovna snadné se dostat z tohodle kousku zemê. Celý kraj je obklopen vysokým a nepâekonatelným horským masivem.
	AI_Output (self, other, "DIA_Kati_HIERWEG_16_02"); //Jediné dvê cesty ven jsou prùsmyk do Hornického údolí a pâístav v Korinidu.
	AI_Output (self, other, "DIA_Kati_HIERWEG_16_03"); //Protože si nemùžeme dovolit cestu lodí a z Hornického údolí není návratu, zùstáváme tady.
};
...
func void DIA_Kati_PASS_Info ()
{
	...
	AI_Output (self, other, "DIA_Kati_PASS_16_01"); //Nikdy jsem tam osobnê nebyla. Musí to být nêkde poblíž Bengarovy farmy na náhorních pastvinách.
};
...
func void DIA_Kati_PERMKAP1_Info ()
{
	...
	AI_Output			(self, other, "DIA_Kati_PERMKAP1_16_01"); //Udêlám, co mùžu.
	AI_StopProcessInfos (self);
};
...
func void DIA_Kati_PERM_Info ()
{
	...
	AI_Output			(self, other, "DIA_Kati_PERM_16_01"); //Dá se to pâežít. Jen by mê zajímalo, jak dlouho tu ještê budeme muset snášet ty ëábly v èerném.
