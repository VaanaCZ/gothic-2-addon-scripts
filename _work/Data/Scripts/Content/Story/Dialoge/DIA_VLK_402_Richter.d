...
{	
	AI_Output (other,self ,"DIA_Richter_Hello_15_00"); //Co tady dêláš?
	AI_Output (self ,other,"DIA_Richter_Hello_10_01"); //Proè mê obtêžuješ? Kdo vlastnê jsi?
	AI_Output (other,self ,"DIA_Richter_Hello_15_02"); //Já, ehm...
	AI_Output (self ,other,"DIA_Richter_Hello_10_03"); //Âekni, neznáme se odnêkud?
...
FUNC VOID DIA_Richter_Hello_DontThinkSo ()
{
	...
	AI_Output (self ,other,"DIA_Richter_Hello_DontThinkSo_10_01"); //To je zvláštní. Pâísahal bych, že už jsme se nêkde vidêli.
	AI_Output (self ,other,"DIA_Richter_Hello_DontThinkSo_10_02"); //Nevadí, však jsi jen ubohý šupák. Všichni vypadáte stejnê.
};

FUNC VOID DIA_Richter_Hello_MayBe ()
{
	AI_Output (other,self ,"DIA_Richter_Hello_MayBe_15_00"); //Možná ano.
	AI_Output (self ,other,"DIA_Richter_Hello_MayBe_10_01"); //Chtêlo by to trochu uctivêjší tón! S kým si vùbec myslíš, že mluvíš?
	AI_Output (self ,other,"DIA_Richter_Hello_MayBe_10_02"); //To nejmenší, co mùžeš udêlat, je mluvit se mnou odpovídajícím tónem.
	AI_Output (self ,other,"DIA_Richter_Hello_MayBe_10_03"); //A teë se mi ztraã z oèí!
...
{	
	...
	{
		AI_Output (self ,other,"DIA_Richter_Perm_10_01"); //Radêji bys mêl vypadnout, ty mizernej žoldáku, než zavolám stráže.
		AI_Output (other,self ,"DIA_Richter_Perm_15_02"); //Klídek. Nepâišel jsem tê okrást. Hledám práci.
		AI_Output (self ,other,"DIA_Richter_Perm_10_03"); //Á. Tak ty pro mê chceš pracovat. Tak to je samozâejmê jiná.
		AI_Output (self ,other,"DIA_Richter_Perm_10_04"); //Ale má to jeden háèek. Nevím, jestli ti mohu vêâit. Budeš mi muset nejprve prokázat svou loajalitu.
		SCIstRichtersLakai = TRUE;
	}
	else
	{
		AI_Output (self ,other,"DIA_Richter_Perm_10_05"); //Neâekl jsem snad jasnê, že s tebou nechci nic mít?
...
{	
	AI_Output (other,self ,"DIA_Richter_RichtersLakai_15_00"); //Otestuj si mê.
	AI_Output (self ,other,"DIA_Richter_RichtersLakai_10_01"); //Mmh. Dobrá. Poslouchej. Pâines mi od ohnivých mágù posvátné kladivo. Ukrývají ho nêkde v podzemí svého kláštera.
	AI_Output (self ,other,"DIA_Richter_RichtersLakai_10_02"); //Jestli to dokážeš splnit, možná budu uvažovat o tom, že bych si tê najal jako svého osobního strážce.
	...
	B_LogEntry (TOPIC_RichterLakai,"Soudce mi nevêâí. Musím mu prokázat svou loajalitu - ukrást ze sklepení kláštera ohnivých mágù posvátné kladivo a pâinést mu je."); 
...
func void DIA_Richter_RichtersLakai_nein ()
{
	...
	AI_Output			(self, other, "DIA_Richter_RichtersLakai_nein_10_01"); //V tom pâípadê táhni do pekel a pâestaà plýtvat mým èasem.
	AI_StopProcessInfos (self);
};
...
func void DIA_Richter_RichtersLakai_ja ()
{
	...
	AI_Output			(self, other, "DIA_Richter_RichtersLakai_ja_10_01"); //Fajn. Ale pamatuj - jestli tê chytí, nikdy jsem o tobê neslyšel.
	AI_StopProcessInfos (self);
};
...
{	
	...
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_01"); //Jsem opravdu pâekvapený. Ty to myslíš vážnê, co? Tady je tvá odmêna.
	CreateInvItems (self, ItMi_Gold, 300);									
	B_GiveInvItems (self, other, ItMi_Gold, 300);					
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_02"); //Fajn. Když se to má takhle, tak to mám pro tebe ještê další úkol.
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_03"); //Pâed nêkolika dny uteklo ze zdejšího vêzení pár trestancù.
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_04"); //Ti zkurvysyni se schovali v horách. Nêkde smêrem ke Xardasovê nové vêži.
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_05"); //Jejich vùdce se jmenuje Morgahard. Chci, abys ho zabil.
	AI_Output (self ,other,"DIA_Richter_KillMorgahard_10_06"); //Pospêš si, než nám proklouzne.
	...
	B_LogEntry (TOPIC_RichterLakai,"Nêkteâí vêzni unikli spravedlnosti. Já je mám dostihnout a zabít jejich velitele Morgaharda. Nejspíš se ukrývají v horách, kdesi smêrem ke Xardasovê nové vêži."); 
...
{	
	...
	AI_Output (self ,other,"DIA_Richter_KilledMorgahard_10_01"); //Á. Jsi dobrej chlap. Tady je tvoje odmêna.
	CreateInvItems (self, ItMi_Gold, 400);									
	B_GiveInvItems (self, other, ItMi_Gold, 400);					
	B_GivePlayerXP (XP_KillMorgahard);
	B_LogEntry (TOPIC_RichterLakai,"Soudce byl zprávou o Morgahardovê smrti oèividnê nadšen. No, nechme ho být. Nesmím odboèovat od poslání, kterým mê povêâil Lee."); 
...
INSTANCE DIA_Richter_KilledMorgahardPERM (C_INFO)
{
	...
	description =	"Mùžu pro tebe ještê nêco udêlat?";
};                       
...
{	
	AI_Output (other,self ,"DIA_Richter_KilledMorgahardPERM_15_00"); //Mùžu pro tebe ještê nêco udêlat?
	AI_Output (self ,other,"DIA_Richter_KilledMorgahardPERM_10_01"); //Teë zrovna ne. Možná pozdêji.
};
...
INSTANCE DIA_Richter_PermissionForShip (C_INFO)
{
	...
	description =	"Dej mi písemné povêâení pro paladinskou loë.";
};                       
...
{	
	AI_Output (other,self ,"DIA_Richter_PermissionForShip_15_00"); //Dej mi písemné povêâení pro paladinskou loë.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_10_01"); //(smích) Ty ses musel zbláznit. Co to po mnê chceš?
	AI_Output (other,self ,"DIA_Richter_PermissionForShip_15_02"); //Slyšel jsi dobâe. Dej mi to písemné oprávnêní, nebo tê prásknu domobranê.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_10_03"); //(vrèí) Nenechám se vydírat! Ne od tebe! Nechám tê sežrat vlkùm, ty mizernej šmejde.
	...
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_10_06"); //Toho budeš litovat. Nikdo mê nebude vydírat, aniž by z toho vyváznul se zdravou kùží.
	AI_Output (self ,other,"DIA_Richter_PermissionForShip_10_07"); //Tady je to oprávnêní. A teë vypadni. Vyâídím si to s tebou pozdêji.
	CreateInvItems (self,ItWr_ForgedShipLetter_Mis,1);
	B_GiveInvItems (self,other,ItWr_ForgedShipLetter_Mis,1);
	B_LogEntry (TOPIC_Ship,"S plnou mocí v kapse mohu získat loë, než bys âekl švec. Jsem zvêdavý, co tomu âekne Lee."); 
...
{	
	AI_Output (self ,other,"DIA_Richter_perm2_10_00"); //Ztraã se mi z oèí.
	AI_StopProcessInfos (self);
};
