...
INSTANCE DIA_Bennet_DI_Hello (C_INFO)
{
	...
	description = "Jak ci się powodzi?";
};    
...
{	
	AI_Output	(other ,self,"DIA_Bennet_DI_Hello_15_00");	//Jak ci się powodzi?

	if (ORkSturmDI == FALSE)
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_01");	//Kuźnia na statku jest trochę przerdzewiała. To przez słoną wodę. Ciężko tu będzie zrobić coś porządnego. Ale poza tym...
	}
	else
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_02");	//Tak, zakładając, że orkowie nie wrócą.
	};
};
...
instance DIA_Bennet_DI_TRADE		(C_INFO)
{
	...
	description	 = 	"Jaką broń możesz mi sprzedać?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_TRADE_15_00"); //Jaką broń możesz mi sprzedać?
	B_GiveTradeInv (self);
	AI_Output			(self, other, "DIA_Bennet_DI_TRADE_06_01"); //Tylko najlepszą. Przecież wiesz.
};
...
instance DIA_Bennet_DI_Smith		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Nauczysz mnie swojego rzemiosła?";
};
...
func void DIA_Bennet_DI_Smith_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_DI_Smith_06_01"); //Zależy, co chcesz zrobić.
...
{	
	...
	AI_Output (self,other ,"DIA_Bennet_TeachSTR_06_01"); //Silna ręka przydaje się w takich czasach.
...
func void DIA_Bennet_DI_DragonEgg_Info ()
{
	...
	AI_Output			(other, self, "DIA_Bennet_DI_DragonEgg_15_02"); //Nooo... Myślałem...
	AI_Output			(self, other, "DIA_Bennet_DI_DragonEgg_06_03"); //Wiem, co sobie myślałeś. Zapomnij o tym i zatrzymaj je. Nie chcę go.
...
instance DIA_Bennet_DI_UndeadDragonDead		(C_INFO)
{
	...
	description	 = 	"Zrobiliśmy wszystko, co było trzeba.";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_UndeadDragonDead_15_00"); //Zrobiliśmy wszystko, co było trzeba.
	AI_Output			(self, other, "DIA_Bennet_DI_UndeadDragonDead_06_01"); //Cieszę się. Miałem już dość tej starej kuźni. Chcę znowu stanąć na pewnym gruncie.
	AI_StopProcessInfos (self);
};
