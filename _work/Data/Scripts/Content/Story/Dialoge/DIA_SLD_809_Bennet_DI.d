...
{	
	...
	if (ORkSturmDI == FALSE)
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_01");	//Kovárna na téhle lodi je tak kapku zarezlá. Mùže za to ta moâská sùl. Vyrobit tu nêco komplikovanêjšího mùže být docela problém. Ale zase na druhou stranu...
	}
	else
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_02");	//Mùžeme zaâídit, aby se skâeti nevrátili zpêt.
	};
};
...
instance DIA_Bennet_DI_TRADE		(C_INFO)
{
	...
	description	 = 	"Jaké zbranê mi mùžeš prodat?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_TRADE_15_00"); //Jaké zbranê mi mùžeš prodat?
	B_GiveTradeInv (self);
...
instance DIA_Bennet_DI_Smith		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Mùžeš mê nauèit své umêní?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_Smith_15_00"); //Mùžeš mê nauèit své umêní?
	AI_Output			(self, other, "DIA_Bennet_DI_Smith_06_01"); //Záleží na tom, co chceš dêlat.
	...
	{
		Info_AddChoice		(DIA_Bennet_DI_Smith, B_BuildLearnString("Nauèit se kováâství"	   , B_GetLearnCostTalent(other, NPC_TALENT_SMITH, WEAPON_Common))			,DIA_Bennet_DI_Smith_Common);
	};
...
{	
	AI_Output (other,self ,"DIA_Bennet_TeachSTR_15_00"); //Chtêl bych se stát silnêjším.
...
func void DIA_Bennet_DI_DragonEgg_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_DI_DragonEgg_06_03"); //Vím, na cos myslel. Zapomeà na to, nech si tu vêc. Já to nechci.
...
instance DIA_Bennet_DI_UndeadDragonDead		(C_INFO)
{
	...
	description	 = 	"Udêlali jsme všechno, co jsme potâebovali udêlat. ";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_UndeadDragonDead_15_00"); //Udêlali jsme všechno, co jsme potâebovali udêlat.
	AI_Output			(self, other, "DIA_Bennet_DI_UndeadDragonDead_06_01"); //To rád slyším. Už jsem mám téhle staré kovárny po krk. Potâebuju se zase postavit na zem pevnýma nohama.
	AI_StopProcessInfos (self);
};
