...
{
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_00"); //Nemáš nêco k pití?
	AI_Output (other,self ,"DIA_Hokurn_Hello_15_01"); //A asi to nemá být voda, co?
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_02"); //To teda ne, ksakru!! Potâebuju nêjakej alkohol, abych se koneènê zbavil toho zatracenýho bolehlavu.
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_03"); //Jestli se brzo nêèeho nenapiju, nejspíš mi pukne hlava.
...
FUNC VOID DIA_Hokurn_Hello_No ()
{
	...
	Info_AddChoice (DIA_Hokurn_Hello,"Ještê jedna vêc.",DIA_Hokurn_Hello_ASK1);
};

FUNC VOID DIA_Hokurn_Hello_ASK1 ()
{
	AI_Output (other,self ,"DIA_Hokurn_Hello_ASK1_15_00"); //Ještê jedna vêc...
	AI_Output (self ,other,"DIA_Hokurn_Hello_ASK1_01_01"); //(povzdech) Tys mi nerozumêl? VYPADNI!!!
...
FUNC VOID B_Hokurn_Sauf ()
{
	...
	AI_Output (self ,other,"B_Hokurn_Sauf_01_01"); //Teë mùžu znova myslet s èistou hlavou. Co pro tebe mùžu udêlat?
};
...
INSTANCE DIA_Hokurn_Drink   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady máš nêco k pití.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_Drink_15_00"); //Tady máš nêco k pití.
	AI_Output (self ,other,"DIA_Hokurn_Drink_01_01"); //(dychtivê) Dej to sem!!
...
{
	AI_Output (other,self ,"DIA_Hokurn_Question_15_00"); //Potâebuju nêjaké informace.
	AI_Output (self ,other,"DIA_Hokurn_Question_01_01"); //(otrávenê) Mêl jsem dojem, že jsme si rozumêli. Mluvím jen s pâáteli.
	AI_Output (self ,other,"DIA_Hokurn_Question_01_02"); //A pâátelé ti nabídnou nêco k pití. Jasný? Teë odpal!
};
...
INSTANCE DIA_Hokurn_Learn   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Hledám nêkoho, kdo by mê mohl nêèemu nauèit.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_Learn_15_00"); //Hledám nêkoho, kdo by mê mohl nêèemu nauèit.
	AI_Output (self ,other,"DIA_Hokurn_Learn_01_01"); //Mùžu ti ukázat pár vêcí. Jsem nejlepší bojovník na míle daleko.
	AI_Output (self ,other,"DIA_Hokurn_Learn_01_02"); //Jelikož jsme pâátelé, udêlám ti zvláštní cenu. 300 zlatých.

	Info_ClearChoices (DIA_Hokurn_Learn);
	Info_AddChoice	(DIA_Hokurn_Learn,"To je pro mê moc drahé.",DIA_Hokurn_Learn_TooExpensive);
...
{
	AI_Output (other,self ,"DIA_Hokurn_Learn_TooExpensive_15_00"); //To je pro mê moc drahé.
	AI_Output (self ,other,"DIA_Hokurn_Learn_TooExpensive_01_01"); //Moc drahé? To je nejmíà, co by si na mém místê âekl kdokoli jiný.
...
INSTANCE DIA_Hokurn_PayTeacher   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady jsou peníze. Chci, abys mê uèil.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_PayTeacher_15_00"); //Tady jsou peníze. Chci, abys mê uèil.
...
INSTANCE DIA_Hokurn_DrinkAndLearn   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady je pro tebe nêco k pití.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_DrinkAndLearn_15_00"); //Tady je pro tebe nêco k pití.
...
{	
	AI_Output (other,self ,"DIA_Hokurn_Teach_15_00"); //Zaènême s tréninkem.
	
	if (HokurnLastDrink < Wld_GetDay ())
	{
		AI_Output (self ,other,"DIA_Hokurn_Teach_01_01"); //Nejdâív mi pâines nêco k pití!
	}
	else
	{	 
		if (hero.guild == GIL_PAL)
		{
			AI_Output (self ,other,"DIA_Hokurn_Teach_01_02"); //Tak se teda uvidí, co mùžeme dostat z têch tvejch zatuhlejch paladinskejch kostí, jasný?
		}
		else if (hero.guild == GIL_KDF)
		{
			AI_Output (self ,other,"DIA_Hokurn_Teach_01_03"); //Co ty víš. Dokonce i mágové používají chladné zbranê.
		};
...
{
	AI_Output(self,other,"B_Hokurn_TeachedEnough_01_00"); //Na tuhle disciplínu už nepotâebuješ žádnýho uèitele.
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_StayHere_15_00"); //A co tady vlastnê dêláš?
	AI_Output (self ,other,"DIA_Hokurn_StayHere_01_01"); //Nemám tušení, proè jsme sem pâišli, nebo kdy to zaène.
...
FUNC VOID DIA_Hokurn_WhereDragon_Info()
{
	...
	if Npc_HasItems (other,ITFO_BOOZE) >=1
	{
		Info_AddChoice (DIA_Hokurn_WhereDragon,"Ještê tu mám láhev ginu!",DIA_Hokurn_WhereDragon_Booze);	 
	};
};
...
FUNC VOID DIA_Hokurn_WhereDragon_FindMyself ()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_WhereDragon_FindMyself_01_01"); //Mêl by sis dávat pozor. Èeká tê spousta bojù.
	Info_ClearChoices (DIA_Hokurn_WhereDragon);
};
...
FUNC VOID DIA_Hokurn_WhereDragon_TooMuch ()
{
	...
	AI_Output 	(self ,other,"DIA_Hokurn_WhereDragon_TooMuch_01_01"); //Tak to na to zapomeà.
	Info_ClearChoices	(DIA_Hokurn_WhereDragon); 
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_WhereDragon_Booze_15_00"); //Ještê tu mám láhev ginu!
	AI_Output (self ,other,"DIA_Hokurn_WhereDragon_Booze_01_01"); //Za poâádnou pitku bych zabil všechny draky svêta.
...
FUNC VOID DIA_Hokurn_Dragon_Info()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_01"); //Pâirozenê ti to neâeknu úplnê pâesnê, ale slyšel jsem, že by tu mêli být celkem ètyâi.
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_02"); //V noci jsme zahlédli nad nejvyšší horou rudou záâi.
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_03"); //Pâísahám na hrob mé matky, pokud hledáš draka, tak když nikde jinde, tam ho urèitê najdeš.
};
...
FUNC VOID DIA_Hokurn_AllDragonsDead_Info()
{
	...
	AI_Output 	(self ,other,"DIA_Hokurn_AllDragonsDead_01_01"); //Udêlej si výlet a vyprávêj své pohádky nêkomu jinému.
};
