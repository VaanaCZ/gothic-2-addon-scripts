...
func void DIA_Angar_DI_HALLO_Info ()
{
	...
	if ((Npc_IsDead(UndeadDragon)) == FALSE)
	{
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_01"); //Od tý doby, co jsme na ostrovê, je ta bolest èím dál tím nesnesitelnêjší.
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_02"); //Èert aby to spral. Už by to snad mohlo skonèit.
	}
	else
	{
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_03"); //Nedêlej si se mnou starosti. Prostê odsud vypadni.
	};
...
{
	AI_Output			(self, other, "DIA_Angar_DI_ORKS_04_00"); //Mêli bysme sebou zatracenê mrsknout. Další skâetí útok nás bude stát život.
	AI_Output			(other, self, "DIA_Angar_DI_ORKS_15_01"); //Dêlám, co mùžu.
	...
	Info_AddChoice	(DIA_Angar_DI_ORKS, "Nech toho skuhrání a radši mi pojë pomoct.", DIA_Angar_DI_ORKS_follow );
};
func void DIA_Angar_DI_ORKS_follow ()
{
	AI_Output			(other, self, "DIA_Angar_DI_ORKS_follow_15_00"); //Tak mi pomoz a pâestaà skuèet.
...
instance DIA_Angar_DI_FOLLOW		(C_INFO)
{
	...
	description  =  "Teë se drž radši vzadu.";
...
{
	AI_Output			(other, self, "DIA_Angar_DI_FOLLOW_15_00"); //Zùstaà vzadu.

	if (Npc_GetDistToWP(self,"SKELETTE")<4000) 
		{
			AI_Output			(other, self, "DIA_Angar_DI_FOLLOW_15_01"); //O zbytek se postarám sám.
			AI_Output			(self ,other, "DIA_Angar_DI_FOLLOW_04_02"); //Hodnê štêstí.
		...
		{	
			AI_Output			(self, other, "DIA_Angar_DI_FOLLOW_04_03"); //Jasná vêc.
...
{
	AI_Output			(self, other, "DIA_Angar_DI_FOLLOWSTOP_04_00"); //Aarh! Ta bolest mê pâivádí k šílenství!
};
...
func void DIA_Angar_DI_UNDEADDRGDEAD_Info ()
{
	...
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_01"); //Mùžeme už odsud koneènê vypadnout?
	AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_02"); //Jo. Nepâítel byl poražen.
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_03"); //Tak to nesmíme dál ztrácet èas. Zajdi kapitánovi âíct, aby zvednul kotvy.

	if (SC_KnowsMadPsi == TRUE)
	{
		AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_04"); //Doufám, že vás, odpadlých èlenù sekty, není víc.
		AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_05"); //Všechno je možné. I když Pátraèi byli velmi dùkladní. Dokonce málem dostali i mê. Kdo ví?
	};

	AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_06"); //Co budeš dêlat dál?
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_07"); //Možná se usadím na venkovê a zaènu farmaâit. Válèení už mê dost unavuje.
	AI_StopProcessInfos (self);
	Npc_ExchangeRoutine	(self,"Zaèít");
};
