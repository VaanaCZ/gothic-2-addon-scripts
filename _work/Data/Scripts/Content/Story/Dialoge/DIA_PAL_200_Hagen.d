...
{
	AI_Output (self, other, "DIA_Hagen_PMSchulden_04_00"); //Dobâe že jsi pâišel. Mùžeš hned zaplatit tu pokutu.
	...
	if (B_GetTotalPetzCounter(self) > Hagen_LastPetzCounter)
	{
		...
		AI_Output (self, other, "DIA_Hagen_PMSchulden_04_01"); //Nebereš zákony mêsta moc vážnê, co?
		AI_Output (self, other, "DIA_Hagen_PMSchulden_04_02"); //Seznam tvých prohâeškù zase narostl.
		if (Hagen_Schulden < 1000)
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_03"); //A neâíkej mi, žes to nevêdêl!
		}
		else
		{
	...
	{
		AI_Output (self, other, "DIA_Hagen_PMSchulden_04_05"); //No, zdá se, že se situace zmênila.
		
		if (Hagen_LastPetzCrime == CRIME_MURDER)
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_06"); //Nejsou už žádní svêdkové tvé vraždy!
		};
		
		if (Hagen_LastPetzCrime == CRIME_THEFT)
		|| ( (Hagen_LastPetzCrime > CRIME_THEFT) && (B_GetGreatestPetzCrime(self) < CRIME_THEFT) )
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_07"); //Nikdo už nebude svêdèit, že tê vidêl krást!
		};
		
		if (Hagen_LastPetzCrime == CRIME_ATTACK)
		|| ( (Hagen_LastPetzCrime > CRIME_ATTACK) && (B_GetGreatestPetzCrime(self) < CRIME_ATTACK) )
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_08"); //Už nezbyli žádní svêdci té tvé rvaèky.
		};
		
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_09"); //Všechna obvinêní proti tobê byla stažena.
		};
		
		AI_Output (self, other, "DIA_Hagen_PMSchulden_04_10"); //Nevím, co se tam ve mêstê stalo - a ani to vêdêt nechci.
		AI_Output (self, other, "DIA_Hagen_PMSchulden_04_11"); //Dávej si prostê pozor, aby ses tady nedostal do problémù.
		// ------- Schulden erlassen oder trotzdem zahlen ------
		if (B_GetGreatestPetzCrime(self) == CRIME_NONE)
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_12"); //Tak nebo tak, rozhodl jsem se prominout ti tvé dluhy.
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_13"); //Dej si pozor, aã se nedostaneš do žádných dalších nesnází.
		...
		{
			AI_Output (self, other, "DIA_Hagen_PMSchulden_04_14"); //Nakonec stejnê zaplatíš všechny své závazky v plné výši.
			B_Say_Gold (self, other, Hagen_Schulden);
	...
	if (B_GetGreatestPetzCrime(self) != CRIME_NONE)
	{
		...
		Info_AddChoice		(DIA_Hagen_PMSchulden,"Nemám dost penêz!",DIA_Hagen_PETZMASTER_PayLater);
...
FUNC VOID DIA_Hagen_PETZMASTER_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_00"); //Tvá povêst tê pâedchází. Porušil jsi zákony mêsta.
	};
	
	if (B_GetGreatestPetzCrime(self) == CRIME_MURDER) 
	{
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_01"); //Dostal ses do pêkné šlamastyky.
		...
		{
			AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_03"); //Nemluvê o tvých dalších prohâešcích.
		};
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_04"); //Stráže mají pâíkazy na místê popravit každého vraha.
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_05"); //Vražda je v tomto mêstê nepâípustná. Mùžeš však dát svou lítost najevo zaplacením pokuty.

	};
		
	if (B_GetGreatestPetzCrime(self) == CRIME_THEFT) 
	{
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_06"); //Jsi obvinên z krádeže!
		if ((PETZCOUNTER_City_Attack + PETZCOUNTER_City_Sheepkiller) > 0)
		{
			AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_07"); //Nemluvê o dalších vêcech, které se ke mnê donesly.
		};
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_08"); //To je porušení mêstských zákonù. Za to zaplatíš.
	...
	if (B_GetGreatestPetzCrime(self) == CRIME_ATTACK)
	{
		...
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_11"); //Porušení zákonù mêsta je porušením zákonù samotného Innose.
	...
	{
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_13"); //Zasedl sis na naše ovce - poprvé jsem tomu ani nechtêl vêâit.
		AI_Output (self, other, "DIA_Hagen_PETZMASTER_04_14"); //Proè se musím poâád zaobírat takovými prkotinami?
	...
	Info_ClearChoices  	(DIA_Hagen_PMSchulden);
	Info_ClearChoices  	(DIA_Hagen_PETZMASTER);
	Info_AddChoice		(DIA_Hagen_PETZMASTER,"Nemám dost penêz!",DIA_Hagen_PETZMASTER_PayLater);
	if (Npc_HasItems(other, itmi_gold) >= Hagen_Schulden)
	{
...
func void DIA_Hagen_PETZMASTER_PayNow()
{
	...
	AI_Output (self, other, "DIA_Hagen_PETZMASTER_PayNow_04_01"); //Dobrá! Dohlédnu na to, aby se o tom dozvêdêl každý ve mêstê - to ti alespoà èásteènê vylepší reputaci.
...
{
	AI_Output (other, self, "DIA_Hagen_PETZMASTER_PayLater_15_00"); //Nemám dost penêz!
	AI_Output (self, other, "DIA_Hagen_PETZMASTER_PayLater_04_01"); //Tak si nêjaké co nejrychleji sežeà.
	AI_Output (self, other, "DIA_Hagen_PETZMASTER_PayLater_04_02"); //A varuji tê: jestli se proviníš ještê nêèím, jen si to zhoršíš.
...
{
	AI_Output (self, other, "DIA_Lord_Hagen_Hallo_04_00"); //Už jsem o tobê slyšel.
	...
	AI_Output (self, other, "DIA_Lord_Hagen_Hallo_04_04"); //Mám spoustu práce. Takže neplýtvej mým èasem - a radêji mi âekni, proè tu jsi.
};
...
INSTANCE DIA_Lord_Hagen_Frieden (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 =  "Pâináším návrh pâímêâí od žoldnéâù!";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Frieden_15_00"); //Pâináším návrh pâímêâí od žoldnéâù!
	...
	AI_Output (self, other, "DIA_Lord_Hagen_Frieden_04_03"); //Považuji ho za èestného muže. Jsem ochoten mu jeho prohâešky odpustit - ale jenom jemu!
	AI_Output (self, other, "DIA_Lord_Hagen_Frieden_04_04"); //To se netýká jeho mužù. Vêtšina z nich jsou hanební hrdloâezové a svùj trest si plnê zasluhují!
	AI_Output (self, other, "DIA_Lord_Hagen_Frieden_04_05"); //Jim rozhodnê nic neprominu. Mùžeš jim to oznámit.
...
INSTANCE DIA_Lord_Hagen_Armee (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 =  "Nedaleko se shromažëují zástupy zlých stvùr. V Hornickém údolí.";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Armee_15_00"); //Nedaleko se shromažëují zástupy zlých stvùr. V Hornickém údolí.
	AI_Output (self, other, "DIA_Lord_Hagen_Armee_04_01"); //V Hornickém údolí? Vyslali jsme tam výpravu. Neustále nám docházejí zprávy o tom, že prùsmyk okupují skâeti.
	AI_Output (self, other, "DIA_Lord_Hagen_Armee_04_02"); //Ale o armádê zla se ke mnê ještê nic nedoneslo.
	if (Npc_KnowsInfo (other, DIA_Lord_Hagen_Frieden))
	{
		AI_Output (self, other, "DIA_Lord_Hagen_Armee_04_03"); //Není to jen nêjaký ubohý trik, který mê má pâesvêdèit o nezbytnosti spojit síly se žoldnéâi?
	...
	AI_Output (other, self, "DIA_Lord_Hagen_Armee_15_06"); //Armáda drakù, která kolem sebe shromáždila zástupy pâisluhovaèù.
	AI_Output (self, other, "DIA_Lord_Hagen_Armee_04_07"); //Draci? Podle starých spisù to už je nêkolik století, co nêkdo naposledy vidêl draka.
	AI_Output (self, other, "DIA_Lord_Hagen_Armee_04_08"); //Povêz - proè bych mêl vêâit jedinému tvému slovu?
	AI_Output (other, self, "DIA_Lord_Hagen_Armee_15_09"); //Jenomže otázka není, jestli mi mùžeš vêâit. Otázka je, jestli si mùžeš dovolit mi NEVÊÂIT v pâípadê, že mluvím pravdu.
...
INSTANCE DIA_Lord_Hagen_Proof (C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 =  "Takže chceš, abych ti pâinesl nêjaký dùkaz?";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Proof_15_00"); //Takže chceš, abych ti pâinesl nêjaký dùkaz?

	IF (hero.guild != GIL_NONE)
	&& (hero.guild != GIL_NOV)
	{
		AI_Output (self, other, "DIA_Lord_Hagen_Proof_04_01"); //Pâesnê tak. Projdi prùsmykem a dostaà se do Hornického údolí. Až tam budeš, najdi mou výpravu - a až se ti to podaâí, promluv si s velitelem Garondem.
		AI_Output (self, other, "DIA_Lord_Hagen_Proof_04_02"); //Pokud nêkdo ví, jaká tam panuje situace, tak je to on.
		...
		AI_Output (self, other, "DIA_Lord_Hagen_Proof_04_05"); //Innosovo oko... dobrá. Pâines mi dùkaz a já pak dohlédnu na to, abys smêl nosit ten amulet.
		AI_Output (other, self, "DIA_Lord_Hagen_Proof_15_06"); //Mám tedy v tomto ohledu tvoje slovo?
		AI_Output (self, other, "DIA_Lord_Hagen_Proof_04_07"); //Samozâejmê - máš ho mít.
...
func void DIA_Lord_Hagen_Auge_Info ()
{
	...
	AI_Output (self, other, "DIA_Lord_Hagen_Auge_04_01"); //Je to posvátný artefakt. (zadumanê) Ve starých proroctvích se âíká, že je nêjak spojen s draky.
	AI_Output (self, other, "DIA_Lord_Hagen_Auge_04_02"); //V têchto spisech je ale také zmínka o tom, že jej mùže nosit jen Innosùv vyvolený.
...
INSTANCE DIA_Lord_Hagen_Pass (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 =  "Jak se mám dostat pâes ten prùsmyk?";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Pass_15_00"); //Jak se mám dostat pâes ten prùsmyk?
	AI_Output (self, other, "DIA_Lord_Hagen_Pass_04_01"); //Dám ti klíè od vrat v prùsmyku. Budeš si ale muset najít zpùsob, jak proklouznout mezi vojskem skâetù.
	AI_Output (self, other, "DIA_Lord_Hagen_Pass_04_02"); //Nechã tê Innos ochraàuje.
	...
	B_LogEntry (Topic_MISOLDWORLD,"Lord Hagen chce, abych mu o armádách zla pâinesl nêjaký dùkaz. Musím se vydat do Hornického údolí a promluvit si tam s velitelem Garondem.");
...
{
	AI_Output	(other, self, "DIA_Addon_Lord_Hagen_GiveOrnament_15_00"); //Hledám kovový ornament. Mêl by být nêkde u kamenného kruhu u Lobartova statku.
	AI_Output	(self, other, "DIA_Addon_Lord_Hagen_GiveOrnament_04_01"); //Jestli myslíš tohleto, tak to jsme považovali za magickou runu, ale pak jsme zjistili, že je to úplnê k nièemu.
	AI_Output	(self, other, "DIA_Addon_Lord_Hagen_GiveOrnament_04_02"); //Klidnê si to vem, mnê to k nièemu není.
...
INSTANCE DIA_Lord_Hagen_Khorinis (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 =  "Proè jste pâišli do Khorinidu?";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Khorinis_15_00"); //Proè jste pâišli do Khorinidu?
	AI_Output (self, other, "DIA_Lord_Hagen_Khorinis_04_01"); //Plníme dùležitý úkol ve prospêch království. Naše rozkazy pocházejí pâímo od krále Rhobara.
...
INSTANCE DIA_Lord_Hagen_Minental (C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 =  "Co dêlají vaši muži v Hornickém údolí?";
};
...
{
	AI_Output (other, self, "DIA_Lord_Hagen_Minental_15_00"); //Co dêlají vaši muži v Hornickém údolí?

	if (Hagen_BringProof == FALSE)
	{
		AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_01"); //Nevidím dùvod, proè bych ti mêl nêco takového âíkat!
	}
	else
	{
		if (Garond.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_02"); //Byl jsi tam. Mêl bys to vêdêt.
		}
		else
		{
			AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_03"); //Dobrá, protože tam máš tak jako tak namíâeno, tak ti to mùžu âíct.
		};
		AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_04"); //Jde o magickou rudu. Mùže rozhodnout o výsledku války.
		AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_05"); //Bez dostatku zbraní ukutých z magické rudy nemá královská armáda ani tu nejmenší šanci obstát v boji proti elitním skâetím váleèníkùm.
		if (other.guild != GIL_SLD)
		{
			AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_06"); //A doly tady na ostrovê jsou poslední, ke kterým ještê máme pâístup.
		};
		AI_Output (self, other, "DIA_Lord_Hagen_Minental_04_07"); //Jakmile naši loë plnê naložíme rudou, vrátíme se zpátky na pevninu.
...
func void DIA_Hagen_CanTeach_Info ()
{
	...
	AI_Output (self, other, "DIA_Hagen_CanTeach_04_01"); //Vážnê? Jednoho jsi právê našel.
	
	LordHagen_Teach2H = TRUE;
	B_LogEntry (TOPIC_CityTeacher,"Lord Hagen mê zasvêtí do tajù boje s obouruèními zbranêmi.");
};
//**************************************
//			Ich will trainieren
//**************************************
INSTANCE DIA_Hagen_Teach(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Tak zaènême (osvojit si dovednost obouruèního boje).";
};                       
//----------------------------------
...
{	
	AI_Output (other,self ,"DIA_Hagen_Teach_15_00"); //Zaènême.
...
FUNC VOID DIA_Hagen_Teach_Back ()
{
	...
	{
		AI_Output (self,other,"DIA_Hagen_Teach_04_00"); //Teë jsi sám dobrým mistrem meèe. Už tê nemohu dál uèit.
		AI_Output (self,other,"DIA_Hagen_Teach_04_01"); //Nechã tvé další skutky provází moudrost mistra meèe.
...
INSTANCE DIA_Lord_Hagen_Knight		(C_INFO)
{
	...
	permanent	 = 	TRUE; 
	description	 = 	"Chtêl bych vstoupit do služby âádu.";
};
...
{
	AI_Output			(other, self, "DIA_Lord_Hagen_Knight_15_00"); //Chtêl bych vstoupit do služby âádu.
	...
	if (MIS_RescueBennet == LOG_SUCCESS)
	{
		...
		AI_Output			(self ,other, "DIA_Lord_Hagen_Knight_04_03"); //Je-li to tvé pâání, vítám tê v našem âádu.
		
		Info_ClearChoices (DIA_Lord_Hagen_Knight);
		Info_AddChoice (DIA_Lord_Hagen_Knight,"Ještê si nejsem tak úplnê jistý.",DIA_Lord_Hagen_Knight_No);
		Info_AddChoice (DIA_Lord_Hagen_Knight,"Jsem pâipraven!",DIA_Lord_Hagen_Knight_Yes);
	}
	else
	{
		AI_Output			(self ,other, "DIA_Lord_Hagen_Knight_04_04"); //Být Innosovým váleèníkem znamená zasvêtit všechny své èiny Innosovým zámêrùm.
		AI_Output			(self ,other, "DIA_Lord_Hagen_Knight_04_05"); //Do âádu jsou pâijati jen nejèestnêjší a nejstateènêjší váleèníci.
		AI_Output			(self ,other, "DIA_Lord_Hagen_Knight_04_06"); //Pokud jsi pevnê rozhodnut stát se paladinem, musíš nejprve prokázat, že jsi toho hoden.
	};
...
{
	AI_Output (other,self ,"DIA_Lord_Hagen_Knight_No_15_00"); //Ještê si nejsem tak úplnê jistý.
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_No_04_01"); //Tak jdi a oèisti své srdce od pochybností. Vraã se, až budeš pâipraven.
...
FUNC VOID DIA_Lord_Hagen_Knight_Yes()
{
	...
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_01"); //(vážnê) Tak budiž dle tvé vùle!
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_02"); //(vážnê) Mnoho mužù se vydalo touto cestou a položili své životy ve jménu Innose.
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_03"); //(vážnê) Pâísaháš, že tvé skutky budou dêlat jejich smrti èest a budou hlásat slávu Innosovu?
	AI_Output (other,self ,"DIA_Lord_Hagen_Knight_Yes_15_04"); //Pâísahám!
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_05"); //Od této chvíle tê tedy prohlašuji èlenem našeho spoleèenstva.
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_06"); //Zároveà tê ustanovuji Innosovým váleèníkem.
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_07"); //Dávám ti zbranê a zbroj rytíâe. Nos je s hrdostí, rytíâi!
	...
	AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_08"); //Dále, na základê tvé hodnosti, ti dovoluji pâístup do kláštera.

	if ((Npc_IsDead(Albrecht))== FALSE)
	{
		AI_Output (self ,other,"DIA_Lord_Hagen_Knight_Yes_04_09"); //Albrecht tê nauèí naší magii, staèí, když za ním zajdeš a promluvíš si s ním.
	};
	
	AI_Output (self ,other,"DIA_Lord_Hagen_Add_04_02"); //A jsou ti samozâejmê otevâeny naše ubikace na horním konci mêsta.
...
FUNC VOID DIA_Lord_Hagen_WhatProof_Info ()
{
	...
	AI_Output			(self ,other, "DIA_Lord_Hagen_WhatProof_04_03"); //Bojujeme proti Beliarovi a jeho pohùnkùm, kteâí chtêjí znièit Innosùv âád.
	...
	AI_Output			(self ,other, "DIA_Lord_Hagen_WhatProof_04_06"); //Každý paladin, jenž se spravedlivê bije, šíâí dál slávu Innose a mnozí z nás již položili život na oltáâ vêèného boje dobra a zla.
...
{	
	...
	if (MIS_OLDWORLD == LOG_SUCCESS)
	{
		...
		AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_05"); //Musíme s têmi draky nêco udêlat.
		if (Hagen_KnowsEyeKaputt == FALSE)
		{
			AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_06"); //(k sobê) Možná by nás teë mohlo zachránit Innosovo oko.
		};
	}
	else
	{
		AI_Output (self ,other,"DIA_Lord_Hagen_KAP3U4_PERM_04_01"); //Asi jsem pâišel o rozum. Jsem voják, ne byrokrat.
		AI_Output (self ,other,"DIA_Lord_Hagen_KAP3U4_PERM_04_02"); //S tímhle vším papírováním, které mám na krku, si sotva dokážu vzpomenout, jaké to je tâímat meè.
	};
};
...
{	
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_08"); //COŽE? U Innose! Cos to udêlal? To Oko potâebujeme!
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_09"); //Promluv si s Pyrokarem! Musí existovat nêjaký zpùsob, jak jej zase opravit.
...
func void DIA_Lord_Hagen_BACKINTOWN_Info ()
{
	...
	AI_Output (other, self, "DIA_Lord_Hagen_BACKINTOWN_15_02"); //Paladinové jsou obklíèeni na hradê v Hornickém údolí, všude kolem jsou skâeti.
	AI_Output (other, self, "DIA_Lord_Hagen_BACKINTOWN_15_03"); //Pâi dolování byly obrovské ztráty a skoro žádná ruda už nezbývá.
	AI_Output (other, self, "DIA_Lord_Hagen_BACKINTOWN_15_04"); //No, pokud mùžu âíct svùj názor, tak když têm chlapíkùm nikdo nepomùže, jsou ztraceni. Tak to alespoà vypadá.
	AI_Output (self, other, "DIA_Lord_Hagen_BACKINTOWN_04_05"); //Najdu zpùsob, jak tu výpravu zachránit. Udêlal jsi toho pro nás tolik. Innos se ti odvdêèí.
	AI_Output (other, self, "DIA_Lord_Hagen_BACKINTOWN_15_06"); //Nechci jeho vdêènost. Chci jeho oko.
	AI_Output (self, other, "DIA_Lord_Hagen_BACKINTOWN_04_07"); //Ano, samozâejmê. Plním své sliby. Vezmi si tenhle dopis. Otevâe ti brány kláštera.
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_00"); //A ještê nêco, než pùjdeš.
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_01"); //Na dùkaz mé vdêènosti si vezmi tuhle runu. Pâenese tê bezpeènê zpátky do mêsta, kdykoliv si zamaneš.
	...
	B_LogEntry (TOPIC_INNOSEYE,"Lord Hagen mi pâedal zprávu, která pâimêje mistra Pyrokara, aby mi v klášteâe vydal Innosovo oko.");
...
FUNC VOID DIA_Lord_Hagen_RescueBennet_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_04_01"); //Ale to je pâece ten žoldák, co zavraždil jednoho z mých mužù.
	...
	Info_AddChoice (DIA_Lord_Hagen_RescueBennet,"Jak si mùžeš být tak jistý, že je vrahem právê Bennet?",DIA_Lord_Hagen_RescueBennet_WhySure);
	/*
	...
	*/
	Info_AddChoice (DIA_Lord_Hagen_RescueBennet,"Vêâím, že je Bennet nevinný.",DIA_Lord_Hagen_RescueBennet_Innoscent);
...
{
	AI_Output			(other, self, "DIA_Lord_Hagen_RescueBennet_WhySure_15_00"); //Jak si mùžeš být tak jistý, že je vrahem právê Bennet?
	AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_WhySure_04_01"); //Máme svêdka.
	AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_WhySure_04_02"); //Jak vidíš, není o vinê toho žoldáka žádných pochyb.
	//neu zusammengefasst M.F.
	AI_Output			(other, self, "DIA_Lord_Hagen_RescueBennet_Witness_15_00"); //Kdo je tím svêdkem?
	AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_Witness_04_01"); //Cornelius, tajemník místodržícího. On tu vraždu vidêl.
	AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_Witness_04_02"); //Jeho popis padne bez jakýchkoliv pochyb na Benneta. Co se mê týèe, vêc je vyâízena.
	AI_Output			(self, other, "DIA_Lord_Hagen_RescueBennet_Witness_04_03"); //Ten žoldák bude viset za velezradu.

	B_LogEntry (TOPIC_RESCUEBENNET,"Svêdkem je tajemník místodržícího, Cornelius. Tvrdí, že celou vraždu vidêl."); 
...
{
	AI_Output			(other, self, "DIA_Lord_Hagen_RescueBennet_Innoscent_15_00"); //Vêâím, že Bennet je nevinný.
	...
	AI_Output			(self ,other, "DIA_Lord_Hagen_RescueBennet_Innoscent_04_03"); //Dávej pozor, co âíkáš. Vznášíš vážné obvinêní.
	AI_Output			(self ,other, "DIA_Lord_Hagen_RescueBennet_Innoscent_04_04"); //Pokud mi nemùžeš pâedložit dùkazy o tom, že svêdek lhal, radêji mlè.
};
...
FUNC VOID DIA_Lord_Hagen_Cornelius_Info ()
{
	...
	AI_Output			(other,self , "DIA_Lord_Hagen_Cornelius_15_02"); //Mám jeho deník. Je v nêm všechno.
	AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_03"); //(zuâivê) Ten slizký bastard!
	AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_04"); //Pod vahou nových dùkazù mi nic jiného nezbývá.
	AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_05"); //Z moci úâadu svêâeného mi králem a církví prohlašuji...
	AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_06"); //...že vêzeà Bennet je zbaven všech obvinêní a tudíž svobodný èlovêk.
	...
	if (Npc_IsDead (Cornelius) == TRUE)
	{
		...
		AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_09"); //V tom pâípadê se mu již dostalo spravedlivého trestu. Dobrá práce.
	}
	else if (CorneliusFlee == TRUE)
	{
		AI_Output			(other,self , "DIA_Lord_Hagen_Cornelius_15_10"); //Ukrývá se.
		AI_Output			(self ,other, "DIA_Lord_Hagen_Cornelius_04_11"); //Dâíve nebo pozdêji se objeví. Pak ho zatkneme.
...
FUNC VOID DIA_Lord_Hagen_AugeAmStart_Info ()
{
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_11"); //(uctivê) Neseš Oko!
	if (Hagen_KnowsEyeKaputt == TRUE)
	{
		AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_12"); //A dal jsi jej zpêt dohromady!
	};
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_13"); //V tom pâípadê jsi Innosùv vyvolený!
	AI_Output (other, self, "DIA_Lord_Hagen_Add_15_14"); //Vyrazím na cestu a zabiju všechny draky v Hornickém údolí!
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_15"); //Nechã tê Innos doprovází a svou mocí znièí zlo!
};
...
instance DIA_Lord_Hagen_ANTIPALADINE(C_INFO)
{
	...
	description	 = 	"Na zemi útoèí elitní skâetí váleèníci.";
};
...
{
	AI_Output			(other, self, "DIA_Lord_Hagen_ANTIPALADINE_15_00"); //Na zemi útoèí elitní skâetí váleèníci.

	Log_CreateTopic (TOPIC_OrcElite, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_OrcElite, LOG_RUNNING);
	B_LogEntry (TOPIC_OrcElite,"Zpravil jsem lorda Hagena o postupujících tlupách skâetích nájezdníkù."); 

	if (TalkedTo_AntiPaladin == TRUE)
	&& (MIS_KillOrkOberst == 0)
		{
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_01"); //Co tê k tomu vede?
			AI_Output			(other, self, "DIA_Lord_Hagen_ANTIPALADINE_15_02"); //Mluvil jsem s jedním z nich. Padlo tvoje jméno.
		};

		AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_03"); //Nesmysl. Moji lidé zatím žádnou masivní invazi skâetù nehlásili.
		AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_04"); //Možná se v nedalekých lesích ztratil nêkterý z jejich zvêdù.

	if (Npc_HasItems (other,ItRi_OrcEliteRing))
		{
			AI_Output			(other, self, "DIA_Lord_Hagen_ANTIPALADINE_15_05"); //Nebyli to žádní zvêdové. Jednomu z nich jsem vzal tento prsten.
			...
			AI_Output			(other, self, "DIA_Lord_Hagen_ANTIPALADINE_15_09"); //Zatím jsem jich moc nevidêl. Hlavnê jejich velitele a jen pár bojovníkù.
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_10"); //Vážnê? V tom pâípadê musejí mít za lubem nêco jiného. To mi ke skâetùm moc nesedne, že by jejich vùdci sami opustili své ochranné palisády.
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_11"); //Mohla by to však být ideální pâíležitost, jak jim uštêdâit citelný úder.
			...
			B_LogEntry (TOPIC_OrcElite,"Podaâilo se mi pâinést lordu Hagenovi dùkaz - prsten skâetího vojevùdce. Požádal mê, abych mu pâinesl všechny, které najdu."); 
	
			if (Npc_IsDead(Ingmar)==FALSE)
			&& (MIS_KillOrkOberst == 0)
			{
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_15"); //Poraë se s Ingmarem. Mùže ti prozradit pár taktik, které by se ti pâi bojích se skâetími veliteli mohly hodit.
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_16"); //Skâetí elitní bojovníci jsou jeho specialita. Mêl s nimi èasto co do èinêní.
			B_LogEntry (TOPIC_OrcElite,"Elitní skâetí váleèníci jsou Ingmarovou specialitou."); 
			};
		...
		else
		{
			...
			{
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_17"); //Tvoje pouhé tvrzení, že jsi zabil jednoho ze skâetích velitelù, mi nestaèí.
			};
			AI_Output			(self, other, "DIA_Lord_Hagen_ANTIPALADINE_04_18"); //Pokud to nemám brát na lehkou váhu, potâebuji nêjaký hmatatelnêjší dùkaz.
		
			B_LogEntry (TOPIC_OrcElite,"Hagen mi poâád nechce vêâit. Žádá dùkaz, že elitní váleèníci skuteènê útoèí na civilizované kraje. No, spíš by mê pâekvapilo, kdyby to bylo naopak."); 
		};
};
...
instance DIA_Lord_Hagen_RINGEBRINGEN		(C_INFO)
{
	...
	description	 = 	"Je tady ještê jedna vêc, kterou musím ohlednê têch skâetích velitelù nahlásit.";
};
...
{
	AI_Output			(other, self, "DIA_Lord_Hagen_RINGEBRINGEN_15_00"); //Je tady ještê jedna vêc, kterou musím ohlednê têch skâetích velitelù nahlásit.
	...
	{
		AI_Output			(self, other, "DIA_Lord_Hagen_RINGEBRINGEN_04_05"); //Možná tam ještê nêkde nêjaký je.
	}
	...
	{
		AI_Output			(self, other, "DIA_Lord_Hagen_RINGEBRINGEN_04_07"); //Vážnê by mê pâekvapilo, kdyby se jich tu potulovalo o mnoho víc.
		AI_Output			(self, other, "DIA_Lord_Hagen_RINGEBRINGEN_04_08"); //Jejich prsteny mi samozâejmê mùžeš nosit i nadále, ale myslím, že skâeti už náš vzkaz pochopili.
		TOPIC_END_OrcElite = TRUE;
	};

	AI_Output			(self, other, "DIA_Lord_Hagen_RINGEBRINGEN_04_09"); //Poèkej. Tady máš nêjaké zlato na své vybavení.
...
{	
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_19"); //Vêdêl jsem, že ti Innos dá sílu porazit draky!
	AI_Output (self ,other,"DIA_Lord_Hagen_AllDragonsDead_04_02"); //Kde je ruda?
	AI_Output (other,self ,"DIA_Lord_Hagen_AllDragonsDead_15_03"); //Skâeti ještê poâád obléhají hrad v Hornickém údolí. Garond nemá žádnou šanci opevnêní opustit, dokud obléhání neskonèí.
	
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_20"); //(zuâivê) Zatracenê!
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_22"); //Pár skâetù mê nezastaví! Mê ne!
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_25"); //To by mêlo staèit na to, abychom s têmi skâety koneènê zametli!
...
{	
	...
	{
		AI_Output (self ,other,"DIA_Lord_Hagen_NeedShip_04_01"); //To hodnê lidí, vojáku.
	}
	else if (hero.guild == GIL_KDF)
	{
		AI_Output (self ,other,"DIA_Lord_Hagen_NeedShip_04_02"); //(zasmêje se) To slyším skoro každý den, ctihodnosti. Ale...
	};

	AI_Output (self ,other,"DIA_Lord_Hagen_NeedShip_04_03"); //Nemáš dokonce ani kapitána, o posádce nemluvê.
	...
	AI_Output (self ,other,"DIA_Lord_Hagen_NeedShip_04_06"); //Ale až to bude za námi, mùžeš se mê zeptat znovu.
};
...
{	
	...
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_30"); //U Innose! Co pâesnê se tam stalo?
	AI_Output (other, self, "DIA_Lord_Hagen_Add_15_31"); //Nêjak se musela otevâít brána.
	AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_32"); //Nêjak?! Jak by to bylo možné... Na hradê musí být zrádce!
};
...
{	
	...
	{
		AI_Output (self ,other, "DIA_Lord_Hagen_Add_04_27"); //Teë, když jsou na hradê skâeti, potâebujeme proviantu ještê víc než pâedtím.
