...
func void DIA_Girion_DI_Hallo_Info ()
{
	...
	if (ORkSturmDI == TRUE)
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_01"); //Skâeti jsou opravdu stateèní, že se vydali proti mnê.
		...
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_03"); //Âekl bych. Zabere ti tvùj úkol tady na útesu hodnê èasu?
		}
	else
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_04"); //Nechovej se ke mnê jako k idiotovi. Udêlej, co musíš, a pâedej mi velení lodi.
		};
};
...
INSTANCE DIA_Girion_DI_Teach(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Když mê vycvièíš, budeme odsud moci rychleji vypadnout!";
};                       
...
{	
	AI_Output (other,self 	,"DIA_Girion_DI_Teach_15_00"); //Uè mê, pak se odsud dostaneme rychleji.
	AI_Output (self ,other 	,"DIA_Girion_DI_Teach_08_01"); //Zdá se, že nemám na výbêr.
...
func void DIA_Girion_DI_Teach_CROSSBOW_1()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_CROSSBOW_1_08_00"); //Nikdy moc nezvedej lokty. Budeš pak mít tuhou mušku a zákonitê zkroutíš zbraà.
	};
...
FUNC VOID DIA_Girion_DI_Teach_1H_1 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_1H_1_08_00"); //Ostâí musí letêt vêtrem jako rákos povêtâím.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_1H_5 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_1H_5_08_00"); //Svou sílu používej s rozmyslem. Když se bezhlavê rozbêhneš proti nepâíteli, moc úspêchù nesklidíš.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_2H_1 ()
{
	...
	{
		AI_Output(self,other,"DIA_DIA_Girion_DI_Teach_2H_1_08_00"); //Nemêj pâíliš pevné sevâení, abys mohl pâesnê trefit.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_2H_5 ()
{
	...
	{
		AI_Output(self,other,"DIA_Girion_DI_Teach_2H_5_08_00"); //Nezapomeà se bránit. Nejlepší obranou je vyhýbat se protivníkovým ranám.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
instance DIA_Girion_DI_OrcEliteRing		(C_INFO)
{
	...
	description	 = 	"Mám prsten skâetího vùdce.";
};
...
{
	AI_Output			(other, self, "DIA_Girion_DI_OrcEliteRing_15_00"); //Mám prsten skâetího vùdce.

	if (DIA_Girion_DI_OrcEliteRing_OneTime == FALSE)
	{
		AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_08_01"); //Myslím, že by s tím mohl lord Hagen nêco udêlat.
...
func void DIA_Girion_DI_OrcEliteRing_geben ()
{
	...
	AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_geben_08_01"); //Díky. Stejnê si nemyslím, že by ti mohl k nêèemu být.
	Info_ClearChoices	(DIA_Girion_DI_OrcEliteRing);
...
func void DIA_Girion_DI_OrcEliteRing_behalten ()
{
	...
	AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_behalten_08_01"); //Jak myslíš. Stejnê ti k nièemu nebude.
	Info_ClearChoices	(DIA_Girion_DI_OrcEliteRing);
};
...
instance DIA_Girion_DI_UndeadDragonDead		(C_INFO)
{
	...
	description = 	"Teë mùžeme koneènê vyrazit.";
};
...
{
	AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_00"); //Teë mùžeme koneènê vyrazit.
	AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_01"); //Bylo naèase. Už jsem si myslel, že tady na tomhle útesu zplesnivím.

	if (DIA_Girion_DI_UndeadDragonDead_OneTime == FALSE)
		{
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_02"); //Teë mi okamžitê tu loë pâedej.
			AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_03"); //Ještê nejsme v Khorinidu. A upâímnê âeèeno, zpátky do Khorinidu se mi moc nechce.
			AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_04"); //Už jsem tam byl dost dlouho.
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_05"); //Myslel jsem si to. Ale teë není ani správný èas, ani správné místo, abychom si nêco takového vyjasàovali. Probereme to, až budeme zpátky na lodi.
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_06"); //No a co. Stejnê jsem rád, že tê mám zpátky na palubê v jednom kuse.
			
			if (hero.guild == GIL_PAL)
			{
				AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_07"); //Jsi skuteèný Innosùv váleèník. Už o tom nemám žádných pochyb. Jsem rád, že tê máme na své stranê.
			}
			else
			{
				AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_08"); //I když jsi lump, zachoval ses nakonec správnê.
			};
			
			DIA_Girion_DI_UndeadDragonDead_OneTime = TRUE;
		};

	AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_09"); //Dobrá. Teë jdi za kapitánem, aã nás odsud vyvede.
	AI_StopProcessInfos (self);
};
