...
FUNC VOID DIA_Addon_Raven_Hi_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_01");//Mùj pán mê varoval, že sem Innos pošle svý poskoky.
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_02");//Jen jsem neèekal, že se tu objevíš tak brzo.
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_03");//Ale když už jsi to dokázal tak rychle (smêje se), budeš mít taky rychlou smrt.
...
FUNC VOID DIA_Addon_Raven_Hi_DU()
{
	...
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_04");//(smêje se) Nejsi pro mê žádná hrozba. Nosím Beliarùv meè.
...
{
	AI_Output (other, self, "DIA_Addon_Raven_Add_15_02");//Jen dêlám, co musí být vykonáno.
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_08");//(smêje se) Jen si ho poslechnête. Poslali jste mi pokornýho služebníka Innosova! (nahlas se smêje)
	
	Info_ClearChoices (DIA_Addon_Raven_Hi);
	Info_AddChoice (DIA_Addon_Raven_Hi,"Chápu. Tak pojëme na to.",DIA_Addon_Raven_Hi_Attack);
	Info_AddChoice (DIA_Addon_Raven_Hi,"Vážnê si myslíš, že sloužím Innosovi?",DIA_Addon_Raven_Hi_Sure);	
};
FUNC VOID DIA_Addon_Raven_Hi_Sure()
{
	AI_Output (other, self, "DIA_Addon_Raven_Add_15_03");//A víš tak jistê, že jsem zrovna Innosùv služebník? 
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_09");//Co? Co to povídáš?
	AI_Output (other, self, "DIA_Addon_Raven_Add_15_04");//Nezvážils nêkdy možnost, že bych mohl být tâebas i služebníkem Adana?
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_10");//(smêje se) Nesmysl!
	AI_Output (other, self, "DIA_Addon_Raven_Add_15_05");//A možná že TAKY sloužím Beliarovi – nebo jenom sám sobê. 
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_11");//(ušklíbá se) No, v tom pâípadê pro mê nejseš vhodným protivníkem.
...
FUNC VOID DIA_Addon_Raven_Hi_Attack()
{
	...
	AI_Output (self, other, "DIA_Addon_Raven_Add_10_12");//(ušklíbá se) To se vážnê tak têšíš na smrt? Ale když na tom trváš.
