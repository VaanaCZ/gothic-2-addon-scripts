...
{
	AI_Output (other,self ,"DIA_Torlof_DI_Hallo_15_00"); //Všechno v pořádku?

	if (ORkSturmDI == FALSE)
	{
		AI_Output (self ,other,"DIA_Torlof_DI_Hallo_01_01"); //Zatím. Ale to se může během okamžiku změnit. Dávej si pozor.
	}
	else
	{
		AI_Output (self ,other,"DIA_Torlof_DI_Hallo_01_02"); //Pokud ti mizerní skřeti zůstanou tam, kde jsou, tak netuším, v čem by mohl být problém.
		B_StartOtherRoutine	(Torlof_DI,"Začít");
	};
	AI_StopProcessInfos (self);
}; 
...
instance DIA_Torlof_DI_UndeadDragonDead		(C_INFO)
{
	...
	description = 	"Zneškodnil jsem nepřítele.";
...
{
	AI_Output			(other ,self, "DIA_Torlof_DI_UndeadDragonDead_15_00"); //Zneškodnil jsem nepřítele.
	AI_Output			(self ,other, "DIA_Torlof_DI_UndeadDragonDead_01_01"); //Nic jiného jsem ani nečekal. Jak to vypadá? Můžeme konečně odplout?

	Info_ClearChoices	(DIA_Torlof_DI_UndeadDragonDead);
	Info_AddChoice	(DIA_Torlof_DI_UndeadDragonDead, "Dej mi ještě pár minut.", DIA_Torlof_DI_UndeadDragonDead_moment );
	Info_AddChoice	(DIA_Torlof_DI_UndeadDragonDead, "Ano. Už je po všem - pojďme na to!", DIA_Torlof_DI_UndeadDragonDead_over );
	
};
func void DIA_Torlof_DI_UndeadDragonDead_moment ()
{
	AI_Output			(other, self, "DIA_Torlof_DI_UndeadDragonDead_moment_15_00"); //Dej mi ještě pár minut.
	AI_Output			(self, other, "DIA_Torlof_DI_UndeadDragonDead_moment_01_01"); //Pospěš si!
	AI_StopProcessInfos (self);
};
