...
func void DIA_Fellan_News_Info ()
{
	...
	{
		 AI_Output (self, other, "DIA_Fellan_News_06_00"); //Dobrá, tak jsi mê dostal. To byl ale hrdinský kousek.
	...
	{
		AI_Output (self, other, "DIA_Fellan_News_06_03");  //Co se dêje? Chceš další ránu pêstí?
	};
	if (self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_CANCEL)
	{
		 AI_Output (self, other, "DIA_Fellan_News_06_04"); //Co seš to za blázna? Bojuj, nebojuj, âekni - co vlastnê chceš?
	};
...
func void DIA_Fellan_HALLO_Info ()
{
	...
	AI_Output (self, other, "DIA_Fellan_HALLO_06_01"); //Nêkolik posledních dní se zoufale snažím zamezit tomu, aby mi zatýkalo do domu.
	AI_Output (self, other, "DIA_Fellan_HALLO_06_02"); //Krov se mi drobí pokaždý, když pâijde vêtší liják. Takže se mi všechno rozsejpá pod rukama!
	AI_StopProcessInfos (self);
};
...
instance DIA_Fellan_Stop		(C_INFO)
{
	...
	permanent	 =  FALSE;
	description	 = 	"Mùžeš s tím boucháním pâestat?";
};
...
{
	AI_Output (other, self, "DIA_Fellan_Stop_15_00"); //Mùžeš pâestat s tím boucháním?
	AI_Output (self, other, "DIA_Fellan_Stop_06_01"); //Ne, musím to dodêlat.
	
	Info_ClearChoices (DIA_Fellan_Stop);
	Info_AddChoice (DIA_Fellan_Stop,"Pâestaà s tím, nebo ti zpâerážím všechny kosti v têle.",DIA_Fellan_Stop_Bones);
	Info_AddChoice (DIA_Fellan_Stop,"Dám ti 10 zlatých, když s tím rámusem pâestaneš.",DIA_Fellan_Stop_Gold);
	Info_AddChoice (DIA_Fellan_Stop,"Prostê pâestaà bušit, jasný?",DIA_Fellan_Stop_Just);
};
FUNC VOID DIA_Fellan_Stop_Bones()
{
	AI_Output (other, self, "DIA_Fellan_Stop_Bones_15_00"); //Pâestaà s tím, nebo ti zpâerážím všechny kosti v têle.
	AI_Output (self, other, "DIA_Fellan_Stop_Bones_06_01"); //Zapomeà na to, nebo ti do tvý palice svym kladivem vtluèu alespoà trochu rozumu.
	AI_StopProcessInfos (self);
};
FUNC VOID DIA_Fellan_Stop_Gold()
{
	...
	AI_Output (self, other, "DIA_Fellan_Stop_Gold_06_02"); //...fajn, ty víš, co se s tim dá dêlat.
};
FUNC VOID DIA_Fellan_Stop_Just()
{
	AI_Output (other, self, "DIA_Fellan_Stop_Just_15_00"); //Prostê pâestaà bušit, fajn?
	AI_Output (self, other, "DIA_Fellan_Stop_Just_06_01"); //Prostê mê zkus neposlouchat, fajn?
};
///////////////////////////////////////////////////////////////////////
//	Info Alles klar?
///////////////////////////////////////////////////////////////////////
instance DIA_Fellan_klar		(C_INFO)
{
	...
	permanent	 =  TRUE;
	description	 = 	"Halóóó - jsi v pohodê?";
};
...
func void DIA_Fellan_klar_Info ()
{
	...
	if (FellanGeschlagen == FALSE) 
	{
		AI_Output (self, other, "DIA_Fellan_klar_06_01"); //Fajn, jestli se mi podaâí tohle všechno vèas opravit.
	}
	else
	{
		AI_Output (self, other, "DIA_Fellan_klar_06_02"); //Ta stâecha je èím dál tím horší. Až pâíštê zaprší, bude mi voda téct rovnou na hlavu. A za to všechno mùžeš ty!
	};
	AI_StopProcessInfos (self);
};
