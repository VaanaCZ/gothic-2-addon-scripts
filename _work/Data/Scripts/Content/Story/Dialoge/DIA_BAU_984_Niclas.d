...
{
	AI_Output (self ,other,"DIA_Niclas_Hello_03_00"); //Taky jsi na útěku?
...
INSTANCE DIA_Niclas_HaveALook   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Jen jsem se tu chtěl trochu porozhlédnout po okolí.";
};
...
{
	AI_Output (other,self ,"DIA_Niclas_HaveALook_15_00"); //Jen jsem se tu chtěl trochu porozhlédnout po okolí.
	...
	Info_AddChoice (DIA_Niclas_HaveALook,"S potěšením.",DIA_Niclas_HaveALook_Yes);
};

FUNC VOID DIA_Niclas_HaveALook_No ()
{
	AI_Output (other,self ,"DIA_Niclas_HaveALook_No_15_00"); //Ne, díky.
	AI_Output (self ,other,"DIA_Niclas_HaveALook_No_03_01"); //Jak je libo. Doufám, že ti nebude vadit, když si sám něco zakousnu.
...
{
	AI_Output (other,self ,"DIA_Niclas_HaveALook_Yes_15_00"); //S potěšením.
...
FUNC VOID DIA_Niclas_WhyHere_Info()
{
	...
	AI_Output (self ,other,"DIA_Niclas_WhyHere_03_01"); //Nalezl jsem tu klid. Ve městě jsem se vždycky cítil tak stísněný.
};
...
INSTANCE DIA_Niclas_ShouldntWork   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Neměl bys pracovat?";
};
...
{
	AI_Output (other,self ,"DIA_Niclas_ShouldntWork_15_00"); //Neměl bys pracovat?
	AI_Output (self ,other,"DIA_Niclas_ShouldntWork_03_01"); //Všechno, co potřebuju, si můžu udělat sám. Vyrábím si šípy a příroda mi poskytuje maso.
	AI_Output (other,self ,"DIA_Niclas_ShouldntWork_15_02"); //Takže jsi lovec.
	AI_Output (self ,other,"DIA_Niclas_ShouldntWork_03_03"); //Ano, pokud tomu tak chceš říkat. Ale lovím jen pro vlastní potřebu. Nelovím pro peníze.
};
...
INSTANCE DIA_Niclas_CanTeachMe   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "V tom případě bys mě mohl něco naučit.";
};
...
{
	AI_Output (other,self ,"DIA_Niclas_CanTeachMe_15_00"); //V tom případě bys mě mohl něco naučit.
	AI_Output (self ,other,"DIA_Niclas_CanTeachMe_03_01"); //Proč ne. Můžu ti ukázat, jak se zachází s lukem.
	
	Log_CreateTopic	(TOPIC_Teacher, LOG_NOTE);
	B_LogEntry		(TOPIC_Teacher, "Niclas mi může předvést, jak se zdokonalit v lukostřelbě.");
};
...
FUNC VOID DIA_Niclas_Teach_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Niclas_Teach_03_01"); //Už jsem tě naučil všemu, co znám. Měl by sis najít jiného učitele.
	}
	else
	{
...
FUNC VOID DIA_Niclas_Teach_Back()
{
	if (other.HitChance[NPC_TALENT_BOW] >= 60)
	{
		AI_Output(self,other,"DIA_Niclas_Teach_03_00"); //Už tě nemůžu naučit ničemu dalšímu.
	};
