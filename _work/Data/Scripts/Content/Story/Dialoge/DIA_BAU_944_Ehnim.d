...
func void DIA_Ehnim_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Ehnim_HALLO_12_03"); //Už tady na farmê pracuju pro Akila nêkolik let.
...
func void DIA_Ehnim_FELDARBEIT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Ehnim_FELDARBEIT_12_02"); //Ale mêl by ses mít na pozoru pâed polními škùdci. Utrhnou ti ruku, ani nemrkneš.
...
instance DIA_Ehnim_FELDRAEUBER		(C_INFO)
{
	...
	description	 = 	"Proè s têmi škùdci nêco neudêláte?";
};
...
{
	AI_Output			(other, self, "DIA_Ehnim_FELDRAEUBER_15_00"); //Proè s têmi škùdci nêco neudêláte?
	AI_Output			(self, other, "DIA_Ehnim_FELDRAEUBER_12_01"); //Zabil už jsem jich víc, než dokážu spoèítat. Jedinej problém je, že zase pâijdou další.
...
instance DIA_Ehnim_STREIT1		(C_INFO)
{
	...
	description	 = 	"Tvùj bratr mi âíkal to samé.";
};
...
{
	AI_Output			(other, self, "DIA_Ehnim_STREIT1_15_00"); //Tvùj bratr mi âíkal to samé.
	...
	AI_Output			(self, other, "DIA_Ehnim_STREIT1_12_02"); //Nemêl by âíkat takový nesmysly.
};
...
instance DIA_Ehnim_STREIT3		(C_INFO)
{
	...
	description	 = 	"Tvùj bratr si myslí, že se akorát vytahuješ.";
};
...
{
	AI_Output			(other, self, "DIA_Ehnim_STREIT3_15_00"); //Tvùj bratr si myslí, že se akorát vytahuješ.
	AI_Output			(self, other, "DIA_Ehnim_STREIT3_12_01"); //Co? To má vážnê odvahu tohle âíct?
	AI_Output			(self, other, "DIA_Ehnim_STREIT3_12_02"); //Radêj by si mêl dávat pozor, než mu uštêdâím poâádnou lekci.
	AI_Output			(self, other, "DIA_Ehnim_STREIT3_12_03"); //Tak mu to bêž âíct.
	AI_StopProcessInfos (self);
};
...
instance DIA_Ehnim_STREIT5		(C_INFO)
{
	...
	description	 = 	"Mám dojem, že byste se mêli oba trochu zklidnit.";
};
...
{
	AI_Output			(other, self, "DIA_Ehnim_STREIT5_15_00"); //Mám dojem, že byste se mêli oba trochu zklidnit.
	AI_Output			(self, other, "DIA_Ehnim_STREIT5_12_01"); //Ten bastard to ještê nevzdal, co?
	AI_Output			(self, other, "DIA_Ehnim_STREIT5_12_02"); //Já ho roztrhnu. Âekni mu to.

	Info_ClearChoices	(DIA_Ehnim_STREIT5);

	Info_AddChoice	(DIA_Ehnim_STREIT5, "Dêlej si, co chceš. Odcházím.", DIA_Ehnim_STREIT5_gehen );
	Info_AddChoice	(DIA_Ehnim_STREIT5, "Proè mu to neâekneš sám?", DIA_Ehnim_STREIT5_Attack );


};
func void DIA_Ehnim_STREIT5_Attack ()
{
	AI_Output			(other, self, "DIA_Ehnim_STREIT5_Attack_15_00"); //Proè mu to neâekneš sám?
	AI_Output			(self, other, "DIA_Ehnim_STREIT5_Attack_12_01"); //To pâesnê udêlám.
...
{
	AI_Output			(other, self, "DIA_Ehnim_STREIT5_gehen_15_00"); //Dêlej si, co chceš. Odcházím.
	AI_Output			(self ,other, "DIA_Ehnim_STREIT5_gehen_12_01"); //Jo, jen se rychle ztraã.
...
{
	AI_Output			(self, other, "DIA_Ehnim_PERMKAP1_12_00"); //Chceš dêlat další problémy? Mám dojem, že bude lepší, když se hned ztratíš.
...
{
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_12_00"); //Ty jsi tu JEŠTÊ.
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_15_01"); //Vypadá to tak. Poâád vytoèenej?
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_12_02"); //Nic se nedêje, zapomeà na to. Âekni, byl jsi poslední dobou na Lobartovê farmê?
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_15_03"); //Možná. Proè?
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_12_04"); //Ále, nic dùležitýho. Jen jsem chtêl mluvit s Vinem o jeho palírnê.
	
	Info_ClearChoices	(DIA_Ehnim_MoleRatFett);
	Info_AddChoice	(DIA_Ehnim_MoleRatFett, "Právê teë nemám èas.", DIA_Ehnim_MoleRatFett_nein );
...
func void DIA_Ehnim_MoleRatFett_tot ()
{
	...
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_tot_12_01"); //Proboha. No nic. Tak to se nedá nic dêlat.

};

func void DIA_Ehnim_MoleRatFett_was ()
{
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_was_15_00"); //O palírnê? O jaké palírnê?
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_12_01"); //Eh. Asi jsem to nemêl âíkat. Vino byl na to své malé tajemství vždycky hodnê citlivý.
	...
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_12_03"); //Nedávno mê žádal o nêco, èím by mohl promazat padací mâíž.
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_12_04"); //V poslední dobê hodnê pršelo a zaèala ho zlobit rez. Teë je naviják zaseknutý a nikdo už se tam nedostane. To jsme v pêkný bryndê.

	Log_CreateTopic (TOPIC_FoundVinosKellerei, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_FoundVinosKellerei, LOG_RUNNING);
	B_LogEntry (TOPIC_FoundVinosKellerei,"Podle Ehnima se Vino stále ukrývá v lesích poblíž Akilova statku. Ale mechanismus dveâí je zadâený a dokud jej nenamažu rysèím sádlem, dovnitâ se nedostanu."); 
	
	Info_AddChoice	(DIA_Ehnim_MoleRatFett, "A? Máš nêjaký mazivo?", DIA_Ehnim_MoleRatFett_was_Fett );
};
func void DIA_Ehnim_MoleRatFett_was_Fett ()
{
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_was_Fett_15_00"); //A? Máš nêjaký mazivo?
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_12_01"); //Jo, jasnê. Nejlepší, co se tu dá sehnat. Rysèí sádlo. Pâíšerná vêc, to ti povím. Taky se používá na promazání lodních dêl.
...
func void DIA_Ehnim_MoleRatFett_was_Fett_habenwill ()
{
	...
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_habenwill_12_01"); //To nebude levný, kámo. V tomhle kraji to je zatracenê vzácná vêc.
	...
	if (B_GiveInvItems (other, self, ItMi_Gold,Ehnim_MoleRatFettOffer))
		{
			...
			if (Npc_HasItems (self,ItMi_Moleratlubric_MIS))
			{
				...
				if (Npc_IsDead(Vino) == FALSE)
					{
						AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_habenwill_ja_12_02"); //(pro sebe) Zatracenê. Vino mê zabije.
					};					
			}
			else
			{
				AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_habenwill_ja_12_03"); //Sakra, kam jsme se to dostali? To je zatracenê mrzutá vêc. Tak fajn, promià. Jak se zdá, už to nemám. Vem si svý prachy zpátky.
				B_GiveInvItems (self, other, ItMi_Gold,Ehnim_MoleRatFettOffer);
				
				if (Npc_IsDead(Egill) == FALSE)
					{
						AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_habenwill_ja_12_04"); //Vsadím se, že to mùj brácha udêlá znovu. Ten bastard.
						AI_StopProcessInfos (self);
						other.aivar[AIV_INVINCIBLE] = FALSE;
						B_Attack (self, Egill, AR_NONE, 0);
					};
		...
		{
			AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_Fett_habenwill_ja_12_05"); //No to je bezvadný. Nejdâív chceš udêlat velkej kšeft a pak nemáš dost prachù. Ztraã se.
		};
	AI_StopProcessInfos (self);
};
...
func void DIA_Ehnim_MoleRatFett_was_Fett_habenwill_zuviel ()
{
	...
	Info_ClearChoices	(DIA_Ehnim_MoleRatFett);
	Info_AddChoice	(DIA_Ehnim_MoleRatFett, "To je ještê poâád moc.", DIA_Ehnim_MoleRatFett_was_Fett_habenwill_zuviel_immernoch );
	Info_AddChoice	(DIA_Ehnim_MoleRatFett, "Dohodnuto.", DIA_Ehnim_MoleRatFett_was_Fett_habenwill_ja );

};
func void DIA_Ehnim_MoleRatFett_was_Fett_habenwill_zuviel_immernoch ()
{
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_was_immernoch_15_00"); //To je ještê poâád moc.
	AI_Output			(self, other, "DIA_Ehnim_MoleRatFett_was_immernoch_12_01"); //(naštvanê) Tak si trhni. Mêj se.
	AI_StopProcessInfos (self);
};

func void DIA_Ehnim_MoleRatFett_nein ()
{
	AI_Output			(other, self, "DIA_Ehnim_MoleRatFett_nein_15_00"); //Právê teë nemám èas.
...
{
	AI_Output			(self, other, "DIA_Ehnim_PERMKAP3_12_00"); //Teë nemám èas.
	AI_StopProcessInfos (self);
};
