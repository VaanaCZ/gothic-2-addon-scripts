...
func void DIA_Oric_HALLO_Info ()
{
	...
	AI_Output (self, other, "DIA_Oric_HALLO_11_01"); //Jsem královský dùstojník pro strategické záležitosti a momentálnê podléhám velení ctihodného velitele Garonda.
	AI_Output (self, other, "DIA_Oric_HALLO_11_02"); //Jsme poslední paladinové tady na hradê. Všichni ostatní odešli nebo padli.
	AI_Output (self, other, "DIA_Oric_HALLO_11_03"); //Na našich bedrech teë spoèívá úspêch celé výpravy. A my se o to postaráme. U Innose - postaráme se o to.
...
func void DIA_Oric_Bruder_Info ()
{
	...
	AI_Output (other, self, "DIA_Oric_Bruder_15_02"); //Tvùj bratr zemâel. Dostal se až do prùsmyku.
	AI_Output (self, other, "DIA_Oric_Bruder_11_03"); //(šeptá)...mùj bratr
	AI_Output (self, other, "DIA_Oric_Bruder_11_04"); //Innos mê krutê zkouší. Ale zemâel jako jeho služebník...
...
FUNC VOID DIA_Oric_ScoutMine_Info()
{
	...
	AI_Output (self ,other,"DIA_Oric_ScoutMine_11_01"); //Dej si pozor, nebude to žádná hraèka. Nejdâíve ze všeho vyhledej paladiny. Oni vedou ty tâi skupiny.
	AI_Output (self ,other,"DIA_Oric_ScoutMine_11_02"); //Pokud bys o tom chtêl vêdêt víc, promluv si s Parcivalem.
};	
///////////////////////////////////////////////////////////////////////
//	Info Perm
///////////////////////////////////////////////////////////////////////
...
func void DIA_Oric_Perm_Info ()
{
	...
	if (MIS_ScoutMine == LOG_RUNNING)
	{
		AI_Output (self, other, "DIA_Oric_Perm_11_01"); //Musíš najít všechny horníky a navíc i jejich rudu. Potom zkusíme pâijít na zpùsob, jak tohle údolí opustit.
	}
	else if (MIS_ScoutMine == LOG_SUCCESS)
 	{
		AI_Output (self, other, "DIA_Oric_Perm_11_02"); //Nadêje výpravy klesly na úplné dno. Ale nevzdáme to, protože Innos je s námi. Vyvede nás bezpeènê z údolí.
	}
	else //noch nicht angenommen
	{
		AI_Output (self, other, "DIA_Oric_Perm_11_03"); //Tvùj pâíchod je pro nás znamením nadêje.
		AI_Output (self, other, "DIA_Oric_Perm_11_04"); //Potâebujeme tê. Bêž si promluvit s Garondem - on ti všechno vysvêtlí.
	};
};
...
{
	AI_Output (other,self ,"DIA_Oric_IAmBack_15_00"); //Už jsem zpêt.
	AI_Output (self ,other,"DIA_Oric_IAmBack_11_01"); //Nikdy bych neâekl, že tê tu ještê uvidím - ne potom, èeho jsem byl svêdkem, když jsi tu byl pâed nêkolika dny.
	
	if (hero.guild == GIL_PAL)
	{
		AI_Output (self, other, "DIA_Oric_IAmBack_11_02"); //Vítám tê jménem našeho âádu.
	}
	...
	AI_Output (self ,other,"DIA_Oric_IAmBack_11_04"); //Kéž by byl tvùj pâíchod dobrým znamením.
};
...
{
	AI_Output (other,self ,"DIA_Oric_CanHelp_15_00"); //Mohu nêjak pomoci?
	AI_Output (self ,other,"DIA_Oric_CanHelp_11_01"); //Momentálnê se zdá, že jsme udêlali všechno, co bylo v našich silách.
	AI_Output (self ,other,"DIA_Oric_CanHelp_11_02"); //Ale pâesto je tady jedna vêc. Nêco velice dùležitého, co bys pro nás mohl udêlat vnê têchto zdí.
	AI_Output (self ,other,"DIA_Oric_CanHelp_11_03"); //Chceme useknout hadovi jeho hlavu.
	
	Info_ClearChoices (DIA_Oric_CanHelp);
	Info_AddChoice (DIA_Oric_CanHelp,"Myslím, že se budeš muset poohlédnout po nêkom jiném.",DIA_Oric_CanHelp_NotYourMan); 
...
{
	AI_Output (other,self ,"DIA_Oric_CanHelp_NotYourMan_15_00"); //Myslím, že byste si mêli najít nêkoho jiného.
	AI_Output (self ,other,"DIA_Oric_CanHelp_NotYourMan_11_01"); //Nemùžu postrádat jediného ze svých lidí - jsi naše jediná nadêje.
};
...
FUNC VOID DIA_Oric_CanHelp_WhatYouMean ()
{
	...
	AI_Output (self ,other,"DIA_Oric_CanHelp_WhatYouMean_11_02"); //Je to jeden z nejvlivnêjších šamanù mezi skâety.
	AI_Output (other,self ,"DIA_Oric_CanHelp_WhatYouMean_15_03"); //A co mám udêlat?
	...
	if (hero.guild == GIL_PAL)
	{
		AI_Output (self, other, "DIA_Oric_CanHelp_WhatYouMean_11_06"); //Jsi jediný, koho mùžeme postrádat. Všechny ostatní rytíâe potâebujeme tady.
	}
	else if (hero.guild == GIL_KDF)
	{
		AI_Output (self, other, "DIA_Oric_CanHelp_WhatYouMean_11_07"); //Není pro mê snadné chtít po tobê nêco takového, mistâe. Ale jsi pro tenhle úkol nejvhodnêjší ze všech.
	}
	else
	{	
		AI_Output (self ,other,"DIA_Oric_CanHelp_WhatYouMean_11_08"); //Chceš nám pâece pomoci, ne? V tom pâípadê...
	};
	
	AI_Output (self ,other,"DIA_Oric_CanHelp_WhatYouMean_11_09"); //Hosh-Pakùv stan je za nepâátelskou linií, na ostrohu na jih odsud.
	AI_Output (self ,other,"DIA_Oric_CanHelp_WhatYouMean_11_10"); //Skoro ho mùžeš vidêt z okna.
...
INSTANCE DIA_Oric_NeedStuff   (C_INFO)
{
	...
	description = "Budu potâebovat nêco výbavy.";
};
...
{
	AI_Output (other,self ,"DIA_Oric_NeedStuff_15_00"); //Budu potâebovat pár vêcí.
...
FUNC VOID DIA_Oric_NeedStuff_Health ()
{
	...
	AI_Output (self ,other,"DIA_Oric_NeedStuff_Health_11_01"); //Dobrá volba. Aã ti jsou k užitku.
...
FUNC VOID DIA_Oric_NeedStuff_Mana ()
{
	...
	AI_Output (self ,other,"DIA_Oric_NeedStuff_Mana_11_01"); //Doufám, že ti pomohou. Nechã tê Innos ochraàuje.
...
FUNC VOID DIA_Oric_NeedStuff_Strength ()
{
	...
	AI_Output (self ,other,"DIA_Oric_NeedStuff_Strength_11_01"); //Tady je, urèitê se ti bude hodit. Pâeji ti hodnê štêstí.
...
FUNC VOID DIA_Oric_NeedStuff_Dexterity ()
{
	...
	AI_Output (self ,other,"DIA_Oric_NeedStuff_Dexterity_11_01"); //Nechã tvoje šípy nikdy neminou cíl. Tady ho máš.
...
INSTANCE DIA_Oric_NoMurder   (C_INFO)
{
	...
	description = "Nezavraždím Hosh-Paka tak zbabêlým zpùsobem.";
};
...
{
	AI_Output (other,self ,"DIA_Oric_NoMurder_15_00"); //Nezavraždím Hosh-Paka tak zbabêlým zpùsobem.
	AI_Output (self ,other,"DIA_Oric_NoMurder_11_01"); //Dokážeš si vùbec pâedstavit, v jak strašné pozici se nacházíme?
	AI_Output (self ,other,"DIA_Oric_NoMurder_11_02"); //Obléhá nás obrovská armáda skâetù. Docházejí nám zásoby.
...
FUNC VOID DIA_Oric_WillHelp_Info()
{
	...
	AI_Output (self ,other,"DIA_Oric_WillHelp_11_02"); //Hosh-Pakùv stan je na malém ostrohu. Už jsi ho musel vidêt.
...
FUNC VOID DIA_Oric_HoshDead_Info()
{
	...
	AI_Output (self ,other,"DIA_Oric_HoshDead_11_01"); //Výbornê, to by mêlo zasadit skâetùm slušnou ránu. Možná máme pâece jenom nêjakou šanci.
	AI_Output (self ,other,"DIA_Oric_HoshDead_11_02"); //Myslím, že bychom tím mêli získat alespoà týden èasu.
...
FUNC VOID DIA_Oric_AnyNews_Info()
{
	...
	{
		AI_Output (self ,other,"DIA_Oric_AnyNews_11_01"); //Myslíš kromê toho, že nás napadli skâeti?
	}
	else if (MIS_AllDragonsDead == TRUE)
	{
		AI_Output (self ,other,"DIA_Oric_AnyNews_11_02"); //Ano. Jsi náš hrdina. Zabít všechny draky není žádná procházka rùžovým sadem, âekl bych.
	}
	else if (MIS_KillHoshPak == LOG_SUCCESS)
	{
		AI_Output (self ,other,"DIA_Oric_AnyNews_11_03"); //Zdá se, že skâetù se hluboce dotkla Hosh-Pakova smrt.
	}
	else
	{
		AI_Output (self ,other,"DIA_Oric_AnyNews_11_04"); //Skâetù je èím dál tím víc. To není dobré.
	};
};
...
func void DIA_Oric_DragonPlettBericht_Info ()
{
	...
	{
		AI_Output (other, self, "DIA_Oric_DragonPlettBericht_15_01"); //Jednoho jsem už zabil. Mùžeš mi âíct, kde najdu ty ostatní?
	}
	else if (MIS_KilledDragons != 0) 
	{
		AI_Output (other, self, "DIA_Oric_DragonPlettBericht_15_02"); //Mám pocit, že jsem se ještê nedostal ke všem drakùm. Že bych nêco pâehlédl?
	};
		...
		||	(Oric_IcedragonInfo_OneTime == FALSE))
	{
		...
		&& (Oric_SwampdragonInfo_OneTime == FALSE)
		{
			AI_Output (self, other, "DIA_Oric_DragonPlettBericht_11_03"); //Na západ od starého hradu vznikla bêhem nêkolika posledních dnù velká bažina. Nezdá se ti to nêjak podezâelé?
			B_LogEntry (TOPIC_DRACHENJAGD,"Oric mi dal aspoà nêjakou stopu: západnê od hradu se pâed nêkolika dny znièehonic objevila hluboká bažina. To je opravdu podezâelé."); 
		...
		&& (Oric_RockdragonInfo_OneTime == FALSE)
		{
			...
			AI_Output (self, other, "DIA_Oric_DragonPlettBericht_11_05"); //Naši špehové hlásí, že ta pevnost je silnê stâežená. Možná se tam ukrývá jeden z nich.
			B_LogEntry (TOPIC_DRACHENJAGD,"Paladinští zvêdové sdêlili Oricovi, že kamenná tvrz za sopkou na jihu je silnê stâežena. Oric se domnívá, že tam nêkde bude i drak."); 
			Oric_RockdragonInfo_OneTime = TRUE;
		}
		else if ((Npc_IsDead(FireDragon))==FALSE)
		&& (Oric_FiredragonInfo_OneTime == FALSE)
		{
			AI_Output (self, other, "DIA_Oric_DragonPlettBericht_11_06"); //Když draci naposledy zaútoèili, âíkalo se, že jeden zmizel poblíž sopky na jih odsud. Mêl bys to tam prozkoumat.
		...
		&& (Oric_IcedragonInfo_OneTime == FALSE)
		{
			AI_Output (self, other, "DIA_Oric_DragonPlettBericht_11_07"); //Na západ odsud je zamrzlý kraj. Nepâekvapilo by mê, kdyby se tam ukrýval jeden z drakù.
			B_LogEntry (TOPIC_DRACHENJAGD,"Podle Orica by drak mohl sídlit v zamrzlé oblasti na západê."); 
...
INSTANCE DIA_Oric_PICKPOCKET (C_INFO)
{
	...
	permanent	= TRUE;
	description = "(Krádež tohoto svitku bude zatracenê têžká.)";
};                       
