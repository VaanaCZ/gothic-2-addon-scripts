...
{
	AI_Output			(other, self, "DIA_Cipher_HALLO_15_00"); //Pêkný tábor.
...
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_15_00"); //Co tady dêláš?
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_01"); //Nêjakou dobu tu jen posedávám a èekám na vhodný okamžik.
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_15_02"); //Vhodný okamžik? Na co?
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_03"); //Tamhle nahoâe by mêl hnízdit drak. Od tý doby, co pâišel, tam vzniknul pêknê smradlavej moèál.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_04"); //Moc dobâe se pamatuju doby, kdy tady kolem bylo úplnê sucho.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_05"); //Ale teë už tam nikdy nepùjdu sám.

	if ((Npc_IsDead(DJG_Rod)) == FALSE)
		{
			AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_06"); //Fajn. Rod je ještê poâád tady. Ale vezme nohy na ramena hned, jak uvidí první žravou štênici.
		};

	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_07"); //A co ty? Ty jsi sem taky nepâišel jen tak pro zábavu, co? Tak tam pùjdem spolu.
	...
	Info_AddChoice	(DIA_CipherDJG_HELLOAGAIN, "Proè ne - mùžu potâebovat nêjakou pomoc.", DIA_CipherDJG_HELLOAGAIN_GoTogether);
...
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_GoAlone_15_00"); //Radêji zùstávám sám.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_GoAlone_07_01"); //Jak je libo. Asi jsem se v tobê dost spletl.
	AI_StopProcessInfos (self);
};

func void DIA_CipherDJG_HELLOAGAIN_GoTogether ()
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_GoTogether_15_00"); //Proè ne - mùžu potâebovat nêjakou pomoc.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_GoTogether_07_01"); //Tak fajn. Koneènê odsud mùžu vypadnout. Ten zápach je k nevydržení. Staèí âíct!
...
func void DIA_CipherDJG_GOTOGETHERAGAIN_Info ()
{
	...
	AI_Output			(self, other, "DIA_CipherDJG_GOTOGETHERAGAIN_07_01"); //Staèí âíct!
	DJG_SwampParty = TRUE;
};
...
{
	AI_Output (self, other,"DIA_CipherDJG_SwampWait2_07_00"); //Ta bažina zaèíná hned za první zatáèkou. Asi bychom mêli jít pâímo tudy.
...
func VOID DIA_CipherDJG_GoForSwampDragon_Info()
{
	...
	{
	AI_Output			(self, other, "DIA_CipherDJG_GoForSwampDragon_07_00"); //(kâièí) Útok!
	AI_StopProcessInfos	(self);
...
instance DIA_CipherDJG_WHATNEXT		(C_INFO)
{
	...
	description	 = 	"Moèálový drak je mrtvý! Co teë budeš dêlat?";
};
...
{
	AI_Output	(other, self, "DIA_CipherDJG_WHATNEXT_15_00"); //Moèálový drak je po smrti! Co budeš dêlat teë?
	AI_Output	(self, other, "DIA_CipherDJG_WHATNEXT_07_01"); //Nemám tušení. Ještê jsem o tom nepâemýšlel, vážnê. Ale v každym pâípadê se teë mùžeš vrátit do Khorinidu jako hrdina.
	AI_Output	(self, other, "DIA_CipherDJG_WHATNEXT_07_02"); //Vsadím se, že z toho jde docela dost vytêžit. Pâemýšlej o tom.

	B_LogEntry (TOPIC_Dragonhunter,"Když je moèálový drak mrtvý, chce z toho Cipher vytâískat nêjaký prachy a stát se velkým 'hrdinou'. No, uvidíme."); 
