...
func void DIA_SylvioDJG_HelloAgain_Info ()
{
	...
	{
		AI_Output			(self, other,"DIA_SylvioDJG_HelloAgain_09_00"); //Dobrá, aã se propadnu! Koukáš, jak si vydêlat, co? Vêdêl jsem to! Máš žoldáckou duši.
		AI_Output			(self, other,"DIA_SylvioDJG_HelloAgain_09_01"); //Hele, jestli si myslíš, že si tu mùžeš nahrabat, tak to se fakt pleteš. Byl jsem tu první.
	}
	else if (other.guild == GIL_KDF)
	{
		AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_02"); //Hej, ty! Mágové mê znervózàují! Bêž nêkam jinam! Tady pro tebe nic není.
	}
	else
	{
		AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_03"); //Hej, ty, paladine! Vraã se zpátky do têch svejch dolù. Tady pro tebe nic není.
	};
	
	AI_Output			(other, self, "DIA_SylvioDJG_HelloAgain_15_04"); //Jasný. Nikdo se nechce dêlit o svou koâist.
	AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_05"); //Pâesnê tak. Zmizni.
};
...
func void DIA_Sylvio_VERSAGER_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_VERSAGER_09_01"); //Nehraj si na tvrëáka, pískle, jinak skonèíš jako ty mizerný svinê, co se válej támhle na snêhu.
};
...
func void DIA_Sylvio_DEINELEUTE_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_DEINELEUTE_09_01"); //Ne tak docela. Žádný velký ztráty. Ti idioti stejnê nestáli za nic.
};
...
func void DIA_Sylvio_WASISTPASSIERT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_09_01"); //Nebyli schopný se dostat tam pâes ty ledový obry. Zùstali tam.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_09_02"); //Když seš tak tvrdej, tak proè to nezkusíš?


	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Tak dobrá. Proè ne?", DIA_Sylvio_WASISTPASSIERT_ok );
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "K èemu ti bude dobrý, když to udêlám?", DIA_Sylvio_WASISTPASSIERT_washastdudavon );
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Co z toho budu mít?", DIA_Sylvio_WASISTPASSIERT_warum );
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Proè to nevyâídíš sám?", DIA_Sylvio_WASISTPASSIERT_selbst );

	Log_CreateTopic (TOPIC_SylvioKillIceGolem, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_SylvioKillIceGolem, LOG_RUNNING);
	B_LogEntry (TOPIC_SylvioKillIceGolem,"Sylvio se bojí dvou ledových golemù pâed vchodem do zamrzlé èásti Hornického údolí."); 

	MIS_DJG_Sylvio_KillIceGolem = LOG_RUNNING;
};
func void DIA_Sylvio_WASISTPASSIERT_selbst ()
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_selbst_15_00"); //Proè to nevyâídíš sám?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_selbst_09_01"); //Hele, nech si ty nafoukaný âeèi.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_selbst_09_02"); //Jak to já vidím, seš podêlanej strachy.
...
func void DIA_Sylvio_WASISTPASSIERT_warum ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_warum_09_01"); //Hodnê. Âeknême... 1000 zlatých. To zní dobâe, ne?
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_15_00"); //K èemu ti bude dobrý, když to udêlám?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_09_01"); //K tomu samýmu co tobê. Ale abych ti odpovêdêl na otázku, chci se dostat do tý zamrzlý oblasti.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_09_02"); //Bohužel, v cestê mi stojej ty zatracený vêci.
	
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Co chceš v té zamrzlé oblasti dêlat?", DIA_Sylvio_WASISTPASSIERT_Eisregion );
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_15_00"); //Co chceš v té zamrzlé oblasti dêlat?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_09_01"); //Ty s têma otázkama nepâestaneš, co? Dobrá, povím ti to.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_09_02"); //Tady, za tím srázem, je taková ledová pustina, jakou jsi nikdy pâedtím nevidêl.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_09_03"); //Spolu s tuènými zásobami tam je ukrytý poklad ledového draka.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_ok_15_00"); //Tak dobrá. Proè ne?
...
instance DIA_Sylvio_ICEGOLEMSKILLED		(C_INFO)
{
	...
	description	 = 	"Máš volnou cestu, ti ledoví obâi už jsou v pánu.";
};
...
{
	AI_Output			(other, self, "DIA_Sylvio_ICEGOLEMSKILLED_15_00"); //Cesta je volná! Ti ledoví obâi už tam zaclánêt nebudou.
	...
	AI_Output			(self, other, "DIA_Sylvio_ICEGOLEMSKILLED_09_03"); //Všechno má svùj èas.
	};
...
func void DIA_Sylvio_WASJETZT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_09_01"); //Fajn. Âekl bych, že to vypadá pêknê na levaèku.
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_09_02"); //Nejlíp bude, když pùjdeš první a já za tebou.
	...
	Info_AddChoice	(DIA_Sylvio_WASJETZT, "Nebudu za tebe dêlat špinavou práci.", DIA_Sylvio_WASJETZT_nein );
	if (DJG_Sylvio_PromisedMoney  == TRUE)
	{
	Info_AddChoice	(DIA_Sylvio_WASJETZT, "Nejdâív bych rád vidêl ty prachy.", DIA_Sylvio_WASJETZT_Geld );
	};
};
func void DIA_Sylvio_WASJETZT_trennen ()
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_trennen_15_00"); //Bojíš se?
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_trennen_09_01"); //Nesmysl, pâestaà žvanit. Jdeme.
...
func void DIA_Sylvio_WASJETZT_ok ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_ok_09_01"); //Fajn, pokraèuj, do toho.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_nein_15_00"); //Nebudu za tebe dêlat špinavou práci.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_Geld_15_00"); //Nejdâív bych rád vidêl ty prachy.
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_Geld_09_01"); //Až dostaneme toho draka, budeš mít penêz, kolik uneseš.
...
func void DIA_Sylvio_WASJETZT_jetztGeld ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_jetztGeld_09_01"); //Buë pùjdeš vpâedu, nebo si to s tebou vyâídim.
};
...
func void DIA_Sylvio_KOMMSTDU_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_KOMMSTDU_09_01"); //Pokraèuj dál. Budu hned za tebou.
...
instance DIA_Sylvio_DUHIER		(C_INFO)
{
	...
	description	 = 	"Uvítal bych nêjakou pomoc.";
};
...
{
	AI_Output			(other, self, "DIA_Sylvio_DUHIER_15_00"); //Uvítal bych nêjakou pomoc.
...
func void DIA_SylvioDJG_WHATNEXT_Info ()
{
	...
	AI_Output	(self, other, "DIA_SylvioDJG_WHATNEXT_09_03"); //Tvoje nepodstatná role je právê u konce!
...
func void DIA_SylvioDJG_WHATNEXT_ATTACK ()
{
	...
	B_StartOtherRoutine (DJG_Bullco,"Zaèít");

	B_LogEntry (TOPIC_Dragonhunter,"Ten hajzl Sylvio si dêlal nároky na moje vítêzství nad ledovým drakem. Trochu jsme se kvùli tomu chytli."); 
...
{
	AI_Output	(self, other, "DIA_SylvioDJG_BUTNOW_09_00"); //Je naèase vyrovnat skóre.
