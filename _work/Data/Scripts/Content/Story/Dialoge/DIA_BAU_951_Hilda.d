...
func void DIA_Hilda_Hallo_Info ()
{
	if (hero.guild == GIL_NONE)
		{
			AI_Output (self, other, "DIA_Hilda_Hallo_17_00"); //Hej, kdo tê sem zval? Okamžitê vypadni. Nikdo tê tu nechce.
		}
	else
		{
...
instance DIA_Hilda_WasZuEssen (C_INFO)
{
	...
	permanent	= TRUE;
	description = "Mùžeš mi dát nêco k jídlu?";
};
	...
	if (hero.guild == GIL_NONE)
		{
			...
			AI_Output (other, self, "DIA_Hilda_WasZuEssen_15_00"); //Mùžeš mi dát nêco k jídlu?
			...
			if (MIS_Lobart_Rueben == LOG_SUCCESS)
				{
					if (! Npc_KnowsInfo (other, DIA_Hilda_PfanneTooLate))
					{
						...
						{
							AI_Output (self, other, "DIA_Hilda_WasZuEssen_17_02"); //Pro další si mùžeš pâijít zase zítra.
							B_GiveInvItems (self, other, ItFo_Stew, 1);
							Hilda_Stew_Day = Wld_GetDay();
							Rueben_TagNull = TRUE;
							
							Log_CreateTopic (Topic_Bonus,LOG_NOTE);
							B_LogEntry (Topic_Bonus,"Každý den dostanu od Hildy trochu dušených tuâínù.");
						}
						else //heute schon bekommen
						{
							AI_Output (self, other, "DIA_Hilda_WasZuEssen_17_03"); //Dnešní porci už jsi dostal! Vraã se pozdêji.
						};
					}
					else //Pfanne zu spät
					{
		...
		{
			AI_Output (self, other, "DIA_Hilda_WasZuEssen_17_06"); //Urèitê máš dost zlata, aby sis mohl sám nêco koupit ve mêstê.
		};
};
...
instance DIA_Hilda_BringBeet (C_INFO)
{
	...
	permanent	= FALSE;
	description	= "Mám tady pro tebe pár tuâínù...";
};
...
{
	AI_Output (other, self, "DIA_Hilda_BringBeet_15_00"); //Mám tady pro tebe pár tuâínù.
	
	if (Npc_HasItems (other,ItPl_Beet) >= 20)
	{
		B_GiveInvItems (other, self, ItPl_Beet, Npc_HasItems (other,ItPl_Beet));
		
		AI_Output (self, other, "DIA_Hilda_BringBeet_17_01"); //Výbornê! (smích) To by mohlo na nakrmení têch chlapù i staèit!
		AI_Output (self, other, "DIA_Hilda_BringBeet_17_02"); //Každopádne, dokud jsi tady - vidêla jsem tudy pâed pár minutami procházet koèovného kupce.
		AI_Output (self, other, "DIA_Hilda_BringBeet_17_03"); //Mám dojem, že na cestê do mêsta na chvíli zastavil. Bêž se ho zeptat, jestli by pro mê nemêl poâádnou pánev.
	...
	else
	{
		...
		AI_Output (self, other, "DIA_Hilda_BringBeet_17_06"); //Bêž na pole a pâines mi další. Chlapi mají hlad!
		AI_StopProcessInfos (self);
	};
};
...
func void DIA_Hilda_Einkaufen_Info ()
{
	AI_Output (other, self, "DIA_Hilda_Einkaufen_15_00"); //Dej mi zlato a já za tím kupcem zajdu.
		...
		if (hero.guild == GIL_NONE)
		{
			AI_Output (self, other, "DIA_Hilda_Einkaufen_17_01"); //Âíkáš, že ti mám vêâit? Tak hlavnê ty peníze nevyhoë za chlast, slyšíš?!
		};
	...
	B_LogEntry (TOPIC_Hilda,"Hilda, žena sedláka Lobarta, mê požádala, abych jí u podomního obchodníka koupil nêjakou pánev.");
};
...
func void DIA_Hilda_PfanneGeholt_Info ()
{
	...
	AI_Output (self, other, "DIA_Hilda_PfanneGeholt_17_01"); //Fajn. To se podívejme, ještê se dá narazit na poctivce.
...
func void DIA_Hilda_PfanneTooLate_Info ()
{
		if (hero.guild == GIL_NONE)
		{
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_00"); //To si ještê dovolíš se sem vracet? Cos udêlal s mými penêzi, ty darmošlape?!
		}
		else
		{
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_01"); //Mêls na to dost èasu. Kde jsou peníze, co jsem ti dala na tu pánev?
		};
		
	if (Npc_HasItems(other, itmi_pan) > 0)
	{
		AI_Output (other, self, "DIA_Hilda_PfanneTooLate_15_02"); //Promià, zapomnêl jsem. Tady je tvoje pánev!
	
		B_GiveInvItems (other, self, itmi_pan, 1);
	
		AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_03"); //Arrh, tak sem tu vêc dej. Taková drzost - neuvêâitelné!
	...
	else //keine Pfanne
	{
		...
		{
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_04"); //Ach! Ty to ještê poâád máš u sebe! Vraã mi to. Hned.
			B_GiveInvItems (other, self, ItMi_Gold, 20); 
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_05"); //Taková drzost - neuvêâitelné!
		}
		else //kein Gold
		{
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_06"); //Ty to ještê nemáš! Dala jsem ti 20 zlatých!
			AI_Output (self, other, "DIA_Hilda_PfanneTooLate_17_07"); //Jdi mi z oèí, ty mizernej zlodêji!
...
func void DIA_Hilda_KRANK_Info ()
{
	...
	AI_Output			(self, other, "DIA_Hilda_KRANK_17_02"); //Mêla bych zajít za mêstským léèitelem, ale jsem na to moc slabá.
	...
	if (DIA_Hilda_KRANK_OnTime == FALSE)
	{
		...
		Info_AddChoice	(DIA_Hilda_KRANK, "Mohu nêjak pomoci?", DIA_Hilda_KRANK_helfen );
...
func void DIA_Hilda_KRANK_besserung ()
{
	...
	AI_Output			(self, other, "DIA_Hilda_KRANK_besserung_17_01"); //Doufejme, že pâijdou lepší èasy.
	AI_StopProcessInfos (self);
};

func void DIA_Hilda_KRANK_helfen ()
{
	AI_Output			(other, self, "DIA_Hilda_KRANK_helfen_15_00"); //Mohu nêjak pomoci?
	AI_Output			(self, other, "DIA_Hilda_KRANK_helfen_17_01"); //Bylo by od tebe moc milé, kdybys zašel za Vatrasem a získal od nêj lék.
	AI_Output			(self, other, "DIA_Hilda_KRANK_helfen_17_02"); //On už bude vêdêt, co má udêlat! Já už to sama nezvládnu.
...
func void DIA_Hilda_HEILUNGBRINGEN_Info ()
{
	...
	AI_Output			(self, other, "DIA_Hilda_HEILUNGBRINGEN_17_01"); //Vážnê. Naše spoleènost by potâebovala víc lidí jako ty. Mockrát díky.
	B_UseItem 			(self,ItPo_HealHilda_MIS);
	AI_Output			(self, other, "DIA_Hilda_HEILUNGBRINGEN_17_02"); //Doufám, že têchhle pár zlatých bude staèit.
...
func void DIA_Hilda_DISTURB_Info ()
{
	...
	AI_Output	(self, other, "DIA_Hilda_DISTURB_17_01"); //Už je mi líp, díky tobê.
	}
	else
	{
