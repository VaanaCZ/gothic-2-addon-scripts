...
{	
	...
	if (ORkSturmDI == TRUE)
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_01"); //Pośpiesz się. Orkowie na pewno wrócą.
	}
	else
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_02"); //Ta grota rzeczywiście jest straszna.
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Lares_DI_Training_09_01"); //Żaden problem.
	...
	{
		Info_AddChoice		(DIA_Lares_DI_Training, "Nauka skradania się"	,DIA_Lares_DI_Training_SNEAK);
	};
...
FUNC VOID DIA_Lares_DI_Training_1H_1 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_1_09_00"); //Musisz lekko ugiąć wysuniętą nogę, a drugą trzymać prosto. Dzięki temu łatwiej zachowasz równowagę.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
FUNC VOID DIA_Lares_DI_Training_1H_5 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_5_09_00"); //Biodra powinny być w ruchu. Będzie ci łatwiej uniknąć ataku.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
func void DIA_Lares_DI_Training_DEX_1 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_1_09_00"); //Górna część ciała musi poruszać się w zgodzie z resztą.
	};
...
func void DIA_Lares_DI_Training_DEX_5 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_5_09_00"); //Cały czas musisz uważać, żeby nie stracić równowagi.
	};
...
func void DIA_Lares_DI_Training_SNEAK ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_SNEAK_09_00"); //Zawsze stawiaj całą stopę, kiedy się skradasz.
	};
...
INSTANCE DIA_Lares_DI_UndeadDragonDead (C_INFO)
{
	...
	description = "Wyglądasz, jakbyś niezbyt się tym przejmował.";
};                       
...
{	
	AI_Output (other,self ,"DIA_Lares_DI_UndeadDragonDead_15_00"); //Wyglądasz, jakbyś niezbyt się tym przejmował.
	AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_01"); //Ech, no wiesz. Myślałem, że ci się uda.

	if (DIA_Lares_DI_UndeadDragonDead_OneTime == FALSE )
	{
		AI_Output (other,self ,"DIA_Lares_DI_UndeadDragonDead_15_02"); //Co zamierzasz teraz zrobić?
		AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_03"); //Nie mam pojęcia. Na razie będę się chyba trzymać ciebie.
