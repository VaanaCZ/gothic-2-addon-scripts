...
{	
	...
	if (ORkSturmDI == TRUE)
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_01"); //Pospêš si. Ti skâeti se urèitê vrátí.
	}
	else
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_02"); //Fajn. Tahle jeskynê je fakt strašidelná.
	};
};
...
INSTANCE DIA_Lares_DI_Training (C_INFO)
{
	...
	description = "Nauè mê nêco z tvých schopností.";
};                       
...
{	
	AI_Output (other,self ,"DIA_Lares_DI_Training_15_00"); //Nauè mê nêco ze svých schopností.
	...
	{
		Info_AddChoice		(DIA_Lares_DI_Training, "Nauèit se plížení"	,DIA_Lares_DI_Training_SNEAK);
	};
...
FUNC VOID DIA_Lares_DI_Training_1H_1 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_1_09_00"); //Musíš pokrèit nohu, co je vpâedu, a tu druhou udržovat napjatou. Tak získáš pevnêjší postoj.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
FUNC VOID DIA_Lares_DI_Training_1H_5 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_5_09_00"); //Zùstaà pružný v kyèlích. To ti umožní uskoèit pâed protiútokem.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
func void DIA_Lares_DI_Training_DEX_1 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_1_09_00"); //Tvá horní polovina musí pracovat v souladu se zbytkem têla.
	};
...
func void DIA_Lares_DI_Training_DEX_5 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_5_09_00"); //Vždycky by sis mêl hlídat své postavení.
	};
...
INSTANCE DIA_Lares_DI_UndeadDragonDead (C_INFO)
{
	...
	description = "Vážnê nevypadáš, že by tê to všechno mohlo zajímat.";
};                       
...
{	
	AI_Output (other,self ,"DIA_Lares_DI_UndeadDragonDead_15_00"); //Vážnê nevypadáš, že by tê to všechno mohlo zajímat.
	AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_01"); //No, vždyã víš. Mêl jsem dojem, že ses o to postaral ty.

	if (DIA_Lares_DI_UndeadDragonDead_OneTime == FALSE )
	{
		AI_Output (other,self ,"DIA_Lares_DI_UndeadDragonDead_15_02"); //Co budeš dêlat dál?
		AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_03"); //Nemám tušení. Hádám, že pùjdu s tebou.
