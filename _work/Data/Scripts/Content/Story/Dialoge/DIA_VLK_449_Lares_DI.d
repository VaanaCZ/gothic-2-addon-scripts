...
{	
	AI_Output (other,self ,"DIA_Lares_DI_Hallo_15_00"); //Come va?

	if (ORkSturmDI == TRUE)
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_01"); //Sbrigati. Gli orchi torneranno sicuramente.
	}
	else
	{
		AI_Output (self ,other,"DIA_Lares_DI_Hallo_09_02"); //Be', questa caverna fa paura.
	};
};
...
{	
	AI_Output (other,self ,"DIA_Lares_DI_Training_15_00"); //Insegnami le tue abilità.
	AI_Output (self ,other,"DIA_Lares_DI_Training_09_01"); //Non c'è problema.
	...
	{
		Info_AddChoice		(DIA_Lares_DI_Training, "Apprendi come muoverti furtivamente."	,DIA_Lares_DI_Training_SNEAK);
	};
...
FUNC VOID DIA_Lares_DI_Training_1H_1 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_1_09_00"); //Devi piegare la gamba davanti e tenere dritta quella dietro, così avrai una postura più stabile.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
FUNC VOID DIA_Lares_DI_Training_1H_5 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Lares_DI_Training_1H_5_09_00"); //Muovi molto il bacino, ti aiuterà a evitare un contrattacco.
	};
	Info_ClearChoices (DIA_Lares_DI_Training);
	Info_AddChoice	  (DIA_Lares_DI_Training, DIALOG_BACK, DIA_Lares_DI_Training_BACK);
	if (Npc_GetTalentSkill (hero, NPC_TALENT_SNEAK) == FALSE)
...
func void DIA_Lares_DI_Training_DEX_1 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_1_09_00"); //La parte superiore del tuo corpo deve muoversi armoniosamente con il resto.
	};
...
func void DIA_Lares_DI_Training_DEX_5 ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_DEX_5_09_00"); //Devi sempre badare a come cammini.
	};
...
func void DIA_Lares_DI_Training_SNEAK ()
{
	...
	{
		AI_Output (self ,other,"DIA_Lares_DI_Training_SNEAK_09_00"); //Usa l'intera pianta del piede per muoverti furtivamente.
	};
...
INSTANCE DIA_Lares_DI_UndeadDragonDead (C_INFO)
{
	...
	description = "Sembra che tutto questo non t'importi.";
};                       
...
{	
	AI_Output (other,self ,"DIA_Lares_DI_UndeadDragonDead_15_00"); //Non sembra che tutto questo ti importi.
	AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_01"); //Ah, sai, sapevo che ce l'avresti fatta.
	...
	if (DIA_Lares_DI_UndeadDragonDead_OneTime == FALSE )
	{
		...
		AI_Output (self ,other,"DIA_Lares_DI_UndeadDragonDead_09_03"); //Non ne ho idea. Per il momento credo che ti seguirò.
