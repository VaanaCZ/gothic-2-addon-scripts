...
{
	AI_Output			(self ,other, "DIA_Kurgan_HELLO_01_00"); //Hej, ty! Chlápci jako ty by si mêli dávat pozor, kam šlapou.
	AI_Output			(other,self , "DIA_Kurgan_HELLO_15_01"); //Co se mi to pokoušíš âíct?
	AI_Output			(self ,other, "DIA_Kurgan_HELLO_01_02"); //Chtêl jsem tím âíct, že zdejší ovzduší není prospêšné tvému zdraví. Tahle oblast je teë prolezlá skâety a dalšíma zrùdama.
	AI_Output			(self ,other, "DIA_Kurgan_HELLO_01_03"); //A to nepoèítám draky. Ber to jen jako dobâe mínênou radu.
	B_LogEntry (TOPIC_Dragonhunter,"U vchodu do Hornického údolí mê pâivítala skupina drakobijcù. Byli têžce ozbrojení, ale obávám se, že na draky tím moc velký dojem neudêlají.");
};
...
instance DIA_Kurgan_ELSE		(C_INFO)
{
	...
	description	= 	"Mùžeš mi âíct nêco, co už bych dávno nevêdêl?";
...
{
	AI_Output			(other,self , "DIA_Kurgan_ELSE_15_00"); //Mùžeš mi âíct nêco, co už bych dávno nevêdêl?
	AI_Output			(self ,other, "DIA_Kurgan_ELSE_01_01"); //Mùžu ti dát dobrou radu, a to dokonce zdarma.
	AI_Output			(self ,other, "DIA_Kurgan_ELSE_01_02"); //Nemùžem tu potâebovat nikoho, kdo se složí ze slabouèkýho draèího prdnutí.
...
func void DIA_Kurgan_Leader_Info ()
{
	...
	AI_Output			(self ,other, "DIA_Kurgan_Leader_01_01"); //Vypadám tak? Jasnê že ne. Nepotâebujeme žádnýho nafoukanýho blba, co by nám nêco rozkazoval.
	AI_Output			(self ,other, "DIA_Kurgan_Leader_01_02"); //Když se Sylvio snažil chovat jako náš šéf, ukázali jsme mu i têm jeho kámošùm, co si o tom myslíme.
	AI_Output			(self ,other, "DIA_Kurgan_Leader_01_03"); //Pohádali jsme se. Nakonec sami odešli.
	AI_Output			(self ,other, "DIA_Kurgan_Leader_01_04"); //Doufám, že Sylvio skonèil v kotli s polívkou nêjakýho skâeta.
};
...
{
	AI_Output			(other,self , "DIA_Kurgan_KillDragon_15_00"); //(posmêšnê) Tak ty chceš zabít draky?
	AI_Output			(self ,other, "DIA_Kurgan_KillDragon_01_01"); //Bláznivej osle. Tak ty víš, jak složit draka?
	AI_Output			(self ,other, "DIA_Kurgan_KillDragon_01_02"); //To si myslíš, že ta bestie si prostê bude jen tak klidnê sedêt ve svym doupêti a èekat, až jí usekneš hlavu?
	AI_Output			(self ,other, "DIA_Kurgan_KillDragon_01_03"); //Nêco takovýho vyžaduje dobrej plán a ráznou akci.
	...
	Info_AddChoice	(DIA_Kurgan_KillDragon, "To bych chtêl vidêt, ale musím se starat o své vêci.", DIA_Kurgan_KillDragon_weg );
...
func void DIA_Kurgan_KillDragon_spott ()
{
	...
	AI_Output			(self, other, "DIA_Kurgan_KillDragon_spott_01_01"); //Co? To chceš, abych ti nêjakou vrazil?
	AI_StopProcessInfos (self);
...
func void DIA_Kurgan_KillDragon_orks ()
{
	...
	AI_Output			(self, other, "DIA_Kurgan_KillDragon_orks_01_01"); //Na tyhle vêci je ještê dost èasu.

};

func void DIA_Kurgan_KillDragon_weg ()
{
	AI_Output			(other, self, "DIA_Kurgan_KillDragon_weg_15_00"); //To bych chtêl vidêt, ale musím se starat o své vêci.
	AI_Output			(self, other, "DIA_Kurgan_KillDragon_weg_01_01"); //Mêl by ses odsud rychle vytratit. Jinak se ti mùže stát, že pâijdeš o hnátu, možná o dvê.
...
instance DIA_Kurgan_SEENDRAGON		(C_INFO)
{
	...
	description	 = 	"Už jsi nêkdy vidêl draka?";
};
...
{
	AI_Output			(other, self, "DIA_Kurgan_SEENDRAGON_15_00"); //Už jsi nêkdy toho draka vidêl?
	AI_Output			(self, other, "DIA_Kurgan_SEENDRAGON_01_01"); //Zatím ne. Ale ta bestie se nemùže schovávat vêènê.
};
...
FUNC VOID DIA_Kurgan_AllDragonsDead_Info()
{
	...
	AI_Output 	(self ,other,"DIA_Kurgan_AllDragonsDead_01_01"); //Ha, a kdo je jako mêl zabít? Paladinové?
	AI_Output 	(other,self ,"DIA_Kurgan_AllDragonsDead_15_02"); //Já.
	AI_Output 	(self ,other,"DIA_Kurgan_AllDragonsDead_01_03"); //(smích) Ha. Tomu sám nevêâíš. Pâestaà si ze mê utahovat.
};
