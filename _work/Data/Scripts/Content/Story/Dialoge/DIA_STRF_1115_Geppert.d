...
func void DIA_Geppert_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Geppert_HALLO_10_01"); //Nepâišel jsi kvùli tomu, abys mê poslal zpátky do dolù, že ne?
	AI_Output			(self, other, "DIA_Geppert_HALLO_10_02"); //V tom pâípadê tê musím zklamat. Není žádnej zpùsob, jak mê dostat zpátky.
	

	Info_ClearChoices	(DIA_Geppert_HALLO);
	Info_AddChoice	(DIA_Geppert_HALLO, "Co tady dêláš?", DIA_Geppert_HALLO_Wasmachtihr );
	Info_AddChoice	(DIA_Geppert_HALLO, "Ty jsi trestanec na útêku, co?", DIA_Geppert_HALLO_Flucht );
};

func void DIA_Geppert_HALLO_Flucht ()
{
	AI_Output			(other, self, "DIA_Geppert_HALLO_Flucht_15_00"); //Ty jsi trestanec na útêku, co?
	AI_Output			(self, other, "DIA_Geppert_HALLO_Flucht_10_01"); //Jak bystrý úsudek. Proè bych se jinak hrabal v týhle špinavý díâe?
	Info_ClearChoices	(DIA_Geppert_HALLO);
};

func void DIA_Geppert_HALLO_Wasmachtihr ()
{
	AI_Output			(other, self, "DIA_Geppert_HALLO_Wasmachtihr_15_00"); //Co tady dêláš?
	...
	if 	((other.guild == GIL_MIL) || (other.guild == GIL_PAL))
			{
				...
				AI_Output			(self, other, "DIA_Geppert_HALLO_Wasmachtihr_10_02"); //Pâestaà chodit kolem horký kaše. Buë mê zabij, nebo se ztraã.
			}
			else
			{
				AI_Output			(self, other, "DIA_Geppert_HALLO_Wasmachtihr_10_03"); //Blbá otázka. Schovávám se, co jinýho? Ty vojenský svinê maj svý špehy úplnê všude.
			};
...
instance DIA_Geppert_BRATEN		(C_INFO)
{
	...
	description	 = 	"Ta peèenê pêknê voní.";
};
...
{
	AI_Output			(other, self, "DIA_Geppert_BRATEN_15_00"); //Ta peèenê pêknê voní.
