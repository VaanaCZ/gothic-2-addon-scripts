...
{	
	AI_Output (other, self, "DIA_Addon_Matt_Alright_15_01"); //Jak se daøí?
	
	if (self.aivar[AIV_PARTYMEMBER] == TRUE)
	{
		if (self.attribute[ATR_HITPOINTS] < 100)
		{
			AI_Output (self ,other,"DIA_Addon_Matt_Alright_10_02"); //Øíkal jsem LÉÈIVEJ LEKTVAR. Nerozumíš èesky?
		}
		else
		{
	...
	&& (!Npc_IsDead(Greg))
	{
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_01"); //To je sranda. Už nemáme žádnou loï.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_02"); //Budu prostì èekat, co Greg vymyslí.
	}
	else
	{
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_03"); //Teï je v táboøe klid, takže si pùjdu taky trochu dáchnout.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_04"); //A bejt tebou, udìlám to samý.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_05"); //Od tý doby, co tu opruzujou ti banditi, se situace mìní rychlejc, než je zdrávo.
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_01"); //Myslíš krom toho, že jsou pro nás smrtelná hrozba a maj nad náma poøádnou pøesilu?
	AI_Output (other,self ,"DIA_Addon_Matt_Bandits_15_02"); //Ano.
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_02"); //Válej se ve zlatì.
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_03"); //Teda aspoò se VÁLELI ve zlatì. Protože za poslední zásilku nikdy nezaplatili.
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Francis_10_03"); //Tak to se musím zamyslet. Hmm... ne. Nic jinýho mì nenapadá.
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_01"); //Teï nemùžu jít, snažím se trochu si dáchnout.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_15_02"); //Jsou to rozkazy od Grega.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_03"); //(rychle) Aha, chápu. To je ovšem nìco jinýho. Teda jako že už jdu, ovšem, samozøejmì.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_04"); //Kam pùjdem?
	
	Info_ClearChoices (DIA_Addon_Matt_Anheuern);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Zavøi klapaèku a radši pojï.",DIA_Addon_Matt_Anheuern_ShutUp);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Máme vyèistit kaòon.",DIA_Addon_Matt_Anheuern_ClearCanyon);
};
...
{
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_00"); //Máme vyèistit kaòon.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_01"); //Si upad, ne? Dyk je plnej potvor. Hlavnì s tìma bøitovcema bysme si nemìli zahrávat.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_02"); //Já vím. Už jdeš?
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_03"); //(povzdychne si) Radši si nabalím pár léèivejch lektvarù, budem je potøebovat.
...
{
	AI_Output (other, self, "DIA_Addon_Matt_ComeOn_15_00"); //Pojï se mnou.
...
{	
	AI_Output (other,self ,"DIA_Addon_Matt_DontNeedYou_15_00"); //Už tì nepotøebuju.
	AI_Output (self, other, "DIA_Addon_Matt_GoHome_10_01"); //(skuhrá si pro sebe) Potøebuju poøádnýho loka koøalky!
...
{	
	AI_Output (self, other, "DIA_Addon_Matt_TooFar_10_01"); //Odsud už mùžeš jít sám (cynicky), kapitáne.
...
{	
	AI_Output (self, other, "DIA_Addon_Matt_Healing_10_01"); //(cynicky) Haló, kapitáne! Hodil by se mi òákej léèivej lektvar.
};
