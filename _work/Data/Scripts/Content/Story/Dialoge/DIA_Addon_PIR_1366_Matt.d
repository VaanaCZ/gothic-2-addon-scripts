...
{	
	AI_Output (other, self, "DIA_Addon_Matt_Alright_15_01"); //Jak se daří?
	
	if (self.aivar[AIV_PARTYMEMBER] == TRUE)
	{
		if (self.attribute[ATR_HITPOINTS] < 100)
		{
			AI_Output (self ,other,"DIA_Addon_Matt_Alright_10_02"); //Říkal jsem LÉČIVEJ LEKTVAR. Nerozumíš česky?
		}
		else
		{
	...
	&& (!Npc_IsDead(Greg))
	{
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_01"); //To je sranda. Už nemáme žádnou loď.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_02"); //Budu prostě čekat, co Greg vymyslí.
	}
	else
	{
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_03"); //Teď je v táboře klid, takže si půjdu taky trochu dáchnout.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_04"); //A bejt tebou, udělám to samý.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_05"); //Od tý doby, co tu opruzujou ti banditi, se situace mění rychlejc, než je zdrávo.
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_01"); //Myslíš krom toho, že jsou pro nás smrtelná hrozba a maj nad náma pořádnou přesilu?
	AI_Output (other,self ,"DIA_Addon_Matt_Bandits_15_02"); //Ano.
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_02"); //Válej se ve zlatě.
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_03"); //Teda aspoň se VÁLELI ve zlatě. Protože za poslední zásilku nikdy nezaplatili.
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Francis_10_03"); //Tak to se musím zamyslet. Hmm... ne. Nic jinýho mě nenapadá.
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_01"); //Teď nemůžu jít, snažím se trochu si dáchnout.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_15_02"); //Jsou to rozkazy od Grega.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_03"); //(rychle) Aha, chápu. To je ovšem něco jinýho. Teda jako že už jdu, ovšem, samozřejmě.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_04"); //Kam půjdem?
	
	Info_ClearChoices (DIA_Addon_Matt_Anheuern);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Zavři klapačku a radši pojď.",DIA_Addon_Matt_Anheuern_ShutUp);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Máme vyčistit kaňon.",DIA_Addon_Matt_Anheuern_ClearCanyon);
};
...
{
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_00"); //Máme vyčistit kaňon.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_01"); //Si upad, ne? Dyk je plnej potvor. Hlavně s těma břitovcema bysme si neměli zahrávat.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_02"); //Já vím. Už jdeš?
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_03"); //(povzdychne si) Radši si nabalím pár léčivejch lektvarů, budem je potřebovat.
...
{
	AI_Output (other, self, "DIA_Addon_Matt_ComeOn_15_00"); //Pojď se mnou.
...
{	
	AI_Output (other,self ,"DIA_Addon_Matt_DontNeedYou_15_00"); //Už tě nepotřebuju.
	AI_Output (self, other, "DIA_Addon_Matt_GoHome_10_01"); //(skuhrá si pro sebe) Potřebuju pořádnýho loka kořalky!
...
{	
	AI_Output (self, other, "DIA_Addon_Matt_Healing_10_01"); //(cynicky) Haló, kapitáne! Hodil by se mi ňákej léčivej lektvar.
};
