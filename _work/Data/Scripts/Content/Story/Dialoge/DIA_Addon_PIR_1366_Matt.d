...
{	
	...
	if (self.aivar[AIV_PARTYMEMBER] == TRUE)
	{
		...
		{
			AI_Output (self ,other,"DIA_Addon_Matt_Alright_10_02"); //Âíkal jsem LÉÈIVEJ LEKTVAR. Nerozumíš èesky?
		}
		else
		{
	...
	&& (!Npc_IsDead(Greg))
	{
		...
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_02"); //Budu prostê èekat, co Greg vymyslí.
	}
	else
	{
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_03"); //Teë je v táboâe klid, takže si pùjdu taky trochu dáchnout.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_04"); //A bejt tebou, udêlám to samý.
		AI_Output (self ,other,"DIA_Addon_Matt_Job_10_05"); //Od tý doby, co tu opruzujou ti banditi, se situace mêní rychlejc, než je zdrávo.
	};
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_02"); //Válej se ve zlatê.
	AI_Output (self ,other,"DIA_Addon_Matt_Bandits_10_03"); //Teda aspoà se VÁLELI ve zlatê. Protože za poslední zásilku nikdy nezaplatili.
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_Francis_10_03"); //Tak to se musím zamyslet. Hmm... ne. Nic jinýho mê nenapadá.
};
...
{	
	...
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_01"); //Teë nemùžu jít, snažím se trochu si dáchnout.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_15_02"); //Jsou to rozkazy od Grega.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_03"); //(rychle) Aha, chápu. To je ovšem nêco jinýho. Teda jako že už jdu, ovšem, samozâejmê.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_10_04"); //Kam pùjdem?
	
	Info_ClearChoices (DIA_Addon_Matt_Anheuern);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Zavâi klapaèku a radši pojë.",DIA_Addon_Matt_Anheuern_ShutUp);
	Info_AddChoice (DIA_Addon_Matt_Anheuern,"Máme vyèistit kaàon.",DIA_Addon_Matt_Anheuern_ClearCanyon);
};
...
{
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_00"); //Máme vyèistit kaàon.
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_01"); //Si upad, ne? Dyk je plnej potvor. Hlavnê s têma bâitovcema bysme si nemêli zahrávat.
	AI_Output (other,self ,"DIA_Addon_Matt_FollowMe_ClearCanyon_15_02"); //Já vím. Už jdeš?
	AI_Output (self ,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_03"); //(povzdychne si) Radši si nabalím pár léèivejch lektvarù, budem je potâebovat.
...
{	
	AI_Output (other,self ,"DIA_Addon_Matt_DontNeedYou_15_00"); //Už tê nepotâebuju.
...
{	
	AI_Output (self, other, "DIA_Addon_Matt_Healing_10_01"); //(cynicky) Haló, kapitáne! Hodil by se mi àákej léèivej lektvar.
};
