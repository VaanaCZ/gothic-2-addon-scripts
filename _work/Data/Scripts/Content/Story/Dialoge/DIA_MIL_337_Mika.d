...
{
	AI_Output			(self, other, "DIA_Mika_WOHIN_12_00"); //Hej, hej. Ne tak rychle. Bloumat tady o samotê mùže být docela nebezpeèný. Odkud jsi?
	...
	Info_AddChoice	(DIA_Mika_WOHIN, "Z mêsta!", DIA_Mika_WOHIN_stadt );
};
func void DIA_Mika_WOHIN_stadt ()
{
	AI_Output			(other, self, "DIA_Mika_WOHIN_stadt_15_00"); //Z mêsta!
	AI_Output			(self, other, "DIA_Mika_WOHIN_stadt_12_01"); //Dobrá, dobrá. Tak proè se potom poflakuješ tady, daleko od bezpeèí mêstskejch hradeb?
...
func void DIA_Mika_WOHIN_Bauern ()
{
	...
	AI_Output			(self, other, "DIA_Mika_WOHIN_Bauern_12_01"); //Farmáâ, co? Hmm. V tom pâípadê by ses nemêl courat po têchhle nebezpeènejch cestách o samotê. Kdo ví, kdo by tady na tebe mohl èíhat.
...
func void DIA_Mika_WOHIN_weg ()
{
	...
	AI_Output			(self, other, "DIA_Mika_WOHIN_weg_12_01"); //Když myslíš. Ale až tê nêkdo sejme, tak si potom nestêžuj. Pâeju hezkej den.
...
func void DIA_Mika_WASGEFAEHRLICH_Info ()
{
	...
	AI_Output			(self, other, "DIA_Mika_WASGEFAEHRLICH_12_01"); //Spousta vêcí.
	if (other.protection[PROT_EDGE]	< ITAR_Leather_L.protection [PROT_EDGE])	//Joly: kleiner als die Lederrüstung
	{
		AI_Output			(self, other, "DIA_Mika_WASGEFAEHRLICH_12_02"); //Za prvé jsou tady banditi. Takovou mêkkotu, jako jsi ty, klidnê zblajznou k snídani.
		AI_Output			(self, other, "DIA_Mika_WASGEFAEHRLICH_12_03"); //A když tê nechytêj banditi, tak se o tebe postaraj divoký zvíâata v lese nebo žoldáci, co se potulujou tady po okolí.
		AI_Output			(self, other, "DIA_Mika_WASGEFAEHRLICH_12_04"); //Takže než nêkam vyrazíš, mêl by sis sehnat nêjaký slušný brnêní.
	};
	AI_Output			(self, other, "DIA_Mika_WASGEFAEHRLICH_12_05"); //Vsadím se, že budeš kâièet o pomoc hned za první zatáèkou.
...
instance DIA_Mika_WASKOSTETHILFE		(C_INFO)
{
	...
	description	 = 	"Jen pro pâípad, že bych mêl pâijmout tvou pomoc.";
};
...
{
	AI_Output			(other, self, "DIA_Mika_WASKOSTETHILFE_15_00"); //Jenom pro pâípad, že bych tvou pomoc opravdu potâeboval. Kolik by mê to stálo?
	AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_12_01"); //Jsem prostý skromný služebník krále a v žádném pâípadê bych nechtêl oškubat bezbranného obèana království.
	AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_12_02"); //Ale, no... když to âíkáš takhle... rozhodnê bych nemêl nic proti nêjakýmu malýmu finanènímu upevnêní našich budoucích obchodních vztahù.
	AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_12_03"); //10 zlatejch by mêlo pro zaèátek staèit. Co ty na to?
	...
	Info_AddChoice	(DIA_Mika_WASKOSTETHILFE, "Proè ne? Tady máš svých 10 zlaãákù.", DIA_Mika_WASKOSTETHILFE_ja );

};

func void DIA_Mika_WASKOSTETHILFE_ja ()
{
	AI_Output			(other, self, "DIA_Mika_WASKOSTETHILFE_ja_15_00"); //Proè ne? Tady máš svých 10 zlaãákù.

	if (B_GiveInvItems (other, self, ItMi_Gold,10))
			{
				AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_ja_12_01"); //Výbornê. Kdybys potâeboval pomoc, víš, kde mê hledat.
				AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_ja_12_02"); //Ale chtêl bych tê poprosit o jednu malièkost. Nechoë za mnou se žádnejma prkotinama. To mê vždycky dokáže vytoèit, rozumíme si?
				
				Mika_Helps = TRUE;
			}
		else
			{
				AI_Output			(self, other, "DIA_Mika_WASKOSTETHILFE_ja_12_03"); //Tolik penêz nemáš. Možná by sis mêl rozmyslet, jestli opravdu mojí pomoc potâebuješ.
			};
	AI_StopProcessInfos (self);
};
...
instance DIA_Mika_UEBERLEGT		(C_INFO)
{
	...
	description	 = 	"Rozmyslel jsem si to. Zaplatím têch 10 zlatých.";
};
...
{
	AI_Output			(other, self, "DIA_Mika_UEBERLEGT_15_00"); //Rozmyslel jsem si to. Zaplatím têch 10 zlatých.

	if (B_GiveInvItems (other, self, ItMi_Gold,10))
		{
			AI_Output			(self, other, "DIA_Mika_UEBERLEGT_12_01"); //Skvêlé. Lepší pozdê než nikdy. A co teë?
			
			Mika_Helps = TRUE;
		}
		else
		{
			AI_Output			(self, other, "DIA_Mika_UEBERLEGT_12_02"); //(rozzlobenê) Vraã se, až budeš mít ty prachy.
			AI_StopProcessInfos (self);	
		};
};
...
func void DIA_Mika_HILFE_Info ()
{
	...
	AI_Output			(self, other, "DIA_Mika_HILFE_12_01"); //Jestli to je naprosto nezbytný... Co se dêje?

	Info_ClearChoices	(DIA_Mika_HILFE);
	
	Info_AddChoice	(DIA_Mika_HILFE, "Pronásledují mê bandité.", DIA_Mika_HILFE_schongut );
	Info_AddChoice	(DIA_Mika_HILFE, "Zaútoèily na mê pâíšery.", DIA_Mika_HILFE_monster );

	if (!Npc_IsDead (Alvares)) 
	&& (!Npc_IsDead (Engardo))
	&& ( (Akils_SLDstillthere == TRUE) || (Npc_KnowsInfo (other, DIA_Sarah_Bauern)) )
	{
		Info_AddChoice	(DIA_Mika_HILFE, "Na farmáâe Akila zaútoèili žoldáci.", DIA_Mika_HILFE_Akil );
	};
};

func void DIA_Mika_HILFE_Akil ()
{
	AI_Output			(other, self, "DIA_Mika_HILFE_Akil_15_00"); //Na farmáâe Akila zaútoèili žoldáci.
	AI_Output			(self, other, "DIA_Mika_HILFE_Akil_12_01"); //(znepokojenê) Cože? Ta lùza vyrazila na Akilovu farmu? Tak to není èasu nazbyt. Pojë za mnou.

	AI_StopProcessInfos (self);
	self.aivar[AIV_PARTYMEMBER] = TRUE;
	B_GivePlayerXP (Xp_Ambient);
	B_LogEntry (TOPIC_AkilsSLDStillthere,"Mika chce, abych vyâešil problém se žoldnéâi na Akilovê farmê."); 
...
{
	AI_Output			(other, self, "DIA_Mika_HILFE_monster_15_00"); //Zaútoèily na mê pâíšery.
	AI_Output			(self, other, "DIA_Mika_HILFE_monster_12_01"); //Ale já žádný pâíšery nevidím. Urèitê sis to jenom vymyslel.
...
{
	AI_Output			(other, self, "DIA_Mika_HILFE_schongut_15_00"); //Pronásledují mê bandité.
	AI_Output			(self, other, "DIA_Mika_HILFE_schongut_12_01"); //Vážnê? A kde tedy jsou? Kdyby tê pronásledovali, urèitê bych mêl vidêt alespoà jednoho, ne?
...
{
	AI_Output			(self, other, "DIA_Mika_Zack_12_00"); //Teë sleduj, jak se to dêlá.
...
FUNC VOID DIA_Mika_Kap3u4u5_PERM_Info()
{
	...
	AI_Output (self ,other,"DIA_Mika_Kap3u4u5_PERM_12_01"); //Ještê poâád jsi naživu. To na mê udêlalo dojem.
};
