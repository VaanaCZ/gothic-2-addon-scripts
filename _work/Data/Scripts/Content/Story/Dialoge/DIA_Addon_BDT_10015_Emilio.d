...
{	
	...
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Hi_10_01");//Taky že JSEM kopáč. Když jsem byl naposledy v dole, nadâel jsem se jak hovado.
...
{	
	AI_Output (other, self, "DIA_Addon_BDT_10015_Emilio_Gold_15_00");//Co dęláte s vytęženým zlatem?
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Gold_10_01");//Thorus je vybírá a rozdęluje. Nikdo si nesmí nechat, co nakope.
	...
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Gold_10_03");//Myslím, že to tak má bejt. Od tý doby, co máme tohle pravidlo, bylo míŕ smrăáků a kopáči furt dostávaj víc než ti, co se poflakujou venku.
};
//----------------------------------------------------------------------
//	Info Stein
//----------------------------------------------------------------------
INSTANCE DIA_Addon_BDT_10015_Emilio_Stein   (C_INFO)
{
	...
	permanent   = FALSE;
	description	= "Co je to s tęmi červenými kameny?";
};
...
{	
	AI_Output (other, self, "DIA_Addon_BDT_10015_Emilio_Stein_15_00");//A co máte s tęmi červenými kameny?
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Stein_10_01");//To vymyslel Thorus s Estebanem.
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Stein_10_02");//Thorus se stará o rozdęlování zlata a Esteban organizuje dęlníky v dole.
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Stein_10_03");//Samozâejmę že nechce bęhat za Thorusem pokaždý, když nękoho pošle do dolu.
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Stein_10_04");//A proto nám dává tyhle červený kamenný tabulky a Thorus nás za nę pouští dovnitâ. Fungujou jako vstupenky.
};
//---------------------------------------------------------------------
//	Info Attentat 
//---------------------------------------------------------------------
...
FUNC VOID DIA_Addon_Emilio_Attentat_Info()
{
	...
	if (Emilio_Switch == 0)
	{
		AI_Output (self, other, "DIA_Addon_Emilio_Attentat_10_01"); //(vydęšenę) Hele, já s tím nemám nic společnýho!
		Emilio_Switch = 1;
	}
	else
	{
		AI_Output (self, other, "DIA_Addon_Emilio_Attentat_10_02"); //(vydęšenę) VŮBEC NIC!
		Emilio_Switch = 0;
	};
};
...
{	
	...
	{
		AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Senyan_10_00");//(zkoumavę) Povęz, PROČ Senyan âval 'A hele, kohopak to tu máme'?
	...
	AI_Output (other, self, "DIA_Addon_BDT_10015_Emilio_Senyan_15_03");//Máš s tím ŕákej problém?
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Senyan_10_04");//(rychle) Ne, s tím žádnej problém nemám.
	AI_Output (self, other, "DIA_Addon_BDT_10015_Emilio_Senyan_10_05");//Pâesnę naopak. (posmęšnę) Ten hajzl pracoval pro Estebana.
	
	Senyan_CONTRA = LOG_SUCCESS;
	B_LogEntry (Topic_Addon_Esteban, "Emilio není na Estabanovę stranę.");
};
...
{	
	...
	AI_Output (self, other, "DIA_Addon_Emilio_Jetzt_10_01"); //(trochu nejistę) Už jsem byl v dole dost dlouho a dâel se do úmoru. Teë potâebuju pár dní odpočinku.
	AI_Output (self, other, "DIA_Addon_Emilio_Jetzt_10_02"); //(trochu nejistę si povzdechne) Než dostanu další červenej kámen.
};
...
INSTANCE DIA_Addon_Emilio_VonEmilio (C_INFO)
{
	...
	permanent   = FALSE;
	description	= "Lennar mi o tobę nęco málo âekl...";
};
...
{	
	AI_Output (other, self, "DIA_Addon_Emilio_VonEmilio_15_00"); //Lennar mi o tobę trochu vyprávęl
	AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_01"); //Lennar? Ten chlap je magor, to sis určitę všimnul.
	AI_Output (other, self, "DIA_Addon_Emilio_VonEmilio_15_02"); //Âíkal, že jsi nebyl v dole od toho útoku.
	AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_03"); //(zastrašenę) Já... já nic nevím!
	
	if (!Npc_IsDead (Senyan))
	{	
		AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_04"); //Ty spolupracuješ s tím Senyanem!
		AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_05"); //Oba jste ve spolku s Estebanem! Moc dobâe jsem slyšel, o čem jste to blábolili!
		AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_06"); //Esteban na nás doteëka kašlal, tak nevím, proč bych męl vęâit jeho lidem.
		AI_Output (self, other, "DIA_Addon_Emilio_VonEmilio_10_07"); //Nech mę bejt.
		AI_StopProcessInfos(self);
	};
...
{	
	...
	AI_Output (other, self, "DIA_Addon_Emilio_HilfMir_15_02"); //Když si dement jako Lennar všimne, že se divnę chováš, zanedlouho to zjistí i Esteban.
	...
	AI_Output (self, other, "DIA_Addon_Emilio_HilfMir_10_05"); //Huno... bęž za Hunem. Ten o tý záležitosti musí nęco vędęt.
	Emilio_TellAll = TRUE;
	
	B_LogEntry (Topic_Addon_Esteban, "Emilio konečnę vyplivnul to jméno: Huno.");
};
...
{	
	...
	AI_Output (self, other, "DIA_Addon_Emilio_GegenEsteban_10_01"); //Ta svinę nemyslí na nic jinýho než na prachy.
	AI_Output (self, other, "DIA_Addon_Emilio_GegenEsteban_10_02"); //Jednou za pár dní nękoho z nás sežere ŕákej důlní červ.
	AI_Output (self, other, "DIA_Addon_Emilio_GegenEsteban_10_03"); //Ale Esteban prostę do dolů neposílá žádný bojovníky.
	AI_Output (self, other, "DIA_Addon_Emilio_GegenEsteban_10_04"); //A proč? TI chlapi patâej k Havranový ochrance a Esteban jim nic neâekne, páč je z nich strachy celej podęlanej.
...
{	
	...
	AI_Output (self, other, "DIA_Addon_BDT_10004_Emilio_Hacker_10_01"); //Dâu se tu jako mezek, ale hlavnę že mę zatím nesežral žádnej důlní červ.
};
