	...
	if (hero.guild == GIL_NONE)
		{
			...
			{
				AI_Output (self, other, "DIA_Maleth_Hallo_08_01"); //Vidêl jsem tê pâicházet z hor.
			};
			AI_Output (self, other, "DIA_Maleth_Hallo_08_02"); //Mùžeš být rád, žes nepâišel pâed tâemi týdny.
			AI_Output (self, other, "DIA_Maleth_Hallo_08_03"); //Zatkli bychom tê jako uprchlého trestance. A udêlali bychom s tebou krátký proces!
			
			if (Npc_HasEquippedArmor(other) == FALSE) 
			{
				AI_Output (self, other, "DIA_Maleth_Hallo_08_04"); //Vypadáš úplnê vyèerpanê.
			}
			else
			{
				AI_Output (self, other, "DIA_Maleth_Hallo_08_05"); //Když jsi pâišel, vypadal jsi, že vypustíš duši.
				AI_Output (self, other, "DIA_Maleth_Hallo_08_06"); //Fajn, teë už vypadáš skoro jako èlovêk!
			};
		};

	AI_Output (self, other, "DIA_Maleth_Hallo_08_07"); //(nedùvêâivê) Co tady chceš?
};
...
instance DIA_Maleth_BANDITS		(C_INFO) //E1
{
	...
	permanent   = FALSE;
	description	= "V horách mê pâepadli bandité.";
};
...
{
	AI_Output (other, self, "DIA_Maleth_BANDITS_15_00"); //V horách mê pâepadli bandité.
	...
	AI_Output (self, other, "DIA_Maleth_BANDITS_08_02"); //Mêls obrovské štêstí. Vêtšina lidí se odtamtud nedostala živá.
};
...
instance DIA_Maleth_BanditsDEAD	(C_INFO) //E2
{
	...
	permanent   = FALSE;
	description	= "Ti bandité už vám nebudou dêlat žádné problémy.";
};
...
{
	AI_Output (other, self, "DIA_Maleth_BanditsDEAD_15_00"); //Ti bandité už vám nebudou dêlat žádné problémy.
	AI_Output (self, other, "DIA_Maleth_BanditsDEAD_08_01"); //Proè? Jsou mrtví?
	AI_Output (other, self, "DIA_Maleth_BanditsDEAD_15_02"); //Vytáhli zbraà na špatného chlapa.
...
func void DIA_Maleth_BanditsALIVE_Info ()
{
	...
	AI_Output (self, other, "DIA_Maleth_BanditsALIVE_08_01"); //Chceš na nê jít? Tak z toho mê vynech! Je to moc nebezpeèný!
	AI_Output (self, other, "DIA_Maleth_BanditsALIVE_08_02"); //A kromê toho, musím dávat pozor na ovce!
	AI_Output (self, other, "DIA_Maleth_BanditsALIVE_08_03"); //Ale jestli si myslíš, že nás té lùzy mùžeš zbavit, bude ti celá farma vdêèná.
	MIS_Maleth_Bandits = LOG_RUNNING;
	
	Log_CreateTopic (TOPIC_Maleth,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_Maleth,LOG_RUNNING);
	B_LogEntry (TOPIC_Maleth,"Jestli se mi podaâí pobít bandity na cestê z Xardasovy vêže na Lobartùv statek, jeho obyvatelé mi budou velmi vdêèní.");
...
instance DIA_Maleth_ToTheCity (C_INFO) //E1
{
	...
	permanent   = FALSE;
	description	= "Jsem na cestê do mêsta.";
};
...
{
	AI_Output (other, self, "DIA_Maleth_ToTheCity_15_00"); //Jsem na cestê do mêsta.
	
	if (Npc_HasEquippedArmor(other) == FALSE) 
	{
		AI_Output (self, other, "DIA_Maleth_ToTheCity_08_01"); //Asi budeš muset podplatit stráže, aby ses dostal, kam chceš.
		AI_Output (self, other, "DIA_Maleth_ToTheCity_08_02"); //A taky musíš vêdêt, co chtêjí slyšet.
	}
	else
	{
		AI_Output (self, other, "DIA_Maleth_ToTheCity_08_03"); //Mùžou tê pustit dovnitâ - když jim âekneš, co chtêj slyšet.
	};
	
	AI_Output (other, self, "DIA_Maleth_ToTheCity_15_04"); //A to jako?
	AI_Output (self, other, "DIA_Maleth_ToTheCity_08_05"); //Tâeba... že jsi z Lobartovy farmy a potâebuješ za mêstským kováâem.
	
	Log_CreateTopic (TOPIC_City,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_City,LOG_RUNNING);
	B_LogEntry (TOPIC_City,"Když strážím u mêstských bran âeknu, že pâicházím z Lobartova statku a že mám namíâeno ke kováâi, pustí mê dovnitâ.");
	
	
	if (Npc_HasEquippedArmor(other) == FALSE) 
	{
		AI_Output (self, other, "DIA_Maleth_ToTheCity_08_06"); //Ale asi ti to nebude moc platné. Nevypadáš jako farmáâ.
		B_LogEntry (TOPIC_City,"Samozâejmê že napâed musím vypadat taky jako sedlák.");
...
func void DIA_Maleth_Equipment_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Maleth_Equipment_08_02"); //Pokud mùžeš zaplatit za to, co chceš, Lobart ti nêco prodá.
		AI_Output (self, other, "DIA_Maleth_Equipment_08_03"); //Jinak se ho také mùžeš zeptat, jestli by pro tebe nemêl nêjakou práci.
	};
};
...
func void DIA_Maleth_LOBART_Info ()
{
	...
	AI_Output (self, other, "DIA_Maleth_LOBART_08_01"); //No, na farmê pâece! Patâí mu to tady!
	AI_Output (self, other, "DIA_Maleth_LOBART_08_02"); //A nezkoušej si s ním nêco zaèínat! Už hodnê tulákù od nêj dostalo nakládaèku a pak letêli z farmy.
};
...
instance DIA_Maleth_PROBLEME		(C_INFO)
{
	...
	description	 = 	"Nêjaký zmêny poslední dobou?";
};
...
{
	AI_Output			(other, self, "DIA_Maleth_PROBLEME_15_00"); //Nêjaký zmêny poslední dobou?
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_08_01"); //Sejdi dolù po cestê k mêstu a uvidíš.
	...
	Info_AddChoice	(DIA_Maleth_PROBLEME, "Už tê ty èerné kápê navštívily?", DIA_Maleth_PROBLEME_beidir );
	
};
func void DIA_Maleth_PROBLEME_beidir ()
{
	AI_Output			(other, self, "DIA_Maleth_PROBLEME_beidir_15_00"); //Už tê ty èerné kápê navštívily?
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_beidir_08_01"); //Jo. Zùstaly tu nêkolik dní. Jestli to chceš vêdêt, tak pâišly rovnou z pekel.
...
func void DIA_Maleth_PROBLEME_schafe ()
{
	...
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_08_01"); //(vztekle) Jako by tê to vùbec zajímalo. Máš jiné problémy.
...
func void DIA_Maleth_PROBLEME_schafe_probleme ()
{
	AI_Output			(other, self, "DIA_Maleth_PROBLEME_schafe_probleme_15_00"); //Problémy? Co ty o tom víš?
		...
		{
			AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_08_01"); //(stroze) Patâíš k mêstským strážím, ne? Tak s têma bastardama v èerných kápích nêco udêlej.
		};

		if ((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
		{
			AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_08_02"); //(vztekle) Vy zatracený žoldáci myslíte jen na jedinou vêc: jak z ostatních dostat co nejvíc penêz do vlastní kapsy.
		};

		if (hero.guild == GIL_KDF)
		{
			AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_08_03"); //Váš vznešený pán kláštera by mêl s têmi postavami v kápích nêco dêlat.
		};
...
func void DIA_Maleth_PROBLEME_schafe_probleme_geldher_auftrag ()
{
	...
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_geldher_auftrag_08_01"); //Urèitê za to mùže ta èerná pálenka, co mi Vino pâed nêkolika týdny podstrèil.
	...
	AI_Output			(other, self, "DIA_Maleth_PROBLEME_schafe_probleme_geldher_auftrag_15_03"); //A kvùli tomu dêláš takovej kravál?
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_geldher_auftrag_08_04"); //Tobê se to âekne, tvoje nebyla.
	
	Log_CreateTopic (TOPIC_MalethsGehstock, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_MalethsGehstock, LOG_RUNNING);
	B_LogEntry (TOPIC_MalethsGehstock,"Maleth se nêkde ožral jak dêlo a ztratil pâitom vycházkovou hùl. Jak ho znám, nikdy se z Lobartova statku moc nevzdálil. Proto by se ta hùl mêla válet nêkde poblíž."); 
...
func void DIA_Maleth_PROBLEME_schafe_probleme_drohen ()
{
	...
	AI_Output			(self, other, "DIA_Maleth_PROBLEME_schafe_probleme_drohen_08_01"); //Jediný, co ti jde, jsou âeèi. Nejlíp udêláš, když pùjdeš pryè.
};
...
instance DIA_Maleth_GEHSTOCK		(C_INFO)
{
	...
	description	 = 	"Mám dojem, že tahle hùl patâí tobê.";
};
...
{
	AI_Output			(other, self, "DIA_Maleth_GEHSTOCK_15_00"); //Mám dojem, že tahle hùl patâí tobê.
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_08_01"); //To je ohromný. Já...
	AI_Output			(other, self, "DIA_Maleth_GEHSTOCK_15_02"); //Tak moment. Nejdâív za ni zaplaã.
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_08_03"); //Ale... Moc toho nemám.
	AI_Output			(other, self, "DIA_Maleth_GEHSTOCK_15_04"); //V tom pâípadê na ni zapomeà.
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_08_05"); //DObrá. A co tohle? Vèera jsem tu vidêl bêžet nêjaký bandity s nêkolika mêšci zlata.
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_08_06"); //Když ti âeknu, kam schovali svou koâist, vrátíš mi za to moji hùl?
	TOPIC_END_MalethsGehstock = TRUE;
	B_GivePlayerXP (XP_FoundMalethsGehstock);

	Info_ClearChoices	(DIA_Maleth_GEHSTOCK);
	Info_AddChoice	(DIA_Maleth_GEHSTOCK, "Promià, ale to mê nezajímá. Chci zlato.", DIA_Maleth_GEHSTOCK_gold );
...
func void DIA_Maleth_GEHSTOCK_ok ()
{
	...
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_ok_08_02"); //V tý jeskyni urèitê nêco bude.

	B_LogEntry (TOPIC_MalethsGehstock,"Maleth mi vyprávêl, že v lesích západnê od Lobartova statku se nachází tábor banditù."); 
...
{
	AI_Output			(other, self, "DIA_Maleth_GEHSTOCK_gold_15_00"); //Promià, ale to mê nezajímá. Chci zlato.
	AI_Output			(self, other, "DIA_Maleth_GEHSTOCK_gold_08_01"); //Tady je všechno, co mám. Mêlo by to staèit.
...
func void DIA_Maleth_PERM3_Info ()
{
	...
	AI_Output			(self, other, "DIA_Maleth_PERM3_08_01"); //(rozèilenê) Zmizni.
	AI_StopProcessInfos (self);
};
