...
{
	AI_Output (other, self, "DIA_Cedric_CanTeach_15_00"); //Możesz mnie wyszkolić?
	
	if (other.guild == GIL_PAL)
	{
		AI_Output (self, other, "DIA_Cedric_CanTeach_12_01"); //Mogę cię nauczyć walki broniami jednoręcznymi.
		Cedric_Teach1H = TRUE;
		B_LogEntry (TOPIC_CityTeacher,"Paladyn Cedrik może mnie nauczyć walki orężem jednoręcznym.");
	}
	else
	{
		AI_Output (self, other, "DIA_Cedric_CanTeach_12_02"); //Szkolę tylko paladynów.
	};
...
INSTANCE DIA_Cedric_Teach(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Chcę się wyszkolić.";
};                       
//----------------------------------
...
{	
	AI_Output (other,self ,"DIA_Cedric_Teach_15_00"); //Chcę się wyszkolić.
	
	if (other.HitChance[NPC_TALENT_1H] >= 90)
	{
		AI_Output (self,other,"DIA_DIA_Cedric_Teach_12_01"); //Wiesz już wszystko o walce mieczem. Nie potrafię ci przekazać nic więcej.
	}
	else
	{
