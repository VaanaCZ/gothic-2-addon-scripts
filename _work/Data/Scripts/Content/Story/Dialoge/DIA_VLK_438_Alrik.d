...
{	
	...
	AI_Output (self ,other,"DIA_Alrik_Hallo_09_04"); //Ale jestli tím myslíš, co dęlám celý den, tak poâádám souboje.
};
...
INSTANCE DIA_Alrik_Regeln(C_INFO)
{
	...
	permanent	= FALSE;
	description = "Má to nęjaký pravidla?";
};                       
...
// ************************************************************
		...
		{
			AI_Output (self, other, "DIA_Alrik_Add_09_03"); //Tak co? Chceš si to se mnou znova rozdat? Myslím, že jsem se od poslednę o nęco zlepšil.
		};
...
FUNC VOID DIA_Alrik_WannaFight_Info()
{
	...
	{
		AI_Output (self ,other,"DIA_Alrik_WannaFight_09_08"); //Bojuju jenom od obęda do večera.
...
FUNC VOID DIA_Alrik_AfterFight_Info()
{
	...
	&& (self.aivar[AIV_ArenaFight] != AF_AFTER_PLUS_DAMAGE)
	{
		...
		{
			AI_Output (self, other,"DIA_Alrik_AfterFight_09_06"); //To byl dobrý boj. Sázku jsi sice prohrál - nic si z toho ale nedęlej, pâispęls na dobrou vęc! (ušklíbne se)
		}
		else //FIGHT_CANCEL
		{
	...
	else //anderer Kampfgrund gegen den Spieler ODER kein sauberer Kampf
	{
		...
		{
			AI_Output (self, other,"DIA_Alrik_AfterFight_09_10"); //Boj skončil, bâídile!
		...
		else //einfach so angegriffen (unvorbereitet)
		{
			if (Alrik_Kaempfe == 0)
			{	
				AI_Output (self, other,"DIA_Alrik_AfterFight_09_13"); //Jestlis se mnou chtęl bojovat, stačilo, abys mę prostę vyzval.
			}
			else // > 0
			{
...
FUNC VOID DIA_Alrik_WerSchwert_Info()
{
	...
	AI_Output (self ,other,"DIA_Alrik_WerSchwert_09_01"); //U jednoho obchodníka na tržišti jsem ho vymęnil za nęjaké zásoby.
...
FUNC VOID DIA_Alrik_Krieg_Info()
{
	...
	AI_Output (self ,other,"DIA_Alrik_Krieg_09_03"); //Ale moc mu to nepomůže - tęch vzpour je už je moc. Jestli válka brzo neskončí, celá âíše se rozpadne.
};
...
FUNC VOID DIA_Alrik_Ausbilden_Info()
{
	...
	else //>0
	{
		...
		B_LogEntry (Topic_CityTeacher,"Alrik mę vycvičí v boji s jednoručními zbranęmi. Potlouká se kdesi za skladištęm v pâístavní čtvrti.");
	};
};
