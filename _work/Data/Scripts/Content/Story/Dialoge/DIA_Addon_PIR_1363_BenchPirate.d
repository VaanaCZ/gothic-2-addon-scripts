...
{	
	...
	if (GregIsBack == TRUE)
	{
		if (randy == 0)
		{
			AI_Output (self ,other,"DIA_Addon_Pir_7_Hello_07_01"); //Greg už je koneènê zpátky. Už bylo naèase.
		}
		else if (randy == 1)
		{
			AI_Output (self ,other,"DIA_Addon_Pir_7_Hello_07_02"); //Francis by mêl obstarat àáký dâevo na palisádu. Doufám, že to na nêj není moc.
		}
		else 
		{
			AI_Output (self ,other,"DIA_Addon_Pir_7_Hello_07_03"); //Tak teda hurá na bandity. Když je na naší stranê Greg, nebojíme se jich.
		};
	}
	else
	{
		if (randy == 0)
		{
			AI_Output (self ,other,"DIA_Addon_Pir_7_Hello_07_04"); //Francis, velitel tábora, není nic než šašek. Copak ho nêkdo bere vážnê?
		}
		else if (randy == 1)
		{
			AI_Output (self ,other,"DIA_Addon_Pir_7_Hello_07_05"); //Greg vêâí Francisovi, protože je moc hloupej na to, aby ho moh podvádêt! Ale to z nêj pâeci nedêlá našeho šéfa.
		}
		else
		{
...
{	
	...
	AI_Output (other,self ,"DIA_Addon_BenchPirate_Anheuern_15_08"); //Pâesnê tak.
};
...
{
	AI_Output	(other, self, "DIA_Addon_BenchPirate_ComeOn_15_01"); //Kaàon èeká.
	if (C_GregsPiratesTooFar() == TRUE)
	{
		AI_Output (self ,other,"DIA_Addon_BenchPirate_ComeOn_07_02"); //Pâesnê tam teë míâím.
		AI_StopProcessInfos (self);
	}
	else
	{
...
{	
	AI_Output (other,self ,"DIA_Addon_BenchPirate_GoHome_15_00"); //Už tê nepotâebuju.
