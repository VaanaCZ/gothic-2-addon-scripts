...
{
	AI_Output (self, other, "DIA_Thekla_HALLO_17_00"); //Co dêláš tady v kuchyni?
};
...
instance DIA_Thekla_Lecker		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Nêco tady výbornê voní!";
};
...
{
	AI_Output (other, self, "DIA_Thekla_Lecker_15_00"); //Nêco tady výbornê voní!
	AI_Output (self, other, "DIA_Thekla_Lecker_17_01"); //No jasnê! Takové jako ty dobâe znám! Takových se tady po okolí potulují spousty.
	AI_Output (self, other, "DIA_Thekla_Lecker_17_02"); //Nejdâív se snaží vetâít do tvojí pâíznê a pak, když je potâebuješ, nejsou nikde k nalezení!
};
...
func void DIA_Thekla_Hunger_Info ()
{
	...
	if (other.guild == GIL_NONE)
	{
		...
		AI_Output (self, other, "DIA_Thekla_Hunger_17_02"); //(pohrdavê) A pro tu žoldáckou chátru, samozâejmê.
	}
	...
	{
		AI_Output (self, other, "DIA_Thekla_Hunger_17_05"); //Jak by se dal odmítnout Innosùv zmocnênec?
		B_GiveInvItems (self, other, ItFo_XPStew, 1);
		Thekla_GaveStew = TRUE;
	};
};
...
func void DIA_Thekla_Arbeit_Info ()
{
	...
	AI_Output (self, other, "DIA_Thekla_Arbeit_17_01"); //Chceš pracovat tady na farmê?
	AI_Output (self, other, "DIA_Thekla_Arbeit_17_02"); //O tom mùže rozhodnout pouze Onar. Farma patâí jemu, stejnê jako celé údolí.
};
...
instance DIA_Thekla_WannaJoin		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Vlastnê jsem se chtêl pâipojit k žoldnéâùm.";
};
...
{
	AI_Output (other, self, "DIA_Thekla_WannaJoin_15_00"); //Vlastnê jsem se chtêl pâipojit k žoldnéâùm.
	AI_Output (self, other, "DIA_Thekla_WannaJoin_17_01"); //Takže ty jsi další z têch lotrù z trestanecké kolonie?
	AI_Output (self, other, "DIA_Thekla_WannaJoin_17_02"); //To se dalo èekat! Nech mê na pokoji! Takových jako ty už je tady v okolí až dost!
};
...
instance DIA_Thekla_Schlafen		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Potâebuju se nêkde vyspat.";
};
...
{
	AI_Output (other, self, "DIA_Thekla_Schlafen_15_00"); //Potâebuju se nêkde vyspat.
	AI_Output (self, other, "DIA_Thekla_Schlafen_17_01"); //Ani nepomysli na to, že by ses mohl vyspat v mojí kuchyni. Najdi si nêco ve stodole.
};
...
func void DIA_Thekla_Problem_Info ()
{
	...
	AI_Output (self, other, "DIA_Thekla_Problem_17_01"); //Ach, ti pitomci mi vážnê lezou na nervy! Obzvlášã Sylvio a jeho tlustej kámoš Bullco.
	...
	if (other.guild == GIL_NONE)
	{
		AI_Output (other, self, "DIA_Thekla_Problem_15_04"); //A proè s tím teda nêco neudêláš?
		AI_Output (self, other, "DIA_Thekla_Problem_17_05"); //A co jako mám dêlat, pane Chytrolín? Praštit je váleèkem? To si mùžu dovolit na farmáâe, ale tihle podlí bastardi mi to prostê vrátí.
	};
};
...
func void DIA_Thekla_Manieren_Info ()
{
	...
	AI_Output (self, other, "DIA_Thekla_Manieren_17_01"); //Udêláš líp, když to nebudeš zkoušet, drahoušku. Co vím, tak Sylvio má magickou zbroj. Jeho nemùžeš porazit.
	AI_Output (other, self, "DIA_Thekla_Manieren_15_02"); //A co Bullco?
	AI_Output (self, other, "DIA_Thekla_Manieren_17_03"); //Ten chlap je silnej jako hovado. Co pamatuju, srazil každýho, kdo jemu nebo Sylviovi dêlal potíže.
};
...
func void DIA_Thekla_AfterFight_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Thekla_AfterFight_17_00"); //Vážnê bys mêl nechat Bullca na pokoji, drahoušku.
	...
	else //Sylvio oder Bullco gewonnen
	{
		...
		AI_Output (self, other, "DIA_Thekla_AfterFight_17_03"); //Neâíkala jsem ti to snad? Teë víš, co jsem mêla na mysli.
		AI_Output (other, self, "DIA_Thekla_AfterFight_15_04"); //Jsem rád, že se dobâe bavíš.
		AI_Output (self, other, "DIA_Thekla_AfterFight_17_05"); //Tak teë tak neprotahuj oblièej. Nejsi první, kdo od toho neâáda dostal napráskáno.
	};
	
	AI_Output (self, other, "DIA_Thekla_AfterFight_17_06"); //Tady, dej si trochu vývaru, aã se ti vrátí síla.
	B_GiveInvItems (self, other, ItFo_XPStew, 1);	
	Thekla_GaveStew = TRUE;
};
...
func void DIA_Thekla_SagittaPaket_Info ()
{
	...
	AI_Output (self, other, "DIA_Thekla_SagittaPaket_17_01"); //Díky moc. Tak jsi byl pâece jen k nêèemu dobrý.
...
instance DIA_Thekla_PERM		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Mùžu dostat ještê trochu toho vývaru?";
};
...
{
	AI_Output (other, self, "DIA_Thekla_PERM_15_00"); //Mùžu dostat ještê trochu toho vývaru?
	...
	{	
		...
		AI_Output (other, self, "DIA_Thekla_PERM_15_04"); //A mùžu alespoà vylízat hrnec?
		AI_Output (self, other, "DIA_Thekla_PERM_17_05"); //Necháš toho?!
		AI_Output (self, other, "DIA_Thekla_PERM_17_06"); //Když tak prahneš po mém vývaru, mêl bys pro to nêco udêlat.
		AI_Output (other, self, "DIA_Thekla_PERM_15_07"); //Co?
		AI_Output (self, other, "DIA_Thekla_PERM_17_08"); //Zajdi za Sagittou, léèitelkou, co žije za Sekobovou farmou, a pâines mi od ní svazek bylinek.
		AI_Output (self, other, "DIA_Thekla_PERM_17_09"); //Když mi pâineseš ty bylinky, udêlám ti trochu vývaru.
		...
		B_LogEntry (TOPIC_TheklaEintopf,"Když Thekle pâinesu byliny od léèitelky Sagitty, uvaâí mi ještê další lektvar. Sagitta pâebývá kousek za Sekobovým statkem.");
	...
	else if (MIS_Thekla_Paket == LOG_SUCCESS)
	{
		if (Kapitel <= 2)
		{
			...
			{
				AI_Output (self, other, "DIA_Thekla_PERM_17_10"); //Dobrá. Tak budu hodná. Tady. Aã mi neumâeš hlady pâed oèima.
				B_GiveInvItems (self, other, ItFo_XPStew, 1);
				Thekla_MehrEintopfKap1 = TRUE;
			}
			else 
			{
				AI_Output (self, other, "DIA_Thekla_PERM_17_11"); //Hej, hej, hej, ne tak rychle! Dám ti vêdêt, až pro tebe najdu nêjakou práci.
		...
		|| (Kapitel == 4)
		{
			...
			{
				AI_Output (self, other, "DIA_Thekla_PERM_17_13"); //Slyšela jsem, žes pomohl Bennetovi z vêzení. Dobrá práce, hochu.
				B_GiveInvItems (self, other, ItFo_XPStew, 1);
			...
			else 
			{
				if (MIS_RescueBennet != LOG_SUCCESS)
				{
					...
					AI_Output (self, other, "DIA_Thekla_PERM_17_15"); //Udêlej mi laskavost a dostaà ho odtamtud, jo? A já ti zatím udêlám trochu toho bájeènýho vývaru.
				}
				else
				{
					AI_Output (self, other, "DIA_Thekla_PERM_17_16"); //Už mi nic nezbylo. Vraã se pozdêjc.
				};
			};
		};
		...
		if (Kapitel >= 5)	
		{
			...
			{
				AI_Output (self, other, "DIA_Thekla_PERM_17_17"); //Ty jsi poâád hladovej, co? A co vlastnê poâád dêláš?
			...
			{
				AI_Output (self, other, "DIA_Thekla_PERM_17_20"); //Pozdê. Už žádný vývar nezbyl.
			};
		};
	}
