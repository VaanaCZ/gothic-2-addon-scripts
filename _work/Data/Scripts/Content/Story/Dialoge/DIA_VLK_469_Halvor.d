...
{
	AI_Output (self, other, "DIA_Halvor_Hallo_06_00"); //Hej, nepotâebuješ nêjaké ryby? Vyber si tady, èerstvé, pâímo z moâských hlubin!
...
instance DIA_Addon_Halvor_MissingPeople		(C_INFO)
{
	...
	description	 = 	"Nevíš nêco o têch pohâešovaných lidech?";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Halvor_MissingPeople_15_00"); //Mùžeš mi âíct nêco o têch zmizelých lidech?
	AI_Output	(self, other, "DIA_Addon_Halvor_MissingPeople_06_01"); //O zmizelých lidech? Tady jsi v pâístavu, kámo. Takový vêci se tu stávají.
	AI_Output	(self, other, "DIA_Addon_Halvor_MissingPeople_06_02"); //Moâe je zrádný a to prostê sem tam nêkoho stojí i život.
	AI_Output	(self, other, "DIA_Addon_Halvor_MissingPeople_06_03"); //Kapitáni velkých lodí si v pâístavech, jako je Khorinis, vždycky doplàují posádku.
	AI_Output	(self, other, "DIA_Addon_Halvor_MissingPeople_06_04"); //A když nenajdou žádné dobrovolníky, prostê nêkoho na loë odvleèou i proti jeho vùli. A takoví lidi se pak pohâešujou.
	...
	Info_AddChoice	(DIA_Addon_Halvor_MissingPeople, "Nevíš o nêkom, kdo se pohâešuje?", DIA_Addon_Halvor_MissingPeople_wer );
	Info_AddChoice	(DIA_Addon_Halvor_MissingPeople, "A ty sám také nêkoho postrádáš?", DIA_Addon_Halvor_MissingPeople_you );
};
...
{
	AI_Output			(other, self, "DIA_Addon_Halvor_MissingPeople_wer_15_00"); //Víš o nêkom, kdo se pohâešuje?
	AI_Output			(self, other, "DIA_Addon_Halvor_MissingPeople_wer_06_01"); //Nemám páru. Na to se zeptej spíš loëaâù.

	Log_CreateTopic (TOPIC_Addon_WhoStolePeople, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Addon_WhoStolePeople, LOG_RUNNING);
	B_LogEntry (TOPIC_Addon_WhoStolePeople,"Prodavaè ryb Halvor mi pouze poradil, že bych se mêl poptat u loëaâù."); 
};
...
instance DIA_Halvor_MESSAGE		(C_INFO)
{
	...
	permanent    =  FALSE;
	description	 = 	"Mám dojem, že tenhle kousek papíru patâí tobê...";
	trade		 =  FALSE;
};
...
{
	AI_Output (other, self, "DIA_Halvor_MESSAGE_15_00"); //Mám dojem, že tenhle kousek papíru patâí tobê.
	...
	AI_Output (self, other, "DIA_Halvor_MESSAGE_06_03"); //(uspêchanê) Ale není žádný dùvod jít pâímo za domobranou, jasný? Jsem si jist, že se nêjak dohodneme, co?
	
	Info_ClearChoices (DIA_Halvor_MESSAGE);
	Info_AddChoice    (DIA_Halvor_MESSAGE,"Tak to mi udêlej nabídku.",DIA_Halvor_MESSAGE_OFFER);
...
{
	AI_Output (other, self, "DIA_Halvor_MESSAGE_OFFER_15_00"); //Tak to mi udêlej nabídku.
	AI_Output (self, other, "DIA_Halvor_MESSAGE_OFFER_06_01"); //Dobrá. Nêco ti povím. Prodávám víc než jen ryby.
	AI_Output (self, other, "DIA_Halvor_MESSAGE_OFFER_06_03"); //Když mi dáš ten smêšný kousek papíru a, ehm... zapomeneš na tu vêc, mùžeme se stát obchodními partnery.
	
	Info_ClearChoices (DIA_Halvor_MESSAGE);
	Info_AddChoice    (DIA_Halvor_MESSAGE,"No dobâe, souhlasím.",DIA_Halvor_MESSAGE_Okay);
	Info_AddChoice    (DIA_Halvor_MESSAGE,"Vypadá to, že patâíš do basy.",DIA_Halvor_MESSAGE_PRISON);
	Info_AddChoice    (DIA_Halvor_MESSAGE,"Vážnê? Co by to tak mohlo být za zboží?",DIA_Halvor_MESSAGE_DEAL);
};	
FUNC VOID DIA_Halvor_MESSAGE_PRISON()
{
	AI_Output (other, self, "DIA_Halvor_MESSAGE_PRISON_15_00"); //Vypadá to, že patâíš do basy.
	AI_Output (self, other, "DIA_Halvor_MESSAGE_PRISON_06_01"); //Ne, poèkej, to nemùžeš udêlat! Vždyã pâece nejsem, vždyã víš... nic jinýho než malá ryba.
	AI_Output (other, self, "DIA_Halvor_MESSAGE_PRISON_15_02"); //To sis mêl rozmyslet dâív. Teë mám v úmyslu tê nahlásit lordu Andremu.
...
{
	AI_Output (other, self, "DIA_Halvor_MESSAGE_DEAL_15_00"); //Vážnê? Co by to tak mohlo být za zboží?
...
FUNC VOID DIA_Halvor_MESSAGE_Okay()
{
	...
	AI_Output (self, other, "DIA_Halvor_MESSAGE_Okay_06_01"); //Díky. Zase se zastav, až budeš poblíž. Urèitê pro tebe budu mít nêco zajímavýho.
...
FUNC VOID DIA_Halvor_Zeichen_Info()
{
	...
	AI_Output (self, other, "DIA_Halvor_Zeichen_06_01");//(potichu) Udêlám ti nabídku. Jestli máš nêjaké stâíbrné tácy nebo poháry, vezmu si je od tebe za dobrou cenu.
...
FUNC VOID DIA_Halvor_Hehlerei_Info()
{
	...
	if  (Halvor_Day != Wld_GetDay())
	{
		...
		|| (Npc_HasItems (other,ItMi_SilverCup) >= 1)
		{
			...
			{
				AI_Output (self, other, "DIA_Halvor_Zeichen_06_06");//Pâinesl jsi moc zboží. Nemùžu ti dát tolik zlata nejednou.
				Info_ClearChoices (DIA_Halvor_Hehlerei);
			};
		}
		else 
		{
			AI_Output (self, other, "DIA_Halvor_Zeichen_06_02");//Vraã se, až budeš mít nêjaké stâíbrné podnosy nebo poháry.
			Info_ClearChoices (DIA_Halvor_Hehlerei);
		};
	}
	else if (Wld_GetDay() == 0)
	{
		AI_Output (self, other, "DIA_Halvor_Zeichen_06_03");//Mùžeš se toho stâíbra zbavit zítra. Dneska je moc brzo.
	}
	else 
	{
		AI_Output (self, other, "DIA_Halvor_Zeichen_06_04");//Lituju, dneska už žádné obchody. Vraã se zítra, dobrá?
	};
...
FUNC VOID DIA_Halvor_Hehlerei_Annehmen()
{
 	...
 	*/
 	...
 	AI_Output (self, other, "DIA_Halvor_Zeichen_Annehmen_06_01");//Udêlali jsme dneska dobrej kšeft. Lepší to bude nejdâív zítra, jinak by nêkdo mohl dostat podezâení, však víš.
...
func void DIA_Halvor_Crew_Info ()
{
	...
	Info_AddChoice (DIA_Halvor_Crew,"To je èistê moje vêc.",DIA_Halvor_Crew_MyThing);
...
{
	AI_Output (other,self ,"DIA_Halvor_Crew_MyThing_15_00"); //To je èistê moje vêc.
	AI_Output (self ,other,"DIA_Halvor_Crew_MyThing_06_01"); //Jak je libo. To opravdu není moje vêc.
...
FUNC VOID DIA_Halvor_Crew_StealShip()
{
	...
	AI_Output (self ,other,"DIA_Halvor_Crew_StealShip_06_01"); //Jo, jasnê. Jinak seš zdravej? Paladinové s tebou udêlaj krátkej proces, až na to pâijdou.
...
FUNC VOID DIA_Halvor_Crew_JoinMe()
{
	...
	AI_Output (self ,other,"DIA_Halvor_Crew_JoinMe_06_01"); //Ne, mám dojem, že ne. Mám tady spoustu práce a upâímnê, netoužím zrovna po tom, abych potkal otrokáâskou galéru plnou skâetù.
	AI_Output (self ,other,"DIA_Halvor_Crew_JoinMe_06_02"); //Ale pâeju ti hodnê štêstí.
};

FUNC VOID DIA_Halvor_Crew_HelpMe()
{
	AI_Output (other,self ,"DIA_Halvor_Crew_HelpMe_15_00"); //Mùžeš mi pomoct?
	AI_Output (self ,other,"DIA_Halvor_Crew_HelpMe_06_01"); //Já na tohle nejsem ten pravej èlovêk. Ale zajdi si promluvit s Jackem. Mêl by se poflakovat nêkde tady po pâístavu. On bude vêdêt nejlíp, co na tu plavbu budeš potâebovat.
};
