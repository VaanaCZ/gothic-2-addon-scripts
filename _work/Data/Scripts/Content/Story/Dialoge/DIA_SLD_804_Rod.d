...
{	
	...
	{
		AI_Output (self, other, "DIA_Rod_Hello_06_01"); //(pohrdavě) Co ode mě chceš, blbečku?
...
{	
	AI_Output (other, self, "DIA_Rod_WannaLearn_15_00"); //Nemůžeš mě naučit, jak zacházet s obouručními zbraněmi?
	...
	{
		AI_Output (self, other, "DIA_Rod_WannaLearn_06_04"); //Poslouchej, chlapče. K tomu, abys mohl nosit obouruční zbraň, potřebuješ mít velkou sílu.
...
{	
	AI_Output (other, self, "DIA_Rod_WannaJoin_15_00"); //Chci se přidat k žoldnéřům!
	...
	|| 		(Rod_WetteGewonnen == TRUE)
	{
		...
		{
			AI_Output (self, other, "DIA_Rod_WannaJoin_06_05"); //No dobře, máš pořádný meč. Zbytek se naučíš.
		};
		
		AI_Output (self, other, "DIA_Rod_WannaJoin_06_06"); //Co se mě týče, tak já tě beru.
		B_LogEntry (TOPIC_SLDRespekt,"Budu-li se chtít připojit k žoldnéřům, Rod se za mě přimluví.");
	}
	else
	{
...
{	
	...
	B_LogEntry (Topic_RodWette,"Žoldnéř Rod si myslí, že nedokážu uzvednout jeho meč.");
};	
...
{	
	...
	B_LogEntry (Topic_RodWette,"Rod se se mnou vsadil o 30 zlaťáků, že neuzvednu jeho meč.");
};
...
func void DIA_Rod_Wette_Yes()
{
	...
	if (B_GiveInvItems (other,self, itmi_gold, 30))
	{
		...
		AI_Output (self, other, "DIA_Rod_Wette_Yes_06_03"); //(škodolibě) V pořádku, podíváme se teda, jak si silnej...
		...
		if (other.attribute[ATR_STRENGTH] >= 30)
		{
			...
			AI_Output (self, other, "DIA_Rod_Wette_Yes_06_05");//(zmatený) Vypadá to, žes mě dostal.
			AI_Output (self, other, "DIA_Rod_Wette_Yes_06_06");//Tohle jsem vůbec nečekal. Opravdu nevypadáš na to, že máš v sobě tolik síly.
...
func void DIA_Rod_Wette_GiveBack()
{
	...
	Info_ClearChoices (DIA_Rod_Wette);
	Info_AddChoice (DIA_Rod_Wette, "(předat mu zbraň)", DIA_Rod_Wette_GiveBack2);
};
...
func void DIA_Rod_Wette_KeepIt()
{
	...
	AI_Output (self, other, "DIA_Rod_Wette_KeepIt_06_01"); //(výhružně) Co to bylo?
	AI_Output (other, self, "DIA_Rod_Wette_KeepIt_15_02"); //Myslím, že si ho ještě  chvíli nechám.
...
{	
	...
	if (Kapitel <= 2)	
	{
		if (other.guild == GIL_NONE)
		{
			...
			{
				AI_Output (self, other, "DIA_Rod_PERM_06_02"); //Jsem zvědavý, jestli ti dovolí se k nim přidat.
			};
		}
	...
	if (Kapitel >= 3)
	{
		AI_Output (self, other, "DIA_Rod_PERM_06_05"); //Jestli jsou všechny ty povídačky o dracích skutečně pravdivé, měli bychom dát pár lidí dohromady a jít ty mrchy pozabíjet!
	};
