...
FUNC VOID DIA_Addon_Franco_EXIT_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Addon_Franco_EXIT_08_00");//Nesmíš zabloudit, jinak si tê žraloci dají jako zákusek.
		Franco_Exit = TRUE;
	};
...
{	
	...
	B_LogEntry (Topic_Addon_Franco,"Když pro Franca odvedu dobrou práci, pustí mê do tábora.");
	
	AI_Output (self,other,"DIA_Addon_Franco_HI_08_00");//Hele, co tu dêláš? Chceš se dostat do tábora?
	...
	AI_Output (self,other,"DIA_Addon_Franco_HI_08_03");//Takže jestli chceš dobrou práci, zavedu tê do tábora.
	
	if (Ramon_News == FALSE)
	{
		AI_Output (self,other,"DIA_Addon_Franco_HI_08_04");//Vrátný Ramon má pâehled, jestli potâebujou nový lidi. Promluv si s ním.
		B_LogEntry (Topic_Addon_Franco,"Mám se optat vrátného Ramona, jestli v táboâe nepotâebují nêkoho nového.");
	};
};
//---------------------------------------------------------------------
...
INSTANCE DIA_Addon_Franco_Hai   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Thorus potâebuje nêkoho nového.";
};
...
{	
	...
	AI_Output (other,self,"DIA_Addon_Franco_Hai_15_00");//Thorus potâebuje novýho èlovêka.
	...
	AI_Output (self,other,"DIA_Addon_Franco_Hai_08_03");//Tys ještê nic neudêlal.
	AI_Output (other,self,"DIA_Addon_Franco_Hai_15_04");//Už jsem toho udêlal spoustu, akorát že o tom nevíš.
	...
	B_LogEntry (Topic_Addon_Franco,"Franco chce, abych napâed pomohl Loganovi. Teprve pak mê pustí do tábora.");
};
//---------------------------------------------------------------------
//	Info Logan Wo?
//---------------------------------------------------------------------
...
{	
	...
	AI_Output (self,other,"DIA_Addon_Franco_Wo_08_01");//Když se postavíš pâed vchod do tábora, dej se podél kamenný stêny nalevo. Je to kousek dál v bažinê.
	AI_StopProcessInfos (self);
};
//---------------------------------------------------------------------
//	Info Logan tot
...
{	
	...
	AI_Output (self,other,"DIA_Addon_Franco_tot_08_01");//Sakra. Byl to nejlepší lovec, jakýho jsme tu mêli. Zatracená vêc!
	AI_Output (self,other,"DIA_Addon_Franco_tot_08_02");//Ale stejnê už jsem do tábora poslal nêkoho jinýho.
...
{	
	...
	AI_Output (self,other,"DIA_Addon_Franco_HaiSuccess_08_02");//No dobâe, tak jsi pobil pár moèálovejch žralokù – pâece jenom nejsi úplnej budižknièemu. Aspoà jsi lepší než vêtšina zdejších chlapù.
	B_GivePlayerXP (XP_Addon_HlpLogan);
	
	B_LogEntry (Topic_Addon_Franco,"Sice jsem pomohl Loganovi, ale Francovi to nestaèilo a stále mê nechce poslat do tábora.");
};
...
{	
	AI_Output (other,self,"DIA_Addon_Franco_MIS2_15_00");//Tak mùžu už teë do tábora?
	AI_Output (self,other,"DIA_Addon_Franco_MIS2_08_01");//Hele, potâebuju od tebe ještê jednu vêc.
	AI_Output (self,other,"DIA_Addon_Franco_MIS2_08_02");//Pâed pár dny jsem poslal do bažin Egora.
	AI_Output (self,other,"DIA_Addon_Franco_MIS2_08_03");//Mêl mi najít starou kamennou tabulku, ale ještê se nevrátil.
	AI_Output (self,other,"DIA_Addon_Franco_MIS2_08_04");//Zjisti, co dêlá, a tu zatracenou tabulku mi dones.
	...
	B_LogEntry (Topic_Addon_Stoneplate,"Franco chce kamennou tabulku, kterou mu mêl pâinést Edgor. Teë to mám vyâídit já.");
	
	B_LogEntry (Topic_Addon_Franco,"Franco chce, abych pomohl Edgorovi. Teprve pak mê pošle do tábora.");
};
...
{	
	...
	AI_Output (other,self,"DIA_Addon_Franco_While_15_02");//Dostanu tu za práci v bažinê nêco zaplaceno?
	AI_Output (self,other,"DIA_Addon_Franco_While_08_03");//(mimoâádnê pâátelsky) Chtêl bys zlato? Ale jistê, kolik by sis tak pâedstavoval? Padesát zlatejch nuggetù? Nebo stovku? Dám ti tolik, kolik uneseš...
	AI_Output (self,other,"DIA_Addon_Franco_While_08_04");//(nahlas) Kdo si myslíš že jsi? Dêláš si ze mê blázny?
	AI_Output (self,other,"DIA_Addon_Franco_While_08_05");//Tak vypal a koukej nêco dêlat! Jinak nedostaneš ANI DROBEK zlata!
	AI_StopProcessInfos(self);
};
...
{	
	...
	AI_Output (self,other,"DIA_Addon_Franco_WOEDGOR_08_01"); //Chceš vêdêt, jak to udêlat rychle, nebo jednoduše.
...
{
	AI_Output (self,other,"DIA_Addon_Franco_There_08_00"); //Je to tu samá rozvalina. Nêkde tam posedává.
	AI_Output (self,other,"DIA_Addon_Franco_There_08_01"); //Poèítám, že byl dost chytrej, aby si rozdêlal oheà. Aspoà TEN bys moh najít.
};
...
func void DIA_Addon_Franco_WOEDGOR_Easy()
{
	...
	AI_Output (self,other,"DIA_Addon_Franco_WOEDGOR_Easy_08_01"); //Bêž po cestê od pâední brány. Nêkde by tam mêl postávat Sancho.
...
func void DIA_Addon_Franco_WOEDGOR_Fast()
{
	...
	AI_Output (self,other,"DIA_Addon_Franco_WOEDGOR_Fast_08_01"); //Fajn. Odsud vyraž úhlopâíènê pâes chodník až do bažiny.
...
{	
	...
	B_LogEntry (Topic_Addon_Franco,"Pâinesl jsem Francovi kamennou tabulku, kterou po mnê chtêl.");
};
...
INSTANCE DIA_Addon_Franco_JemandAnderen   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "A co bude teda s tím táborem? Už mê tam koneènê pustíš?";
};
...
{	
	AI_Output (other,self,"DIA_Addon_Franco_JemandAnderen_15_00");//A co bude s tím táborem? Mùžu už koneènê dovnitâ?
	...
	{
		AI_Output (self,other,"DIA_Addon_Franco_JemandAnderen_08_02");//Ne, už jsem tam poslal nêkoho jinýho.
	};
	
	AI_Output (self,other,"DIA_Addon_Franco_JemandAnderen_08_03");//A zrovna teë už v táboâe nikoho dalšího nepotâebujou.
	AI_Output (self,other,"DIA_Addon_Franco_JemandAnderen_08_04");//Krom toho bych tê fakt využil tady, (šklebí se), když už tu není Logan.
		
	B_LogEntry (Topic_Addon_Franco,"Franco mê do tábora prostê nepustí. Myslím, že mu to budu muset vysvêtlit ruènê.");
};
...
{	
	AI_Output (other,self,"DIA_Addon_Franco_Fight_15_00"); //OKAMŽITÊ mê tam pošli!
	AI_Output (self,other,"DIA_Addon_Franco_Fight_08_01"); //(nebezpeènê pâátelsky) A co když âeknu ne?
	
	Info_ClearChoices (DIA_Addon_Franco_Fight);
	Info_AddChoice (DIA_Addon_Franco_Fight, "Tak teda ještê chvíli poèkám." ,DIA_Addon_Franco_Fight_Nothing);
...
{
	AI_Output (other,self,"DIA_Addon_Franco_Fight_Duel_15_00"); //Pak toho budeš prostê litovat.
	AI_Output (self,other,"DIA_Addon_Franco_Fight_Duel_08_01"); //Ty se mi opovažuješ vyhrožovat? TY chceš vyhrožovat MNÊ? Tak to si koleduješ vo poâádnou lekci!
	Info_ClearChoices (DIA_Addon_Franco_Fight);
	AI_StopProcessInfos(self);
...
func void DIA_Addon_Franco_Fight_Nothing()
{
	...
	AI_Output (self,other,"DIA_Addon_Franco_Fight_Nothing_08_01"); //(posmêšnê) Pâesnê tak.
...
INSTANCE DIA_Addon_Franco_Pig   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Ty hajzle! Za tohle tê dostanu!";
};
...
{	
	AI_Output (other,self,"DIA_Addon_Franco_Pig_15_00");//Ty svinê! Já tê dostanu!
	AI_Output (self,other,"DIA_Addon_Franco_Pig_08_01");//(podráždênê) Ty mizernej malej ubožáku, ty si dovoluješ na mê? Jen poèkej!
