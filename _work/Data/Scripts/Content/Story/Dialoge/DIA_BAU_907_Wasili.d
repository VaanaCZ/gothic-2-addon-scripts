...
{
	AI_Output (self, other, "DIA_Wasili_HALLO_01_00"); //Neodvažuj se ani pomyslet na to, že bys tu mohl nêco ukrást. Jinak slízneš ránu do hlavy, jasný?
};
...
func void DIA_Wasili_Job_Info ()
{
	...
	AI_Output (self, other, "DIA_Wasili_Job_01_01"); //Onar po mnê chce, abych celý den dohlížel na jeho vêci.
	AI_Output (self, other, "DIA_Wasili_Job_01_02"); //Dêlá si starosti, že se nêco pokazí. A já taky - na tomhle místê...
	AI_Output (self, other, "DIA_Wasili_Job_01_03"); //Vêtšina z têch žoldákù, co najal, jsou bývalí vêzni z trestanecké kolonie.
...
func void DIA_Wasili_Sammler_Info ()
{
	...
	AI_Output (self, other, "DIA_Wasili_Sammler_01_01"); //Jo, a spousta z nêj je hodnê drahá. Onar sbírá cennosti.
	AI_Output (self, other, "DIA_Wasili_Sammler_01_02"); //Obyèejný èlovêk jako já si takový luxus nemùže dovolit. Záleží mi na jiných vêcech.
...
func void DIA_Wasili_FirstOldCoin_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Wasili_FirstOldCoin_01_03"); //Dám ti za nê zlaãák. Pâesnê tolik, jaká je jejich hodnota.
	}
	else
	{
		AI_Output			(self, other, "DIA_Wasili_FirstOldCoin_01_04"); //Ty moc dobâe víš, kolik jsem za nê ochoten zaplatit, ty pse. Jeden zlatý. Nic víc.
	};
...
func void DIA_Wasili_FirstOldCoin_ok ()
{
	...
	{
		AI_Output			(self, other, "DIA_Wasili_FirstOldCoin_ok_01_03"); //Kdybys jich mêl takových víc, víš, kde mê najdeš.
...
func void DIA_Wasili_FirstOldCoin_mehr ()
{
	...
	else
	{
		...
		Info_AddChoice	(DIA_Wasili_FirstOldCoin, "V tom pâípadê nejsou 3 tak špatné.", DIA_Wasili_FirstOldCoin_ZumTeufel );
	};
};
...
{
	AI_Output			(other, self, "DIA_Wasili_FirstOldCoin_ZumTeufel_15_00"); //V tom pâípadê nejsou 3 tak špatné.
...
func void DIA_Wasili_BringOldCoin_Info ()
{
	...
	AI_Output			(self, other, "DIA_Wasili_BringOldCoin_01_01"); //Jasnê. Máš ještê nêjaké?
...
instance DIA_Wasili_PERM	(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Už se nêkdo pokusil nêco ukrást?";
};
...
{
	AI_Output (other, self, "DIA_Wasili_PERM_15_00"); //Už se nêkdo pokusil nêco ukrást?

	if (Kapitel <= 2)
	{
		if (PETZCOUNTER_Farm_Theft > 0)
		{
			AI_Output (self, other, "DIA_Wasili_PERM_01_01"); //Myslíš kromê tebe?
		};
	...
	if (Kapitel == 3)
	{
		AI_Output (self, other, "DIA_Wasili_PERM_01_03"); //Pâed pár dny se v noci jeden ze žoldnéâù plížil po domê.
		AI_Output (self, other, "DIA_Wasili_PERM_01_04"); //Mêl na sobê èernou róbu s kapucí, takže jsem ho nemohl poznat.
