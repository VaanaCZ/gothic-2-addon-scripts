...
{
	AI_Output (self, other, "DIA_Addon_Fisk_Hi_12_00");//Hej, jestli tady v táboâe hledáš ŕákýho slušnýho obchodníka, tak jsi ho právę našel.
	AI_Output (self, other, "DIA_Addon_Fisk_Hi_12_01");//Mám chatrč pâímo za Estebanem, takže kdybys nęco potâeboval, zastav se.
...
FUNC VOID DIA_Addon_Fisk_Attentat_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Attentat_12_01"); //(nevinnę) Mám jiný starosti, já sám jsem obęă.
	AI_Output (other, self, "DIA_Addon_Fisk_Attentat_15_02"); //Jak to?
	AI_Output (self, other, "DIA_Addon_Fisk_Attentat_12_03"); //Za poslední dodávku jsem vysolil pęknou hromadu zlata.
	AI_Output (self, other, "DIA_Addon_Fisk_Attentat_12_04"); //Ale nikdy jsem za ni nic nedostal! Ŕákej hajzl to prostę stopil!
	AI_Output (self, other, "DIA_Addon_Fisk_Attentat_12_05"); //(rozzuâenę) Okradli mę, a nejspíš nękdo z našich vlastních lidí!
};
...
FUNC VOID DIA_Addon_Fisk_Lieferung_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Lieferung_12_01"); //Balík paklíčů. Nękdo mi je męl vyzvednout u pirátů.
	AI_Output (self, other, "DIA_Addon_Fisk_Lieferung_12_02"); //Ale jistej Juan mi je čmajznul. (vztekle) Včetnę zlata, který jsem za nę zaplatil!
	AI_Output (self, other, "DIA_Addon_Fisk_Lieferung_12_03"); //Ten šmejd se teë ukrejvá nękde v bažinę.
	...
	B_LogEntry (Topic_Addon_Fisk,"Jistý Juan ukradl Fiskovi zásilku zboží (balík paklíčů). Teë se skrývá kdesi v bažinách.");
};
...
//---------------------------------------------------------------------
	...
	{	
		if (Npc_IsDead(Juan))
		{
			...
			AI_Output (self, other, "DIA_Addon_Fisk_AboutJuan_12_01"); //Dobâe, aspoŕ už od nęj bude pokoj.
		}
		else
		{
			...
			AI_Output (self, other, "DIA_Addon_Fisk_AboutJuan_12_05"); //Nevadí – já se s tęma hajzlama vypoâádám pozdęjc.
		};
	};
//---------------------------------------------------------------------
	func void B_Addon_Fisk_Belohnung()
	{
		...
		AI_Output (self, other, "DIA_Addon_Fisk_Belohnung_12_02"); //Dohlídnu, aby ti odteëka prodával brnęní fakt levnę.
		...
		if (other.guild  == GIL_KDF)
		{
			...
			AI_Output (self, other, "DIA_Addon_Fisk_Belohnung_12_04"); //Je to jediná vęc, co pâežila můj útęk z bariéry. Myslím, že ho využiješ víc jak já.
		...
		AI_Output (self, other, "DIA_Addon_Fisk_Belohnung_12_06"); //(uličnicky) Ruka ruku myje – to se nikdy nezmęní.
	};
//---------------------------------------------------------------------
...
FUNC VOID DIA_Addon_Fisk_GivePaket_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_GivePaket_12_01"); //(potęšenę) Vážnę! (pâekvapenę) A co Juan?
...
instance DIA_Addon_Fisk_PaketOpen (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Otevâel jsem tvůj balík!";
};
...
{
	AI_Output (other, self, "DIA_Addon_Fisk_PaketOpen_15_00"); //Otevâel jsem ten tvůj balík!
	AI_Output (self, other, "DIA_Addon_Fisk_PaketOpen_12_01"); //(smęje se) Samozâejmę! Ses prostę nemoh dočkat, co?
	AI_Output (self, other, "DIA_Addon_Fisk_PaketOpen_12_02"); //Máš aspoŕ ještę ty paklíče?
	AI_Output (self, other, "DIA_Addon_Fisk_PaketOpen_12_03"); //A hlavnę, co je s Juanem?
...
FUNC VOID DIA_Addon_Fisk_GivePicks_Info()
{
	...
	{
		AI_Output (self, other, "DIA_Addon_Fisk_GivePicks_12_01"); //Výbornę. Pár lidí už na tuhle dodávku čeká jak na smilování.
		B_Addon_Fisk_Belohnung();
	...
	{
		AI_Output (self, other, "DIA_Addon_Fisk_GivePicks_12_02"); //Tohle není tucet! (potutelnę) Chceš mę snad napálit?
	};
};
...
FUNC VOID DIA_Addon_Fisk_Inspektor_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_12_01"); //(pâekvapenę) Cože?
	AI_Output (other, self, "DIA_Addon_Fisk_Inspektor_15_02"); //Nehraj si se mnou! Tys o tom vędęl!
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_12_03"); //(klidnę) No dobâe, máš pravdu. Ale doufám, že si taky uvędomuješ, proč jsem ti to neâek.
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_12_04"); //Všichni v táboâe si myslí, že pracuješ pro Estebana.
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_12_05"); //A já se do tý záležitosti rozhodnę nechci namočit jenom proto, že seš celej žhavej, jen aby ti dal červenej kámen.
	
	Info_ClearChoices (DIA_Addon_Fisk_Inspektor);
	Info_AddChoice (DIA_Addon_Fisk_Inspektor,"Já vlastnę nepracuju pro Estebana.",DIA_Addon_Fisk_Inspektor_AntiEsteban);
...
func void DIA_Addon_Fisk_Inspektor_DontBelieve()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_DontBelieve_12_01"); //Vęâ si, čemu chceš, ale varuju tę! Jestli o mnę chceš Estebanovi vyprávęt ŕáký pohádky, pak budeš další na âadę!
...
{
	AI_Output (other, self, "DIA_Addon_Fisk_Inspektor_AntiEsteban_15_00"); //Já pro Estebana až tak úplnę nepracuju.
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_AntiEsteban_12_03"); //Nevęâ NIKOMU, kdo má nęco společnýho s Estebanem, ani mnę. Kapišto?
	AI_Output (self, other, "DIA_Addon_Fisk_Inspektor_AntiEsteban_12_04"); //A teë už toho nechme, aă nás nękdo neslyší.
...
FUNC VOID DIA_Addon_Fisk_Meeting_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Meeting_12_02");//Nikoho z lidí, co pro mę pracujou, nechce pustit do dolů. Dává si bacha, aby VŠICHNI makali pro nęj a jedinę pro nęj.
	...
	{
		Info_AddChoice (DIA_Addon_Fisk_Meeting,"Taky bych tę mohl Estebanovi prodat.",DIA_Addon_Fisk_Meeting_sell);
	};
};
FUNC VOID DIA_Addon_Fisk_Meeting_You()
{
	AI_Output (other, self, "DIA_Addon_Fisk_Meeting_You_15_00");//A ty ho chceš nahradit?
	AI_Output (self, other, "DIA_Addon_Fisk_Meeting_You_12_01");//Ne, já jsem kupec – jen si chci v klidu hledęt svýho.
};
FUNC VOID DIA_Addon_Fisk_Meeting_sell()
{
	AI_Output (other, self, "DIA_Addon_Fisk_Meeting_sell_15_00");//Mohl bych tę Estebanovi prodat.
	AI_Output (self, other, "DIA_Addon_Fisk_Meeting_sell_12_01");//A co? Co si myslíš, že se pak stane? Myslíš, že ti dá, co chceš?
	AI_Output (self, other, "DIA_Addon_Fisk_Meeting_sell_12_02");//Zapomeŕ na to – mám lepší plán.
};
FUNC VOID DIA_Addon_Fisk_Meeting_now()
{
	...
	if (!Npc_IsDead (Esteban))
	{
		...
		AI_Output (self, other, "DIA_Addon_Fisk_Meeting_now_12_03");//Pak je musíme pâimęt, k odchodu – povíme jim pravdu.
		AI_Output (other, self, "DIA_Addon_Fisk_Meeting_now_15_04");//Jakou pravdu?
		AI_Output (self, other, "DIA_Addon_Fisk_Meeting_now_12_05");//Žes udęlal, co ti nakázal, a že ten útok jsem spískal já.
		AI_Output (self, other, "DIA_Addon_Fisk_Meeting_now_12_06");//Povęz to Estebanovi a on pošle svý hochy, aby mę zabili. A já si tu na nę počkám.
	...
	{
		AI_Output (self, other, "DIA_Addon_Fisk_Meeting_now_12_08"); //Esteban je mrtvý. Už se zase můžu vęnovat jenom obchodům.
...
FUNC VOID DIA_Addon_Fisk_Sieg_Info()
{
	...
	AI_Output (self, other, "DIA_Addon_Fisk_Sieg_12_02");//Myslím, že teë máš na práci další vęci a já se musím vrátit zpátky k obchodům.
	AI_Output (self, other, "DIA_Addon_Fisk_Sieg_12_03");//Takže když budeš potâebovat nęco dalšího, víš, kde mę najít.
