...
func void DIA_Rosi_HALLO_Info ()
{
	...
	AI_Output			(self, other, "DIA_Rosi_HALLO_17_01"); //Dobrá, je to tak, tak. Bolí mê záda ze vší té tvrdé dâiny. Co tady dêláš? Návštêvníky tu nemáme pâíliš èasto.

	if ((hero.guild != GIL_MIL))
	{
		AI_Output			(self, other, "DIA_Rosi_HALLO_17_02"); //Když už, tak ty hrdloâezy z hor nebo vojáky z mêstské domobrany.
		AI_Output			(self, other, "DIA_Rosi_HALLO_17_03"); //Poslední dobou rabujou naší farmu èím dál tím èastêji. Ale ty nevypadáš jako jeden z nich, nebo se pletu?
	};
};
...
{
	AI_Output			(other, self, "DIA_Rosi_WASMACHSTDU_15_00"); //Co tady dêláš?

	if (Npc_IsDead(Sekob)== FALSE)
	{
	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_01"); //Tak to se ptám sama sebe už nêkolik let. Sekob, mùj manžel, už se tady pohádal snad s každým.
	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_02"); //Všude má dluhy. Proto krade zboží z Onarových zásob a prodává je ve mêstê.
	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_03"); //A z té své podfukaâiny už nastâádal celé jmêní.
	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_04"); //Dâe naše dêlníky, dokud jim neznièí záda. Jeho vlastní lidé mu už neâeknou jinak než otrokáâ.
	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_05"); //Nejsem vùbec pyšná na to, že se mùžu nazývat Sekobovou ženou, to mi vêâ. Obèas si pâeju, aby bariéra byla zùstala na svém místê.
	};

	AI_Output			(self, other, "DIA_Rosi_WASMACHSTDU_17_06"); //Nemáš zájem o nêco k jídlu nebo nêco, co by se ti hodilo pro lov v lesích?
	Log_CreateTopic (Topic_OutTrader,LOG_NOTE);
	B_LogEntry (Topic_OutTrader,"Rosi prodává na Sekobovê statku rùzné vêci.");
};
...
func void DIA_Rosi_BARRIERE_Info ()
{
	...
	AI_Output			(self, other, "DIA_Rosi_BARRIERE_17_02"); //Svrhli tam spoustu lidí, co se dopustili nêjaké nezákonné èinnosti. A nêkteâí z nich už se nikdy nevrátili zpêt.
...
instance DIA_Rosi_DuInBarriere		(C_INFO)
{
	...
	description	 = 	"Byla jsi nêkdy za bariérou?";
};
...
{
	AI_Output			(other, self, "DIA_Rosi_DuInBarriere_15_00"); //Byla jsi nêkdy za bariérou?
	AI_Output			(self, other, "DIA_Rosi_DuInBarriere_17_01"); //Ne. Jen jsme o ní slyšeli. Bengar, farmáâ z náhorních pastvin, ti o tom urèitê poví víc.
...
func void DIA_Rosi_BENGAR_Info ()
{
	...
	AI_Output			(self, other, "DIA_Rosi_BENGAR_17_01"); //Bêž se zeptat Balthazara. To je náš ovèák. Obèas svoje zvíâata vodí na pastvu nahoru na Bengarovy pozemky.
	AI_Output			(self, other, "DIA_Rosi_BENGAR_17_02"); //Mêl by být schopen ti âíct, jak se tam dostat.
...
instance DIA_Rosi_Miliz		(C_INFO)
{
	...
	description	 = 	"Proè vás domobrana na farmê napadá?";
};
...
{
	AI_Output			(other, self, "DIA_Rosi_Miliz_15_00"); //Proè vás domobrana na farmê napadá?
	...
	AI_Output			(self, other, "DIA_Rosi_Miliz_17_02"); //Král je daleko a nám nezbývá než dâít pro Onara a doufat, že ten vydâiduch pošle pomoc, když to bude opravdu zapotâebí.
};
...
func void DIA_Rosi_ONAR_Info ()
{
	...
	AI_Output			(self, other, "DIA_Rosi_ONAR_17_01"); //Obèas se k nám dostane varování, že se k nám chystá pár vojákù z domobrany, aby se nás pokusili okrást na vlastní pêst.
	AI_Output			(self, other, "DIA_Rosi_ONAR_17_02"); //Pak pošleme k velkostatkáâi nêkoho pro pomoc.
...
func void DIA_Rosi_PERMKAP1_Info ()
{
	...
	if ((MIS_bringRosiBackToSekob == LOG_SUCCESS))
		{
			AI_Output			(self, other, "DIA_Rosi_PERMKAP1_17_01"); //Bêž si skoèit do jezera.
		}
	else
		{
...
instance DIA_Rosi_FLEEFROMSEKOB		(C_INFO)
{
	...
	description	 = 	"Co dêláš tady v divoèinê?";
};
...
{
	AI_Output			(other, self, "DIA_Rosi_FLEEFROMSEKOB_15_00"); //Co dêláš tady v divoèinê?
	AI_Output			(self, other, "DIA_Rosi_FLEEFROMSEKOB_17_01"); //Na Sekobovê farmê už se to nedalo vydržet. Sekob dostával jeden hysterický záchvat za druhým.
	AI_Output			(self, other, "DIA_Rosi_FLEEFROMSEKOB_17_02"); //Nakonec už se s ním nedalo vùbec mluvit. Nakonec na všechny akorát âval.
...
instance DIA_Rosi_HILFE		(C_INFO)
{
	...
	description	 = 	"Vezmu tê odsud pryè.";
};
...
{
	AI_Output			(other, self, "DIA_Rosi_HILFE_15_00"); //Vezmu tê odsud pryè.
	...
	if ((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL))
		{
			AI_Output			(other, self, "DIA_Rosi_HILFE_15_01"); //Mohl bych tê vzít do mêsta.
		};

		if ((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
		{
			AI_Output			(other, self, "DIA_Rosi_HILFE_15_02"); //Vezmu tê na farmu velkostatkáâe.
		};
		...
		AI_Output			(self, other, "DIA_Rosi_HILFE_17_04"); //Nikdy ti nezapomenu, cos pro mê udêlal. Samozâejmê ti zaplatím.

			if (Npc_IsDead(Till))
			{
				AI_Output			(self, other, "DIA_Rosi_HILFE_17_05"); //Bêž první, budu tê následovat.
			}
			else
			{
				AI_Output			(self, other, "DIA_Rosi_HILFE_17_06"); //Bêž první, budeme tê následovat.
			};
		...
		B_LogEntry (TOPIC_RosisFlucht,"Rosi už to nemohla na Sekobovê statku déle vydržet a teë neví, kam jít. Vyvedu ji ven z divoèiny."); 
...
func void DIA_Rosi_ANGEKOMMEN_Info ()
{
	...
	MIS_RosisFlucht = LOG_SUCCESS;
		AI_Output			(self, other, "DIA_Rosi_ANGEKOMMEN_17_02"); //Prosím, vezmi si tenhle skromný dárek jako vyjádâení mých díkù.
...
func void DIA_Rosi_TRAIT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Rosi_TRAIT_17_01"); //Ty jsi ten nejhnusnêjší kýbl hnoje, co jsem kdy ve svým životê potkala, ty mizerná svinê.
	AI_StopProcessInfos (self);
	self.aivar[AIV_PARTYMEMBER] = FALSE;
	Till.aivar[AIV_PARTYMEMBER] = FALSE;
...
func void DIA_Rosi_MinenAnteil_Info ()
{
	...
	AI_Output (self, other, "DIA_Rosi_Minenanteil_17_01"); //Ne. Z nêèeho žít musím. A taky nejsem jediný, kdo je pustil do obêhu.
	B_GivePlayerXP (XP_Ambient);
};
