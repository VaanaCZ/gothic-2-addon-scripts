...
{	
	...
	AI_Output	(self ,other,"DIA_Gerold_Hallo_12_01");//Vêznê.
	AI_Output	(other,self ,"DIA_Gerold_Hallo_15_02");//A kdo to je?
	AI_Output	(self ,other,"DIA_Gerold_Hallo_12_03");//Pár trestancù a žoldnéâ jménem Gorn. Tuhý chlapík - je silný jako býk.
	AI_Output	(self ,other,"DIA_Gerold_Hallo_12_04");//Potâebovali jsme ètyâi chlapy, abychom ho mohli dát pod zámek.
};	
// ************************************************************
// 	Kerker
// ************************************************************
INSTANCE DIA_Gerold_Jail (C_INFO)
{
	...
	permanent	= FALSE;
	description ="Pustíš mê do vêzení?";
};                       
...
{	
	AI_Output	(other,self ,"DIA_Gerold_Jail_15_00");//Pustíš mê do vêzení?
	AI_Output	(self ,other,"DIA_Gerold_Jail_12_01");//Ne. NIKDO do vêzení nesmí. To platí pro KAŽDÉHO. Pro tebe, pro Miltena a pro všechny ostatní.
...
INSTANCE DIA_Gerold_Ausnahme (C_INFO)
{
	...
	permanent	= FALSE;
	description ="Nemùžeš pro mê udêlat výjimku?";
};                       
...
{	
	AI_Output	(other,self ,"DIA_Gerold_Ausnahme_15_00");//Nemùžeš pro mê udêlat výjimku?
...
INSTANCE DIA_Gerold_Deal (C_INFO)
{
	...
	permanent	= FALSE;
	description ="Mám tu zprávu pro jednoho z vêzàù...";
};                       
...
{	
	AI_Output	(other,self ,"DIA_Gerold_Deal_15_00");	//Mám tady vzkaz pro jednoho z vêzàù. Mùžeš mu ho pâedat?
	AI_Output	(self ,other,"DIA_Gerold_Deal_12_01");	//Hm... v zásadê nevidím jediný dùvod, proè ne. Pro koho je ten vzkaz?
	AI_Output	(other,self ,"DIA_Gerold_Deal_15_02");	//Pro Gorna.
	AI_Output	(self ,other,"DIA_Gerold_Deal_12_03");	//Velice dùležitý vêzeà. Nemyslím, že by s tím Garond souhlasil. Ale pokud to je pro tebe tak podstatné, jsem si jist, že se nêjak dohodneme.
	AI_Output	(other,self ,"DIA_Gerold_Deal_15_04");	//Co chceš?
	AI_Output	(self ,other,"DIA_Gerold_Deal_12_05");	//Naše pâídêly jídla se výraznê snížily. Rád bych se pro zmênu jednou poâádnê najedl.
	AI_Output	(self ,other,"DIA_Gerold_Deal_12_06");	//Pâines mi nêjakou slušnou klobásku a èerstvý krajíc chleba - a trochu sladkého medu.
	AI_Output	(self ,other,"DIA_Gerold_Deal_12_07");	//A k tomu ještê láhev vína... Jo, to by mêlo staèit. Urèitê se ti to podaâí. Vraã se, až to budeš mít.
...
INSTANCE DIA_Gerold_Stuff (C_INFO)
{
	...
	permanent	= TRUE;
	description = "Nêco tu pro tebe mám...";
};
//-----------------------------------                       
...
{	
	AI_Output	(other,self ,"DIA_Gerold_Stuff_15_00");	//Nêco pro tebe mám. Jídlo, které jsi chtêl.
	...
	*/
		...
		AI_Output   (other,self ,"DIA_Gerold_Stuff_15_02"); //Ano, tady. Nezapomeà, je to pro Gorna.
		AI_Output 	(self ,other,"DIA_Gerold_Stuff_12_03"); //Vraã se zítra, tou dobou už bude ten vzkaz mít.
		
		B_GiveInvItems (other,self ,ItWr_LetterForGorn_MIS,1); 
		DayContactGorn = Wld_GetDay();
		DIA_Gerold_Stuff_permanent = TRUE;
		
		B_LogEntry (TOPIC_RescueGorn,"Gerold dostal, co chtêl, a teë pâedá zprávu pâíslušnému adresátovi.");
	}
	else 
	{
		AI_Output (self ,other,"DIA_Gerold_Stuff_12_04"); //Vraã se, až budeš mít vêci, které jsem chtêl.
	};
};
// ************************************************************
// 	Deal Antwort
...
{	
	...
	{
		AI_Output	(self ,other,"DIA_Gerold_Antwort_12_01");//Ano, a mám ti od nêj vyâídit tohle:
		AI_Output	(other,self ,"DIA_Gerold_Antwort_15_02");//A?
		AI_Output	(self ,other,"DIA_Gerold_Antwort_12_03");//Je to u jižní brány.
		AI_Output	(other,self ,"DIA_Gerold_Antwort_15_04");//Âíkal ještê nêco?
		AI_Output	(self ,other,"DIA_Gerold_Antwort_12_05");//Ne. Netuším, co to mêlo znamenat. Ty to víš?
		AI_Output	(other,self ,"DIA_Gerold_Antwort_15_06");//Možná. Ale radši bys o tom nemêl moc pâemýšlet.
	...
	{
		AI_Output	(self ,other,"DIA_Gerold_GornsAnswer_12_08");//Ne, ještê ne. Vraã se pozdêji.
	};
...
INSTANCE DIA_Gerold_SetGornFree (C_INFO)
{
	...
	permanent	= FALSE;
	description = "Pusã Gorna na svobodu.";
};                       
...
{	
	AI_Output	(other,self ,"DIA_Gerold_SetGornFree_15_00");//Propusã Gorna na svobodu. Je to rozkaz od velitele Garonda.
	AI_Output	(self ,other,"DIA_Gerold_SetGornFree_12_01");//Už o tom vím. Tady máš klíè, mùžeš dovnitâ.

	CreateInvItems (self, ItKe_PrisonKey_MIS,1);
	B_GiveInvItems	(self,other,ItKe_PrisonKey_MIS,1);
	B_LogEntry (TOPIC_RescueGorn,"Gerold mi svêâil klíè od vêznice.");
};
// ************************************************************
// 	Perm
// ************************************************************
...
{	
	...
	AI_Output	(self ,other,"DIA_Gerold_Perm_12_01");//Ne. Innos sám mi dal sílu, abych nikdy nepotâeboval odpoèinek.
};	
//#####################################################################
//##
//##
...
func void DIA_Gerold_KAP4_ALLESRUHIG_Info ()
{
	...
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_12_01"); //V poâádku, jistê.
	AI_Output			(other, self, "DIA_Gerold_KAP4_ALLESRUHIG_15_02"); //Ale?
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_12_03"); //Pupek mi drhne o páteâ, chlape. Garond zase zmenšil pâídêly jídla.
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_12_04"); //Jestli nedostanu do châtánu nêjaký slušný dlabanec, zahryznu se do té Garondovy tlusté... zadku.
	...
	Info_AddChoice	(DIA_Gerold_KAP4_ALLESRUHIG, "Možná bych ti mohl nêco dát.", DIA_Gerold_KAP4_ALLESRUHIG_geben );
};
func void DIA_Gerold_KAP4_ALLESRUHIG_geben ()
{
	AI_Output			(other, self, "DIA_Gerold_KAP4_ALLESRUHIG_geben_15_00"); //Možná bych ti mohl nêco dát.
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_geben_12_01"); //Zbláznil ses? Tady v žádném pâípadê. Kdyby to vidêli ostatní, budu mít hned tunu nových pâátel, jestli chápeš, co tím chci âíct.
	if 	(Wld_IsTime	(23,10,08,00) == FALSE)
	{
		AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_geben_12_02"); //Až pùjdou všichni spát, najdeš mê v mágovê svatyni.
	}
	else
	{
		AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_geben_12_03"); //Rychle, pojë za mnou do mágovy svatynê, než se ostatní vyhrabou. Nechce se mi èekat do zítâka.
	};
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_geben_12_04"); //V tuhle dobu tam stejnê nikdo nebývá. Poèkám tam na tebe.
	...
	B_LogEntry (TOPIC_GeroldGiveFood,"Garond opêt omezil pâídêly potravin. Radši bych mêl Geroldovi dát nêco k jídlu, nebo z hladu vyvede nêjakou hroznou pitomost. Mám se s ním setkat u pâíbytku mágù na hradê, až budou všichni spát."); 
...
func void DIA_Gerold_KAP4_ALLESRUHIG_Nein ()
{
	...
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_Nein_12_01"); //Mùžeš mluvit. Tobê Garond urèitê pâídêl nesníží.
	AI_Output			(self, other, "DIA_Gerold_KAP4_ALLESRUHIG_Nein_12_02"); //I když by to neuznal, má strachy stažený zadek a v tobê vidí jedinou spásu.
...
func void DIA_Gerold_MoreFood ()
{
	...
	if (Gerold_FoodCounter > 8)
	{
		...
		AI_Output			(self, other, "DIA_Gerold_MoreFood_12_01"); //Tady máš peníze. Tady si za nê stejnê nic nekoupím, takže si je klidnê vezmi.
		AI_Output			(self, other, "DIA_Gerold_MoreFood_12_02"); //Už bych asi mêl jít, než si nás nêkdo všimne.
		AI_StopProcessInfos (self);
	...
	else
	{
		if (Gerold_FoodCounter == 0)
			{
				Info_AddChoice	(DIA_Gerold_FOOD, "Teë momentálnê nic nemám.", DIA_Gerold_FOOD_nichts );
			}
		else
			{
				...
				{
					AI_Output			(self, other, "DIA_Gerold_MoreFood_12_03"); //Jasan. Dej to sem. Máš ještê další?
				}
				else
				{
...
{
	AI_Output			(self, other, "DIA_Gerold_FOOD_12_00"); //Tady máš. Takže co? Máš pro mê nêco k jídlu?
...
{
	AI_Output			(other, self, "DIA_Gerold_FOOD_nichts_15_00"); //Teë momentálnê nic nemám.
	AI_Output			(self, other, "DIA_Gerold_FOOD_nichts_12_01"); //Nejdâív kolem toho dêláš takový humbuk a nakonec mê chceš podvést?
...
func void DIA_Gerold_FOOD_kaese_nichtmehr ()
{
	...
	AI_Output			(self, other, "DIA_Gerold_FOOD_kaese_nichtmehr_12_01"); //A tomu mám vêâit? No dobrá, lepší nêco než nic. Tady máš pár zlaãákù.
	CreateInvItems 		(self, ItMi_Gold, 50);									
	B_GiveInvItems 		(self, other, ItMi_Gold, 50);					
	AI_Output			(self, other, "DIA_Gerold_FOOD_kaese_nichtmehr_12_02"); //No. Mêl bych se vrátit na svoje místo.
	AI_StopProcessInfos (self);
...
{
	AI_Output			(other, self, "DIA_Gerold_FOOD_kaese_15_00"); //Co takhle pêkný špalík sýra?
...
{
	AI_Output			(other, self, "DIA_Gerold_FOOD_schinken_15_00"); //Mùžu ti dát tuhle šunku.
...
{
	AI_Output			(other, self, "DIA_Gerold_PERM4_15_00"); //Ještê máš hlad?
	
	if (MIS_OCGateOpen == TRUE)
	{
	AI_Output			(self, other, "DIA_Gerold_PERM4_12_01"); //Pokud skâeti znovu zaútoèí, nepomùže mi ani plné bâicho.
	}
	else if (MIS_GeroldGiveFood == LOG_SUCCESS)
	{
	AI_Output			(self, other, "DIA_Gerold_PERM4_12_02"); //To si dêláš srandu. Už jsem plný. Ještê jednou dêkuju.
	}
	else if (MIS_GeroldGiveFood == LOG_OBSOLETE)
	{
	AI_Output			(self, other, "DIA_Gerold_PERM4_12_03"); //Hm. Ale i tak ti dêkuju.
	}
	else
	{
