...
func void DIA_Bosper_HALLO_Info ()
{
	...
	B_LogEntry (TOPIC_CityTrader,"Bosper vyrábí luky a obchoduje s kùžemi. Jeho obchod najdu v dolní èásti mêsta, u východní brány.");
};
...
func void DIA_Bosper_IntoOV_Info ()
{
	...
	AI_Output (self, other, "DIA_Bosper_IntoOV_11_02"); //Nejdâív se musíš stát ctihodným mêšãanem, nebo si aspoà najít slušné zamêstnání.
	AI_Output (self, other, "DIA_Bosper_IntoOV_11_03"); //Jako cizinec se tam rozhodnê dostat nemùžeš.
	...
	B_LogEntry (TOPIC_OV,"Pokud se chci dostat do horní ètvrti, musím se stát buëto váženým obèanem, nebo si najít práci.");
};
...
func void DIA_Bosper_SeekWork_Info ()
{
	...
	AI_Output (self, other, "DIA_Bosper_SeekWork_11_01"); //Hmm - nový uèedník by se mi hodil.
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Fur] == FALSE)
	{
		...
		AI_Output (self, other, "DIA_Bosper_SeekWork_11_05"); //Mùžu tê nauèit, jak stahovat zvíâata.
		AI_Output (self, other, "DIA_Bosper_SeekWork_11_06"); //Za každou kùži, kterou mi pâineseš, ti dobâe zaplatím.
	}
	else
	{
		AI_Output (other, self, "DIA_Bosper_SeekWork_15_07"); //Mùžu ti dát pár kùží, jestli o nê máš zájem.
		AI_Output (self, other, "DIA_Bosper_SeekWork_11_08"); //Výbornê! Pâines mi tolik zvíâecích kùži, kolik mùžeš - koupím je od tebe za dobrou cenu.
	};
...
instance DIA_Bosper_LEHRLING		(C_INFO)
{
	...
	permanent   = TRUE;
	description = "Chci se stát tvým uèedníkem!";
};
...
func void DIA_Bosper_LEHRLING_Info ()
{
	...
	AI_Output (other, self, "DIA_Bosper_LEHRLING_15_00"); //Chci se stát tvým uèedníkem!

	if (MIS_Bosper_WolfFurs == LOG_SUCCESS)
	{
		// ------ Bosper ------
		AI_Output (self, other, "DIA_Bosper_LEHRLING_11_01"); //(zakâení se) Skvêlé! Âekl bych, že základy už máš.
		stimmen = stimmen + 1;
		...
		if (Harad.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			...
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_02"); //Harad si myslí, že jsi dobrý èlovêk.
				stimmen = stimmen + 1;
			}
			else
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_03"); //Ale Harad není o tvých schopnostech zrovna pâesvêdèen.
			};
		}
		else //noch kein Dialog
		{
			AI_Output (self, other,"DIA_Bosper_LEHRLING_11_04"); //Ale Harad tê prý ještê vùbec nevidêl.
		};
		...
		if (Thorben.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			...
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_05"); //Thorben ti dal svùj hlas. Já sice nejsem tak pobožný, ale vážím si toho.
				stimmen = stimmen + 1;
			}
			else
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_06"); //Thorben ti dá svolení jen tehdy, pokud si vyprosíš požehnání do bohù.
			};
		}
		else //noch kein Dialog
		{
		...
		if (Constantino.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			...
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_08"); //Constantino prohlašuje, že se mùžeš zapsat, u koho chceš.
				stimmen = stimmen + 1;
			}
			else
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_09"); //Constantino prohlašuje, žes byl obvinên ze zloèinu - doufám, že šlo jen o nêjakou banalitu.
		...
		if (Matteo.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			...
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_12"); //A Matteo se nechal slyšet, že by tê mêli vyvažovat zlatem.
				stimmen = stimmen + 1;
			}
			else if (MIS_Matteo_Gold == LOG_RUNNING)
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_13"); //A Matteo se zmínil o nêjaké pùjèce - aã už tím myslel cokoliv, mêl by sis s ním radši promluvit.
			}
			else	
			{	
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_14"); //Matteo tvrdí, že s tebou o téhle vêci zatím nemluvil.
			};
		}
		else //noch kein Dialog
		{
			AI_Output (self, other,"DIA_Bosper_LEHRLING_11_15"); //Matteo tvrdí, že tê nikdy v životê nevidêl.
		};		
		...
		if (stimmen >= 4)
		{
			if (stimmen == 5)
			{
				AI_Output (self, other,"DIA_Bosper_LEHRLING_11_16"); //To znamená, žes získal doporuèení ode všech mistrù!
			}
			else // == 4
			{
			...
			AI_Output (self, other,"DIA_Bosper_LEHRLING_11_18"); //Mùžeš pro mê zaèít pracovat, kdykoliv budeš chtít.
		...
		else // stimmen < 4
		{
			...
			AI_Output (self, other,"DIA_Bosper_LEHRLING_11_20"); //Takže by sis mêl promluvit se všemi, kteâí ti zatím souhlas nedali.
		};
	}
	else //keine WolfFurs
	{
		AI_Output (self, other, "DIA_Bosper_LEHRLING_11_21"); //Ještê než tê pâijmu, musím samozâejmê vêdêt, jestli jsi aspoà k nêèemu dobrý.
...
instance DIA_Bosper_OtherMasters (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Co když se budu chtít zapsat u nêkterého z ostatních mistrù?";
};
...
{
	AI_Output (other, self, "DIA_Bosper_OtherMasters_15_00"); //Co když se budu chtít zapsat u nêkterého z ostatních mistrù?
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_01"); //(mrzutê) Nesmysl!
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_02"); //Harad a Matteo už uèedníky mají.
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_03"); //Alchymista Constantino je starý samotáâ - ten žádného uèedníka nepâijal už celé roky.
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_04"); //A co se týèe Thorbena, tak o nêm každý ví, že je úplnê švorc - nejspíš by ti nemohl zaplatit ani mêëák.
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_05"); //Ovšem já velmi nutnê potâebuji uèedníka - a taky dobâe platím.
	AI_Output (self, other, "DIA_Bosper_OtherMasters_11_06"); //Ale aã už se chceš zapsat u kohokoliv, budeš k tomu potâebovat souhlas všech ostatních mistrù z dolní èásti mêsta.
...
instance DIA_Bosper_Bartok (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Proè to tvùj minulý uèedník vzdal?";
};
...
{
	AI_Output (other, self, "DIA_Bosper_Bartok_15_00"); //Proè to tvùj minulý uèedník vzdal?
	AI_Output (self, other, "DIA_Bosper_Bartok_11_01"); //Nechal se slyšet, že je tu poslední dobou pâíliš nebezpeèno.
	AI_Output (self, other, "DIA_Bosper_Bartok_11_02"); //Jestli tê to ale opravdu zajímá, mùžeš se ho zeptat sám.
...
func void DIA_Bosper_ZUSTIMMUNG_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_01"); //(zklamanê) Doufal jsem, že se rozhodneš pro mê.
		AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_02"); //Ale když to chceš takhle...
		AI_Output (other, self, "DIA_Bosper_ZUSTIMMUNG_15_03"); //Tím mi chceš naznaèit, že mê nedoporuèíš?
		AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_04"); //Když ostatní mistâi nebudou proti, pak ano.
		if (MIS_Bosper_Bogen == LOG_SUCCESS)
		{
			AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_05"); //Když už nic jiného, aspoà jsi mi vrátil luk.
		};
		if (MIS_Bosper_WolfFurs == LOG_SUCCESS)
		{
			AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_06"); //Ale urèitê by z tebe byl dobrý lovec.
		};
		...
		B_LogEntry (Topic_Lehrling,"Bosper mi dá doporuèení, když se budu chtít stát uèedníkem u nêkoho jiného.");
	}
	else
	{
		...
		AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_10"); //A kdo ví - možná se ti zalíbí natolik, že zùstaneš u mê.
		AI_Output (self, other, "DIA_Bosper_ZUSTIMMUNG_11_11"); //A když jsi dost dobrý, abych tê pâijal JÁ, urèitê si povedeš dobâe i u ostatních.
		Bosper_HintToJob = TRUE;
	};	
};
...
func void DIA_Bosper_Job_Info ()
{
	...
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Fur] == FALSE)
	{
		AI_Output (self, other, "DIA_Bosper_Job_11_01"); //Nauèím tê, jak stahovat kùži ze zvíâat, a ty mi pak pâineseš, âeknême, pùl tuctu vlèích kùží.
		
		Log_CreateTopic (TOPIC_CityTeacher,LOG_NOTE);
		B_LogEntry	(TOPIC_CityTeacher,"Bosper mê nauèí stahovat zvíâecí kùže.");
	}
	else
	{
		AI_Output (self, other, "DIA_Bosper_Job_11_02"); //Pâines mi pùl tuctu vlèích kùží.
	};
	
	AI_Output (self, other, "DIA_Bosper_Job_11_03"); //Tak poznám, že ses své âemeslo nauèil.
	AI_Output (self, other, "DIA_Bosper_Job_11_04"); //Pokud ti to nepotrvá vêènost a jestli ty kùže budou ve slušném stavu, pâijmu tê, když budeš chtít.
	if (Npc_KnowsInfo(other, DIA_Bosper_OtherMasters))
	{
		AI_Output (self, other, "DIA_Bosper_Job_11_05"); //Nebo (povzdech) mùžeš zaèít u nêkterého z ostatních mistrù - budeš-li si to opravdu pâát.
	};
	...
	B_LogEntry (TOPIC_BosperWolf,"Musím Bosperovi pâinést šest vlèích kùží - pak pro nêj budu moci pracovat, nebo mê doporuèí do uèení u jiného mistra.");
	
	if (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Fur] == FALSE)
	{
		B_LogEntry (TOPIC_BosperWolf,"Mêl bych ho pâimêt, aby mê nauèil stahovat zvíâecí kùže.");
	};
};
...
instance DIA_Bosper_BringFur (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Co se týèe têch vlèích kùží...";
};
...
{
	AI_Output (other, self, "DIA_Bosper_BringFur_15_00"); //Co se týèe têch vlèích kùží...
	
	if (Player_IsApprentice > APP_NONE)
	{
		AI_Output (self, other, "DIA_Bosper_BringFur_11_01"); //Už jsi vstoupil do uèení k jinému mistrovi. Kùže od tebe proto odkoupím za bêžnou cenu.
	...
	if (B_GiveInvItems (other, self, ItAt_WolfFur, 6))		
	{
		...
		AI_Output (self, other, "DIA_Bosper_BringFur_11_03"); //Skvêlé! Já vêdêl, že se pro tuhle práci hodíš!
		...
		AI_Output (self, other, "DIA_Bosper_BringFur_11_05"); //No a? Co si myslíš? Není to lepší práce než celé dny mlátit do kovadliny nebo se v nevêtraném kutlochu patlat s nêjakými lahvièkami?
	...
	{
		AI_Output (self, other, "DIA_Bosper_BringFur_11_06"); //Dohodli jsme se na pùltuctu - ještê máš ale èas. Pokraèuj a opatâi mi ty kùže.
	};
...
instance DIA_Bosper_TeachFUR (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Vysvêtli mi, jak stahovat zvíâata z kùže!";
};
...
{
	AI_Output (other, self, "DIA_Bosper_TeachFUR_15_00"); //Vysvêtli mi, jak stahovat zvíâata z kùže!

	if (B_TeachPlayerTalentTakeAnimalTrophy (self, other, TROPHY_Fur))
	{
		AI_Output (self, other, "DIA_Bosper_TeachFUR_11_01"); //No tak poslouchej, je to jednoduché, opravdu.
		AI_Output (self, other, "DIA_Bosper_TeachFUR_11_02"); //Staèí vzít ostrý nùž a rozâíznout zvíâeti bâicho. Pak po stranách trochu naâízni kùži na nohou a mùžeš celou kožešinu smêle stáhnout.
		if (MIS_Bosper_WolfFurs == LOG_RUNNING)
		{
			AI_Output (self, other, "DIA_Bosper_TeachFUR_11_03"); //Pâines mi vlèí kùže a pak se uvidí.
			B_LogEntry (TOPIC_BosperWolf,"Bosper mê nauèil stahovat zvíâecí kùže.");
		};
	};
};
...
func void DIA_Bosper_Trade_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Bosper_Trade_11_01"); //Nejsi tady, aby sis prohlížel moje zboží - máš mi pâece pâinést ty kùže!
...
instance DIA_Bosper_BogenRunning (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 = "Slyšel jsem, že tê prý nêkdo okradl.";
};
...
{
	AI_Output (other, self, "DIA_Bosper_LukRunning_15_00"); //Slyšel jsem, že tê prý nêkdo okradl.
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_01"); //Kdo ti to âekl? Nejspíš Bartok, že? To ti nemohl povêdêt nic lepšího? No tak jo.
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_02"); //Âeknu ti, aã se ten hajzl modlí, abych ho nedostal do ruky!
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_03"); //Jen na chvilièku jsem odešel z krámu a když jsem se vrátil, spatâil jsem jenom, jak utíká - s mým lukem na rameni.
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_04"); //Okamžitê jsem zavolal stráže, ale ten lump bêžel smêrem k pâístavu. A tam se jim ztratil.
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_05"); //Trochu jsem jim pâipálil koudel u zadku, takže prohledali celou pâístavní ètvrã, ale nenašli vùbec nic.
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_06"); //Jsou to prostê mamlasové!
	AI_Output (self, other, "DIA_Bosper_LukRunning_11_07"); //Vsadím se, že mùj luk bude poâád ještê nêkde ve mêstê. Informoval jsem strážné u obou mêstských bran, a tak by nikoho s mým lukem ven nepustili.
	...
	B_LogEntry	(TOPIC_BosperBogen,"Bosperovi nêkdo ukradl luk. Zlodêj uprchl do pâístavu, ale pak nêkam zmizel. Domobrana prohledala celou ètvrã, ale nikdo nic nenašel, i když luk musí být stále ještê kdesi ve mêstê.");
};
...
instance DIA_Bosper_BogenSuccess (C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 = "Mám pocit, že tenhle luk je tvùj.";
};
...
{
	AI_Output (other, self, "DIA_Bosper_LukSuccess_15_00"); //Mám pocit, že tenhle luk je tvùj.
	...
	AI_Output (self, other, "DIA_Bosper_LukSuccess_11_03"); //Doufám, žes s tím nemêl nêjaké trable.
	AI_Output (other, self, "DIA_Bosper_LukSuccess_15_04"); //Ne - s takovými vêcmi už jsem se vypoâádal dâív.
	AI_Output (self, other, "DIA_Bosper_LukSuccess_11_05"); //Hm, aã je to, jak chce, dêkuji ti. Jsem tvým dlužníkem!
...
instance DIA_Bosper_PERM		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Co novýho ve mêstê?";
};
...
{
	AI_Output (other, self, "DIA_Bosper_PERM_15_00"); //Co novýho ve mêstê?
	
	if (Bosper_Island == FALSE)
	{
		AI_Output (self, other, "DIA_Bosper_PERM_11_01"); //Jestli nás skâeti opravdu obklíèí, zaène to tu být velmi nepâíjemné.
		AI_Output (self, other, "DIA_Bosper_PERM_11_02"); //Jediná loë, která tu kotví, patâí paladinùm, a ti si ji taky poâádnê hlídají. A nemyslím, že by ji použili na záchranu mêšãanù.
...
FUNC VOID DIA_Bosper_AlsLehrling_Info()
{
	...
	{
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_00"); //Takže ty ses pâidal k domobranê?
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_01"); //Ne že by mi to nêjak vadilo. Kromê skâetù a banditù mùžeš vždycky ulovit i pár vlkù (ušklíbne se).
	...
	{
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_02"); //Koukám, že tvoje kariéra letí rychle vzhùru, královský paladine!
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_03"); //Aã už odsud pùjdeš kamkoliv, nezapomeà na svého starého mistra.
	...
	{
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_04"); //Tak tys vstoupil do kláštera, jo? Doufám, že tê obèas taky pustí ven, abys mê mohl i nadále zásobovat kùžemi.
	...
	&& (other.guild != GIL_KDF)
	{
		...
		AI_Output (self, other,"DIA_Bosper_AlsLehrling_11_06"); //Potâebuji další kùže, pâinesls mi je?
		Bosper_Lehrling_Day = Wld_GetDay();
	}
...
func void DIA_Bosper_Aufgaben_Info ()
{
	...
	AI_Output (self, other, "DIA_Bosper_Aufgaben_11_02"); //Zaplatím ti za nê daleko lepší cenu, než jakou by ti mohli nabídnout ostatní obchodníci.
	...
	if (other.guild == GIL_NONE)
	{
		...
		AI_Output (self, other, "DIA_Bosper_Aufgaben_11_05"); //Bohužel tu pro tebe nemám žádný volný pokoj. Ale v hostinci na tržišti tê nêkam uložit musí.
	};
	
	Log_CreateTopic (Topic_Bonus,LOG_NOTE);
	B_LogEntry (Topic_Bonus,"Bosper mi za zvíâecí kùže zaplatí velmi pâíznivou cenu.");
};
...
instance DIA_Bosper_SellFur (C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Mám tu pro tebe pár kùží.";
};
...
{
	AI_Output (other, self, "DIA_Bosper_SellFur_15_00"); //Mám tu pro tebe pár kùží.
	...
	|| (Npc_HasItems(other, ItAt_Addon_KeilerFur) > 0)
	{
		...
		if (Npc_HasItems(other, ItAt_SheepFur) > 0)
		{
			AI_Output (self, other, "DIA_Bosper_SellFur_11_01"); //Ovèí kùže? Doufám, žes tu ovci nezabil nêjakému sedlákovi na pastvê.
		...
		{
			AI_Output (self, other, "DIA_Bosper_SellFur_11_03"); //Vlèí kùže, ty jsou dobré.
		...
		{
			AI_Output (self, other, "DIA_Bosper_SellFur_11_04"); //Kùže ze skâetího psa? To jsou pêkné bestie.
		...
		{
			AI_Output (self, other, "DIA_Bosper_SellFur_11_05"); //Páni, dokonce kùže stínové šelmy - ty jsou velmi cenné.
		...
		|| (Npc_HasItems(other, ItAt_TrollBlackFur) > 0)
		{
			...
			{
				AI_Output (self, other, "DIA_Bosper_SellFur_11_06"); //A co je ksakru TOHLE za kùži?
			...
			{
				AI_Output (self, other, "DIA_Bosper_SellFur_11_09"); //Další velká trolí kùže - tos ty potvory vážnê zabil?
				AI_Output (other, self, "DIA_Bosper_SellFur_15_10"); //To víš - když narazím na nêjakou nestvùru, prostê chytím pâíležitost za paèesy.
			};
			...
			{
				AI_Output (self, other, "DIA_Bosper_SellFur_11_11"); //A dokonce kùže z èerného trola!
	...
	{
		AI_Output (self, other, "DIA_Bosper_SellFur_11_13"); //Ale vždyã víš, že mê zajímají pouze kùže z vlkù, stínových šelem a tak podobnê.
	};
};
...
INSTANCE DIA_Bosper_Minenanteil (C_INFO)
{
	...
	description = "Vidím, že máš na prodej nêjaké dùlní akcie.";
};   
...
{	
	AI_Output (other, self, "DIA_Bosper_Minenanteil_15_00"); //Vidím, že máš na prodej nêjaké dùlní akcie.
