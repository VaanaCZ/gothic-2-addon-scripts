...
func void DIA_Meldor_Interessantes_Info ()
{
	...
	AI_Output (self, other, "DIA_Meldor_Interessantes_07_01"); //Najdeš tu nevêstinec i hospodu. Hostinský se jmenuje Kardif a jestli hledáš nêjaké informace, pak je on ten pravý, s kým si máš promluvit.
...
func void DIA_Meldor_Lehmar_Info ()
{
	...
	AI_Output (self, other, "DIA_Meldor_Lehmar_07_02"); //Urèitê ti pár zlaãákù pùjèí, jen bêž dál.
...
func void DIA_Meldor_InsOV_Info ()
{
	...
	AI_Output (self, other, "DIA_Meldor_InsOV_07_01"); //(sarkasticky) No jasnê. A já si zase chtêl najmout loë, aby mê zavezla pâímo na královský dvùr k audienci.
...
func void DIA_Meldor_Smoke_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Meldor_Smoke_07_02"); //Hmm, být tebou, zkusím štêstí u Èervené lucerny.
	};
};
...
func void DIA_Meldor_PERM_Info ()
{
	...
	if (Kapitel <= 1)
	{
		AI_Output (self, other, "DIA_Meldor_PERM_07_01"); //Pâed nedávnem obrátily stráže celou pâístavní ètvrã vzhùru nohama.
		AI_Output (self, other, "DIA_Meldor_PERM_07_02"); //Hledaly nêjakou odcizenou vêc - poslední dobou se tu krádeže dost rozrostly, zvláštê v lepších ètvrtích.
		AI_Output (self, other, "DIA_Meldor_PERM_07_03"); //Oèividnê se snaží ty chudáky z pâístavu obvinit úplnê ze všeho.
	}
	else if (Andre_Diebesgilde_aufgeraeumt == TRUE)
	&&		(Meldor_DGNews == FALSE)
	{
		AI_Output (self, other, "DIA_Meldor_PERM_07_04"); //Doneslo se mi, že dole ve stokách byl rozprášen nêjaký zlodêjský klan. Všichni kradáci byli zabiti.
		Meldor_DGNews = TRUE;
	}
	else if (Kapitel == 3)
	{
		AI_Output (self, other, "DIA_Meldor_PERM_07_05"); //V Hornickém údolí jsou prý draci. Zajímalo by mê, jestli odsud teë paladinové vyrazí a pustí se s nimi do boje.
	}
	else if (Kapitel == 5)
	{
		AI_Output (self, other, "DIA_Meldor_PERM_07_06"); //Podle toho, co se âíká, tak už jsou všichni draci mrtví. Nêkdo je vymetl železným koštêtem.
	}
	else
	{
