...
{
	AI_Output (self, other,"DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_00"); //HALTE !
	
	if (MIS_Send_Buddler == LOG_SUCCESS)
	{
		//kairo: aioutput-string des spielers gab es doppelt. umbenannt!
		
		AI_Output (other, self,"DIA_Addon_BDT_1088_Torwache_FirstWarn_15_01"); //Hors de mon chemin, garde.
		AI_Output (self, other,"DIA_Addon_BDT_10018_Torwache_FirstWarn_06_02"); //Heu... d'accord, vous pouvez passer.
	...
	&& 		(MIS_Send_Buddler != LOG_SUCCESS)
	{
		...
		{
			AI_Output (self, other,"DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_03"); //Tant que Thorus ne vous aura pas remis de pierre rouge, vous ne serez pas le bienvenu ici.
			Torwache_Greetings = 1;
		}
		else if (Torwache_Greetings == 1)
		{
			AI_Output (self, other,"DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_04"); //Vous ne comprenez pas ce que je vous dis ? (fermement) FICHEZ LE CAMP !
			Torwache_Greetings = 2;
		}			
		else
		{
			AI_Output (self, other,"DIA_Addon_BDT_1088_Addon_Torwache_FirstWarn_06_05"); //(fermement) FICHEZ LE CAMP !
		};
	}
	else if (MIS_Send_Buddler == LOG_RUNNING)
	{
...
func void DIA_BDT_1088_Addon_Torwache_Attack_Info()
{
	...
	AI_Output (self, other,"DIA_Addon_BDT_1088_Addon_Torwache_Attack_06_00"); //(soupire) Vous croyez que je plaisante ? Je vais vous montrer...
