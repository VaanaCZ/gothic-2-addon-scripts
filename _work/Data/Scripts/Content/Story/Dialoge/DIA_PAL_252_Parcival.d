...
instance DIA_Parcival_Schurfer		(C_INFO)
{
	...
	permanent 	 =  FALSE;
	description	 =	"Co mi mùžeš âíct o hornících?";
};
...
{
	AI_Output (other, self, "DIA_Parcival_Schurfer_15_00"); //Co mi mùžeš âíct o hornících?
	...
	AI_Output (self, other, "DIA_Parcival_Schurfer_13_03"); //Marcosova skupina vyrazila smêrem ke starému dolu - vedl ji starý kopáè jménem Grimes.
	AI_Output (self, other, "DIA_Parcival_Schurfer_13_04"); //Další dvê skupiny šly spoleènê.
	AI_Output (self, other, "DIA_Parcival_Schurfer_13_05"); //Podle Jergana, jednoho z našich zvêdù, rozbili tábor poblíž velké vêže.
	
	B_LogEntry (TOPIC_ScoutMine,"Každý oddíl kopáèù vede jeden z paladinù - Marcos, Fajeth a Silvestro.");
	B_LogEntry (TOPIC_ScoutMine,"Marcosova skupina se vydala tam, kde kdysi býval starý dùl. Jejich vyzvêdaèem je Grimes."); 
	B_LogEntry (TOPIC_ScoutMine,"Další dvê skupiny se spojily dohromady a usadily se poblíž velké vêže."); 
...
instance DIA_Parcival_Diego		(C_INFO)
{
	...
	permanent 	 =  FALSE;
	description	 =	"K jaké skupinê se pâipojil Diego?";
};
...
{
	AI_Output (other, self, "DIA_Parcival_Diego_15_00"); //K jaké skupinê se pâipojil Diego?
	AI_Output (self, other, "DIA_Parcival_Diego_13_01"); //Ten trestanec - Diego? Šel se skupinou paladina Silvestra.
	
	B_LogEntry (TOPIC_ScoutMine,"Diego se pâipojil ke skupinê, kterou vede paladin Silvestro."); 
};
///////////////////////////////////////////////////////////////////////
//	Info Weg
///////////////////////////////////////////////////////////////////////
...
func void DIA_Parcival_Weg_Info ()
{
	...
	AI_Output (self, other, "DIA_Parcival_Weg_13_01"); //Do toho údolí nevede žádná bezpeèná cesta. Ale zdá se mi rozumné nejít pâímým smêrem.
	AI_Output (self, other, "DIA_Parcival_Weg_13_02"); //Snaž se vyhnout skâetùm a lesùm - a nechã tê Innos ochraàuje.
	
	B_LogEntry (TOPIC_ScoutMine,"Bude moudâejší nejít nejkratší cestou, to by mohlo být nebezpeèné. Zvlášã bych se mêl vyhýbat lesùm a skâetùm."); 
};
...
func void DIA_Parcival_DRAGON_Info ()
{
	...
	AI_Output (self, other, "DIA_Parcival_DRAGON_13_01"); //Tábor obklíèili skâeti. Doslova se tam zakopali.
	AI_Output (self, other, "DIA_Parcival_DRAGON_13_02"); //Ale mnohem víc znepokojující jsou útoky drakù. Zpustošili už celé vnêjší opevnêní.
	AI_Output (self, other, "DIA_Parcival_DRAGON_13_03"); //Ještê jeden draèí útok a budeme mít têžké ztráty.
};
///////////////////////////////////////////////////////////////////////
//	Info DRAGONS
///////////////////////////////////////////////////////////////////////
...
func void DIA_Parcival_DRAGONS_Info ()
{
	...
	AI_Output (self, other, "DIA_Parcival_DRAGONS_13_01"); //Netušíme, kolik jich je dohromady, ale urèitê to je víc než jeden.
...
func void DIA_Parcival_BRAVE_Info ()
{
	if (Parcival_BRAVE_LaberCount < 6)
	{
		...
		if randy == 0
		{
			AI_Output			(self, other, "DIA_Parcival_BRAVE_13_00"); //Všude panuje klid. To se ale mùže rychle zmênit.
		};
		if randy == 1
		{
		...
		if randy == 2
		{
			AI_Output			(self, other, "DIA_Parcival_BRAVE_13_02"); //Innos nám pomùže. Jeho svêtlo záâí v našich srdcích!
		};
	
	}
	else 
	{
	AI_Output			(self, other, "DIA_Parcival_BRAVE_13_03"); //Hej! Nemáš na práci nic dùležitêjšího než se mnou poâád tlachat? Mazej!
...
func void DIA_Parcival_ALLESKLAR_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Parcival_ALLESKLAR_13_02"); //Jo, zatracenê.
	}	
	else if (Parcival_BRAVE_LaberCount > 6)
	{
		AI_Output			(self, other, "DIA_Parcival_ALLESKLAR_13_03"); //(zasmêje se) Ale. Teë chápu, co máš za lubem. Ne, pâíteli. Tentokrát ne.
	}
	else
	{
...
instance DIA_Parcival_AnyNews		(C_INFO)
{
	...
	description  =	"Stalo se nêco dùležitého?";		
};
...
{
	AI_Output	(other,self ,"DIA_Parcival_AnyNews_15_00"); //Stalo se nêco dùležitého?
	if (hero.guild == GIL_DJG)
	{
		AI_Output 	(self ,other,"DIA_Parcival_AnyNews_13_01"); //Ty patâíš k té spodinê, co si âíká drakobijci?
		AI_Output 	(self ,other,"DIA_Parcival_AnyNews_13_02"); //Vážnê jsem si myslel, že v sobê máš dost cti, než aby ses spolèoval s takovými lidmi.
	
	}
	else
	{
		AI_Output 	(self ,other,"DIA_Parcival_AnyNews_13_03"); //Dêlám si starosti. Velké starosti.
	...
	AI_Output 	(self ,other,"DIA_Parcival_AnyNews_13_07"); //Kdyby záleželo na mnê, vyhnal bych je z hradu. Aã se o nê postarají skâeti.
	...
	Info_AddChoice	(DIA_Parcival_AnyNews,"Možná bys jim mêl dát šanci.",DIA_Parcival_AnyNews_Chance);
...
FUNC VOID DIA_Parcival_AnyNews_LordHagen ()
{
	...
	AI_Output (self ,other,"DIA_Parcival_AnyNews_LordHagen_13_01"); //(trpce se zasmêje) Ano, bohužel jsou to jediné, co tady máme.
	AI_Output (self ,other,"DIA_Parcival_AnyNews_LordHagen_13_02"); //Innos nás opravdu velice têžce zkouší.
...
{
	AI_Output (other,self ,"DIA_Parcival_AnyNews_Chance_15_00"); //Možná bys jim mêl dát šanci.
	AI_Output (self ,other,"DIA_Parcival_AnyNews_Chance_13_01"); //Už ji dostali. Bohužel.
	AI_Output (self ,other,"DIA_Parcival_AnyNews_Chance_13_02"); //Garond je pâesvêdèen, že by nám mohli pomoci.
	AI_Output (self ,other,"DIA_Parcival_AnyNews_Chance_13_03"); //Ale já z nich nespustím oko. Máme už dost problémù se skâety, nepotâebujeme další potíže.
...
FUNC VOID DIA_Parcival_AnyNews_Overact ()
{
	...
	AI_Output (self ,other,"DIA_Parcival_AnyNews_Overact_13_01"); //To rozhodnê ne. V našem postavení potâebujeme muže, kteâí by stáli pâíkladem.
	AI_Output (self ,other,"DIA_Parcival_AnyNews_Overact_13_02"); //Bojovníky, kteâí by v srdcích našich vojákù zažhnuli Innosùv oheà.
...
func void DIA_Parcival_Jan_Info ()
{
	...
	AI_Output	(other,self ,"DIA_Parcival_Jan_15_04"); //Chce pracovat v kovárnê.
	AI_Output 	(self ,other,"DIA_Parcival_Jan_13_05"); //Nepâipadá v úvahu. Je jedním z nich a já jim nevêâím.
}; 
...
instance DIA_Parcival_ThinkAgain		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description  =	"Nemùžeš tu vêc s Janem ještê jednou zvážit?";		
};
...
{
	AI_Output	(other,self ,"DIA_Parcival_ThinkAgain_15_00"); //Nemùžeš tu vêc s Janem ještê jednou zvážit?
...
func void DIA_Parcival_TalkedGarond_Info ()
{
	...
	AI_Output 	(self ,other,"DIA_Parcival_TalkedGarond_13_01"); //Hmm. Pokud to je tak, aã si tu kovárnu klidnê má.
	AI_Output 	(self ,other,"DIA_Parcival_TalkedGarond_13_02"); //I když si myslím, že vêâit tomu Janovi je velká chyba.
};  
...
{
	AI_Output			(other, self, "DIA_Parcival_PERMKAP4_15_00"); //A kromê toho?
	AI_Output			(self, other, "DIA_Parcival_PERMKAP4_13_01"); //Ale, nech mê už být!
