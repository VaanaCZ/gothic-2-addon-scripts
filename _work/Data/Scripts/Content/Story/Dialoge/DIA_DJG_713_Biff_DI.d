...
func void DIA_Biff_DI_HALLO_Info ()
{
	...
	if (Npc_KnowsInfo(other, DIA_Biff_DI_ORKS)==FALSE)
		{
				AI_Output			(other, self, "DIA_Biff_DI_HALLO_15_01"); //A co jsem âíkal pâedtím, ještê na moâi?
		};
	...
	AI_Output			(self, other, "DIA_Biff_DI_HALLO_07_04"); //Do kelu. Kdybych to byl vêdêl, zùstal bych radêj v Khorinidu.
	AI_StopProcessInfos (self);
};
...
instance DIA_Biff_DI_perm		(C_INFO)
{
	...
	description	 = 	"Na palubê všechno v poâádku?";
};
...
{
	AI_Output			(other, self, "DIA_Biff_DI_perm_15_00"); //Na palubê všechno v poâádku?
	AI_Output			(self, other, "DIA_Biff_DI_perm_07_01"); //(podráždênê) Jo, jo. Je tu klid.
	AI_StopProcessInfos (self);
};
...
func void DIA_Biff_DI_ORKS_Info ()
{
	...
	AI_Output			(other, self, "DIA_Biff_DI_ORKS_15_01"); //Co tady nahoâe ksakru dêláš? Mêls hlídat loë.
...
func void DIA_Biff_DI_UNDEADDRGDEAD_Info ()
{
	...
	AI_Output			(self, other, "DIA_Biff_DI_UNDEADDRGDEAD_07_02"); //Mùžu teë...
	AI_Output			(other, self, "DIA_Biff_DI_UNDEADDRGDEAD_15_03"); //Mùžeš vyrabovat celej ostrov, když na to pâijde.
	AI_Output			(self, other, "DIA_Biff_DI_UNDEADDRGDEAD_07_04"); //Skvêlý.
