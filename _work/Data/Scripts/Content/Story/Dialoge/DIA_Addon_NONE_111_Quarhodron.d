...
func void DIA_Addon_Quarhodron_Hello_Info ()
{
	...
	|| (PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_3] == TRUE)
	{
		...
		AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_11_01"); //(rozzuøenì) Povìz, èeho ode mne žádáš?
		 
		Info_ClearChoices	(DIA_Addon_Quarhodron_Hello);
		Info_AddChoice	(DIA_Addon_Quarhodron_Hello, "Co je v tom Adanovì chrámu?", DIA_Addon_Quarhodron_Hello_schwert );
		Info_AddChoice	(DIA_Addon_Quarhodron_Hello, "Nìkdo násilím vnikl do Adanova chrámu.", DIA_Addon_Quarhodron_Hello_raven );
		Info_AddChoice	(DIA_Addon_Quarhodron_Hello, "Umožni mi pøístup do Adanova chrámu.", DIA_Addon_Quarhodron_Hello_tempel );
		Info_AddChoice	(DIA_Addon_Quarhodron_Hello, "V zemi vypukla zemìtøesení.", DIA_Addon_Quarhodron_Hello_erdbeben );
	...
	else
	{
		...
		AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_11_03"); //(hlasitì) Bengla anthani?
		AI_StopProcessInfos (self);
	};
};
...
func void B_Quarhodron_Hello_KommZumPunkt ()
{
	...
	{
		Info_AddChoice	(DIA_Addon_Quarhodron_Hello, "Moudrost našich pøedkù je skuteènì nesmírná.", DIA_Addon_Quarhodron_Hello_frech );
...
{
	AI_Output			(other, self, "DIA_Addon_Quarhodron_Hello_erdbeben_15_00"); //Zemi nièí zemìtøesení. Musíme nìco udìlat, jinak se celý ostrov potopí do moøe.
	AI_Output			(self, other, "DIA_Addon_Quarhodron_Hello_erdbeben_11_01"); //Není nièeho, co by mohlo býti vykonáno.
	AI_Output			(self, other, "DIA_Addon_Quarhodron_Hello_erdbeben_11_02"); //Hnìv Adanùv padl na Jharkendar, by ztrestal nevìøící.
...
func void DIA_Addon_Quarhodron_Hello_raven ()
{
	...
	AI_Output			(self, other, "DIA_Addon_Quarhodron_Hello_raven_11_01"); //Hloupost. Já sám dveøe zapeèetil. Nelze se tam dostati.
...
{
	AI_Output			(other, self, "DIA_Addon_Quarhodron_Hello_tempel_15_00"); //Pus mì do Adanova chrámu.
	AI_Output			(self, other, "DIA_Addon_Quarhodron_Hello_tempel_11_01"); //Chrám zùstane navìky uzavøen. Tak rozhodla rada.
	DIA_Addon_Quarhodron_Hello_ChoiceCounter = (DIA_Addon_Quarhodron_Hello_ChoiceCounter + 1);
	B_Quarhodron_Hello_KommZumPunkt ();
};
func void DIA_Addon_Quarhodron_Hello_schwert ()
{
	AI_Output			(other, self, "DIA_Addon_Quarhodron_Hello_schwert_15_00"); //Co v tom Adanovì chrámu vlastnì je?
	AI_Output			(self, other, "DIA_Addon_Quarhodron_Hello_schwert_11_01"); //Mùj nejhlubší žal, mé nejvìtší zklamání.
	DIA_Addon_Quarhodron_Hello_ChoiceCounter = (DIA_Addon_Quarhodron_Hello_ChoiceCounter + 1);
	B_Quarhodron_Hello_KommZumPunkt ();
};
func void DIA_Addon_Quarhodron_Hello_frech ()
{
	AI_Output	(other, self, "DIA_Addon_Quarhodron_Hello_frech_15_00"); //Moudrost našich pøedkù je skuteènì ohromná.
	AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_frech_11_01"); //Muž tvého postavení se mnou ale nemùže hovoøiti.
	AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_frech_11_02"); //Mám neodbytný dojem, že nejsi tím, kým se zdáš.
	AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_frech_11_03"); //Potøebuješ-li mou pomoc, musíš nejprve dokázat, že jsi jí hoden.
	AI_Output	(self, other, "DIA_Addon_Quarhodron_Hello_frech_11_04"); //Odpovìz na mé otázky, abych mìl jistotu, že nejsi cizincem.
	
	Log_CreateTopic (TOPIC_Addon_Quarhodron, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Addon_Quarhodron, LOG_RUNNING);
	B_LogEntry (TOPIC_Addon_Quarhodron,"Quarhodron mi pomùže, pouze když mu prokážu svou cenu. Položil mi nìkolik obtížných otázek - dokud na nì neodpovím, nemohu s jeho pomocí poèítat."); 
...
{
		AI_Output			(self, other, "DIA_Addon_Quarhodron_TestFailed_11_00"); //Špatné odpovìdi odhalily, že nemáš dobré úmysly.
...
func void B_Quarhodron_Fragen_Choices ()
{
	...
	else
	{
		...
		&& (Quarhodrons_RichtigeAntworten == 0)
		{
			...
			AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_01"); //Vra se, teprve až se rozhodneš øíci mi pravdu.
			AI_StopProcessInfos (self);
		}
		else 
		{
			if (Quarhodrons_NextQuestion == 1)
			{
				...
				{
					AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_02"); //(pøemýšlí) Máš schopnosti strážcù smrti, jinak bys mì neprobudil. To je zjevné.
					AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_03"); //Jsi-li vskutku tím, kým tvrdíš, mìl bys znát odpovìdi na všechny mé otázky.
			...
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_04"); //Kdo uzavøel portál, aby svìt ochránil pøed zlem?
				Log_AddEntry  (TOPIC_Addon_Quarhodron," --- Kdo uzavøel portál, aby zem zachránil pøed zlem? ---"); 
				Quarhodrons_NextQuestion = Quarhodron_AlleFragenGestellt;
			}
			else if (Quarhodrons_NextQuestion == 6)
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_05"); //Kdo zmíròuje utrpení a stará se o nemocné?
			...
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_06"); //Kdo má poslední slovo v Radì pìti?
				Log_AddEntry  (TOPIC_Addon_Quarhodron," --- Kdo mìl poslední slovo v Radì pìti? ---"); 
				Quarhodrons_NextQuestion = 6;
			}	
			else if (Quarhodrons_NextQuestion == 4)
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_07"); //Kdo na nás pøivolal zlo?
				Log_AddEntry  (TOPIC_Addon_Quarhodron," --- Kdo má na svìdomí zlo, jež zachvátilo lid Jharkendaru? ---"); 
				Quarhodrons_NextQuestion = 5;
			}
			else if (Quarhodrons_NextQuestion == 3)
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_08"); //Kdo mi mùže udìliti pøímý pøíkaz?
				Log_AddEntry  (TOPIC_Addon_Quarhodron," --- Kdo mùže Quarhodronovi pøímo rozkazovat? ---"); 
				Quarhodrons_NextQuestion = 4;
			}	
			else if (Quarhodrons_NextQuestion == 2)
			{
				AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_Choices_11_09"); //Kdo kdysi chránil jharkendarský lid pøed útoky nepøátel?
			...
			if (Quarhodrons_NextQuestion >= Quarhodron_AlleFragenGestellt)
			{
				Info_AddChoice	(DIA_Addon_Quarhodron_Fragen, "Tohle nemùžu vìdìt.", DIA_Addon_Quarhodron_Fragen_NoPlan );
			}
			else
			{
			...
			Info_AddChoice	(DIA_Addon_Quarhodron_Fragen, "Knìží.", DIA_Addon_Quarhodron_Fragen_prie );
...
func void DIA_Addon_Quarhodron_Fragen_Info ()
{
	...
	if (DIA_Addon_Quarhodron_Fragen_Info_OneTime == FALSE)
	{
		...
		AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_11_02"); //Probudil jsi mì.
		
		DIA_Addon_Quarhodron_Fragen_Info_OneTime = TRUE;
	};
	
	AI_Output	(self, other, "DIA_Addon_Quarhodron_Fragen_11_03"); //Ke které jharkendarské kastì náležíš?
...
func void DIA_Addon_Quarhodron_Fragen_NoPlan ()
{
	...
	{
		AI_Output			(self, other, "DIA_Addon_Quarhodron_Fragen_NoPlan_11_01"); //Dobøe. Vìøím ti a pomohu ti.
	...
	{	
		AI_Output			(self, other, "DIA_Addon_Quarhodron_Fragen_NoPlan_11_02"); //Tak už mì nezdržuj.
		AI_StopProcessInfos (self);	
	};
};
...
func void DIA_Addon_Quarhodron_GibMirKey_Info ()
{
	...
	AI_Output	(other, self, "DIA_Addon_Quarhodron_GibMirKey_15_01"); //Pokud mì hned teï nepustíš do chrámu, nejspíš už bude pøíliš pozdì a všechno bude ztraceno.
	AI_Output	(self, other, "DIA_Addon_Quarhodron_GibMirKey_11_02"); //To není možné! Veleknìz KHARDIMON a já jsme v Jharkendaru jediní, kdo znají tajemství otevøení brány do Adanova chrámu.
	AI_Output	(other, self, "DIA_Addon_Quarhodron_GibMirKey_15_03"); //No, pak to ten tvùj kámoš KHARDIMON nejspíš vyžvanil.
	AI_Output	(other, self, "DIA_Addon_Quarhodron_GibMirKey_15_04"); //Povídám ti, že brána je otevøená – vidìl jsem to na vlastní oèi.
	...
	AI_Output	(self, other, "DIA_Addon_Quarhodron_GibMirKey_11_07"); //Pak si vezmi heslo. Jest vepsáno na této kamenné tabulce. Vyslov je u zavøené chrámové brány a otevøe se.
	CreateInvItems (self, ItMi_TempelTorKey, 1);									
	B_GiveInvItems (self, other, ItMi_TempelTorKey, 1);		
	AI_Output	(self, other, "DIA_Addon_Quarhodron_GibMirKey_11_08"); //(už polomrtvý) Mùj èas je u konce. Bìda, už ti více nepomohu.
	...
	AI_Output	(other, self, "DIA_Addon_Quarhodron_GibMirKey_15_10"); //Poèkej. Co je s tìmi komnatami?
	AI_Output	(self, other, "DIA_Addon_Quarhodron_GibMirKey_11_11"); //(už polomrtvý) Síly mne opouštìjí. Sbohem. Sejdeme se v øíši mrtvých.
	AI_StopProcessInfos (self);
	
	B_LogEntry (TOPIC_Addon_Quarhodron,"Quarhodron mi dal kamennou tabulku, která mi otevøe vstup do Adanova chrámu."); 
	
	Log_CreateTopic (TOPIC_Addon_Kammern, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Addon_Kammern, LOG_RUNNING);
	B_LogEntry (TOPIC_Addon_Kammern,"Quarhodron se zmínil o 'komnatách Adanových', kterých bych se mìl vyvarovat. Mìl bych zjistit, co tím pøesnì myslel, jinak mùžu v chrámu padnout do pìkné pasti."); 
