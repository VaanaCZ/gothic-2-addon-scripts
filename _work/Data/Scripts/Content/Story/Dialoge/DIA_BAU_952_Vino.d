...
func void DIA_Vino_HALLO_Info ()
{
	...
	AI_Output (self, other, "DIA_Vino_HALLO_05_01"); //Jako vždycky. Spousta dâiny, málo penêz a s trochou smùly zejtra pâijdou skâeti a srovnají naši farmu se zemí.
	
	if (hero.guild == GIL_NONE)
		{
			AI_Output (self, other, "DIA_Vino_HALLO_05_02"); //Královští paladinové obsadili celé mêsto. Ale nechce se mi vêâit, že by odtamtud zvedli ty svý velebný zadky a šli nám pomoct, až nás napadnou skâeti.
		};
};
...
func void DIA_Vino_SeekWork_Info ()
{
	...
	if (hero.guild == GIL_NONE)
	{
		AI_Output (self, other, "DIA_Vino_SeekWork_05_01"); //Víš nêco o polních pracech?
		AI_Output (other, self, "DIA_Vino_SeekWork_15_02"); //Co je tom tak složitého?
		AI_Output (self, other, "DIA_Vino_SeekWork_05_03"); //Ach! V tom pâípadê... Mám dojem, že si vystaèíme sami. Díky.
		
		if (!Npc_IsDead(Lobart))
		{
			AI_Output (self, other, "DIA_Vino_SeekWork_05_04"); //Pokud chceš pro Lobarta pracovat jako nádeník, musím tê varovat. Lidem jako ty platí fakt mizernê!
			...
			{
				AI_Output (other, self, "DIA_Vino_SeekWork_15_05"); //Nabídl, že mi prodá nêjaké šaty se slevou, když mu pomùžu na farmê.
				AI_Output (self, other, "DIA_Vino_SeekWork_05_06"); //Hmm. Nemám pro tebe nic, ale mùžeš mnê a chlapùm pâinést nêco k pití.
				AI_Output (self, other, "DIA_Vino_SeekWork_05_07"); //Pâines mi láhev vína a já âeknu Lobartovi, žes nám opravdu pomohl (s výsmêšným úšklebkem).
			...
			{	
				AI_Output (self, other, "DIA_Vino_SeekWork_05_08"); //Ale ode mê to nemáš, jasný?
			};
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Vino_SeekWork_05_09"); //Neâekl bych. Práce, kterou nabízím, by nêkoho jako ty urèitê nezajímala.
	};
};
...
func void DIA_Vino_BringWine_Info ()
{
	...
	AI_Output (self, other, "DIA_Vino_BringWine_05_01"); //Radêji se nemám ptát, kdes ho sebral, co? (smích) Koho to zajímá?
	if (!Npc_IsDead (Lobart))
	{
		...
		if (hero.guild == GIL_NONE)
		{
			AI_Output (self, other, "DIA_Vino_BringWine_05_03"); //Lobart o tobê uslyší jen to nejlepší.
		};
	};
...
instance DIA_Vino_ToTheCity (C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Mám namíâeno do mêsta.";
};
...
{
	AI_Output (other, self, "DIA_Vino_ToTheCity_15_00"); //Mám namíâeno do mêsta.
	AI_Output (self, other, "DIA_Vino_ToTheCity_05_01"); //A?
	AI_Output (other, self, "DIA_Vino_ToTheCity_15_02"); //Mùžeš mi o mêstê âíct nêco zajímavého?
	AI_Output (self, other, "DIA_Vino_ToTheCity_05_03"); //Ne. Ale Maleth tam obèas zajde - snad se od nêj nêco dozvíš - myslím nêco, co by tê mohlo zajímat.
};
...
instance DIA_Vino_PERM (C_INFO)
{
	...
	permanent 	= TRUE;
	description	= "Nêjaké zajímavé novinky?";
};
...
{
	AI_Output (other, self, "DIA_Vino_PERM_15_00"); //Nêjaké zajímavé novinky?
	
	if (Vino_Gossip_Orks == FALSE)
	{
		AI_Output (self, other, "DIA_Vino_PERM_05_01"); //Pâísahal bych, že jsem pâede dvêma dny na kraji lesa zahlédl skâeta.
		AI_Output (self, other, "DIA_Vino_PERM_05_02"); //Od té doby mám v noci vždycky alespoà jedno oko otevâené.
	...
	{
		AI_Output (self, other, "DIA_Vino_PERM_05_03"); //Ti obrovští odporní brouci už jsou vážnê hodnê otravní. Jsou všude. Sežerou, co uvidí, když na to nedáš pozor.
		AI_Output (self, other, "DIA_Vino_PERM_05_04"); //Pâed nêkolika dny jsem ležel v trávê, myslel si na své, chvilku jsem si zdâímnul a najednou koukám, jak mi jedna z têch zatracenejch vêcí ožužlává botu!
		AI_Output (self, other, "DIA_Vino_PERM_05_05"); //Mêls mê vidêt, jak jsem mazal. Od tý doby se nemùžu ani poâádnê vyspat.
		Vino_Gossip_Bugs = TRUE;
	}
	else
	{
		AI_Output (self, other, "DIA_Vino_PERM_05_06"); //Nêco, co by stálo za âeè? Nic.
	};
};
...
func void DIA_Vino_DMTAMSTART_Info ()
{
	...
	&& (hero.guild != GIL_MIL)
	{
		...
		AI_Output			(self, other, "DIA_Vino_DMTAMSTART_05_02"); //Doufám, že nikdy nezjistí, že patâí mnê.
		B_GivePlayerXP (XP_AmbientKap3);
		DIA_Vino_DMTAMSTART_OneTime = TRUE;
	}
	else
	{
		AI_Output			(self, other, "DIA_Vino_DMTAMSTART_05_03"); //Ti bastardi v èerných kápích jsou snad všude. Nêkdo by jim ty jejich kápê mêl omotat kolem krku.
	};
};
...
func void DIA_Vino_Obesessed_Info ()
{
	...
	AI_Output			(self, other, "DIA_Vino_Obesessed_05_01"); //(âve) Zatracenê, zmizni. Jinak mê zabijou.
...
instance DIA_Vino_Heilung		(C_INFO)
{
	...
	description	 = 	"Ty vùbec nejsi pâi smyslech.";
};
...
{
	AI_Output			(other, self, "DIA_Vino_Heilung_15_00"); //Ty vùbec nejsi pâi smyslech.
	AI_Output			(self, other, "DIA_Vino_Heilung_05_01"); //Moje hlava... Už to nemùžu dýl vydržet.

	if (DIA_Vino_Heilung_oneTime == FALSE)
	{
		AI_Output			(other, self, "DIA_Vino_Heilung_15_02"); //Mêli bychom zajít do kláštera. Pyrokar, nejvyšší mág, ti snad pomùže.
		AI_Output			(self, other, "DIA_Vino_Heilung_05_03"); //Myslíš? Fajn. Tak to zkusíme.
		B_NpcClearObsessionByDMT (self);
		B_StartOtherRoutine  (Vino,"Klášter"); 
		
		B_LogEntry (TOPIC_DEMENTOREN,"Vino je posedlý. Poslal jsem ho do kláštera, kde ho snad vyléèí. Doufám, že tam v poâádku dorazí."); 
...
func void DIA_Vino_PERM4OBSESSED_Info ()
{
	...
	AI_Output			(self, other, "DIA_Vino_PERM4OBSESSED_05_01"); //Jo, jo, už je to v poâádku. Mám dojem, že mi tihle chlapci mùžou pomoct. Ale ještê je mi dost divnê.

	if (DIA_Vino_PERM4OBSESSED_XP_oneTime == FALSE)
	{
	AI_Output			(self, other, "DIA_Vino_PERM4OBSESSED_05_02"); //Ale musím uznat, že jejich víno je to nejlepší, co jsem zatím mêl možnost ochutnat.
...
func void DIA_Vino_PERM45UND6_Info ()
{
	...
	{
	AI_Output			(self, other, "DIA_Vino_PERM45UND6_05_01"); //Poèet skâetù v okolí se nepâíjemnê zvýšil.
	AI_Output			(self, other, "DIA_Vino_PERM45UND6_05_02"); //Vy paladinové s nima zatoèíte, ne?
	}
	else	//hero.guild == GIL_DJG
	{
	AI_Output			(self, other, "DIA_Vino_PERM45UND6_05_03"); //Bojím se, že už brzy budeme muset všeho nechat a pâidat se k vám, žoldákùm na Onarovê farmê.
	};
	
	if (FoundVinosKellerei == TRUE)
	&& (hero.guild != GIL_MIL)
	{
		AI_Output			(self, other, "DIA_Vino_PERM45UND6_05_04"); //Bohužel, domobrana mê ještê nepâestala hledat. Jen doufám, že mê nechytí.
	};
};
