...
FUNC VOID Use_BookstandALCHEMY1_S1()		//Alchemiebücher
{
	...
	{	
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "Alchymista ke své práci vždy potâebuje laboratorní lahvièku, do které pak nalije hotový lektvar."	); 
					Doc_PrintLine	( nDocID,  1, ""						);
					...
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "K namíchání lektvarù trvale mênících nêkterou z duševních èi têlesných vlastností potâebuješ urèitou rostlinu:"						);
					Doc_PrintLines	( nDocID,  1, "Královský šãovík"		);
					Doc_Show		( nDocID );
...
FUNC VOID Use_BookstandALCHEMY3_S1()		//Alchemiebücher
{
	...
	{	
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "1 chàapavèí býlí - k výrobê tohoto lektvaru nepotâebuješ královský šãovík, nýbrž luèní pohanku."						);
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "Elixír síly"						);
					Doc_PrintLine	( nDocID,  0, "1 draèí koâen"						);
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "Elixír života"						);
					Doc_PrintLine	( nDocID,  0, "1 léèivý koâen"						);
					Doc_PrintLine	( nDocID,  0, ""						);
					...
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "Tyto lektvary mùže namíchat jen skuteèný mistr alchymie. Jednou z pâísad do têchto vzácných elixírù bývá obvykle královský šãovík."); 
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "Lektvar rychlosti namícháš mnohem snáze, zvláštê proto, že k jeho vytvoâení nepotâebuješ královský šãovík."						);
					Doc_PrintLine	( nDocID,  1, ""						);
