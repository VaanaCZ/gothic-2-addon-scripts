...
FUNC VOID PC_Goldhacken_Addon_Hour_Info()
{
	...
	if (B_GoldMob_Bestimmung() == TRUE)
	{
		...
		{
			PrintScreen ("Všude lítají odštěpky zlata...", -1, -1, FONT_ScreenSmall, 2);	
...
INSTANCE PC_Goldhacken_Addon_TSchlag (C_Info)
{
	...
	permanent		= TRUE;
	description		= "Pořádně do toho prašti."; 
};
...
FUNC VOID PC_Goldhacken_Addon_TSchlag_Info()
{
	...
	if (TruemmerChance < 5)
	{
			PrintScreen ("Ani ťuk...", -1, -1, FONT_ScreenSmall, 2);	
	}
	else
	{
...
INSTANCE PC_Goldhacken_Addon_Chance (C_Info)
{
	...
	permanent		= TRUE;
	description		= "(ohodnoť si vlastní schopnosti)"; 
};
...
FUNC VOID PC_Goldhacken_Addon_Chance_Info()
{
	...
	{
		ConcatText = ConcatStrings ("mizerný začátečník (", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 40) 
	{
		ConcatText = ConcatStrings ("obstojný kopáč (" , IntToString (Hero_HackChance));
	}
	...
	{
		ConcatText = ConcatStrings ("zatraceně dobrý zlatokop ( ", IntToString (Hero_HackChance));
	}
