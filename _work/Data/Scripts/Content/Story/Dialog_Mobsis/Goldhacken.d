...
FUNC VOID PC_Goldhacken_Addon_Hour_Info()
{
	...
	if (B_GoldMob_Bestimmung() == TRUE)
	{
		...
		{
			PrintScreen ("Všude lítají odštępky zlata...", -1, -1, FONT_ScreenSmall, 2);	
...
INSTANCE PC_Goldhacken_Addon_TSchlag (C_Info)
{
	...
	permanent		= TRUE;
	description		= "Poâádnę do toho prašti."; 
};
...
FUNC VOID PC_Goldhacken_Addon_TSchlag_Info()
{
	...
	if (TruemmerChance < 5)
	{
			PrintScreen ("Ani ăuk...", -1, -1, FONT_ScreenSmall, 2);	
	}
	else
	{
...
INSTANCE PC_Goldhacken_Addon_Chance (C_Info)
{
	...
	permanent		= TRUE;
	description		= "(ohodnoă si vlastní schopnosti)"; 
};
...
FUNC VOID PC_Goldhacken_Addon_Chance_Info()
{
	...
	{
		ConcatText = ConcatStrings ("zatracenę dobrý zlatokop ( ", IntToString (Hero_HackChance));
	}
