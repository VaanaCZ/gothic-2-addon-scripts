...
FUNC VOID PC_Goldhacken_Addon_Chance_Info()
{
	...
	{
		ConcatText = ConcatStrings ("nieopierzony żółtodziób (", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 40) 
	{
		ConcatText = ConcatStrings ("przeciętny kopacz (" , IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 55) 
	{
		ConcatText = ConcatStrings ("doświadczony kopacz (", IntToString (Hero_HackChance));
	}
