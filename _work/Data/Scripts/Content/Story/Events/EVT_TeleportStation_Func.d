	...
	if (CurrentLevel == NEWWORLD_ZEN)
	{
		if (Npc_GetDistToWP(hero,"NW_TELEPORTSTATION_CITY")<3000)
		{
			...
			if (SCUsed_NW_TELEPORTSTATION_CITY == FALSE)
			{
				...
				B_LogEntry (TOPIC_Addon_TeleportsNW,"Kamień teleportacyjny w jaskini na wschód od miasta przenosi do karczmy 'Martwa Harpia'."); 
			};
		...
		else if (Npc_GetDistToWP(hero, "NW_TELEPORTSTATION_TAVERNE")<3000)
		{
			...
			if (SCUsed_NW_TELEPORTSTATION_TAVERNE == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsNW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsNW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsNW,"Kamień teleportacyjny w pobliżu gospody 'Martwa Harpia' przenosi do portalu tajemniczych budowniczych."); 
			};
		...
		else if (Npc_GetDistToWP(hero, "NW_TELEPORTSTATION_MAYA")<3000)
		{
			...
			if (SCUsed_NW_TELEPORTSTATION_MAYA == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsNW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsNW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsNW,"Kamień teleportacyjny przy portalu tajemniczych budowniczych przenosi do jaskini na wschód od miasta."); 
			};
	...
	else if (CurrentLevel == ADDONWORLD_ZEN)
	{
		if	Hlp_StrCmp  (Npc_GetNearestWP(hero),"ADW_ENTRANCE_TELEPORT_NORTH_WP")
		{
			...
			if (SCUsed_ADW_TELEPORTSTATION_PORTALTEMPEL == FALSE)
			{
				...
				B_LogEntry (TOPIC_Addon_TeleportsADW,"Uaktywniłem kamień teleportacyjny w pobliżu portalu, który łączy się z Khorinis."); 
		...
		else if	Hlp_StrCmp  (Npc_GetNearestWP(hero),"ADW_ENTRANCE_TELEPORT_EAST_WP")
		{
			...
			if (SCUsed_ADW_TELEPORTSTATION_ADANOSTEMPEL == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsADW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsADW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsADW,"Udało mi się uaktywnić kamień teleportacyjny na górnym poziomie obozu bandytów."); 
				B_GivePlayerXP (XP_Ambient);
			};	
		...
		else if	Hlp_StrCmp  (Npc_GetNearestWP(hero),"ADW_ENTRANCE_TELEPORT_SOUTHEAST_WP")
		{
			...
			if (SCUsed_ADW_TELEPORTSTATION_SOUTHEAST == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsADW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsADW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsADW,"Uaktywniłem kamień teleportacyjny na bagnie, tuż na południe od obozu bandytów."); 
				B_GivePlayerXP (XP_Ambient);
			};	
		...
		else if	Hlp_StrCmp  (Npc_GetNearestWP(hero),"ADW_ENTRANCE_TELEPORT_SOUTHWEST_WP")
		{
			...
			if (SCUsed_ADW_TELEPORTSTATION_SOUTHWEST == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsADW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsADW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsADW,"Znalazłem kamień teleportacyjny na południowym zachodzie."); 
				B_GivePlayerXP (XP_Ambient);
			};	
		...
		else if	Hlp_StrCmp  (Npc_GetNearestWP(hero),"ADW_ENTRANCE_TELEPORT_WEST_WP")
		{
			...
			if (SCUsed_ADW_TELEPORTSTATION_PIRATES == FALSE)
			{
				Log_CreateTopic (TOPIC_Addon_TeleportsADW, LOG_MISSION);
				Log_SetTopicStatus(TOPIC_Addon_TeleportsADW, LOG_RUNNING);
				B_LogEntry (TOPIC_Addon_TeleportsADW,"Wewnątrz jaskini w tym kanionie znajduje się kamień teleportacyjny. Uaktywniłem go."); 
				B_GivePlayerXP (XP_Ambient);
			};	
