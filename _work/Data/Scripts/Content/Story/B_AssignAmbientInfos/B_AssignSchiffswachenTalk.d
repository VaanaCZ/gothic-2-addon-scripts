...
{	
	...
	||	(MIS_SCVisitShip == LOG_RUNNING))//Joly: Für die Kurzzeitige Erlaubnis, das Schiff zu besuchen! Muß nach der Aktion wieder geändert werden, z. B = 0;
	{
			...
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_Ambient_09_01"); //Dokud jsi tady, nedêlej žádné potíže.
			};
			if (Schiffswache.voice == 12)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_Ambient_12_02"); //Ani nepomysli na to, že bys tu mohl nêco ukrást, jasné?
			};
	...
	if (Mis_OCGateOpen == TRUE)
		{
			...
			if (Schiffswache.voice == 9)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_09_01"); //Kdybychom chytili toho zrádce, co otevâel hlavní bránu do hradu, udêlali bychom s ním krátký proces.
			};
			if (Schiffswache.voice == 12)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_12_02"); //Už nemùžeme déle èekat. Chlapi v Hornickém údolí potâebují naši pomoc dâív, než zaútoèí další vlna.
			};
		}
	else
		{
			if (Schiffswache.voice == 4)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_04_03"); //Garond žádá úplnou mobilizaci. Do Hornického údolí se vydáme co nevidêt.
			};
			if (Schiffswache.voice == 9)
			{
			...
			if (Schiffswache.voice == 12)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_12_05"); //Nemùžu se doèkat, až têm skâetùm dáme co proto. Zaèneme už brzo.
			};
		};
	AI_StopProcessInfos	(Schiffswache);
};
