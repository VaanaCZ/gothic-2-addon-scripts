...
{	
	...
	||	(MIS_SCVisitShip == LOG_RUNNING))//Joly: Für die Kurzzeitige Erlaubnis, das Schiff zu besuchen! Muß nach der Aktion wieder geändert werden, z. B = 0;
	{
			...
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_Ambient_12_02"); //Ani nepomysli na to, že bys tu mohl něco ukrást, jasné?
			};
	...
	if (Mis_OCGateOpen == TRUE)
		{
			if (Schiffswache.voice == 4)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_04_00"); //Ti zatracení skřeti vzali útokem Garondův hrad. Musíme jednat rychle.
			};
			if (Schiffswache.voice == 9)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_09_01"); //Kdybychom chytili toho zrádce, co otevřel hlavní bránu do hradu, udělali bychom s ním krátký proces.
			};
			if (Schiffswache.voice == 12)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_12_02"); //Už nemůžeme déle čekat. Chlapi v Hornickém údolí potřebují naši pomoc dřív, než zaútočí další vlna.
			};
		}
	else
		{
			...
			if (Schiffswache.voice == 12)
			{
				AI_Output (self,other,"DIA_Pal_Schiffswache_AmbientKap5_12_05"); //Nemůžu se dočkat, až těm skřetům dáme co proto. Začneme už brzo.
			};
		};
	AI_StopProcessInfos	(Schiffswache);
};
