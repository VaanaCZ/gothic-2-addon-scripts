...
{	
	...
	AI_Output (self, other, "DIA_PAL_9_JOIN_09_01"); //Jen ten, kdo v Innosových èi králových službách prokáže, že je toho hoden, se mùže stát vyvoleným!
	AI_Output (self, other, "DIA_PAL_9_JOIN_09_02"); //Jinak âeèeno: Vybráni mohou být jen ti nejlepší. Ale TY do toho máš hodnê daleko, hochu!
};
...
INSTANCE DIA_PAL_9_LOCATION(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Proè paladinové pâišli do Khorinidu?";
};                       
...
{	
	AI_Output (other, self, "DIA_PAL_9_LOCATION_15_00"); //Proè jste vy, paladinové, pâišli do Khorinidu?
	AI_Output (self, other, "DIA_PAL_9_LOCATION_09_01"); //Do toho ti nic není, pâíteli! Musí ti staèit, že jsme tu v Innosových a králových službách!
};
...
{	
	...
	|| (other.guild == GIL_KDF)
	{
		if (Kapitel <= 4)
		{
			...
			{
				AI_Output (self, other, "DIA_PAL_9_STANDARD_09_01"); //Teë když víme, že máme co do èinêní s draky, zaène náš velitel urèitê brzy nêco dêlat.
			}
			else
			{
				AI_Output (self, other, "DIA_PAL_9_STANDARD_09_02"); //Ještê stále nemáme žádné zprávy od našich mužù z Hornického údolí. To je velmi znepokojivé.
			};
		};

		if (Kapitel >= 5)
		{
			AI_Output (self, other, "DIA_PAL_9_STANDARD_09_03"); //Díky Innosovi! Draèí hrozba byla zažehnána. Teë už se musíme jen vypoâádat s têmi skâety a cesta k rudê je volná.
		};
	}
	else //KEIN Paladin
	{
		AI_Output (self, other, "DIA_PAL_9_STANDARD_09_04"); //Jestli je to všechno, co mi mùžeš âíct, tak už se ztraã, pâíteli!
	};
};
