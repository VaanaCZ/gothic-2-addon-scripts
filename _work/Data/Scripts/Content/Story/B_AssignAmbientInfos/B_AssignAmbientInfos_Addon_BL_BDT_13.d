...
{	
	...
	if (RavenIsDead == TRUE)
	{
		...
		{
			AI_Output (self, other, "DIA_Addon_BL_BDT_13_Chef_13_01");//(nadšenê) Náš slavnej velitel Thorus. Udêlá z nás ze všech boháèe!
		}
		else
		{
			AI_Output (self, other, "DIA_Addon_BL_BDT_13_Chef_13_02");//Náš slavnej velitel... no, vlastnê nevím.
		};
	}
	else
	{
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_Chef_13_03");//(nadšenê) Náš slavnej velitel Havran. Udêlá z nás ze všech boháèe!
...
{	
	...
	AI_Output (self, other, "DIA_Addon_BL_BDT_13_Lager_13_01");//Jestli hledáš informace, promluv si se Snafem. Ten kuchaâ toho ví spoustu.
...
{	
	AI_Output (other, self, "DIA_Addon_BL_BDT_13_News_15_00");//Nêco nového?
	
	if (RavenIsDead == TRUE)
	{
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_News_13_01");//Havran je mrtvý. Teë už se z tohohle ostrova NIKDY nedostanem!
	}
	else
	{
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_News_13_02");//Piráti sedli na loë a odjeli.
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_News_13_03");//Na pláži zùstalo sedêt jen pár debilù, kteâí ani nemaj poâádnou loë.
...
instance DIA_Addon_BL_BDT_13_Gold(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Jak bych si rychle vydêlal peníze?";
};                       
...
{	
	...
	if (BDT_13_Nerver <= 2)
	{
		...
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_Gold_13_02");//Prostê si u Thoruse vyzvedni èervenej kámen a pak už mùžeš jít do dolu.
	}
	else if (BDT_13_Kill == FALSE)
	{
		BDT_13_Nerver = 0;
		BDT_13_Kill = TRUE;
		AI_Output (self, other, "DIA_Addon_BL_BDT_13_Gold_13_03");//Si dêláš kozy, ne? Jdi do prdele!
		AI_StopProcessInfos (self);
	}
	else 
	{
