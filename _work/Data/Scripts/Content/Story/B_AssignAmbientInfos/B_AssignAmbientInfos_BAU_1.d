...
INSTANCE DIA_BAU_1_JOIN(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Âekni mi víc o têch žoldnéâích.";
};                       
...
{	
	AI_Output (other, self, "DIA_BAU_1_JOIN_15_00"); //Povêz mi víc o têch žoldnéâích.
	AI_Output (self, other, "DIA_BAU_1_JOIN_01_01"); //Jediný, co ti mùžu poradit, je, aby ses jim zdaleka vyhnul.
	AI_Output (self, other, "DIA_BAU_1_JOIN_01_02"); //Jestli se nêkterýmu z nich nebude zamlouvat tvùj ksicht, prostê ti krapet vylepší nos.
	AI_Output (self, other, "DIA_BAU_1_JOIN_01_03"); //Pak si mùžeš u Leeho stêžovat, ale ten nos už stejnê zùstane zlomenej.
};
...
{	
	...
	AI_Output (self, other, "DIA_BAU_1_PEOPLE_01_02"); //Dovol, abych ti dal menší radu: moc ty žoldáky neprovokuj. S têmahle není radno si zahrávat.
...
{	
	...
	AI_Output (self, other, "DIA_BAU_1_LOCATION_01_01"); //Jen pole a farmy. A spousta žoldákù. Ale vêtšina jich je na východê na Onarovê farmê.
...
{	
	...
	if (Kapitel == 1)
	{
		AI_Output (self,other,"DIA_BAU_1_STANDARD_01_01"); //Onar si najal žoldáky, aby udrželi domobranu ve mêstê. Jinak by nám odvedli i tu poslední ovci!
	};

	if (Kapitel == 2)
	{
		AI_Output (self,other,"DIA_BAU_1_STANDARD_01_02"); //Nic moc. Poâád ty samý problémy. Domobrana, skâeti a polní škùdci.
	};

	if (Kapitel == 3)
	{
		AI_Output (self,other,"DIA_BAU_1_STANDARD_01_03"); //Zaèaly se tu dít divný vêci. Pâed pár dny jsem v noci zahlídl èernou siluetu. A nebyl to èlovêk.
	};

	if (Kapitel == 4)
	{
		AI_Output (self,other,"DIA_BAU_1_STANDARD_01_04"); //Èím dál tím èastêjc tu vídáme skâety. Obèas mám dojem, jako by se jich za každýho mrtvýho objevilo nêkolik dalších.
	};
	
	if (Kapitel >= 5)
	{
		AI_Output (self,other,"DIA_BAU_1_STANDARD_01_05"); //Paladinové vyrazili do Hornickýho údolí. Co to všechno má znamenat? Nemùžeme si dovolit ztratit jedinýho muže, abysme udrželi domobranu na uzdê.
	};
};
