...
{	
	AI_Output (other, self, "DIA_NOV_3_Fegen_15_00"); //Potřebuji pomoci zamést pokoje noviců.
	...
	if (Hlp_GetInstanceID (Feger1)  == Hlp_GetInstanceID (self))
	{
		...
		{
			AI_Output (self, other, "DIA_NOV_3_Fegen_03_01"); //Nikdo ti nechce jen tak pomoct, co? Pomůžu ti jen v případě, že se ti podaří přesvědčit ještě někoho dalšího, aby do toho šel taky.
	...
	{	
		if (Feger2_Permanent == FALSE)
		{
			...
			AI_Output (self, other, "DIA_NOV_3_Fegen_03_09"); //Potřebuju 50 zlatých, protože jsem stále ještě nezaplatil Parlanovi.
			
			B_LogEntry 	(Topic_ParlanFegen,"Novic před kostelem mi pomůže, pokud mu zaplatím padesát zlaťáků.");
...
FUNC VOID DIA_NOV_3_Fegen_Ja()
{
	...
	B_LogEntry 	(Topic_ParlanFegen,"Novic před kostelem mi nyní pomůže s úklidem komnat.");
...
{	
	...
	AI_Output (self, other, "DIA_NOV_3_PEOPLE_03_01"); //My novicové sloužíme mágům kruhu ohně. Ti se zodpovídají nejvyššímu shromáždění, které sestává ze tří nejmocnějších čarodějů.
	AI_Output (self, other, "DIA_NOV_3_PEOPLE_03_02"); //Ale za vše, co se týká noviců, je zodpovědný otec Parlan. Veškerý svůj čas tráví na nádvoří, kde dohlíží na práci noviců.
};
...
{	
	...
	AI_Output (self, other, "DIA_NOV_3_LOCATION_03_03"); //My novicové se do ní podíváme, jen když sem pro něco jdeme mágům kruhu ohně.
};
...
{	
	...
	if (Kapitel == 1)
	{
		if (hero.guild == GIL_KDF)
		{
			...
			AI_Output (self,other,"DIA_NOV_3_STANDARD_03_02"); //Jen málokdy se stane, že by se takový nováček jako ty stal vyvoleným kruhu ohně.
		}
		else
		{
	...
	|| (Kapitel == 3)
	{
		...
		else //Kap 2 - 3a
		{
			...
			{
				AI_Output (self,other,"DIA_NOV_3_STANDARD_03_10"); //Říká se, že od paladinů, kteří se vydali do Hornického údolí, zatím nedorazily žádné zprávy. Nejvyšší rada bude vědět, co má být učiněno.
			};
		};	
	};
