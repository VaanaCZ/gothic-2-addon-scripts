...
{	
	...
	AI_Output (self, other, "DIA_VLK_16_JOIN_16_01"); //Pomùžeš nám bránit mêsto proti skâetùm?
	AI_Output (self, other, "DIA_VLK_16_JOIN_16_02"); //Pokud jsi obèanem Khorinidu, mùžeš se pâidat k domobranê - promluv si nêkým od mêstských stráží.
};
...
{	
	AI_Output (other, self, "DIA_VLK_16_PEOPLE_15_00"); //Kdo v tomhle mêstê požívá nejvêtší vážnosti?
...
{	
	...
	if (Kapitel == 1)
	{
		AI_Output (self,other,"DIA_Addon_VLK_16_STANDARD_16_00"); //Doufám, že domobrana koneènê zjistí, proè se khorinidská populace tak tenèí.
	...
	if (Kapitel == 2)
	{
		AI_Output (self,other,"DIA_VLK_16_STANDARD_16_01"); //Už jsi to slyšel? Za hradbami byli vidêni skâeti. Doufejme, že paladinové mají dostatek mužù.
	...
	if (Kapitel == 3)
	{
		...
		{
			AI_Output (self,other,"DIA_VLK_16_STANDARD_16_03"); //Žoldnéâ, o kterém se âíkalo, že zavraždil Lothara, je nevinný. Objevily se dùkazy o tom, že na nêj nêkdo ušil boudu. Zaslechl jsem to na tržišti.
		}
		else
		{
	...
	if (Kapitel == 4)
	{
		AI_Output (self,other,"DIA_VLK_16_STANDARD_16_06"); //Slyšel jsem, že ti šibenièníci opustili Onarovu farmu. Âíká se, že se ze strachu pâed lordem Hagenem stáhli zpêt do Hornického údolí.
	};
