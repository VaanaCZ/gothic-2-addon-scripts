...
{	
	...
	{
		B_Say (self,other,"$SHEEPKILLER_CRIME"); //Einfach unsere Schafe zu schlachten! Mach, daЯ du hier wegkommst!
	};
				
	if (B_GetPlayerCrime(self) == CRIME_ATTACK)
	{
		B_Say (self,other,"$ATTACK_CRIME"); //Mit miesen Schlдgern rede ich nicht!
	};
	...
	if (C_NpcBelongsToCity(self))
	{
		...
		&& (Hagen_Schulden <= 0)
		{
			B_Say (self,other,"$PAL_CITY_CRIME"); //Du bist eine Schande fьr deinen Orden! Lord Hagen wird toben vor Wut!
		}
		else if (other.guild == GIL_MIL)
		&& (Andre_Schulden <= 0)
		{
			B_Say (self,other,"$MIL_CITY_CRIME"); //Du bist eine Schande fьr die Stadtwache! Lord Andre wird dir das sicher austreiben!
		}
