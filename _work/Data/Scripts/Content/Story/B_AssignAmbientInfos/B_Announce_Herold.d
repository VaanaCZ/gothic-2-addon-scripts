...
func void B_Announce_Herold ()
{
	...
	AI_Output (self ,self,"DIA_Herold_Announce_04_00"); //Slyšte, obèané Khorinidu! Podle pøímého rozkazu ctihodného lorda Hagena vstupuje v platnost následující naøízení.
	...
	if (Kapitel <= 2)
	{
		...
		if (randy == 0)
		{
			AI_Output (self ,self,"DIA_Herold_Announce_04_01"); //Z bezpeènostních dùvodù se vstup do lesù a divoèiny kolem mìsta dùraznì nedoporuèuje.
		...
		else if (randy == 1)
		{
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_04"); //Všichni obèané našeho mìsta, kteøí dokážou udržet zbraò, jsou naléhavì žádáni, aby posílili øady královské domobrany.
		}
		else if (randy == 2)
		{
		...
		else if (randy == 3)
		{
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_10"); //Každý obyvatel Khorinidu, který se proviní nezákonnou èinností, se bude okamžitì zodpovídat lordu Andremu.
		}
		else
		{
	...
	{
			AI_Output (self ,self,"DIA_Herold_Announce_04_16"); //Zvìsti, které vypovídají o pøítomnosti drakù v Hornickém údolí Khorinidu, se nezakládají na pravdì.
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_18"); //Abychom tyto smìšné zvìsti vyvrátili, byla do Hornického údolí vyslána jednotka stateèných paladinù pod vedením jednoho z nich, jenž se v té oblasti dokonale vyzná.
	}
	else	//Kapitel 5
	{
		randy = Hlp_Random (2);
