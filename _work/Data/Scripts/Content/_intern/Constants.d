...
// ****************************
// Spellkosten fьr ALLE SCrolls
// ****************************
const int SPL_Cost_Scroll = 5;


// MH: 19.11.01 Gildennamen geдndert --> Дnderungen in Text.d (Character-Screen Gildenbezeichnungen) und Species.d (Gildenanhдngige Bewegungswerte wie z.B Kletterhцhe)
// MH: 19.11.01 Talente hinzugefьgt
// MH: 15.12.01 Neue Spells hinzugefьgt
// MH: 15.12.01 Neue Player Talente hinzugefьgt
...
const int ATR_STRENGTH				=  4;	// Stдrke
...
//
//	INVENTORY CAPACITIES		// --- werden vom Programm ignoriert - INV ist unendlich groЯ! ---
//
...
CONST INT PERC_ASSESSSURPRISE							=  30					; //wird beim Zurьckverwandeln gesendet
...
//	MISSION STATUS
//
const INT LOG_RUNNING									= 1						;		//	Mission lдuft gerade
...
//	VAR INT		JUMPUP_FORCE							[GIL_MAX]				;
	...
	VAR INT		FALLDOWN_DAMAGE							[GIL_MAX]				;		//									Schaden fьr jeden weiteren angefangenen Meter.
...
const int SPL_DONTINVEST 		= 0;		// Es kцnnen keine weiteren Manapunkte investiert werden. Erst durch CTRL loslassen geht der Spell ab
const int SPL_RECEIVEINVEST		= 1;		// Wirkung durchgefьhrt, es kцnnen weitere Invest kommen, zB.bei Heal nach jedem Pцppel
...
const int SPL_NEXTLEVEL			= 4;		// setze den Spruch auf den nдchsten Level
const int SPL_STATUS_CANINVEST_NO_MANADEC=8;
const int SPL_FORCEINVEST		 = 1 << 16;	// zieht auf jeden Fall einen Manapunkt ab, egal ob timePerMana abgelaufen ist, oder nicht (sinnvoll fьr Investierzauber, die zumindest einen Manapunkt abziehen sollen, obwohl timePerMana noch nicht abgelaufen ist.
...
const int NPC_TALENT_ACROBAT			= 11;	//--> Anis дndern!

// NEW Talents //werden komplett auf Scriptebene umgesetzt - Programm braucht sie nur fьr Ausgabe im Characterscreen
...
var int PLAYER_TALENT_RUNES[MAX_SPELL];				//Die SPL_ Konstanten werden hierfьr als Kennung verwendet
